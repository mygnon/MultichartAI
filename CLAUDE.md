...
| Instrument | TF | Target | Status |
|---|---|---|---|
| BNBUSDT | Hourly | >100K | R3 complete — **NP-max NEW $36,703** (−63.3%; **LL=229** SL=3.8 LS=22-23 SS=4.2-4.3 10tr MDD=−$14,441 Obj=93,284; found by LL 170-300 gap sweep — gap rule 3rd payoff; A09=A10=A11 conv). ⭐⭐⭐ **Obj-max (live use): LL=122 SL=4.025 LS=29 SS=4.2 NP=$35,112 MDD=−$7,112 24tr Obj=173,342** (7-conv; R3 A04 retest exact). LL=229 regime: +4.5% NP for 2× MDD — worse risk-adjusted. R2→R3 +4.53% not converged — R4 ready. BNB/BTC 8.83× |
...
**Exhaustive checkbox index is dynamic**: for N params, the Exhaustive option checkbox is at index `N+1`. `mc_automation.py` uses `len(cfg.params) + 1` — never hardcode this.
