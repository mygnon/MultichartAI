...
**HUNTER2 exit modules (BTC + ETH Hourly):** like SuperTrend (trend strategy), exit modules **HELP** (BTC 5/6, ETH 6/6 positive in-sample, ETH 0 hurt). **M5 QuantPass_PT_Exit best: BTC PT=0.671 +10.2% NP AND MDD↓; ETH PT=0.677 +9.52% NP, MDD flat** (IS). Full-period OOS (both): **NO module reduces the main-dominated Max DD** (BTC −$411, ETH −$874), so no strict NP↑-AND-MDD↓ PASS — but **M5 adds NP at equal MDD** (BTC +6.7%, ETH +10.09%) = best "free profit"; ETH M6 RescueTeam highest NP (+16.86%) but MDD +$7 deeper; ETH M3 EntryBarsAfterExit IS-overfit (IS +4.87% → full −0.70%). **M5 QuantPass_PT_Exit = the consistent NP-booster across SuperTrend (BTC/ETH) + HUNTER2 (BTC/ETH).** `search_{btc,eth}_hunter2_exit_modules.py` + `..._oos_validation.py`. (ETH scripts add `ensure_chart_ready` before the chart-trim — connect() picks the largest MC window, which is the Study Editor when open; ensure_chart_ready matches the workspace stem in the chart title.)
