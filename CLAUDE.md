...
| 17 | ~~ConsensusBreakout~~ **TESTED 2026-07-11/12** | COMPOSITION-AND verdict: **IS pays (BNB Obj 64,193 < both parents' 100K/95K) but OOS collects — BNB strict PASS 1.00× (+$14,719 = largest strict-PASS OOS profit)**; 5/6 OOS-profitable, mild breaks; BandMult interior on all 6 (both parents alternate as the binding gate). NQ stack $401K→$594K (+48%) RoMaD 7.64; modules 6/6 KEEP (72/72). Union (OR) + RegimeBlend (BLEND) will complete the operator triad |
| 18 | ~~DecayChannelBreakout~~ **TESTED 2026-07-12/13** | MEMORY-DECAY axis CONFIRMED: **BandMult interior on all 6 (0.44-2.63, none at the Donchian-0 endpoint) — stale resistance SHOULD fade; BNB Obj 117,985 = #2 overall**. But OOS = low-lag camp (3/6 profitable, no PASS; ETH 3.37×/NQ 2.95×) — the decayed level converging toward price is another low-lag construction (lag law 5th confirmation). **GC kept ALL 6 modules (first ever); ETH M5 step +251% RoMaD = record**. Modules 6/6 KEEP (78/78) |
| 19 | ~~UnionBreakout~~ **TESTED 2026-07-13** | COMPOSITION-OR verdict: **the operator is an IS↔OOS DIAL** — OR (earlier fills) BNB Obj 104,077 BEATS both parents (100K/95K) but OOS all-broke no PASS (1.3-2.6×); mirror of AND (64K IS / strict PASS). The operator moves position on the trade-off curve, it does not create alpha. GC M5 step +243% (2nd largest); NQ stack +64% RoMaD 8.02; modules 6/6 KEEP (84/84). RegimeBlend (dynamic dial) completes the triad |
| 20 | ~~MADBandBreakout~~ **TESTED 2026-07-14** | ROBUST DISPERSION CONFIRMED: **BNB Obj 138,777 = #2 overall (1% behind InnerChannel; +46% over Bollinger's stdev 95K on the same SMA center)** — on fat tails MAD ≫ stdev. OOS near-robust: BNB 1.056×/BTC 1.028× near-PASS, 5/6 profitable (ETH 2.47× the outlier). BTC stack RoMaD 10.64, NQ 14.47; modules 6/6 KEEP (90/90) |
| 21 | ~~SemiBandBreakout~~ **TESTED 2026-07-15** | DIRECTIONAL DISPERSION verdict: **splitting the dispersion per side COSTS — BNB Obj 109,136 < MADBand's pooled 138,777** (same center family); dispersion axis final: MAD 139K > Semi 109K > stdev 95K (robustness helps, direction split hurts — halved per-side samples add noise). OOS mixed (4/6 profitable, no PASS; ETH 1.31×/BNB 1.50× mild). ETH stack RoMaD 11.58, TXF M4 step +71%; modules 6/6 KEEP (96/96) |
| 22 | ~~TypicalChannelBreakout~~ **TESTED 2026-07-15** | Wick-weight ⅓ is its OWN sweet spot, not an interpolation: **BNB Obj 119,203 (#3 overall, > both bracket ends Donchian 100K/Close 98K); ETH strict PASS 1.00×; GC 176/0.5625/1.75/20 stack RoMaD 17.54 = NEW ALL-TIME RECORD** (dethrones Inner-BNB 16.36); NQ 14.73; TXF kept ALL 6 modules (2nd full-keep). Field axis final: Inner 140K > Typ 119K > Donch 100K > Close 98K — monotone INWARD. Modules 6/6 KEEP (102/102) |
| 23 | ~~POCBreakout~~ **TESTED 2026-07-15/16** | VOLUME-LOCATED event anchor verdict: **Length collapses to 2-11 bars on all 6 — the battle-zone that matters is the max-volume bar of the LAST FEW HOURS only** (multi-day volume memory rejected; TXF BandMult→0.0 = pure max-vol-bar H/L). Weak-main family (BNB Obj 46,522 — above Pivot 40K, below OpenRange 60K); **BNB strict PASS 1.00× (+$7,592)**, BTC 1.216× mild, futures all broke (but GC OOS +78K); 3/6 OOS-profitable. **NQ M5 step +170.7% RoMaD (top-3 all-time), stack $287K→$633K (+120%)**; GC RoMaD 9.90. Modules 6/6 KEEP (108/108). Awaits RangeSpike (price-only control) to isolate whether VOLUME added info |
| 24 | ~~RangeSpikeBreakout~~ **TESTED 2026-07-16** | POC-CONTROL verdict: **VOLUME adds OOS ROBUSTNESS, not IS edge** — price-only spike anchor is IS-strong (BNB 38/2.125/7/23 Obj 106,997 = 2.3× POC's 46.5K) but OOS-fragile (**0 PASS; BNB flips to worst cell: all-4-broke-and-lost, −$3,916, RoMaD 2.91** vs POC's strict PASS). Length does NOT collapse (2-57 dispersed vs POC's 2-11) — volume spike = hours-recent event, price spike = multi-day memory: different objects. 4/6 OOS-profitable (TXF +249,800 mildest 1.353×); **GC kept ALL 6 modules (3rd full-keep ever)**; BTC M6 step +144.8%. Modules 6/6 KEEP (114/114). BodySpike (conviction \|C−O\|) completes the event-anchor trio |
| 25 | ~~HeavyChannelBreakout~~ **TESTED 2026-07-16/17** | VOLUME-VALIDATE verdict: **the volume axis closes as VALIDATE > LOCATE > WEIGHT** — mid IS (BNB Obj 60,885 ≈ OpenRange) but best volume-family OOS: **BNB strict PASS 1.00× +$14,223 (#2 all-time strict-PASS profit; BOTH S2 candidates passed, first ever), NQ 1.012× near-PASS RoMaD 16.22 (#2 all-time cell)**, 5/6 OOS-profitable, mild breaks. **BandMult→0.0 on BNB/BTC/TXF: the volume filter itself replaces the ATR buffer** (heavy-volume extremes need no price confirmation). TXF IS Obj 32.77M = futures top-tier. Modules 6/6 KEEP (120/120). ETH+TXF Stage-4 flake-recovered same-day |
| 26 | ~~MultiScaleBreakout~~ **TESTED 2026-07-17** | SCALE-BLEND verdict: **level-space averaging ≠ parameter-space plateau** — no strict PASS (BTC 1.049× near-PASS +$593 the closest); base Length collapses to 2-5 on 5/6 (blend degenerates to short-window smoothing) EXCEPT TXF 70 (=70/140/280 genuine multi-scale memory → best futures cell: kept 5 modules, RoMaD 10.77). IS low-mid (BNB Obj 52,033 < Heavy 61K); 5/6 OOS-profitable, GC only loser. BTC M6 IS +101.6% but Stage-4 DISCARDED (full RoMaD −16%) — full-period validation again decisive. NQ RoMaD 12.46. Modules 6/6 KEEP (126/126). First zero-flake full run (401 min) |
| 27 | ~~RangeFracBreakout~~ **TESTED 2026-07-17** | NUMERAIRE verdict: **channel-width fraction WORKS — third ref that is BOTH IS-top-tier AND strict PASS** (after ERBand, joining the anti-correlation breakers): BNB IS Obj 111,638 (#4 tier) AND deploy 8/0.9/7/14 **strict PASS 1.00× +$13,402** with 5 modules kept → **stack $51.1K RoMaD 10.73 (#2 all-time stacked NP, behind Inner $70.6K)**. Fraction lands INSIDE the channel (0.75-0.93 on BNB/BTC/TXF) — the inward-field law re-confirmed in continuous form; NQ/GC took >1. **TXF IS Obj 38.55M = new futures record** (OOS +1.01M); **BTC M6 step +326% = #2 all-time**. 5/6 OOS-profitable. Modules 6/6 KEEP (132/132). 2nd consecutive zero-flake run (379 min) |
| 28 | ~~PolarityChannelBreakout~~ **TESTED 2026-07-17/18** | POLARITY-VALIDATE verdict: **the VALIDATION family generalizes — 2nd validator ref (after Heavy) with BNB strict PASS + BandMult→0.0** (filtering WHICH bars' extremes count replaces the price buffer). **BNB: IS champ 10/0.0/7/14 ITSELF strict PASS 1.00× +$13,437** (rare champ-holds; stack $50.9K RoMaD 10.94 = #3 all-time NP, 5 modules); **6/6 OOS-profitable** with mild breaks (ETH 1.218×, TXF 1.347× +1.07M, BTC 1.185×); IS mid-tier (BNB Obj 68,587 ≈ KAMA/MidChannel). 3/6 winners = IS champ (candidate quality high); 4 cells RoMaD >10 (NQ 11.71, ETH 10.97, BNB 10.94, TXF 10.07) = most balanced matrix yet. Modules 6/6 KEEP (138/138). 3rd consecutive zero-flake run |
| 29 | ~~TrimChannelBreakout~~ **TESTED 2026-07-18** | ORDER-STATISTIC verdict: **K lands INTERIOR (2-3) on 5/6 — the single extreme is noise, the 2nd-3rd extreme is the robust level** = 3rd validation-family member (rank-validate, after volume/polarity). **BNB (all candidates K=2): strict PASS 1.00× +$13,554 (#3 all-time strict-PASS profit; 2 of 4 candidates PASS)**, IS Obj 91,827 high-tier, stack $45.3K RoMaD 9.49; **TXF IS Obj 42.96M = NEW futures record** (5/2.0/4.75/11; OOS re-picks K=1 +434,600). 5/6 OOS-profitable (ETH −167 only loser). Modules 6/6 KEEP (144/144). BTC+BNB Stage-4 flakes recovered same-day |
| 30 | ~~FeedbackChannelBreakout~~ **TESTED 2026-07-18/19** | FEEDBACK verdict: **loss-streak escalation REJECTED — BandMult collapses to 0/near-0 on 5/6** (BTC/TXF/GC 0.0, ETH 0.06, BNB 0.25; the ReentryBars cooldown already covers it — law (4) extends to its evidence-based sibling). But the collapsed bare skeleton delivers **2 strict PASSes: BTC +$1,028 AND GC +$59,130 = GC's FIRST strict PASS in 30 references** (OOS re-picks a totally different regime 185/0.0/2.0/20, IS $78.8K ≪ champ $144.9K — OOS-law textbook case; **GC stack RoMaD 15.48 = #4 all-time cell**); TXF 1.054× near-PASS +508,400. 5/6 OOS-profitable. Modules 6/6 KEEP (150/150). BNB Stage-4 flake recovered same-day |
| 31 | ~~WickBlendBreakout~~ **TESTED 2026-07-19/20** | CONTINUOUS FIELD verdict: **the wick-weight optimum is INTERIOR w ≈ 0.625-0.75** (BNB 0.75, TXF 0.625, GC-IS 0.75) — not Donchian(1), not Close(0): **BNB 11/0.75/7/17 Obj 125,760 = #3 overall, beating BOTH endpoints AND Typical 119K**; confirmation trades ~70% of the Close→High distance, cross-confirming RangeFrac's 0.75-0.93 channel fraction — "inward but not central" is the general law. **TXF IS Obj 40.77M = #2 futures** (stack 5.77M +45% RoMaD 10.47); BNB stack $49.4K RoMaD 10.87 (#4 NP). OOS 0 PASS, 5/6 profitable (NQ only loser); w→0 on NQ/GC OOS picks (futures prefer the Close end). Modules 6/6 KEEP (162/162). BTC Stage-4 flake recovered same-day |
| 37 | ~~BodySpikeBreakout~~ **TESTED 2026-07-19** | EVENT-ANCHOR TRIO CLOSES: **body ≈ range (BNB Obj 107,357 ≈ RangeSpike 106,997 ≫ POC 46,522) — \|C−O\| conviction and true-range are interchangeable price-magnitude measures; only VOLUME carries independent OOS information** (POC strict PASS vs both spikes' BNB all-broke: Body least-bad −$1,276/2.43×, Range −$3,916). 0 PASS, BTC 1.049× near-PASS; **GC OOS +$135,970 = largest GC OOS profit ever** (1.534×, RoMaD 9.72); NQ ultra-long L≈300 regime (2-week body event). Modules 6/6 KEEP (156/156). ⚠️ NQ exposed the seed/zoom range-inversion bug (champion drifted above LEN_HI via unclamped len_fine) — fixed in bodyspike + 12 untested clones (5b46b3b) |
