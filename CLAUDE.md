...
**HUNTER2 exit modules (Hourly: BTC/ETH/BNB; Daily: BNB/BTC):** trend-strategy pattern, but **modules help on Hourly, NOT on Daily**. Hourly IS: all positive (BTC 5/6, ETH 6/6, **BNB 6/6 — M5 QuantPass_PT_Exit PT=0.069 +30.66% NP AND MDD↓25% = strongest M5 ever**; BTC PT=0.671 +10.2%; ETH PT=0.677 +9.52%). **Daily IS: modules HURT NP** (BNB max +4.78% M4; **BTC ALL negative −1.2..−5.5%** — low-freq daily already tight). **M5 QuantPass_PT_Exit flips role by TF: Hourly = NP-booster (NP↑+MDD↓); Daily = MDD-slasher** (BNB M5 −1.5%NP/−45%MDD; BTC M5 −5.5%NP/−42%MDD). Full-period OOS (Hourly BTC/ETH): no module reduces main-dominated MDD but M5 adds NP at equal MDD (BTC +6.7%, ETH +10.09%). ⚠️ **M6 RescueTeamExit Status-checkbox toggle fails on Daily runs** (BNB+BTC Daily M6 never completed — MC64 UI flakiness on that signal). **M5 QuantPass_PT_Exit = consistent across SuperTrend + HUNTER2.** `search_{btc,eth,bnb}_hunter2{,_daily}_exit_modules.py` + `..._oos_validation.py`. (All scripts call `ensure_chart_ready` before the chart-trim — connect() picks the largest MC window, the Study Editor when open; ensure_chart_ready matches the workspace stem in the chart title.)
