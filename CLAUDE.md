...
**11 self-authored strategies (clean-IS BNB Hourly), all confirmed: Donchian v2 $24.4K (ONLY OOS PASS, DN3 +$5,704) ≈ ADXtrend $23.9K (OOS fail) > VolatilityBreakout $22.0K (OOS fail) > HeikinAshi $20.7K (OOS fail) > BBSqueeze $16.1K (OOS fail) > Keltner $10.1K > RSIPullback ~$8K > ROCmomentum $5.6K > ParabolicSAR $4.6K (broken) ; TurtleChannel all-negative.**
**FINAL LAWS: (1) only Donchian (channel breakout + WIDE ATR chandelier trail) is both IS-strong AND OOS-robust — every other self-authored strategy is IS-strong-but-OOS-fragile (all break MDD OOS). (2) The EXIT is decisive IN-SAMPLE — wide ATR trail >> channel exit (Turtle all-negative) / SAR reversal (broken); HeikinAshi + VolatilityBreakout reconfirm (tight-trail all-negative/inferior IS; wide trail wins IS). BUT OOS the wide trail can amplify tail losses (VolatilityBreakout VB1 collapse). (3) Entry filters/buffers (ADX gate, squeeze, RSI/trend, volatility buffer) can boost IS but ALL fail OOS; BNB drives most to floor (EntryMult→0, TrendLen→floor) except the ADX gate. (4) IS strength != OOS robustness — the IS NP/Obj champion is repeatedly the OOS-WORST (HA1, VB1); IS-inferior higher-freq/tighter-trail regimes generalize best OOS (HA3, VB3).**
