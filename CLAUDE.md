...
| BTCUSDT | Hourly (champion OOS robustness) | pick most-robust converged champion | **WINNER C1: LL=107 SL=4.1 LS=139 SS=4.7** (full-period NP=$3,726, Max DD=−$799). Re-tested all 9 converged champions (R1-R5) on full data incl. the recent decline (BTC 80k→61k). **Drawdown-break test (full MDD vs each champion's established MDD) flipped the in-sample ranking**: the low-MDD in-sample stars BROKE their DD in the recent drop — C9 Obj-max −431→−725 (1.7×), C4/C5 LS165 −498→−948 (1.9×), C8 −464→−725, C7 −490→−654 — while the "plain" NP-max LS≈139 trio (C1/C2/C3, MDD −$799) did NOT deepen AND had the highest full NP. Classic overfit-to-calm-period signal: prefer C1 for live. NB: true OOS-only could not be isolated (signal date no-op; chart-trim didn't stick) so this is full-period robustness, not OOS-only. `search_btc_ct_oos_champion_select.py` |
