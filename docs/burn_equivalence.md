...
4. **手動編譯燒錄訊號**(無自動化):PowerLanguage Editor → New Signal →
   ...
   **MC 編譯器已實證拒絕**:`WriteFile` 的 lplong byref(v3,"Incorrect
   argument type")— DefineDLLFunc 一律避免指標型參數;**v4 新增待驗**:
   `_lcreat`、`_lwrite`、`_lclose`(kernel32 legacy,全值傳遞)。
   任一不過 → 回報,調整 templates.py。
   v4 emit 行為:零拋錯 + 自持 handle — 訊號檔寫入 `Z:\oms\signals\`(ramdisk),
   每次 emit 自癒建目錄(重開機清空自動恢復);**寫檔用 _lcreat/_lwrite/_lclose
   而非 FileAppend**(實測 2026-07-24:MC 的 FileAppend 開檔後永不釋放 handle,
   rename/刪除全部 sharing violation,v2 的 .json 從未發佈);Z: 未掛載、
   開檔失敗、寫入長度不符或 rename 重試 5 次仍失敗 → 本 bar 靜默跳過
