...
### 2.1.1 寫入端零拋錯規範(template v4,burner/templates.py)

EL 無 try/catch,檔案內建函數一拋 runtime error 就會解除該策略的 AOE;且 **MC 的 FileAppend 開檔後永不釋放 handle**(實測 2026-07-24:v2 的 .tmp 全數被 MC 鎖住,rename/刪除全部 sharing violation);MC 的 DefineDLLFunc 也**拒絕指標型 byref 參數**(v3 的 WriteFile lplong → "Incorrect argument type")。因此 v4 emit 區塊完全不用 EL 檔案內建函數、只用值傳遞的 WinAPI:
...
3. **自持 handle 寫檔**:`_lcreat`(建立或截斷)→ `_lwrite`(回傳實寫位元組數,與 StrLen 比對)→ `_lclose`(rename 前必先 close)— kernel32 legacy API,全部值傳遞、每步回傳值可檢查,永不拋錯。
