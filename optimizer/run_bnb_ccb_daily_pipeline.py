"""
run_bnb_ccb_daily_pipeline.py -- ONE-BAT unattended 4-stage pipeline for
SFJ_CloseChannelBreakout_crypto on BNBUSDT HOT DAILY.

  Stage 1  IS optimization (R1 + R2 confirm, take Obj-max)            2022/01-2026/01
...
One MC64 connection, params handed stage->stage in memory, decisions made in code:
  - convergence: FIXED R1+R2, champion = Obj-max (NP^2/|MDD|); daily surface is sparse/noisy.
...
PREREQUISITES (MC64 as Administrator):
  20260622_SFJ_CloseChannelBreakout_crypto_AI.wsp open; BNBUSDT HOT DAILY tab active+visible;
  data to 2026/06/10; Binance connected; Study Editor CLOSED; main + 6 module signals inserted
  (Status arbitrary -- the orchestrator sets all Status & the fixed inputs).

CLI:
  py run_bnb_ccb_daily_pipeline.py                 # full pipeline (auto-elevates)
  py run_bnb_ccb_daily_pipeline.py --from-stage 3  # resume from stage 3 (uses state.json)
  py run_bnb_ccb_daily_pipeline.py --from-csv      # re-analyse existing CSVs, no MC64
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ccb_daily_pipeline")
...
def _main_cfg(name, L, A, R):
    ...
    return StrategyConfig(name=name, mc_signal_name=MAIN_SIGNAL, timeframe="daily", bar_period=1440,
...
def _module_cfg(name, signal, axes):
    ...
    if combos > 5000:
        log.warning("  %s: %d combos >5000!", name, combos)
    return StrategyConfig(name=name, mc_signal_name=signal, timeframe="daily", bar_period=1440,
...
def _stage1_round(label, attempts, seed, conn, from_csv):
    """Run a set of static attempts + 3 adaptive zooms; return list of obj_max row-dicts."""
    rows: List[dict] = []
    best = {"objective": -1e18}
    bx, ba, br = seed
    for nm, L, A, R in attempts:
        ...
        log.info("  [%s %-16s] Obj-max L=%.4g A=%.4g R=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
    ...
    for zi, (rl, rs, ra, rast, rr, rrst) in enumerate(
        ...
        if om["objective"] > best["objective"]:
            best = om; bx, ba, br = om["Length"], om["ATRMult"], om["ReentryBars"]
        log.info("  [%s zoom%d] Obj-max L=%.4g A=%.4g R=%.4g Obj=%.0f", label, zi,
                 om["Length"], om["ATRMult"], om["ReentryBars"], om["objective"])
    return rows


def stage1(conn, from_csv, state):
    log.info("############ STAGE 1: IS optimization (R1 + R2) ############")
    if conn is not None:
    ...
    # R2 confirm seeded at R1 Obj-max
    r2_attempts = [
    ...
    rows += _stage1_round("R2", r2_attempts, (sx, sa, sr), conn, from_csv)
    champ = max(rows, key=lambda r: r["objective"])
    npmax = max(rows, key=lambda r: r["net_profit"])
    # candidate set: champion(Obj) + NP-max + 2 distinct high-Obj by Length bucket
    cands = []
    seen = set()
    ...
    _save_state(state)
    log.info(">>> Stage1 Obj-max champion: L=%.4g A=%.4g R=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
...
def stage2(conn, from_csv, state):
    ...
    if passing:
        ...
        # de-facto: max OOS, tie-break mildest break
...
def _set_signal_inputs(conn, signal, params):
    cfg = StrategyConfig(name=f"SET_{signal[:20]}", mc_signal_name=signal, timeframe="daily",
                         bar_period=1440, params=[ParamAxis(k, v, v, 1.0) for k, v in params.items()],
                         chart_workspace=WORKSPACE, chart_symbol=SYMBOL, insample=INSAMPLE)
...
def stage3(conn, from_csv, state):
    ...
    if conn is not None:
        ...
        mc.set_instrument_data_range(conn, *IS_RANGE)
        # set MAIN inputs to champion (the one programmatic main set)
...
def stage4(conn, from_csv, state):
    ...
    if conn is not None:
        ...
        _set_signal_inputs(conn, MAIN_SIGNAL, champ)
        for label, info in mods.items():       # set each module's IS-best inputs
            _set_signal_inputs(conn, info["signal"], info["params"])
    # A00 main-only full baseline
    if conn is not None:
        mc.set_signal_statuses(conn, {n: False for n in ALL_MODULE_NAMES}, verify=True, protected=[MAIN_SIGNAL])
    ...
    for step, (label, info) in enumerate(order, 1):
        ...
        cfg = StrategyConfig(name=f"S4_S{step}_{label}", mc_signal_name=signal, timeframe="daily",
                             bar_period=1440,
        ...
        steps.append(ent)
        state["stage4"] = {"baseline": {"net_profit": float(row0["NetProfit"]), "max_intraday_drawdown": cur_midd if not kept else None},
    ...
    log.info(">>> Stage4 FINAL kept-set: %s  RoMaD %.4f -> %.4f", kept or "NONE", cur_romad, cur_romad)
    return state
