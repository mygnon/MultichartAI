"""
run_btc_rcb_hourly_pipeline.py -- ONE-BAT unattended 4-stage pipeline for the NEW
self-authored SFJ_RegChannelBreakout_crypto on BTCUSDT HOT HOURLY.

  Stage 1  IS optimization (R1 broad + confirm rounds until converged, Obj-max)  2022/01-2026/01
  Stage 2  OOS champion-select among Stage-1 candidates              IS vs FULL 2021/03-2026/06/10
  Stage 3  exit-module IS optimization (6 modules, main fixed)        2022/01-2026/01
  Stage 4  CUMULATIVE greedy full-period OOS stack (keep iff RoMaD up) 2021/03-2026/06/10
           + write the final best params back into Format Objects Input Strings.

Strategy = linear-regression channel breakout (4 params: RegLen, BandMult, ATRMult, ReentryBars);
_Crypto1MUSD sizing lives in the .txt. One MC64 connection; params handed stage->stage in memory;
state.json after every stage (--from-stage N resumes; --from-csv re-analyzes). Decisions in code:
  - convergence: SELF-JUDGED -- R1 broad, confirm rounds seeded at running Obj-max until
    round-over-round Obj gain <= 0.5% (min 2, max 5). champion = Obj-max (NP^2/|MDD|).
  - Stage-2 pick: strict PASS (|MDD_full|<=|MDD_is|) max OOS, else de-facto (max OOS, mildest break).
  - RoMaD = NetProfit / |Max Intraday Drawdown| (= MCReport MaxDrawdown column).
Programmatic Format-Objects input-setting is read-back-verified -> ABORT loudly on mismatch.

PREREQUISITES (MC64 as Administrator):
  SFJ_RegChannelBreakout_AI.wsp open; BTCUSDT HOT HOURLY tab active+visible; data to 2026/06/10;
  Binance connected; Study Editor CLOSED; main SFJ_RegChannelBreakout_crypto + 6 module signals
  inserted (Status arbitrary -- the orchestrator sets all Status & the fixed inputs).

CLI:
  py run_btc_rcb_hourly_pipeline.py                 # full pipeline (auto-elevates)
  py run_btc_rcb_hourly_pipeline.py --from-stage 3  # resume from stage 3 (uses state.json)
  py run_btc_rcb_hourly_pipeline.py --from-csv      # re-analyse existing CSVs, no MC64
"""
...
SYMBOL      = "BTCUSDT HOT"
MAIN_SIGNAL = "SFJ_RegChannelBreakout_crypto"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_rcb_hourly_pipeline")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")          # wide no-op; chart-trim is control
...
def _stage1_round(label, attempts, seed, conn, from_csv):
    ...
    # 3 adaptive zooms around running Obj-max (counts kept <=5000)
...
def stage1(conn, from_csv, state):
    ...
    if conn is not None:
        mc.ensure_chart_ready(conn, _main_cfg("S1_seed", (18, 22, 2), (1.0, 2.0, 0.5), (6.5, 7.5, 0.5), (11, 13, 1)))
    ...
    _save_state(state)
    log.info(">>> Stage1 Obj-max: Reg=%.4g Band=%.4g Atr=%.4g Re=%.4g NP=%.0f MDD=%.0f Obj=%.4g tr=%d",
             champ["RegLen"], champ["BandMult"], champ["ATRMult"], champ["ReentryBars"],
             champ["net_profit"], champ["max_drawdown"], champ["objective"], champ["total_trades"])
...
def stage2(conn, from_csv, state):
    ...
    for period, rng in (("is", IS_RANGE), ("full", FULL_RANGE)):
        if conn is not None:
            c0 = cands[0]
            mc.ensure_chart_ready(conn, _micro_main_cfg("S2_probe", c0["RegLen"], c0["BandMult"], c0["ATRMult"], c0["ReentryBars"]))
            mc.set_signal_statuses(conn, {n: False for n in ALL_MODULE_NAMES}, verify=True, protected=[MAIN_SIGNAL])
        ...
        for i, c in enumerate(cands):
            ...
            if row is not None:
                ...
                log.info("  [%s C%d %s] NP=%.0f MDD=%.0f", period, i,
                         (c["RegLen"], c["BandMult"], c["ATRMult"], c["ReentryBars"]),
    ...
    _save_state(state)
    log.info(">>> Stage2 WINNER %s OOS=%.0f pass=%s", winner["params"], winner["oos_np"], winner["pass"])
    return state
...
def stage3(conn, from_csv, state):
    ...
    if conn is not None:
        mc.ensure_chart_ready(conn, _micro_main_cfg("S3_probe", *ct))
        mc.set_instrument_data_range(conn, *IS_RANGE)
    ...
    for (label, signal, axes) in MODULES:
        ...
        log.info("  %s %s NP=%.0f (%+.2f%%) MDD=%.0f", label, params, np_, mods[label]["delta_np_pct"] or 0, mdd)
...
def stage4(conn, from_csv, state):
    ...
    if conn is not None:
        mc.ensure_chart_ready(conn, _micro_main_cfg("S4_probe", *ct))
    ...
    for step, (label, info) in enumerate(order, 1):
        ...
        if row is not None:
            ...
            ent["valid"] = False
            log.error("  Stage4 S%d %s INVALID -- skipping", step, label)
        steps.append(ent)
        ...
        _save_state(state)
    if conn is not None:
        # write the FINAL best params into Format Objects Input Strings: main champ + kept modules at IS-best
        _set_signal_inputs(conn, MAIN_SIGNAL, champ)
        for k in kept:
        ...
        log.info("Final Format Objects inputs applied: main=%s kept=%s",
                 champ, {k: mods[k]["params"] for k in kept})
...
def main():
    ...
    log.info("  Stage1 champ: %s", {k: state["stage1"]["champion_obj"][k] for k in P + ("objective",)})
