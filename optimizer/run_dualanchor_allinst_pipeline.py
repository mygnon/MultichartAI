"""
run_dualanchor_allinst_pipeline.py -- ONE-BAT unattended 4-stage pipeline for
DualAnchorBreakout run across SIX instruments living in ONE workspace
(DualAnchorBreakout_AI.wsp), switching to the correct chart Window per instrument.

  crypto charts (signal DualAnchorBreakout_crypto):  BTCUSDT, ETHUSDT, BNBUSDT
  futures charts (signal DualAnchorBreakout_NQ):      TWF.TXF, CME.NQ, CME.GC
...
Strategy params (4): Length, BandMult, ATRMult, ReentryBars (dual-anchor gate (outermost of yesterday's extreme and rolling Donchian)
breakout). Date ranges by asset class. Each instrument writes results/<sub>_dualanchor_hourly_pipeline/
state.json (+ CSVs).
...
CLI:
  py run_dualanchor_allinst_pipeline.py                    # all 6 instruments x 4 stages (auto-elevates)
  py run_dualanchor_allinst_pipeline.py --probe-windows    # read-only: dump open chart titles
  py run_dualanchor_allinst_pipeline.py --instrument "BTCUSDT HOT" [--from-stage N]
  py run_dualanchor_allinst_pipeline.py --resume-gaps      # re-run any instrument missing a complete stage4
  py run_dualanchor_allinst_pipeline.py --from-csv         # re-analyse existing CSVs, no MC64
"""
...
WORKSPACE = r"C:\Users\Tim\Downloads\Multichart64\Tim\DualAnchorBreakout_AI.wsp"
...
SIG_CRYPTO = "DualAnchorBreakout_crypto"
SIG_NQ     = "DualAnchorBreakout_NQ"

# (key, symbol, match-tokens, signal, is_range, full_range, target, out_subdir)
INSTRUMENTS = [
    ("btc", "BTCUSDT HOT", ["BTCUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "btc_dualanchor_hourly_pipeline"),
    ("eth", "ETHUSDT HOT", ["ETHUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL,  10_000.0, "eth_dualanchor_hourly_pipeline"),
    ("bnb", "BNBUSDT HOT", ["BNBUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "bnb_dualanchor_hourly_pipeline"),
    ("txf", "TWF.TXF HOT", ["TXF"],     SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "txf_dualanchor_hourly_pipeline"),
    ("nq",  "CME.NQ HOT",  ["CME.NQ"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "nq_dualanchor_hourly_pipeline"),
    ("gc",  "CME.GC HOT",  ["CME.GC"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "gc_dualanchor_hourly_pipeline"),
]

# ---- 4-param search space (Length, BandMult, ATRMult, ReentryBars) ----
LEN_LO, LEN_HI   = 2.0, 300.0
BAND_LO, BAND_HI = 0.0, 8.0     # ATR buffer; 0 = pure touch
ATR_LO, ATR_HI   = 0.5, 20.0
RE_LO,  RE_HI    = 0.0, 50.0
SEED = (20.0, 1.0, 7.0, 13.0)   # Length, BandMult, ATRMult, ReentryBars
CONV_THRESH = 0.005
...
TARGET_NP = 0.0
OUTPUT_DIR: Path = RESULTS / "dualanchor_allinst_pipeline"
STATE_JSON: Path = OUTPUT_DIR / "state.json"

RESULTS.mkdir(parents=True, exist_ok=True)
(RESULTS / "dualanchor_allinst_pipeline").mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-8s -- %(message)s",
                    datefmt="%H:%M:%S",
                    handlers=[logging.StreamHandler(sys.stdout),
                              logging.FileHandler(str(RESULTS / "dualanchor_allinst_pipeline" /
...
def stage1(conn, from_csv, state):
    ...
    r1_attempts = [
        ...
        ("04_band_fine",   (10, 30, 5),  (0.0, 3.0, 0.125),(6.0, 8.0, 1.0),  (12, 16, 2)),
        ...
        ("09_global_bound",(5, 300, 20), (0.0, 5.0, 0.5),  (2.0, 18.0, 4.0), (0, 40, 20)),
    ]
