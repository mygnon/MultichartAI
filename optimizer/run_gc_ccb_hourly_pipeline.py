"""
run_gc_ccb_hourly_pipeline.py -- ONE-BAT unattended 4-stage pipeline for the
FUTURES strategy SFJ_CloseChannelBreakout_NQ on CME.GC HOT HOURLY.
...
sizing (Format Signals -> Properties -> default contracts). Run on CME.GC HOT data.

PREREQUISITES (MC64 as Administrator):
  SFJ_CloseChannelBreakout_AI.wsp open; CME.GC HOT HOURLY tab active+visible;
...
CLI:
  py run_gc_ccb_hourly_pipeline.py                 # full pipeline (auto-elevates)
  py run_gc_ccb_hourly_pipeline.py --from-stage 3  # resume from stage 3 (uses state.json)
  py run_gc_ccb_hourly_pipeline.py --from-csv      # re-analyse existing CSVs, no MC64
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\gc_ccb_hourly_pipeline")
