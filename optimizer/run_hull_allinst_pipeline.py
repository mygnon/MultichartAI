"""
run_hull_allinst_pipeline.py -- ONE-BAT unattended 4-stage pipeline for
SFJ_HullBreakout run across SIX instruments living in ONE workspace
(SFJ_HullBreakout_AI.wsp), switching to the correct chart Window per instrument.

  crypto charts (signal SFJ_HullBreakout_crypto):  BTCUSDT, ETHUSDT, BNBUSDT
  futures charts (signal SFJ_HullBreakout_NQ):      TWF.TXF, CME.NQ, CME.GC
...
Strategy params (4): Length, BandMult, ATRMult, ReentryBars (Hull-MA +/- ATR band breakout). Date ranges by asset class. Each instrument writes results/<sub>_hull_hourly_pipeline/
state.json (+ CSVs).
...
CLI:
  py run_hull_allinst_pipeline.py                    # all 6 instruments x 4 stages (auto-elevates)
  py run_hull_allinst_pipeline.py --probe-windows    # read-only: dump open chart titles
  py run_hull_allinst_pipeline.py --instrument "BTCUSDT HOT" [--from-stage N]
  py run_hull_allinst_pipeline.py --resume-gaps      # re-run any instrument missing a complete stage4
  py run_hull_allinst_pipeline.py --from-csv         # re-analyse existing CSVs, no MC64
"""
...
WORKSPACE = r"C:\Users\Tim\Downloads\Multichart64\Tim\SFJ_HullBreakout_AI.wsp"
...
SIG_CRYPTO = "SFJ_HullBreakout_crypto"
SIG_NQ     = "SFJ_HullBreakout_NQ"

# (key, symbol, match-tokens, signal, is_range, full_range, target, out_subdir)
INSTRUMENTS = [
    ("btc", "BTCUSDT HOT", ["BTCUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "btc_hull_hourly_pipeline"),
    ("eth", "ETHUSDT HOT", ["ETHUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL,  10_000.0, "eth_hull_hourly_pipeline"),
    ("bnb", "BNBUSDT HOT", ["BNBUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "bnb_hull_hourly_pipeline"),
    ("txf", "TWF.TXF HOT", ["TXF"],     SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "txf_hull_hourly_pipeline"),
    ("nq",  "CME.NQ HOT",  ["CME.NQ"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "nq_hull_hourly_pipeline"),
    ("gc",  "CME.GC HOT",  ["CME.GC"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "gc_hull_hourly_pipeline"),
]
...
TARGET_NP = 0.0
OUTPUT_DIR: Path = RESULTS / "hull_allinst_pipeline"
STATE_JSON: Path = OUTPUT_DIR / "state.json"

RESULTS.mkdir(parents=True, exist_ok=True)
(RESULTS / "hull_allinst_pipeline").mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-8s -- %(message)s",
                    datefmt="%H:%M:%S",
                    handlers=[logging.StreamHandler(sys.stdout),
                              logging.FileHandler(str(RESULTS / "hull_allinst_pipeline" /
...
# ----------------------------------------------------------------------------- helpers
def _seed_cfg():
    return StrategyConfig(name="HULL_seed", mc_signal_name=MAIN_SIGNAL, timeframe=TF, bar_period=BARP,
