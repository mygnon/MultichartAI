"""
run_multiscale_allinst_pipeline.py -- ONE-BAT unattended 4-stage pipeline for
MultiScaleBreakout run across SIX instruments living in ONE workspace
(MultiScaleBreakout_AI.wsp), switching to the correct chart Window per instrument.

  crypto charts (signal MultiScaleBreakout_crypto):  BTCUSDT, ETHUSDT, BNBUSDT
  futures charts (signal MultiScaleBreakout_NQ):      TWF.TXF, CME.NQ, CME.GC
...
Strategy params (4): Length, BandMult, ATRMult, ReentryBars (multi-scale channel (avg Donchian over L/2L/4L) +/- ATR buffer
breakout). Date ranges by asset class. Each instrument writes results/<sub>_multiscale_hourly_pipeline/
state.json (+ CSVs).
...
CLI:
  py run_multiscale_allinst_pipeline.py                    # all 6 instruments x 4 stages (auto-elevates)
  py run_multiscale_allinst_pipeline.py --probe-windows    # read-only: dump open chart titles
  py run_multiscale_allinst_pipeline.py --instrument "BTCUSDT HOT" [--from-stage N]
  py run_multiscale_allinst_pipeline.py --resume-gaps      # re-run any instrument missing a complete stage4
  py run_multiscale_allinst_pipeline.py --from-csv         # re-analyse existing CSVs, no MC64
"""
...
WORKSPACE = r"C:\Users\Tim\Downloads\Multichart64\Tim\MultiScaleBreakout_AI.wsp"
...
SIG_CRYPTO = "MultiScaleBreakout_crypto"
SIG_NQ     = "MultiScaleBreakout_NQ"

# (key, symbol, match-tokens, signal, is_range, full_range, target, out_subdir)
INSTRUMENTS = [
    ("btc", "BTCUSDT HOT", ["BTCUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "btc_multiscale_hourly_pipeline"),
    ("eth", "ETHUSDT HOT", ["ETHUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL,  10_000.0, "eth_multiscale_hourly_pipeline"),
    ("bnb", "BNBUSDT HOT", ["BNBUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "bnb_multiscale_hourly_pipeline"),
    ("txf", "TWF.TXF HOT", ["TXF"],     SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "txf_multiscale_hourly_pipeline"),
    ("nq",  "CME.NQ HOT",  ["CME.NQ"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "nq_multiscale_hourly_pipeline"),
    ("gc",  "CME.GC HOT",  ["CME.GC"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "gc_multiscale_hourly_pipeline"),
]

# ---- 4-param search space (Length, BandMult, ATRMult, ReentryBars) ----
LEN_LO, LEN_HI   = 2.0, 75.0    # BASE scale; longest window = 4*Length <= 300
BAND_LO, BAND_HI = 0.0, 8.0     # 0 = pure touch of the averaged level
ATR_LO, ATR_HI   = 0.5, 20.0
RE_LO,  RE_HI    = 0.0, 50.0
SEED = (15.0, 0.5, 7.0, 13.0)   # Length, BandMult, ATRMult, ReentryBars
CONV_THRESH = 0.005
...
TARGET_NP = 0.0
OUTPUT_DIR: Path = RESULTS / "multiscale_allinst_pipeline"
STATE_JSON: Path = OUTPUT_DIR / "state.json"

RESULTS.mkdir(parents=True, exist_ok=True)
(RESULTS / "multiscale_allinst_pipeline").mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-8s -- %(message)s",
                    datefmt="%H:%M:%S",
                    handlers=[logging.StreamHandler(sys.stdout),
                              logging.FileHandler(str(RESULTS / "multiscale_allinst_pipeline" /
...
def stage1(conn, from_csv, state):
    ...
    r1_attempts = [
        ("01_global_wide", (2, 74, 4),   (0.0, 3.0, 0.5),  (2.0, 14.0, 2.0), (0, 20, 10)),
        ("02_classic",     (5, 35, 2),   (0.0, 1.5, 0.25), (5.0, 9.0, 1.0),  (10, 20, 5)),
        ...
        ("09_global_bound",(3, 75, 6),   (0.0, 5.0, 0.5),  (2.0, 18.0, 4.0), (0, 40, 20)),
    ]
