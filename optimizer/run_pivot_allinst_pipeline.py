"""
run_pivot_allinst_pipeline.py -- ONE-BAT unattended 4-stage pipeline for
SFJ_PivotBreakout run across SIX instruments living in ONE workspace
(SFJ_PivotBreakout_AI.wsp), switching to the correct chart Window per instrument.

  crypto charts (signal SFJ_PivotBreakout_crypto):  BTCUSDT, ETHUSDT, BNBUSDT
  futures charts (signal SFJ_PivotBreakout_NQ):      TWF.TXF, CME.NQ, CME.GC
...
Strategy params (3): PivMult, ATRMult, ReentryBars (daily-pivot breakout). Date ranges by asset class.
Each instrument writes results/<sub>_pivot_hourly_pipeline/state.json (+ CSVs).
...
mismatch). set_signal_statuses retries the M5/M6 checkbox-revert + dialog-open flakiness.

CLI:
  py run_pivot_allinst_pipeline.py                    # all 6 instruments x 4 stages (auto-elevates)
  py run_pivot_allinst_pipeline.py --probe-windows    # read-only: dump open chart titles
  py run_pivot_allinst_pipeline.py --instrument "BTCUSDT HOT" [--from-stage N]
  py run_pivot_allinst_pipeline.py --resume-gaps      # re-run only the instruments that failed
  py run_pivot_allinst_pipeline.py --from-csv         # re-analyse existing CSVs, no MC64
"""
...
WORKSPACE = r"C:\Users\Tim\Downloads\Multichart64\Tim\SFJ_PivotBreakout_AI.wsp"
...
SIG_CRYPTO = "SFJ_PivotBreakout_crypto"
SIG_NQ     = "SFJ_PivotBreakout_NQ"

# (key, symbol, match-tokens, signal, is_range, full_range, target, out_subdir)
INSTRUMENTS = [
    ("btc", "BTCUSDT HOT", ["BTCUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "btc_pivot_hourly_pipeline"),
    ("eth", "ETHUSDT HOT", ["ETHUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL,  10_000.0, "eth_pivot_hourly_pipeline"),
    ("bnb", "BNBUSDT HOT", ["BNBUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "bnb_pivot_hourly_pipeline"),
    ("txf", "TWF.TXF HOT", ["TXF"],     SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "txf_pivot_hourly_pipeline"),
    ("nq",  "CME.NQ HOT",  ["CME.NQ"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "nq_pivot_hourly_pipeline"),
    ("gc",  "CME.GC HOT",  ["CME.GC"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "gc_pivot_hourly_pipeline"),
]

# ---- 3-param search space (PivMult, ATRMult, ReentryBars) ----
PIV_LO, PIV_HI = 0.05, 5.0
ATR_LO, ATR_HI = 0.5, 20.0
RE_LO,  RE_HI  = 0.0, 50.0
SEED = (0.5, 7.0, 13.0)   # PivMult, ATRMult, ReentryBars
CONV_THRESH = 0.005
MIN_ROUNDS, MAX_ROUNDS = 2, 5
P = ("PivMult", "ATRMult", "ReentryBars")
BOUNDS = {"PivMult": (PIV_LO, PIV_HI), "ATRMult": (ATR_LO, ATR_HI), "ReentryBars": (RE_LO, RE_HI)}
...
TARGET_NP = 0.0
OUTPUT_DIR: Path = RESULTS / "pivot_allinst_pipeline"
STATE_JSON: Path = OUTPUT_DIR / "state.json"

RESULTS.mkdir(parents=True, exist_ok=True)
(RESULTS / "pivot_allinst_pipeline").mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-8s -- %(message)s",
                    datefmt="%H:%M:%S",
                    handlers=[logging.StreamHandler(sys.stdout),
                              logging.FileHandler(str(RESULTS / "pivot_allinst_pipeline" /
...
# ----------------------------------------------------------------------------- helpers
def _seed_cfg():
    return StrategyConfig(name="PIV_seed", mc_signal_name=MAIN_SIGNAL, timeframe=TF, bar_period=BARP,
                          params=[ParamAxis("PivMult", 0.4, 0.6, 0.1), ParamAxis("ATRMult", 6.5, 7.5, 0.5),
                                  ParamAxis("ReentryBars", 11, 13, 1)],
...
def _main_cfg(name, Pv, A, Re):
    Pv = _safe(*Pv, PIV_LO, PIV_HI); A = _safe(*A, ATR_LO, ATR_HI); Re = _safe(*Re, RE_LO, RE_HI)
    combos = n_vals(*Pv) * n_vals(*A) * n_vals(*Re)
    if combos > 5000:
    ...
    return StrategyConfig(name=name, mc_signal_name=MAIN_SIGNAL, timeframe=TF, bar_period=BARP,
                          params=[ParamAxis("PivMult", *Pv), ParamAxis("ATRMult", *A),
                                  ParamAxis("ReentryBars", *Re)],
                          chart_workspace=WORKSPACE, chart_symbol=SYMBOL, insample=INSAMPLE)
...
def _row_dict(row):
    return {"PivMult": float(row["PivMult"]), "ATRMult": float(row["ATRMult"]),
...
def _stage1_round(label, attempts, seed, conn, from_csv):
    ...
    best = {"objective": -1e18}
    bP, bA, bRe = seed
    for nm, Pv, A, Re in attempts:
        cfg = _main_cfg(f"S1_{label}_{nm}", Pv, A, Re)
        ...
        if om["objective"] > best["objective"]:
            best = om; bP, bA, bRe = om["PivMult"], om["ATRMult"], om["ReentryBars"]
        log.info("  [%s %-16s] Obj-max Piv=%.4g Atr=%.4g Re=%.4g NP=%.0f MDD=%.0f Obj=%.4g tr=%d",
                 label, nm, om["PivMult"], om["ATRMult"], om["ReentryBars"], om["net_profit"],
                 om["max_drawdown"], om["objective"], om["total_trades"])
    for zi, (rP, sP, rA, sA, rRe, sRe) in enumerate(
            [(0.3, 0.05, 2.0, 0.5, 4, 1),
             (0.2, 0.05, 1.0, 0.25, 3, 1),
             (0.15, 0.025, 0.75, 0.25, 2, 1)], 1):
        Pz = (max(PIV_LO, _snap(bP - rP, sP)), min(PIV_HI, _snap(bP + rP, sP)), sP)
        Az = (max(ATR_LO, _snap(bA - rA, sA)), min(ATR_HI, _snap(bA + rA, sA)), sA)
        Rez = (max(RE_LO, _snap(bRe - rRe, sRe)), min(RE_HI, _snap(bRe + rRe, sRe)), sRe)
        cfg = _main_cfg(f"S1_{label}_zoom{zi}", Pz, Az, Rez)
        df = run_or_load(cfg, conn, from_csv)
        if df is None or df.empty or not _valid_main(df):
            continue
        d = _with_obj(df); pos = d[d["NetProfit"] > 0]
        ...
        if om["objective"] > best["objective"]:
            best = om; bP, bA, bRe = om["PivMult"], om["ATRMult"], om["ReentryBars"]
        log.info("  [%s zoom%d] Obj-max Piv=%.4g Atr=%.4g Re=%.4g Obj=%.4g", label, zi,
                 om["PivMult"], om["ATRMult"], om["ReentryBars"], om["objective"])
    return rows


def _confirm_attempts(sP, sA, sRe):
    return [
        ("01_retest", (max(PIV_LO, sP-0.2), sP+0.2, 0.05), (max(ATR_LO, sA-1), sA+1, 0.5), (max(RE_LO, sRe-2), sRe+2, 1)),
        ("02_piv_fine", (max(PIV_LO, sP-0.5), min(PIV_HI, sP+0.5), 0.05), (sA, sA, 0.5), (sRe, sRe, 1)),
        ("03_atr_fine", (sP, sP, 0.05), (max(ATR_LO, sA-3), min(ATR_HI, sA+3), 0.25), (sRe, sRe, 1)),
        ("04_re_fine", (sP, sP, 0.05), (sA, sA, 0.5), (0, 30, 1)),
        ("05_combo", (max(PIV_LO, sP-0.3), min(PIV_HI, sP+0.3), 0.1),
...
def stage1(conn, from_csv, state):
    ...
    r1_attempts = [
        ("01_global_wide", (0.1, 2.0, 0.1),   (2.0, 16.0, 2.0), (0, 30, 10)),
        ("02_classic",     (0.2, 1.0, 0.05),  (5.0, 9.0, 1.0),  (8, 20, 4)),
        ("03_piv_fine",    (0.05, 1.5, 0.05), (6.0, 8.0, 1.0),  (10, 16, 2)),
        ("04_atr_fine",    (0.3, 0.7, 0.1),   (2.0, 16.0, 0.5), (10, 16, 2)),
        ("05_reentry_fine",(0.3, 0.7, 0.1),   (6.0, 8.0, 1.0),  (0, 30, 1)),
        ("06_wide_trail",  (0.25, 1.5, 0.25), (8.0, 20.0, 1.0), (5, 20, 5)),
        ("07_low_piv",     (0.05, 0.6, 0.05), (4.0, 12.0, 1.0), (5, 20, 5)),
        ("08_high_piv",    (1.0, 3.0, 0.1),   (3.0, 10.0, 1.0), (0, 20, 5)),
        ("09_global_bound",(0.1, 5.0, 0.25),  (2.0, 18.0, 4.0), (0, 40, 20)),
    ]
    ...
    for rnd in range(2, MAX_ROUNDS + 1):
        ...
        s = (champ["PivMult"], champ["ATRMult"], champ["ReentryBars"])
    ...
    cands = []; seen = set()
    def _key(r): return (round(r["PivMult"], 3), round(r["ATRMult"], 2), round(r["ReentryBars"]))
    ...
    _save_state(state)
    log.info(">>> [%s] Stage1 Obj-max: Piv=%.4g Atr=%.4g Re=%.4g NP=%.0f Obj=%.4g tr=%d", SYMBOL,
             champ["PivMult"], champ["ATRMult"], champ["ReentryBars"],
             champ["net_profit"], champ["objective"], champ["total_trades"])
    return state


# ----------------------------------------------------------------------------- Stage 2
def _micro_main_cfg(name, Pv, A, Re):
    return StrategyConfig(name=name, mc_signal_name=MAIN_SIGNAL, timeframe=TF, bar_period=BARP,
                          params=[ParamAxis("PivMult", max(PIV_LO, Pv-0.05), Pv+0.05, 0.05),
                                  ParamAxis("ATRMult", max(ATR_LO, A-0.5), A+0.5, 0.5),
                                  ParamAxis("ReentryBars", max(RE_LO, Re-1), Re+1, 1.0)],
                          chart_workspace=WORKSPACE, chart_symbol=SYMBOL, insample=INSAMPLE)


def _pick_main(df, Pv, A, Re):
    m = df
    for nm, v in (("PivMult", Pv), ("ATRMult", A), ("ReentryBars", Re)):
        if nm in m.columns:
...
def stage2(conn, from_csv, state):
    ...
    for period, rng in (("is", IS_RANGE), ("full", FULL_RANGE)):
        ...
        for i, c in enumerate(cands):
            cfg = _micro_main_cfg(f"S2_{period}_C{i}", c["PivMult"], c["ATRMult"], c["ReentryBars"])
            df = run_or_load(cfg, conn, from_csv)
            row = _pick_main(df, c["PivMult"], c["ATRMult"], c["ReentryBars"]) if df is not None and not df.empty else None
            if row is not None:
    ...
    for i, c in enumerate(cands):
        ...
        rep.append({"idx": i, "params": {"PivMult": c["PivMult"], "ATRMult": c["ATRMult"], "ReentryBars": c["ReentryBars"]},
...
def _champ_tuple(champ):
    return (champ["PivMult"], champ["ATRMult"], champ["ReentryBars"])
...
def main():
    ...
    if args.probe_windows:
        mc.ensure_chart_ready(conn, StrategyConfig(name="probe", mc_signal_name=SIG_CRYPTO, timeframe=TF,
                              bar_period=BARP, params=[ParamAxis("PivMult", 0.4, 0.6, 0.1)],
