"""
run_rangefrac_allinst_pipeline.py -- ONE-BAT unattended 4-stage pipeline for
RangeFracBreakout run across SIX instruments living in ONE workspace
(RangeFracBreakout_AI.wsp), switching to the correct chart Window per instrument.

  crypto charts (signal RangeFracBreakout_crypto):  BTCUSDT, ETHUSDT, BNBUSDT
  futures charts (signal RangeFracBreakout_NQ):      TWF.TXF, CME.NQ, CME.GC
...
Strategy params (4): Length, BandMult, ATRMult, ReentryBars (range-fraction gate (levels in channel-width units; 1.0 = Donchian)
breakout). Date ranges by asset class. Each instrument writes results/<sub>_rangefrac_hourly_pipeline/
state.json (+ CSVs).
...
CLI:
  py run_rangefrac_allinst_pipeline.py                    # all 6 instruments x 4 stages (auto-elevates)
  py run_rangefrac_allinst_pipeline.py --probe-windows    # read-only: dump open chart titles
  py run_rangefrac_allinst_pipeline.py --instrument "BTCUSDT HOT" [--from-stage N]
  py run_rangefrac_allinst_pipeline.py --resume-gaps      # re-run any instrument missing a complete stage4
  py run_rangefrac_allinst_pipeline.py --from-csv         # re-analyse existing CSVs, no MC64
"""
...
WORKSPACE = r"C:\Users\Tim\Downloads\Multichart64\Tim\RangeFracBreakout_AI.wsp"
...
SIG_CRYPTO = "RangeFracBreakout_crypto"
SIG_NQ     = "RangeFracBreakout_NQ"

# (key, symbol, match-tokens, signal, is_range, full_range, target, out_subdir)
INSTRUMENTS = [
    ("btc", "BTCUSDT HOT", ["BTCUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "btc_rangefrac_hourly_pipeline"),
    ("eth", "ETHUSDT HOT", ["ETHUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL,  10_000.0, "eth_rangefrac_hourly_pipeline"),
    ("bnb", "BNBUSDT HOT", ["BNBUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "bnb_rangefrac_hourly_pipeline"),
    ("txf", "TWF.TXF HOT", ["TXF"],     SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "txf_rangefrac_hourly_pipeline"),
    ("nq",  "CME.NQ HOT",  ["CME.NQ"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "nq_rangefrac_hourly_pipeline"),
    ("gc",  "CME.GC HOT",  ["CME.GC"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "gc_rangefrac_hourly_pipeline"),
]

# ---- 4-param search space (Length, BandMult, ATRMult, ReentryBars) ----
LEN_LO, LEN_HI   = 2.0, 300.0
BAND_LO, BAND_HI = 0.3, 2.5     # channel-width fraction; 1.0 = exact Donchian
ATR_LO, ATR_HI   = 0.5, 20.0
RE_LO,  RE_HI    = 0.0, 50.0
SEED = (20.0, 1.0, 7.0, 13.0)  # band seed 1.0 = the Donchian point   # Length, BandMult, ATRMult, ReentryBars
CONV_THRESH = 0.005
...
TARGET_NP = 0.0
OUTPUT_DIR: Path = RESULTS / "rangefrac_allinst_pipeline"
STATE_JSON: Path = OUTPUT_DIR / "state.json"

RESULTS.mkdir(parents=True, exist_ok=True)
(RESULTS / "rangefrac_allinst_pipeline").mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-8s -- %(message)s",
                    datefmt="%H:%M:%S",
                    handlers=[logging.StreamHandler(sys.stdout),
                              logging.FileHandler(str(RESULTS / "rangefrac_allinst_pipeline" /
...
def stage1(conn, from_csv, state):
    ...
    r1_attempts = [
        ("01_global_wide", (5, 80, 5),   (0.4, 2.2, 0.3),  (2.0, 14.0, 2.0), (0, 20, 10)),
        ("02_classic",     (10, 40, 3),  (0.7, 1.3, 0.1),  (5.0, 9.0, 1.0),  (10, 20, 5)),
        ("03_len_fine",    (2, 60, 2),   (0.8, 1.2, 0.1),  (6.0, 8.0, 1.0),  (12, 16, 2)),
        ("04_band_fine",   (10, 30, 5),  (0.3, 2.5, 0.05), (6.0, 8.0, 1.0),  (12, 16, 2)),
        ("05_atr_fine",    (10, 30, 5),  (0.85, 1.15, 0.15),(3.0, 16.0, 0.5), (12, 16, 2)),
        ("06_reentry_fine",(10, 30, 5),  (0.85, 1.15, 0.15),(6.0, 8.0, 1.0),  (0, 30, 1)),
        ("07_wide_trail",  (5, 50, 5),   (0.5, 2.0, 0.25), (8.0, 20.0, 1.0), (5, 20, 5)),
        ("08_short_len",   (2, 20, 1),   (0.5, 2.0, 0.25), (5.0, 12.0, 1.0), (5, 20, 5)),
        ("09_global_bound",(5, 300, 20), (0.3, 2.5, 0.2),  (2.0, 18.0, 4.0), (0, 40, 20)),
    ]
