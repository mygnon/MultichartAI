"""
run_rcb_allinst_pipeline.py -- ONE-BAT unattended 4-stage pipeline for
SFJ_RegChannelBreakout run across SIX instruments living in ONE workspace
(SFJ_RegChannelBreakout_AI.wsp), switching to the correct chart Window per instrument.

  crypto charts (signal SFJ_RegChannelBreakout_crypto):  BTCUSDT, ETHUSDT, BNBUSDT
  futures charts (signal SFJ_RegChannelBreakout_NQ):      TWF.TXF, CME.NQ, CME.GC

Per instrument it runs all 4 stages:
  Stage 1  IS optimization (R1 broad + confirm rounds until converged, Obj-max)
  Stage 2  OOS champion-select among Stage-1 candidates (IS vs FULL)
  Stage 3  exit-module IS optimization (6 modules, main fixed)
  Stage 4  CUMULATIVE greedy full-period OOS stack (keep iff RoMaD up) + write final params
           back into Format Objects Input Strings.

Strategy params (4): RegLen, BandMult, ATRMult, ReentryBars. Date ranges by asset class.
Each instrument writes its own results/<sub>_rcb_hourly_pipeline/state.json (+ CSVs).

KEY: all 6 charts share one MC workspace window. mc.activate_chart_by_symbol() MDI-activates +
MAXIMIZES the target instrument's chart child before every wizard/dialog op so the wizard hits
the right chart; it read-back-verifies the active child title and ABORTs on mismatch.

CLI:
  py run_rcb_allinst_pipeline.py                      # all 6 instruments x 4 stages (auto-elevates)
  py run_rcb_allinst_pipeline.py --probe-windows      # read-only: dump the open chart titles
  py run_rcb_allinst_pipeline.py --instrument "BTCUSDT HOT"   # one instrument only
  py run_rcb_allinst_pipeline.py --instrument "BTCUSDT HOT" --from-stage 3
  py run_rcb_allinst_pipeline.py --from-csv           # re-analyse existing CSVs, no MC64
"""
...
WORKSPACE = r"C:\Users\Tim\Downloads\Multichart64\Tim\SFJ_RegChannelBreakout_AI.wsp"
INSAMPLE  = DateRange("2017/01/01", "2027/01/01")   # wide no-op; chart-trim is the control
TF, BARP  = "hourly", 60
...
SIG_CRYPTO = "SFJ_RegChannelBreakout_crypto"
SIG_NQ     = "SFJ_RegChannelBreakout_NQ"

# (key, symbol, match-tokens, signal, is_range, full_range, target, out_subdir)
INSTRUMENTS = [
    ("btc", "BTCUSDT HOT", ["BTCUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "btc_rcb_hourly_pipeline"),
    ("eth", "ETHUSDT HOT", ["ETHUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL,  10_000.0, "eth_rcb_hourly_pipeline"),
    ("bnb", "BNBUSDT HOT", ["BNBUSDT"], SIG_CRYPTO, CRYPTO_IS, CRYPTO_FULL, 100_000.0, "bnb_rcb_hourly_pipeline"),
    ("txf", "TWF.TXF HOT", ["TXF"],     SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "txf_rcb_hourly_pipeline"),
    ("nq",  "CME.NQ HOT",  ["CME.NQ"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "nq_rcb_hourly_pipeline"),
    ("gc",  "CME.GC HOT",  ["CME.GC"],  SIG_NQ,     FUT_IS,    FUT_FULL, 10_000_000.0, "gc_rcb_hourly_pipeline"),
]

# ---- 4-param search space (shared across instruments) ----
REG_LO, REG_HI   = 2.0, 300.0
BAND_LO, BAND_HI = 0.25, 6.0
ATR_LO, ATR_HI   = 0.5, 20.0
RE_LO,  RE_HI    = 0.0, 50.0
SEED = (20.0, 1.5, 7.0, 13.0)   # RegLen, BandMult, ATRMult, ReentryBars
CONV_THRESH = 0.005
MIN_ROUNDS, MAX_ROUNDS = 2, 5
P = ("RegLen", "BandMult", "ATRMult", "ReentryBars")
BOUNDS = {"RegLen": (REG_LO, REG_HI), "BandMult": (BAND_LO, BAND_HI),
...
TARGET_NP = 0.0
OUTPUT_DIR: Path = RESULTS / "rcb_allinst_pipeline"
STATE_JSON: Path = OUTPUT_DIR / "state.json"

RESULTS.mkdir(parents=True, exist_ok=True)
(RESULTS / "rcb_allinst_pipeline").mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-8s -- %(message)s",
                    datefmt="%H:%M:%S",
                    handlers=[logging.StreamHandler(sys.stdout),
                              logging.FileHandler(str(RESULTS / "rcb_allinst_pipeline" /
...
# ----------------------------------------------------------------------------- helpers
def _seed_cfg():
    return StrategyConfig(name="RCB_seed", mc_signal_name=MAIN_SIGNAL, timeframe=TF, bar_period=BARP,
                          params=[ParamAxis("RegLen", 18, 22, 2), ParamAxis("BandMult", 1.0, 2.0, 0.5),
...
def _activate(conn):
    """Bring workspace to front, close any blocking report, then MDI-activate+maximize the
    current instrument's chart so the next wizard/dialog op hits the RIGHT chart."""
    if conn is None:
        return
...
def _main_cfg(name, R, B, A, Re):
    R = _safe(*R, REG_LO, REG_HI); B = _safe(*B, BAND_LO, BAND_HI)
    A = _safe(*A, ATR_LO, ATR_HI); Re = _safe(*Re, RE_LO, RE_HI)
    combos = n_vals(*R) * n_vals(*B) * n_vals(*A) * n_vals(*Re)
    if combos > 5000:
    ...
    return StrategyConfig(name=name, mc_signal_name=MAIN_SIGNAL, timeframe=TF, bar_period=BARP,
                          params=[ParamAxis("RegLen", *R), ParamAxis("BandMult", *B),
...
def run_or_load(cfg, conn, from_csv):
    ...
    for attempt in (1, 2):
        try:
            _activate(conn)       # re-assert correct chart before every wizard launch
        except Exception as e:
...
def _row_dict(row):
    return {"RegLen": float(row["RegLen"]), "BandMult": float(row["BandMult"]),
...
def _stage1_round(label, attempts, seed, conn, from_csv):
    ...
    best = {"objective": -1e18}
    bR, bB, bA, bRe = seed
    for nm, R, B, A, Re in attempts:
        cfg = _main_cfg(f"S1_{label}_{nm}", R, B, A, Re)
        ...
        if om["objective"] > best["objective"]:
            best = om; bR, bB, bA, bRe = om["RegLen"], om["BandMult"], om["ATRMult"], om["ReentryBars"]
        log.info("  [%s %-16s] Obj-max Reg=%.4g Band=%.4g Atr=%.4g Re=%.4g NP=%.0f MDD=%.0f Obj=%.4g tr=%d",
                 label, nm, om["RegLen"], om["BandMult"], om["ATRMult"], om["ReentryBars"],
                 om["net_profit"], om["max_drawdown"], om["objective"], om["total_trades"])
    for zi, (rR, sR, rB, sB, rA, sA, rRe, sRe) in enumerate(
            [(4, 1, 0.375, 0.125, 1.5, 0.5, 4, 1),
             (3, 1, 0.25, 0.125, 1.0, 0.25, 3, 1),
             (2, 1, 0.25, 0.0625, 0.75, 0.25, 2, 1)], 1):
        Rz = (max(REG_LO, _snap(bR - rR, sR)), min(REG_HI, _snap(bR + rR, sR)), sR)
        ...
        cfg = _main_cfg(f"S1_{label}_zoom{zi}", Rz, Bz, Az, Rez)
        df = run_or_load(cfg, conn, from_csv)
        if df is None or df.empty or not _valid_main(df):
            continue
        d = _with_obj(df); pos = d[d["NetProfit"] > 0]
        ...
        if om["objective"] > best["objective"]:
            best = om; bR, bB, bA, bRe = om["RegLen"], om["BandMult"], om["ATRMult"], om["ReentryBars"]
        log.info("  [%s zoom%d] Obj-max Reg=%.4g Band=%.4g Atr=%.4g Re=%.4g Obj=%.4g", label, zi,
                 om["RegLen"], om["BandMult"], om["ATRMult"], om["ReentryBars"], om["objective"])
    return rows


def _confirm_attempts(sR, sB, sA, sRe):
    return [
        ("01_retest", (max(REG_LO, sR-2), sR+2, 1), (max(BAND_LO, sB-0.5), sB+0.5, 0.25),
         (max(ATR_LO, sA-1), sA+1, 0.5), (max(RE_LO, sRe-2), sRe+2, 1)),
        ("02_reg_fine", (max(REG_LO, sR-8), sR+8, 1), (sB, sB, 0.25), (sA, sA, 0.5), (sRe, sRe, 1)),
        ("03_band_fine", (sR, sR, 1), (max(BAND_LO, sB-1), min(BAND_HI, sB+1), 0.125), (sA, sA, 0.5), (sRe, sRe, 1)),
        ("04_atr_fine", (sR, sR, 1), (sB, sB, 0.25), (max(ATR_LO, sA-3), min(ATR_HI, sA+3), 0.25), (sRe, sRe, 1)),
        ("05_re_fine", (sR, sR, 1), (sB, sB, 0.25), (sA, sA, 0.5), (0, 30, 1)),
        ("06_combo", (max(REG_LO, sR-6), sR+6, 2), (max(BAND_LO, sB-0.75), min(BAND_HI, sB+0.75), 0.25),
...
def stage1(conn, from_csv, state):
    ...
    r1_attempts = [
        ...
        ("02_classic",     (10, 40, 3),  (1.0, 2.5, 0.25), (5.0, 9.0, 1.0),  (10, 20, 5)),
        ("03_reg_fine",    (2, 60, 2),   (1.25, 2.0, 0.25),(6.0, 8.0, 1.0),  (12, 16, 2)),
        ("04_band_fine",   (10, 30, 5),  (0.5, 3.0, 0.125),(6.0, 8.0, 1.0),  (12, 16, 2)),
        ("05_atr_fine",    (10, 30, 5),  (1.25, 2.0, 0.25),(3.0, 16.0, 0.5), (12, 16, 2)),
        ("06_reentry_fine",(10, 30, 5),  (1.25, 2.0, 0.25),(6.0, 8.0, 1.0),  (0, 30, 1)),
        ("07_wide_trail",  (5, 50, 5),   (1.0, 2.0, 0.5),  (8.0, 20.0, 1.0), (5, 20, 5)),
        ("08_short_reg",   (2, 20, 1),   (1.0, 2.5, 0.5),  (5.0, 12.0, 1.0), (5, 20, 5)),
        ("09_global_bound",(5, 300, 20), (0.5, 4.0, 0.5),  (2.0, 18.0, 4.0), (0, 40, 20)),
    ]
    ...
    for rnd in range(2, MAX_ROUNDS + 1):
        ...
        s = (champ["RegLen"], champ["BandMult"], champ["ATRMult"], champ["ReentryBars"])
    ...
    cands = []; seen = set()
    def _key(r): return (round(r["RegLen"]), round(r["BandMult"], 3), round(r["ATRMult"], 2), round(r["ReentryBars"]))
    ...
    _save_state(state)
    log.info(">>> [%s] Stage1 Obj-max: Reg=%.4g Band=%.4g Atr=%.4g Re=%.4g NP=%.0f Obj=%.4g tr=%d", SYMBOL,
             champ["RegLen"], champ["BandMult"], champ["ATRMult"], champ["ReentryBars"],
             champ["net_profit"], champ["objective"], champ["total_trades"])
    return state


# ----------------------------------------------------------------------------- Stage 2
def _micro_main_cfg(name, R, B, A, Re):
    return StrategyConfig(name=name, mc_signal_name=MAIN_SIGNAL, timeframe=TF, bar_period=BARP,
                          params=[ParamAxis("RegLen", max(REG_LO, R-1), R+1, 1.0),
...
def _pick_main(df, R, B, A, Re):
    m = df
    for nm, v in (("RegLen", R), ("BandMult", B), ("ATRMult", A), ("ReentryBars", Re)):
        if nm in m.columns:
...
def stage2(conn, from_csv, state):
    ...
    for period, rng in (("is", IS_RANGE), ("full", FULL_RANGE)):
        ...
        for i, c in enumerate(cands):
            ...
            df = run_or_load(cfg, conn, from_csv)
            row = _pick_main(df, c["RegLen"], c["BandMult"], c["ATRMult"], c["ReentryBars"]) if df is not None and not df.empty else None
            if row is not None:
    ...
    for i, c in enumerate(cands):
        ...
        rep.append({"idx": i, "params": {"RegLen": c["RegLen"], "BandMult": c["BandMult"],
...
def _set_signal_inputs(conn, signal, params):
    cfg = StrategyConfig(name=f"SET_{signal[:20]}", mc_signal_name=signal, timeframe=TF,
                         ...
                         chart_workspace=WORKSPACE, chart_symbol=SYMBOL, insample=INSAMPLE)
    last = None
    for a in range(1, 4):       # retry: Format Objects occasionally fails to open
        try:
...
def _champ_tuple(champ):
    return (champ["RegLen"], champ["BandMult"], champ["ATRMult"], champ["ReentryBars"])
...
def main():
    ...
    if args.probe_windows:
        mc.ensure_chart_ready(conn, StrategyConfig(name="probe", mc_signal_name=SIG_CRYPTO, timeframe=TF,
                              bar_period=BARP, params=[ParamAxis("RegLen", 18, 22, 2)],
        ...
        return 0
    # plan = list of (instrument-entry, from_stage)
    if args.resume_gaps:
        gaps = {"btc": 1, "bnb": 4, "nq": 4}
        plan = [(i, gaps[i[0]]) for i in INSTRUMENTS if i[0] in gaps]
    else:
        todo = INSTRUMENTS
        if args.instrument:
