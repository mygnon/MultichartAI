"""
search_bnb_adxtrend_hourly_oos_champion_select.py -- OOS selection among the converged
BNB Hourly SFJ_ADXtrend_crypto champions (R1-R2, results/bnb_adxtrend_hourly{,2}_search/).

3 params DMILen, ADXThresh, ATRMult; ADX-gated DMI crossover + ATR(14) chandelier trailing exit,
long+short, _Crypto1MUSD. R2 ceiling = 2nd-strongest non-CT BNB strategy (~ Donchian):
  best (B10)  DMILen=12 ADXThresh=26 ATRMult=7.8 (NP 23938 MDD-6658 155tr Obj 86068)
  stable      DMILen=12 ADXThresh=26 ATRMult=8.0 (NP 22286 MDD-6886 152tr, 5-conv)
ADX gate confirmed active (26 is the true peak); wide ATR trail (~8); the only self-authored
strategy where the trend-strength FILTER genuinely helps.
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_ADXtrend_crypto_AI.wsp open, BNBUSDT HOT HOURLY chart tab ACTIVE,
    SFJ_ADXtrend_crypto signal ON. Full data to 2026/06/10. Binance source connected.
    CLOSE the Study Editor. (Uses the mc_automation Settings-tab retry fix.)

CLI:
  py search_bnb_adxtrend_hourly_oos_champion_select.py                  # merged verdict
  py search_bnb_adxtrend_hourly_oos_champion_select.py --candidate AD1  # smoke one
  py search_bnb_adxtrend_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_adxtrend_hourly_oos_champion_select_search")
PREFIX      = "BNBADXSEL_"
...
STEP = {"DMILen": 1.0, "ADXThresh": 1.0, "ATRMult": 0.5}
LO   = {"DMILen": 2.0, "ADXThresh": 5.0, "ATRMult": 0.5}
HI   = {"DMILen": 200.0, "ADXThresh": 60.0, "ATRMult": 20.0}

# Converged BNB Hourly ADXtrend candidates (bnb_adxtrend_hourly{,2}_search).
# id, regime, DMILen, ADXThresh, ATRMult, prior IS NP, prior IS MDD.
CANDIDATES = [
    ("AD1", "R2 best ***",          12.0, 26.0, 7.8, 23938.0, -6658.0),  # 155tr highest NP/Obj
    ("AD2", "high-ADX low-freq",    14.0, 34.0, 8.0, 11250.0, -4837.0),  # 69tr, tight IS MDD
    ("AD3", "short-DMILen hi-freq",  5.0, 30.0, 8.0, 16791.0, -5886.0),  # 193tr, distinct
    ("AD4", "long-DMILen low-MDD",  24.0, 22.0, 8.5, 11436.0, -4295.0),  # 103tr, lowest IS MDD
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_adxtrend_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, dmilen, adxth, mult) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="hourly",
        bar_period=60,
        params=[_axis("DMILen", dmilen), _axis("ADXThresh", adxth), _axis("ATRMult", mult)],
...
def _pick(df, dmilen, adxth, mult):
    m = df
    for nm, v in (("DMILen", dmilen), ("ADXThresh", adxth), ("ATRMult", mult)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_adxtrend_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_adxtrend_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("AD1", "is", *CANDIDATES[0][2:5]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly ADXtrend -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
        ...
        for (cid, regime, dmilen, adxth, mult, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, dmilen, adxth, mult)
            log.info("--- [%s] %s %s DMILen=%g ADXThresh=%g ATRMult=%g [%s] ---",
                     period, cid, regime, dmilen, adxth, mult, datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"DMILen": dmilen, "ADXThresh": adxth, "ATRMult": mult},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, dmilen, adxth, mult) if (df is not None and not df.empty) else None
            if row is not None:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly SFJ_ADXtrend_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="ADn", default=None)
