"""
search_bnb_bbsqueeze_hourly_oos_champion_select.py -- OOS selection among the converged
BNB Hourly SFJ_BBSqueeze_crypto champions (R1-R2, results/bnb_bbsqueeze_hourly{,2}_search/).

4 params BBLen, BBmult, SqueezeLen, ATRMult; Bollinger squeeze breakout (volatility-regime filter)
+ ATR(14) chandelier trailing exit, long+short, _Crypto1MUSD. R2 ceiling = 3rd-strongest self-authored:
  best (3-conv) BBLen=14 BBmult=1.5 SqueezeLen=50 ATRMult=8 (NP 16119 MDD-7058 185tr Obj 36811)
Wide ATR trail (~8) does the work; squeeze filter (SqueezeLen~50) active. < Donchian/ADXtrend.
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_BBSqueeze_crypto_AI.wsp open, BNBUSDT HOT HOURLY chart tab ACTIVE,
    SFJ_BBSqueeze_crypto signal ON. Full data to 2026/06/10. CLOSE the Study Editor.

CLI:
  py search_bnb_bbsqueeze_hourly_oos_champion_select.py                  # merged verdict
  py search_bnb_bbsqueeze_hourly_oos_champion_select.py --candidate BS1  # smoke one
  py search_bnb_bbsqueeze_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_bbsqueeze_hourly_oos_champion_select_search")
PREFIX      = "BNBBBSSEL_"
...
STEP = {"BBLen": 1.0, "BBmult": 0.25, "SqueezeLen": 1.0, "ATRMult": 0.5}
LO   = {"BBLen": 2.0, "BBmult": 0.5, "SqueezeLen": 5.0, "ATRMult": 0.5}
HI   = {"BBLen": 200.0, "BBmult": 6.0, "SqueezeLen": 300.0, "ATRMult": 20.0}

# Converged BNB Hourly BBSqueeze candidates (bnb_bbsqueeze_hourly{,2}_search).
# id, regime, BBLen, BBmult, SqueezeLen, ATRMult, prior IS NP, prior IS MDD.
CANDIDATES = [
    ("BS1", "R2 best ***",          14.0, 1.5,  50.0,  8.0, 16119.0, -7058.0),  # 185tr highest NP/Obj
    ("BS2", "ultra-wide-M low-freq",12.0, 1.75, 40.0, 15.0, 13496.0, -5832.0),  # 78tr lowest IS MDD
    ("BS3", "R2 alt Sqz=45",        14.0, 1.5,  45.0,  8.0, 14437.0, -7045.0),  # 186tr
    ("BS4", "long-BBLen",           55.0, 2.0,  20.0,  8.0, 12856.0, -7252.0),  # 162tr distinct
]
...
def _cfg(cid, period, bblen, bbmult, sqzlen, mult) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="hourly",
        bar_period=60,
        params=[_axis("BBLen", bblen), _axis("BBmult", bbmult),
                _axis("SqueezeLen", sqzlen), _axis("ATRMult", mult)],
...
def _pick(df, bblen, bbmult, sqzlen, mult):
    m = df
    for nm, v in (("BBLen", bblen), ("BBmult", bbmult), ("SqueezeLen", sqzlen), ("ATRMult", mult)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_bbsqueeze_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_bbsqueeze_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BS1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly BBSqueeze -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
        ...
        for (cid, regime, bblen, bbmult, sqzlen, mult, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, bblen, bbmult, sqzlen, mult)
            log.info("--- [%s] %s %s BBLen=%g BBm=%g Sqz=%g M=%g [%s] ---",
                     period, cid, regime, bblen, bbmult, sqzlen, mult,
                     datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"BBLen": bblen, "BBmult": bbmult, "SqueezeLen": sqzlen, "ATRMult": mult},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, bblen, bbmult, sqzlen, mult) if (df is not None and not df.empty) else None
            if row is not None:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly SFJ_BBSqueeze_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="BSn", default=None)
