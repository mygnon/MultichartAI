"""
search_bnb_bollingerbreakout_daily_oos_champion_select.py -- OOS selection among the converged
BNB Daily SFJ_BollingerBreakout_crypto champions (R1-R2, results/bnb_bollingerbreakout_daily{,2}_search/).

4 params BBLen, BBmult, ATRMult, ReentryBars; pure Bollinger-band breakout STOP + cooldown + ATR chandelier exit,
long+short, _Crypto1MUSD. BNB DAILY R2 ceiling ~$25.6-27.7K (BBLen=2-3 BBmult=1 ATRMult=5 Re=12, sparse 11-13tr):
STRONGEST cell of whole matrix, Daily>Hourly +20-30% (BNB is the Daily-works coin). R1 BBLen=3 $27,720 vs
R2 BBLen=2 $25,576 (Rule 5 drift). BNB Hourly already OOS-PASS (2); Daily check (sparse).
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, BNBUSDT HOT DAILY chart tab ACTIVE,
    SFJ_BollingerBreakout_crypto signal ON. Full data to 2026/06/10. Binance source connected.
    CLOSE the Study Editor. (Uses the mc_automation Settings-tab retry fix.)

CLI:
  py search_bnb_bollingerbreakout_daily_oos_champion_select.py                  # merged verdict
  py search_bnb_bollingerbreakout_daily_oos_champion_select.py --candidate BD1  # smoke one
  py search_bnb_bollingerbreakout_daily_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_bollingerbreakout_daily_oos_champion_select_search")
PREFIX      = "BNBBODSEL_"
...
# BNB Daily BollingerBreakout candidates (bnb_bollingerbreakout_daily{,2}; strongest cell $25-28K, sparse 11-28tr).
# id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday).
CANDIDATES = [
    # id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (sparse 11-28tr)
    ("BD1", "R1 champ (13tr) ***", 3.0, 1.0,  5.0, 12.0, 27720.0, -6839.0),  # R1 NP/Obj-max
    ("BD2", "R2 champ BBLen=2 (11tr)", 2.0, 1.0,  5.0, 12.0, 25576.0, -6839.0),  # R2 re-measured (Rule5 drift)
    ("BD3", "high-freq (28tr)",     2.0, 0.75, 3.9, 7.0,  16619.0, -9178.0),  # densest = OOS-law candidate
    ("BD4", "longer-BBLen (11tr)",  6.0, 1.0,  5.0, 12.0, 26828.0, -7400.0),  # distinct longer BBLen
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_bollingerbreakout_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_bollingerbreakout_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_bollingerbreakout_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BD1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Daily BollingerBreakout -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Daily SFJ_BollingerBreakout_crypto OOS champion selection")
