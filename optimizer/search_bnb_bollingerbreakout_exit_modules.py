"""
search_bnb_bollingerbreakout_exit_modules.py -- Exit-module independent optimization on top of the
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, BNBUSDT HOT HOURLY tab active
...
CLI:
  py search_bnb_bollingerbreakout_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_bnb_bollingerbreakout_exit_modules.py --module 3       # only module 3
  py search_bnb_bollingerbreakout_exit_modules.py --module 0       # baseline A00 only
  py search_bnb_bollingerbreakout_exit_modules.py --smoke --module 1
  py search_bnb_bollingerbreakout_exit_modules.py --manual-status  # user toggles Status by hand
  py search_bnb_bollingerbreakout_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_bnb_bollingerbreakout_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_bollingerbreakout_exit_modules_search")
...
# Reference baseline = BO2 main-only IS (bnb_bollingerbreakout_hourly_oos_champion_select, IS pass)
BASELINE_REF = {"net_profit": 18628.0, "max_drawdown": -4814.0, "total_trades": 233}
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_bollingerbreakout_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    ...
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("BBLen",       7.0,  9.0,  1.0),
            ParamAxis("BBmult",      1.45, 1.55, 0.05),
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs BBLen=8 BBmult=1.5 ATRMult=7 ReentryBars=14)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_bnb_bollingerbreakout_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_bnb_bollingerbreakout_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("  Baseline ref (BO2 IS): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
