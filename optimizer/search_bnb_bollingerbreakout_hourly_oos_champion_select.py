"""
search_bnb_bollingerbreakout_hourly_oos_champion_select.py -- OOS selection among the converged
...
long+short, _Crypto1MUSD. R2 ceiling $21,210 = #2 self-authored by Obj (95,486):
BBLen=13 BBmult=1.55 ATRMult=7 ReentryBars=13; statistical band beats Donchian channel on
risk-adjusted return (lowest MDD -$4,711); ReentryBars cooldown engaged.
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, BNBUSDT HOT HOURLY chart tab ACTIVE,
...
CLI:
  py search_bnb_bollingerbreakout_hourly_oos_champion_select.py                  # merged verdict
  py search_bnb_bollingerbreakout_hourly_oos_champion_select.py --candidate BO1  # smoke one
  py search_bnb_bollingerbreakout_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_bollingerbreakout_hourly_oos_champion_select_search")
PREFIX      = "BNBBOSEL_"
...
# Converged BNB Hourly BollingerBreakout candidates (bnb_bollingerbreakout_hourly{,2}_search).
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_bollingerbreakout_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_bollingerbreakout_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_bollingerbreakout_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BO1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly BollingerBreakout -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly SFJ_BollingerBreakout_crypto OOS champion selection")
