"""
search_bnb_closechannelbreakout_exit_modules.py -- Exit-module independent optimization on top of the
SFJ_BollingerBreakout_crypto BNB Hourly champion.

Main signal FIXED at BO2 (short-BBLen, OOS-high-profit regime: BBLen=8 BBmult=1.5 ATRMult=7
ReentryBars=14), Status ON. Six exit modules tested INDEPENDENTLY (one enabled at a time),
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260622_SFJ_CloseChannelBreakout_crypto_AI.wsp open, BNBUSDT HOT HOURLY tab active
  - Main signal SFJ_BollingerBreakout_crypto inputs set to BBLen=8 BBmult=1.5 ATRMult=7 ReentryBars=14 (BO2), Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved
  - CLOSE the Study Editor window

CLI:
  py search_bnb_closechannelbreakout_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_bnb_closechannelbreakout_exit_modules.py --module 3       # only module 3
  py search_bnb_closechannelbreakout_exit_modules.py --module 0       # baseline A00 only
  py search_bnb_closechannelbreakout_exit_modules.py --smoke --module 1
  py search_bnb_closechannelbreakout_exit_modules.py --manual-status  # user toggles Status by hand
  py search_bnb_closechannelbreakout_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_bnb_closechannelbreakout_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_closechannelbreakout_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # signal date no-op; chart-trim is the control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# BO2 (short-BBLen, OOS-high-profit regime) — main signal set to these in MC64 (Format Objects, Status ON)
CHAMPION = {
    "Length":      8.0,
    "ATRMult":     7.0,
    "ReentryBars": 14.0,
}
# Reference baseline = CC1 main-only IS (bnb_closechannelbreakout_hourly{,2} R1/R2 10-conv champion)
BASELINE_REF = {"net_profit": 22008.1, "max_drawdown": -4922.57, "total_trades": 226}
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_closechannelbreakout_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    # 81-combo micro-grid around BO2: all 4 params vary (Critical Rule 1);
    # grid points include the EXACT BO2 8/1.5/7/14 so A00 reads main-only at BO2.
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("Length",      7.0,  9.0,  1.0),
            ParamAxis("ATRMult",     6.5,  7.5,  0.5),
            ParamAxis("ReentryBars", 13.0, 15.0, 1.0),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs Length=8 ATRMult=7 ReentryBars=14)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_bnb_closechannelbreakout_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_bnb_closechannelbreakout_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly DonchianATR v2 exit-module optimization%s", " [SMOKE]" if smoke else "")
    log.info("  Main fixed: BBLen=8 BBmult=1.5 ATRMult=7 ReentryBars=14 (BO2, Status ON)")
    log.info("  Baseline ref (CC1 IS): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
...
def main():
    ap = argparse.ArgumentParser(description="BNB DonchianATR v2 exit-module independent optimization")
