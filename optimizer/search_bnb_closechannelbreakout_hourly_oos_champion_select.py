"""
search_bnb_closechannelbreakout_hourly_oos_champion_select.py -- OOS selection among the converged
...
CLI:
  py search_bnb_closechannelbreakout_hourly_oos_champion_select.py                  # merged verdict
  py search_bnb_closechannelbreakout_hourly_oos_champion_select.py --candidate VB1  # smoke one
  py search_bnb_closechannelbreakout_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_closechannelbreakout_hourly_oos_champion_select_search")
PREFIX      = "BNBCCBSEL_"
...
CANDIDATES = [
    ("CC1", "champion (Obj #1) ***",   8.0,  7.0,  14.0, 22008.0, -4922.6),  # R1/R2 10-conv NP/Obj-max, 226tr
    ("CC2", "mid-len high-freq",       13.0, 7.0,  15.0, 16977.0, -6623.0),  # R1 A07, 218tr
    ("CC3", "short-len zoom",          10.0, 7.0,  12.0, 19124.0, -5519.0),  # R1/R2 zoom1, 227tr
    ("CC4", "wide-trail low-freq",     15.0, 19.5, 15.0, 14869.0, -7170.0),  # R1 A06 wide trail, 46tr
]
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_closechannelbreakout_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_closechannelbreakout_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_closechannelbreakout_hourly_oos_champion_select.json"
