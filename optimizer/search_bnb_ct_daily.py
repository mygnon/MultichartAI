"""
search_bnb_ct_daily.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BNBUSDT HOT Daily, Round 1
...
attempts (DTN_DATETIMECHANGE nudge fix makes it apply).  insample stays WIDE = no-op.

Objective = NetProfit^2 / |MaxDrawdown| (plateau.compute_objective); target NP > 10,000 USD.

Priors (no BNB Daily CT prior — first time):
  BNB Hourly CT ceiling $36,703 — ASYMMETRIC long-LL / short-LS: NP-max LL=222-236 SL=3.8 LS=22 SS=4.2
    (10tr); Obj-max LL=122 SL=4.025 LS=29 SS=4.2 (24tr).  BNB ~8.8x BTC notional.
  BTC Daily CT LL=49 SL=2.4 LS=123 SS=1.2 (16tr, low-freq long-LS).
  ETH Daily CT LL=5 SL=1.75 LS=37 SS=1.375 (47tr, ultra-short-LL high-freq).
  Futures CT Daily: NQ LL=8 LS=47 / GC LL=3 LS=50.
...
R1 Plan (11 attempts, <=5000 combos each):
  ...
  A02 bnb_asym_longLL      LL(60-240 s20)  SL(3.0-4.5 s0.25) LS(12-40 s2)    SS(3.8-4.4 s0.2)   = 4200
  A03 btc_daily_regime     LL(30-70 s5)    SL(1.5-3.0 s0.25) LS(100-150 s5)  SS(0.8-2.0 s0.2)   = 4851
  A04 eth_daily_hifreq     LL(2-15 s1)     SL(1.0-2.5 s0.5)  LS(20-60 s5)    SS(0.8-2.0 s0.2)   = 3528
  A05 mid_period           LL(15-60 s5)    SL(1.0-4.0 s0.5)  LS(15-60 s5)    SS(1.0-4.0 s0.5)   = 4900
  A06 high_stddev          LL(10-90 s10)   SL(3.5-6.0 s0.5)  LS(10-90 s10)   SS(3.5-6.0 s0.5)   = 2916
  A07 futures_daily        LL(2-14 s2)     SL(0.5-2.0 s0.25) LS(35-60 s5)    SS(1.0-3.0 s0.25)  = 2646
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ct_daily_search")
...
# No BNB Daily prior — neutral seed at signal defaults (LENGTH=46, STDDEV=3.8)
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_ct_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "priors":   "BNB Hourly CT $36,703 ASYM LL=222-236 LS=22 SS=4.2 (Obj-max LL=122 LS=29); BTC Daily LL=49 LS=123; ETH Daily LL=5 LS=37 hi-freq",
            "r1_plan":  "Global coarse + BNB-asym-longLL + BTC-daily-regime + ETH-daily-hifreq + mid period + high stddev + futures-daily + tight SL + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_ct_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Daily countertrend_LS_crypto NP>10K -- Round 1")
    log.info("  IS window %s ~ %s  (chart-trimmed)", IS_RANGE[0], IS_RANGE[1])
    log.info("  Priors: BNB Hourly ASYM LL=229 LS=22; BTC Daily LL=49 LS=123; ETH Daily LL=5 LS=37")
    log.info("==============================================================")
    ...
    attempts_config = [
        ...
        ("02_bnb_asym_longLL",   (60, 240, 20),  (3.0, 4.5, 0.25),  (12, 40, 2),    (3.8, 4.4, 0.2)),
        ("03_btc_daily_regime",  (30, 70, 5),    (1.5, 3.0, 0.25),  (100, 150, 5),  (0.8, 2.0, 0.2)),
        ("04_eth_daily_hifreq",  (2, 15, 1),     (1.0, 2.5, 0.5),   (20, 60, 5),    (0.8, 2.0, 0.2)),
        ("05_mid_period",        (15, 60, 5),    (1.0, 4.0, 0.5),   (15, 60, 5),    (1.0, 4.0, 0.5)),
        ("06_high_stddev",       (10, 90, 10),   (3.5, 6.0, 0.5),   (10, 90, 10),   (3.5, 6.0, 0.5)),
        ("07_futures_daily",     (2, 14, 2),     (0.5, 2.0, 0.25),  (35, 60, 5),    (1.0, 3.0, 0.25)),
    ...
    log.info("==============================================================")
    log.info("  BNB Daily countertrend_LS_crypto Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Daily countertrend_LS_crypto NP>10K Round-1 search")
