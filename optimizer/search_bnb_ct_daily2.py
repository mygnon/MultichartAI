"""
search_bnb_ct_daily2.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BNBUSDT HOT Daily, Round 2

R1 (search_bnb_ct_daily.py) champion: LL=24 SL=1.75 LS=23 SS=3 NP=$35,182 MDD=-$5,200 Obj=238,043 16tr
  (TARGET MET >10K).  A10=A11 converged but the mid-LL/short-LS regime was found late (A05) and the
  zoom was still nudging up (+1% A09->A10).  R2 confirms the peak + maps the basin + finds Obj-max.
  Long-LL (BNB Hourly asym), BTC-daily long-LS, and high-SS regions returned 0 rows in R1 (barren).
...
R2 Plan (11 attempts, <=5000 combos each) — confirm/refine the mid-LL short-LS peak:
  A01 LS_sweep        LL(22-26 s1)  SL(1.5-2.0 s0.1)  LS(12-40 s2)  SS(2.7-3.3 s0.1)   = 3150
  A02 SS_sweep        LL(22-26 s1)  SL(1.5-2.0 s0.1)  LS(20-26 s1)  SS(1.5-4.5 s0.2)   = 3360
  A03 SL_sweep        LL(22-26 s1)  SL(0.5-3.0 s0.1)  LS(20-26 s1)  SS(2.8-3.2 s0.1)   = 4550
  A04 LL_sweep        LL(10-45 s2)  SL(1.5-2.0 s0.1)  LS(20-26 s1)  SS(2.8-3.2 s0.1)   = 3780
  A05 longLS_check    LL(22-26 s1)  SL(1.5-2.0 s0.1)  LS(40-90 s5)  SS(2.5-3.5 s0.25)  = 1650
  A06 high_freq_alt   LL(2-10 s1)   SL(1.0-2.0 s0.2)  LS(35-65 s5)  SS(0.8-2.5 s0.2)   = 3402
  A07 mid_LL_wider    LL(15-40 s2)  SL(1.5-2.5 s0.2)  LS(15-35 s2)  SS(2.5-3.5 s0.25)  = 4290
  A08 obj_lowMDD      LL(20-30 s2)  SL(1.5-2.5 s0.2)  LS(18-30 s2)  SS(2.5-4.0 s0.25)  = 1764
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ct_daily2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_ct_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_champion": "LL=24 SL=1.75 LS=23 SS=3 NP=35182 MDD=-5200 Obj=238043 16tr (TARGET MET)",
            "r2_plan":  "Confirm mid-LL short-LS peak: LS/SS/SL/LL sweeps + longLS check + high-freq alt + mid-LL wider + obj-lowMDD + adaptive zoom. (Long-LL/long-LS/high-SS confirmed barren in R1.)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_ct_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Daily countertrend_LS_crypto NP>10K -- Round 2")
    log.info("  IS window %s ~ %s  (chart-trimmed)", IS_RANGE[0], IS_RANGE[1])
    log.info("  R1 champion LL=24 SL=1.75 LS=23 SS=3 NP=35182 (confirming peak)")
    log.info("==============================================================")

    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (22, 26, 1), (1.5, 2.0, 0.1),
                                         (20, 26, 1), (2.8, 3.2, 0.1)))
    ...
    attempts_config = [
        ("01_LS_sweep",       (22, 26, 1),   (1.5, 2.0, 0.1),   (12, 40, 2),    (2.7, 3.3, 0.1)),
        ("02_SS_sweep",       (22, 26, 1),   (1.5, 2.0, 0.1),   (20, 26, 1),    (1.5, 4.5, 0.2)),
        ("03_SL_sweep",       (22, 26, 1),   (0.5, 3.0, 0.1),   (20, 26, 1),    (2.8, 3.2, 0.1)),
        ("04_LL_sweep",       (10, 45, 2),   (1.5, 2.0, 0.1),   (20, 26, 1),    (2.8, 3.2, 0.1)),
        ("05_longLS_check",   (22, 26, 1),   (1.5, 2.0, 0.1),   (40, 90, 5),    (2.5, 3.5, 0.25)),
        ("06_high_freq_alt",  (2, 10, 1),    (1.0, 2.0, 0.2),   (35, 65, 5),    (0.8, 2.5, 0.2)),
        ("07_mid_LL_wider",   (15, 40, 2),   (1.5, 2.5, 0.2),   (15, 35, 2),    (2.5, 3.5, 0.25)),
        ("08_obj_lowMDD",     (20, 30, 2),   (1.5, 2.5, 0.2),   (18, 30, 2),    (2.5, 4.0, 0.25)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BNB Daily countertrend_LS_crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Daily countertrend_LS_crypto NP>10K Round-2 search")
