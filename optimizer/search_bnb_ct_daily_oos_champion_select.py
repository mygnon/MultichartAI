"""
search_bnb_ct_daily_oos_champion_select.py — Out-of-sample (OOS) selection among the converged
BNB *Daily* CT champions (rounds 1-3, results/bnb_ct_daily{,2,3,4}_search/).
...
TASK: re-test each previously-converged BTC Daily champion on this workspace and pick the
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_Bollinger_AI.wsp open, BNBUSDT HOT **DAILY** chart tab ACTIVE, the
...
CLI:
  py search_bnb_ct_daily_oos_champion_select.py                 # merged: IS then FULL, then verdict
  py search_bnb_ct_daily_oos_champion_select.py --period is     # one pass only
  py search_bnb_ct_daily_oos_champion_select.py --candidate BD1  # smoke one candidate
  py search_bnb_ct_daily_oos_champion_select.py --from-csv      # recompute from existing CSVs
  py search_bnb_ct_daily_oos_champion_select.py --probe-instrument  # verify auto data-range
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ct_daily_oos_champion_select_search")
PREFIX      = "BNBCTDSEL_"
...
# 4 DISTINCT converged BNB Daily regimes (verified vs bnb_ct_daily{,2,3,4}_search JSONs).
# id, regime, LL, SL, LS, SS, prior IS NP, prior IS MDD (= Max Intraday DD; reference only —
# prior chart start differed, so a fresh IS re-measure with drift is expected).
CANDIDATES = [
    ("BD1", "NP/Obj-max R3=R4 ***", 18.0, 2.175, 21.0, 3.15, 42546.0, -6645.0),  # mid-LL short-LS, 14tr champion
    ("BD2", "R1/R2 mid regime",     24.0, 1.75,  23.0, 3.0,  35182.0, -5200.0),  # 16tr (the false-ceiling regime)
    ("BD3", "long-LS low-MDD",      15.0, 1.5,   85.0, 3.5,  26990.0, -4942.0),  # 15tr, longer-LS (OOS-robust type)
    ("BD4", "ultra-high-freq",       4.0, 1.5,    9.0, 1.2,  23489.0, -4702.0),  # 109tr, distinct regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_ct_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_ct_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_ct_daily_oos_champion_select.json"
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Daily CT — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Daily CT OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="Dn", default=None)
