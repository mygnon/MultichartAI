"""
search_bnb_ct_hourly.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BNBUSDT HOT Hourly, Round 1

Priors from completed searches:
  BTC Hourly CT (R1-R5, ceiling $4,155): LL=104 SL=4.05 LS=165 SS=4.95 (22tr);
    Obj-max LL=104 SL=4.15 LS=68 SS=4.95
  ETH Hourly CT (R1-R3, ceiling $5,005): LL=111 SL=4.025 LS=115 SS=4.725 (25tr);
    Obj-max plateau LL 109-110 SL 4.3-4.4 LS 109-110 SS 4.7-4.75
  Crypto CT = LONG-PERIOD RARE-EXTREME regime (LL~104-111, SL~4.0-4.2, SS~4.7-5.0)
  BNB scaling on other strategies: BNB/BTC ~12-13x (QPATR_Breakout $35,634 vs $2,748;
    QPATRex $39,921 vs $3,293) -> BNB CT estimate ~$50K vs target $100K
  BUT BNB champions elsewhere had different Len regimes (Breakout NP-max Len=3 82tr;
    QPATRex Obj-max Len=94 188tr) — BNB may support higher-frequency regimes.
...
R1 Plan (11 attempts, <=5000 combos each):
  ...
  A02 BTCETH_regime     LL(85-130 s5)xSL(3.6-4.8 s0.2)xLS(60-130 s10)xSS(4.3-5.3 s0.25)   = 2800
  A03 longLS_regime     LL(85-130 s10)xSL(3.6-4.8 s0.2)xLS(130-210 s10)xSS(4.3-5.3 s0.25) = 1890
  A04 highfreq_short    LL(2-20 s2)xSL(1.0-4.0 s0.5)xLS(2-20 s2)xSS(1.0-4.0 s0.5)         = 4900
  ...
  A08 tight_SL_futures  LL(5-30 s5)xSL(0.1-1.0 s0.15)xLS(30-70 s5)xSS(0.2-2.0 s0.2)       = 3780
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ct_hourly_search")
...
# No BNB prior — neutral seed at signal defaults (LENGTH=46, STDDEV=3.8)
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_ct_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "priors":   "BTC CT $4,155 (LL=104 SL=4.05 LS=165 SS=4.95); ETH CT $5,005 (LL=111 SL=4.025 LS=115 SS=4.725); BNB/BTC ~12-13x on QPATRex/Breakout -> estimate ~$50K vs target $100K; BNB elsewhere has short-Len and mid-freq regimes too",
            "r1_plan":  "Global coarse + BTC/ETH regime + long-LS + high-freq short + mid/long period + high stddev + tight-SL futures + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_ct_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto NP>100K -- Round 1")
    log.info("  Priors: BTC CT $4,155 / ETH CT $5,005 (long-period regime)")
    log.info("          BNB/BTC ~12-13x elsewhere -> estimate ~$50K")
    log.info("==============================================================")
    ...
    attempts_config = [
        ...
        ("02_BTCETH_regime",    (85, 130, 5),   (3.6, 4.8, 0.2),   (60, 130, 10),  (4.3, 5.3, 0.25)),
        ("03_longLS_regime",    (85, 130, 10),  (3.6, 4.8, 0.2),   (130, 210, 10), (4.3, 5.3, 0.25)),
        ("04_highfreq_short",   (2, 20, 2),     (1.0, 4.0, 0.5),   (2, 20, 2),     (1.0, 4.0, 0.5)),
        ...
        ("08_tight_SL_futures", (5, 30, 5),     (0.1, 1.0, 0.15),  (30, 70, 5),    (0.2, 2.0, 0.2)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Hourly countertrend_LS_crypto NP>100K Round-1 search")
