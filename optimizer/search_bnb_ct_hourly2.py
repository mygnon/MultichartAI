"""
search_bnb_ct_hourly2.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BNBUSDT HOT Hourly, Round 2

R1 Summary (11 attempts):
  Champion ⭐ A10 (=A11): LL=123 SL=4.05 LS=29 SS=4.2 NP=$35,112.20 MDD=-$7,112.33 24tr Obj=173,342
  Plateau: LL 122-123, SL 4.025-4.05.
  NEW ASYMMETRIC regime unique to BNB: long LL (~123) + SHORT LS (~29)
    — BTC/ETH regimes both inferior on BNB (LS=60: $24.5K; LS=160: $23.0K).
  SS=4.2 lower than BTC (4.95) / ETH (4.725); SL~4.05 consistent across symbols.
  Futures tight-SL fails ($6,655 at 536tr). BNB/BTC 8.45x.
  Zoom still climbing: A01 $27.6K -> A09 $33.9K -> A10 $35.1K (+3.5%) -> A11 same.
  vs other BNB Hourly: QPATRex $39,921 / Breakout $35,634 / CT $35,112 (#3, within 2% of #2).

R2 Plan (11 attempts, <=5000 combos each):
  Converge champion + fine bridges on every gap (bridge rule: paid off on BTC R4 and ETH R2).

  A01 champion_microfine LL(118-128 s1)xSL(3.95-4.15 s0.05)xLS(26-32 s1)xSS(4.1-4.3 s0.05)  = 1925
  A02 LS_2_26            LL(110-135 s5)xSL(3.8-4.3 s0.1)xLS(2-26 s2)xSS(3.9-4.5 s0.1)       = 3276
  A03 LS_33_60_bridge    LL(110-135 s5)xSL(3.8-4.3 s0.1)xLS(33-60 s3)xSS(4.0-4.6 s0.1)      = 2520
  A04 SS_4p4_5p2         LL(115-130 s3)xSL(3.9-4.2 s0.05)xLS(25-35 s2)xSS(4.4-5.2 s0.05)    = 4284
  A05 SS_3p4_4p1         LL(115-130 s3)xSL(3.9-4.2 s0.05)xLS(25-35 s2)xSS(3.4-4.1 s0.05)    = 3780
  A06 SL_above_4p2       LL(110-135 s5)xSL(4.2-5.2 s0.1)xLS(25-35 s2)xSS(4.0-4.5 s0.1)      = 2376
  A07 SL_below_3p9       LL(110-135 s5)xSL(3.0-3.9 s0.1)xLS(25-35 s2)xSS(4.0-4.5 s0.1)      = 2160
  A08 LL_130_170         LL(130-170 s2)xSL(3.9-4.2 s0.1)xLS(25-35 s2)xSS(4.0-4.4 s0.1)      = 2520
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ct_hourly2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_ct_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result":"R1 champion A10=A11: LL=123 SL=4.05 LS=29 SS=4.2 NP=$35,112 MDD=-$7,112 24tr Obj=173,342; NEW asymmetric long-LL short-LS regime; BNB/BTC 8.45x",
            "r2_plan":  "Champion microfine + fine bridges (LS 2-26, 33-60; SS 3.4-4.1, 4.4-5.2; SL 3.0-3.9, 4.2-5.2; LL 130-170) + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_ct_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto NP>100K -- Round 2")
    log.info("  R1 champion: LL=123 SL=4.05 LS=29 SS=4.2 NP=$35,112 Obj=173,342")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_champion_microfine", (118, 128, 1),  (3.95, 4.15, 0.05), (26, 32, 1),   (4.1, 4.3, 0.05)),
        ("02_LS_2_26",            (110, 135, 5),  (3.8, 4.3, 0.1),    (2, 26, 2),    (3.9, 4.5, 0.1)),
        ("03_LS_33_60_bridge",    (110, 135, 5),  (3.8, 4.3, 0.1),    (33, 60, 3),   (4.0, 4.6, 0.1)),
        ("04_SS_4p4_5p2",         (115, 130, 3),  (3.9, 4.2, 0.05),   (25, 35, 2),   (4.4, 5.2, 0.05)),
        ("05_SS_3p4_4p1",         (115, 130, 3),  (3.9, 4.2, 0.05),   (25, 35, 2),   (3.4, 4.1, 0.05)),
        ("06_SL_above_4p2",       (110, 135, 5),  (4.2, 5.2, 0.1),    (25, 35, 2),   (4.0, 4.5, 0.1)),
        ("07_SL_below_3p9",       (110, 135, 5),  (3.0, 3.9, 0.1),    (25, 35, 2),   (4.0, 4.5, 0.1)),
        ("08_LL_130_170",         (130, 170, 2),  (3.9, 4.2, 0.1),    (25, 35, 2),   (4.0, 4.4, 0.1)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Hourly countertrend_LS_crypto NP>100K Round-2 search")
