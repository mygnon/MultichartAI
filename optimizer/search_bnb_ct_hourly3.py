"""
search_bnb_ct_hourly3.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BNBUSDT HOT Hourly, Round 3

R1+R2 Summary (22 attempts):
  Champion ⭐ 6-conv (R1 A10/A11 + R2 A01/A09/A10/A11):
    LL=122 SL=4.025-4.05 LS=29 SS=4.2 NP=$35,112.20 MDD=-$7,112.33 24tr Obj=173,342
  Champion is ALSO Obj-max — no separate high-SL Obj regime (unlike BTC/ETH):
    R2 A06 SL 4.2-5.2 best Obj only 122,314.
  R1->R2 gain +0.00%. All near bridges worse:
    LS 2-26 ($33.2K MDD -14.4K), LS 33-60 ($29.9K), SS 3.4-4.1 ($30.3K),
    SS 4.4-5.2 ($33.2K), SL 3.0-3.9 ($33.2K), SL 4.2-5.2 ($28.0K), LL 130-170 ($34.8K).
  REMAINING GAP: LL 85-118 x LS~29 never tested (R1 low-LL sweeps all used LS>=60).

R3 Plan (11 attempts, <=5000 combos each) — FINAL VERIFICATION ROUND:
  Close every remaining gap before declaring ceiling (BTC false-ceiling lesson).

  A01 LL_90_118_gap     LL(90-118 s2)xSL(3.9-4.2 s0.05)xLS(26-32 s2)xSS(4.1-4.3 s0.05)  = 2100
  A02 LL_170_300        LL(170-300 s10)xSL(3.9-4.3 s0.1)xLS(25-35 s2)xSS(4.0-4.4 s0.1)  = 2100
  A03 LL_2_90           LL(2-90 s8)xSL(3.8-4.3 s0.1)xLS(25-35 s2)xSS(4.0-4.4 s0.1)      = 2160
  A04 champion_ultrafine LL(120-125 s1)xSL(3.975-4.1 s0.025)xLS(27-31 s1)xSS(4.15-4.25 s0.025) = 900
  A05 highSS_shortLS_redo LL(100-140 s8)xSL(3.8-4.4 s0.1)xLS(20-35 s3)xSS(4.4-6.0 s0.2) = 2268
  A06 global_verify     LL(90-180 s10)xSL(3.7-4.5 s0.2)xLS(5-65 s5)xSS(3.8-4.8 s0.25)   = 3250
  A07 verylong_LL       LL(300-500 s25)xSL(3.8-4.6 s0.2)xLS(20-40 s4)xSS(3.9-4.5 s0.2)  = 1080
  A08 LS_60_100_recheck LL(115-130 s3)xSL(3.9-4.2 s0.1)xLS(60-100 s4)xSS(4.0-4.4 s0.1)  = 1320
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ct_hourly3_search")
...
# R1+R2 champion as seed (6-conv)
SEED_LL, SEED_SL = 122.0, 4.025
SEED_LS, SEED_SS = 29.0, 4.2
SEED_NP          = 35112.2

PREFIX = "BNBCT3_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_ct_hourly3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r2_result":"R2: champion 6-conv LL=122 SL=4.025-4.05 LS=29 SS=4.2 NP=$35,112 Obj=173,342 (also Obj-max); R1->R2 +0.00%; all near bridges worse",
            "r3_plan":  "Final verification: close LL 85-118 x LS=29 gap + LL 2-90/170-500 + high-SS redo + global verify + LS 60-100 recheck + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_ct_hourly3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto NP>100K -- Round 3 (VERIFY)")
    log.info("  Champion 6-conv: LL=122 SL=4.025 LS=29 SS=4.2 NP=$35,112")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_LL_90_118_gap",      (90, 118, 2),   (3.9, 4.2, 0.05),   (26, 32, 2),   (4.1, 4.3, 0.05)),
        ("02_LL_170_300",         (170, 300, 10), (3.9, 4.3, 0.1),    (25, 35, 2),   (4.0, 4.4, 0.1)),
        ("03_LL_2_90",            (2, 90, 8),     (3.8, 4.3, 0.1),    (25, 35, 2),   (4.0, 4.4, 0.1)),
        ("04_champion_ultrafine", (120, 125, 1),  (3.975, 4.1, 0.025),(27, 31, 1),   (4.15, 4.25, 0.025)),
        ("05_highSS_shortLS_redo",(100, 140, 8),  (3.8, 4.4, 0.1),    (20, 35, 3),   (4.4, 6.0, 0.2)),
        ("06_global_verify",      (90, 180, 10),  (3.7, 4.5, 0.2),    (5, 65, 5),    (3.8, 4.8, 0.25)),
        ("07_verylong_LL",        (300, 500, 25), (3.8, 4.6, 0.2),    (20, 40, 4),   (3.9, 4.5, 0.2)),
        ("08_LS_60_100_recheck",  (115, 130, 3),  (3.9, 4.2, 0.1),    (60, 100, 4),  (4.0, 4.4, 0.1)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto Round-3 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Hourly countertrend_LS_crypto NP>100K Round-3 search")
