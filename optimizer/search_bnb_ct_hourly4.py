"""
search_bnb_ct_hourly4.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BNBUSDT HOT Hourly, Round 4

R1-R3 Summary (33 attempts):
  R3's LL 170-300 gap sweep found a NEW NP-max regime (gap rule payoff #3):
  NP-max ⭐ R3 A09=A10=A11: LL=229 SL=3.8 LS=22-23 SS=4.2-4.3
    NP=$36,703.20 MDD=-$14,441 10tr Obj=93,284 (MDD/NP 39.3%)
  Obj-max ⭐⭐⭐ LIVE-USE champion (7-conv, R3 A04 retest exact):
    LL=122 SL=4.025-4.05 LS=29 SS=4.2 NP=$35,112.20 MDD=-$7,112 24tr Obj=173,342
  LL=229 regime: +4.5% NP for 2x MDD — strictly worse risk-adjusted.
  R2->R3 gain +4.53% — NOT converged. Heavy export truncation in 10-trade regime.
  Bounded: LL 2-118 worse, LL 300-500 worse ($31.8K), LS 60-100 worse, global verified.

R4 Plan (11 attempts, <=5000 combos each):
  Converge the LL=229 NP regime + fine bridges on its flanks + Obj-max drift check.

  A01 NPmax_LL229_microfine LL(222-236 s2)xSL(3.7-3.9 s0.05)xLS(20-26 s1)xSS(4.15-4.35 s0.05) = 1400
  A02 LL_170_225_fine       LL(170-225 s5)xSL(3.7-4.0 s0.1)xLS(20-28 s2)xSS(4.1-4.4 s0.1)     = 960
  A03 LL_240_300_fine       LL(240-300 s5)xSL(3.7-4.0 s0.1)xLS(20-28 s2)xSS(4.1-4.4 s0.1)     = 1040
  A04 Objmax_LL122_retest   LL(120-125 s1)xSL(3.975-4.1 s0.025)xLS(27-31 s1)xSS(4.15-4.25 s0.025) = 900
  A05 LL229_SL_3p4_3p75     LL(222-236 s2)xSL(3.4-3.75 s0.05)xLS(20-26 s2)xSS(4.2-4.35 s0.05) = 1024
  A06 LL229_SL_3p9_4p3      LL(222-236 s2)xSL(3.9-4.3 s0.05)xLS(20-26 s2)xSS(4.15-4.35 s0.1)  = 864
  A07 LL229_SS_wide         LL(224-234 s2)xSL(3.75-3.85 s0.05)xLS(20-26 s2)xSS(3.8-5.0 s0.05) = 1800
  A08 LL229_LS_2_40         LL(224-234 s2)xSL(3.7-3.9 s0.1)xLS(2-40 s2)xSS(4.2-4.4 s0.1)      = 1080
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_ct_hourly4_search")
...
# R3 NP-max as seed (A09=A10=A11)
SEED_LL, SEED_SL = 229.0, 3.8
SEED_LS, SEED_SS = 23.0, 4.3
SEED_NP          = 36703.2

PREFIX = "BNBCT4_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_ct_hourly4_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r3_result":"R3: NP-max NEW LL=229 SL=3.8 LS=22-23 SS=4.2-4.3 NP=$36,703 MDD=-$14,441 Obj=93,284 (LL 170-300 gap); Obj-max LL=122 champion 7-conv intact ($35,112 Obj=173,342); R2->R3 +4.53%",
            "r4_plan":  "Converge LL=229 NP regime (microfine + LL 170-225/240-300 fine + SL/SS/LS wide) + LL=122 Obj-max drift retest + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_ct_hourly4.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto NP>100K -- Round 4")
    log.info("  NP-max: LL=229 SL=3.8 LS=23 SS=4.3 NP=$36,703 (R3 new regime)")
    log.info("  Obj-max: LL=122 SL=4.025 LS=29 SS=4.2 NP=$35,112 Obj=173,342")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_NPmax_LL229_microfine", (222, 236, 2),  (3.7, 3.9, 0.05),   (20, 26, 1),  (4.15, 4.35, 0.05)),
        ("02_LL_170_225_fine",       (170, 225, 5),  (3.7, 4.0, 0.1),    (20, 28, 2),  (4.1, 4.4, 0.1)),
        ("03_LL_240_300_fine",       (240, 300, 5),  (3.7, 4.0, 0.1),    (20, 28, 2),  (4.1, 4.4, 0.1)),
        ("04_Objmax_LL122_retest",   (120, 125, 1),  (3.975, 4.1, 0.025),(27, 31, 1),  (4.15, 4.25, 0.025)),
        ("05_LL229_SL_3p4_3p75",     (222, 236, 2),  (3.4, 3.75, 0.05),  (20, 26, 2),  (4.2, 4.35, 0.05)),
        ("06_LL229_SL_3p9_4p3",      (222, 236, 2),  (3.9, 4.3, 0.05),   (20, 26, 2),  (4.15, 4.35, 0.1)),
        ("07_LL229_SS_wide",         (224, 234, 2),  (3.75, 3.85, 0.05), (20, 26, 2),  (3.8, 5.0, 0.05)),
        ("08_LL229_LS_2_40",         (224, 234, 2),  (3.7, 3.9, 0.1),    (2, 40, 2),   (4.2, 4.4, 0.1)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly countertrend_LS_crypto Round-4 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Hourly countertrend_LS_crypto NP>100K Round-4 search")
