"""
search_bnb_donchian_hourly.py -- SFJ_DonchianATR_crypto on BNBUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_DonchianATR_crypto.txt): Donchian breakout + ATR chandelier trailing stop,
long+short, MARKET entries, _Crypto1MUSD sizing.
  Long  entry : Close > Highest(High, Length)[1]   -> Buy  next bar market
  Short entry : Close < Lowest (Low , Length)[1]   -> SellShort next bar market
  Exits : ATR trailing stop (ATRMult x AvgTrueRange(ATRLength)) under/over the running extreme.

3 params: Length (Donchian, int), ATRLength (ATR period, int), ATRMult (trail multiple, fractional).
...
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "SFJ_DonchianATR_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchian_hourly_search")
...
SEED_LEN, SEED_ATRL, SEED_MULT = 20.0, 14.0, 3.0

PREFIX = "BNBDON_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchian_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "Donchian breakout entries (Close vs Highest/Lowest(Length)[1]); ATR chandelier trailing-stop exits (ATRMult x ATR(ATRLength)); long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "Length (Donchian period int), ATRLength (ATR period int), ATRMult (trail multiple frac); defaults 20/14/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_donchian_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto on BNBUSDT HOT Hourly -- Round 1 (IS 2022/01-2026/01)")
    log.info("  Params: Length, ATRLength, ATRMult.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_atrl, seed_mult = om["Length"], om["ATRLength"], om["ATRMult"]
        if not np_best or nm["net_profit"] > np_best.get("net_profit", -1):
            ...
            if nm["net_profit"] > TARGET_NP:
                target_met = True
        log.info("  [A%02d %-16s] obj_max LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["Length"], om["ATRLength"], om["ATRMult"],
    ...
    # static attempts (3D)
    _do(1, "01_global_wide",  _cfg("01_global_wide",  (5,100,5),  (7,28,7),   (1.5,6.0,0.5)))
    _do(2, "02_default_zone", _cfg("02_default_zone", (10,40,2),  (10,20,2),  (2.0,4.0,0.5)))
    _do(3, "03_len_fine",     _cfg("03_len_fine",     (5,60,2),   (10,18,4),  (2.5,4.5,0.5)))
    _do(4, "04_atrl_fine",    _cfg("04_atrl_fine",    (15,35,5),  (5,40,2),   (2.5,4.0,0.5)))
    _do(5, "05_mult_fine",    _cfg("05_mult_fine",    (15,35,5),  (10,20,3),  (1.0,8.0,0.25)))
    _do(6, "06_short_len",    _cfg("06_short_len",    (3,25,1),   (8,20,4),   (2.0,5.0,0.5)))
    _do(7, "07_long_len",     _cfg("07_long_len",     (40,150,5), (10,25,5),  (2.0,5.0,0.5)))
    _do(8, "08_wide_mult",    _cfg("08_wide_mult",    (10,50,5),  (8,24,4),   (1.0,10.0,0.5)))
    _do(9, "09_global_bound", _cfg("09_global_bound", (5,200,15), (5,50,9),   (1.0,9.0,1.0)))
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto BNBUSDT Hourly Round-1 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["Length"], obj_best["ATRLength"], obj_best["ATRMult"],
    ...
    if np_best:
        log.info("  NP-max: LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f Obj=%.0f",
                 np_best["Length"], np_best["ATRLength"], np_best["ATRMult"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_DonchianATR_crypto BNBUSDT HOT Hourly R1 search")
