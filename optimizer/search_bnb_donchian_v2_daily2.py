"""
search_bnb_donchian_v2_daily.py -- SFJ_DonchianATR_crypto_v2 on BNBUSDT HOT Daily, R1 (IS 2022/01-2026/01)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchian_v2_daily2_search")
...
SEED_LEN, SEED_ATRL, SEED_MULT, SEED_TR, SEED_RE = 10.0, 10.0, 4.5, 65.0, 12.0

PREFIX = "BNBDONV2D2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchian_v2_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_donchian_v2_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 on BNBUSDT HOT Daily -- Round 2 boundary confirm (IS 2022/01-2026/01)")
    log.info("  Params: Length, ATRLength, ATRMult, TrendLen, ReentryBars.  Obj=NP^2/|MDD|.  Target %.0f", TARGET_NP)
    log.info("  R1: NP=NP-max $19,177 (L10/ATRL10/M4.5/T65/R12, 11tr SPARSE); Daily<Hourly.")
    log.info("  R2 careful zoom (sparse) + denser-regime probe.  Run start: %s", datetime.now().isoformat())
    log.info("==============================================================")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (8, 12, 2), (8, 12, 2),
                                                 (4, 5, 0.5), (55, 75, 10), (10, 14, 2)))
    ...
    # static attempts (5D R2 boundary confirm: sparse-trade careful zoom + denser probe)
    _do(1, "01_rule5_retest", _cfg("01_rule5_retest", (7,13,1),  (8,14,2),  (3.5,5.5,0.5),(55,75,10), (8,16,4)))   # 1260
    _do(2, "02_len_fine",     _cfg("02_len_fine",     (3,25,1),  (8,12,2),  (4,5,0.5),    (55,75,10), (10,14,4)))  # L fine
    _do(3, "03_mult_scan",    _cfg("03_mult_scan",    (8,14,2),  (8,14,2),  (2.5,8,0.5),  (55,75,10), (8,16,4)))   # M 2.5-8
    _do(4, "04_trend_scan",   _cfg("04_trend_scan",   (8,14,2),  (8,14,2),  (4,5,0.5),    (20,120,10),(8,16,4)))   # TrendLen
    _do(5, "05_reentry_scan", _cfg("05_reentry_scan", (8,14,2),  (8,14,2),  (4,5,0.5),    (55,75,10), (0,24,2)))   # R 0-24
    _do(6, "06_atrl_scan",    _cfg("06_atrl_scan",    (8,14,2),  (4,24,2),  (4,5,0.5),    (55,75,10), (8,16,4)))   # ATRL 4-24
    _do(7, "07_denser_reg",   _cfg("07_denser_reg",   (15,40,2), (8,14,3),  (2.5,5,0.5),  (40,80,20), (0,6,3)))    # denser/more-trade
    _do(8, "08_wide_mult",    _cfg("08_wide_mult",    (8,16,2),  (8,14,2),  (5,12,0.5),   (55,75,20), (8,16,4)))   # wide trail
    _do(9, "09_short_lowT",   _cfg("09_short_lowT",   (3,15,1),  (8,14,2),  (3.5,5.5,0.5),(30,60,15), (8,16,8)))   # short L low TrendLen
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 BNBUSDT Daily Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_DonchianATR_crypto_v2 BNBUSDT HOT Daily R1 search")
