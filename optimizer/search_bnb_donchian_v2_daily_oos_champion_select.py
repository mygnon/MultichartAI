"""
search_bnb_donchian_v2_daily_oos_champion_select.py -- OOS selection among the converged
BNB Daily SFJ_DonchianATR_crypto_v2 champions (R1-R2, results/bnb_donchian_v2_daily{,2}_search/).

NOTE: v1 (SFJ_DonchianATR_crypto) lost on EVERY combo (whipsaw) -> no profitable champions to test.
The deployable champions are all v2 (trend filter + STOP-breakout entry + ReentryBars cooldown).
This OOS test therefore uses signal SFJ_DonchianATR_crypto_v2.

5 params Length, ATRLength, ATRMult (frac), TrendLen, ReentryBars; Donchian breakout + ATR chandelier
trailing stop, long+short, _Crypto1MUSD. DAILY R2 ceiling (sparse 10-11tr, 5-conv):
  Obj-max Length=10 ATRLength=10 ATRMult=4.5 TrendLen=65 ReentryBars=14 (NP 19045 MDD-7041 10tr)
  NP-max  Length=2  ATRLength=16 ATRMult=5   TrendLen=5  ReentryBars=6  (NP 20461 MDD-9097 10tr ultra-short)
...
METHOD -- MERGED single-run, AUTO data-range:
  ...
  Each candidate = a 5D micro-grid (all params +/-1 step, Critical Rule 1); exact center row picked.
  Metric = Max Intraday Drawdown. Winner = max OOS_NP among PASS.

PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_DonchianATR_AI.wsp open, BNBUSDT HOT DAILY chart tab ACTIVE,
    SFJ_DonchianATR_crypto_v2 signal ON. Full data to 2026/06/10. CLOSE the Study Editor.

CLI:
  py search_bnb_donchian_v2_daily_oos_champion_select.py                  # merged verdict
  py search_bnb_donchian_v2_daily_oos_champion_select.py --candidate BND1  # smoke one
  py search_bnb_donchian_v2_daily_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchian_v2_daily_oos_champion_select_search")
PREFIX      = "BNBDONDSEL_"
...
STEP = {"Length": 1.0, "ATRLength": 1.0, "ATRMult": 0.5, "TrendLen": 1.0, "ReentryBars": 1.0}
LO   = {"Length": 2.0, "ATRLength": 2.0, "ATRMult": 0.5, "TrendLen": 2.0, "ReentryBars": 0.0}
HI   = {"Length": 500.0, "ATRLength": 100.0, "ATRMult": 20.0, "TrendLen": 1000.0, "ReentryBars": 200.0}

# Converged BNB Daily v2 candidates (bnb_donchian_v2_daily{,2}_search).
# id, regime, Length, ATRLength, ATRMult, TrendLen, ReentryBars, prior IS NP, prior IS MDD.
CANDIDATES = [
    ("BND1", "R2 Obj-max short-L ***", 10.0, 10.0, 4.5, 65.0,  14.0, 19045.0, -7041.0),  # 10tr, 5-conv
    ("BND2", "R2 NP-max ultra-short",   2.0, 16.0, 5.0,  5.0,   6.0, 20461.0, -9097.0),  # 10tr, highest NP deep MDD
    ("BND3", "R1 long-L",              40.0, 15.0, 5.0, 100.0,  0.0, 16902.0, -10745.0), # 10tr, distinct long regime
    ("BND4", "R1 low-MDD hi-cooldown", 15.0, 14.0, 4.0, 80.0,  18.0, 13310.0, -6862.0),  # 13tr, lowest IS MDD
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchian_v2_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, length, atrl, mult, trend, reentry) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="daily",
        bar_period=1440,
        params=[_axis("Length", length), _axis("ATRLength", atrl), _axis("ATRMult", mult),
                _axis("TrendLen", trend), _axis("ReentryBars", reentry)],
...
def _pick(df, length, atrl, mult, trend, reentry):
    m = df
    for nm, v in (("Length", length), ("ATRLength", atrl), ("ATRMult", mult),
                  ("TrendLen", trend), ("ReentryBars", reentry)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_donchian_v2_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_donchian_v2_daily_oos_champion_select.json"
    periods = [] if from_csv else ([only_period] if only_period else PERIOD_ORDER)

    payload = {
        "symbol": SYMBOL, "signal": SIGNAL, "timeframe": "Daily (1440 min)",
        "method": "MERGED auto-range; set_instrument_data_range per pass; 5D micro-grid MCReport CSV; "
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BND1", "is", *CANDIDATES[0][2:7]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Daily DonchianATR v2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
            mc.ensure_chart_ready(conn, _cfg(CANDIDATES[0][0], period, *CANDIDATES[0][2:7]))
        ...
        for (cid, regime, length, atrl, mult, trend, reentry, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, length, atrl, mult, trend, reentry)
            log.info("--- [%s] %s %s L=%g ATRL=%g M=%g T=%g R=%g [%s] ---",
                     period, cid, regime, length, atrl, mult, trend, reentry,
                     datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"Length": length, "ATRLength": atrl, "ATRMult": mult,
                              "TrendLen": trend, "ReentryBars": reentry},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, length, atrl, mult, trend, reentry) if (df is not None and not df.empty) else None
            if row is not None:
    ...
    log.info("  %-3s %-26s %9s %9s %9s %9s %4s", "id", "regime", "NP_is", "NP_full", "OOS_NP", "MDD_full", "pass")
    for c in sorted(valid, key=lambda c: c["oos_np"], reverse=True):
        log.info("  %-3s %-26s %9.0f %9.0f %9.0f %9.0f %4s", c["id"], c["regime"],
...
def main():
    ap = argparse.ArgumentParser(description="BNB Daily SFJ_DonchianATR_crypto_v2 OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="BNDn", default=None)
