"""
search_bnb_donchian_v2_exit_modules.py -- Exit-module independent optimization on top of the
SFJ_DonchianATR_crypto_v2 BNB Hourly champion.

NOTE: v1 (SFJ_DonchianATR_crypto) lost on every combo; the deployable optimized main signal is v2.
Main signal FIXED at the OOS-winner champion DN3 (Length=4 ATRLength=16 ATRMult=7 TrendLen=5
ReentryBars=17), Status ON. Six exit modules tested INDEPENDENTLY (one enabled at a time),
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260622_SFJ_DonchianATR_AI.wsp open, BNBUSDT HOT HOURLY tab active
  - Main signal SFJ_DonchianATR_crypto_v2 inputs set to 4/16/7/5/17, Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved
  - CLOSE the Study Editor window

CLI:
  py search_bnb_donchian_v2_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_bnb_donchian_v2_exit_modules.py --module 3       # only module 3
  py search_bnb_donchian_v2_exit_modules.py --module 0       # baseline A00 only
  py search_bnb_donchian_v2_exit_modules.py --smoke --module 1
  py search_bnb_donchian_v2_exit_modules.py --manual-status  # user toggles Status by hand
  py search_bnb_donchian_v2_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_bnb_donchian_v2_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
SYMBOL      = "BNBUSDT HOT"
MAIN_SIGNAL = "SFJ_DonchianATR_crypto_v2"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchian_v2_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # signal date no-op; chart-trim is the control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# OOS-winner champion DN3 — main signal must be set to these in MC64 (Format Objects, Status ON)
CHAMPION = {
    "Length":      4.0,
    "ATRLength":   16.0,
    "ATRMult":     7.0,
...
# Reference baseline = DN3 in-sample (from bnb_donchian_v2_hourly_oos_champion_select, IS pass)
BASELINE_REF = {"net_profit": 20808.0, "max_drawdown": -4678.0, "total_trades": 210}
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchian_v2_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    # 243-combo micro-grid around the champion: all 5 params vary (Critical Rule 1)
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("Length",      3.0,  5.0,  1.0),
            ParamAxis("ATRLength",   15.0, 17.0, 1.0),
            ParamAxis("ATRMult",     6.5,  7.5,  0.5),
            ParamAxis("TrendLen",    4.0,  6.0,  1.0),
            ParamAxis("ReentryBars", 16.0, 18.0, 1.0),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs 4/16/7/5/17)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_bnb_donchian_v2_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_bnb_donchian_v2_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("  Main fixed: L=4 ATRL=16 M=7 T=5 R=17 (Status ON)")
    log.info("  Baseline ref (DN3 IS): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
