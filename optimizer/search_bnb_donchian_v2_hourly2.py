...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchian_v2_hourly2_search")
...
SEED_LEN, SEED_ATRL, SEED_MULT, SEED_TR, SEED_RE = 5.0, 14.0, 7.0, 5.0, 14.0

PREFIX = "BNBDONV22_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchian_v2_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_donchian_v2_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 on BNBUSDT HOT Hourly -- Round 2 boundary confirm (IS 2022/01-2026/01)")
    log.info("  Params: Length, ATRLength, ATRMult, TrendLen, ReentryBars.  Obj=NP^2/|MDD|.  Target %.0f", TARGET_NP)
    log.info("  R1: NP $17,237 (L5/ATRL14/M7/T5/R14); wide ATRMult+cooldown work, TrendLen at floor.")
    log.info("  R2 push M>7, test TrendLen mid, fine Length.  Run start: %s", datetime.now().isoformat())
    log.info("==============================================================")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (4, 8, 2), (12, 16, 2),
                                                 (6.5, 7.5, 0.5), (5, 15, 5), (12, 16, 2)))
    ...
    # static attempts (5D boundary confirm around L5/ATRL14/M7/T5/R14)
    _do(1, "01_rule5_retest", _cfg("01_rule5_retest", (3,10,1),  (12,16,2), (6,8,0.5),    (5,25,10),  (10,16,3)))   # 1080
    _do(2, "02_mult_push",    _cfg("02_mult_push",    (4,10,2),  (12,16,2), (6,12,0.5),   (5,15,5),   (10,16,3)))   # push M>7
    _do(3, "03_trend_test",   _cfg("03_trend_test",   (5,9,2),   (12,16,2), (6.5,8,0.5),  (5,205,20), (10,16,3)))   # does TrendLen mid help?
    _do(4, "04_len_fine",     _cfg("04_len_fine",     (2,15,1),  (12,16,2), (6.5,8,0.5),  (5,25,10),  (11,14,3)))   # 1008
    _do(5, "05_reentry_scan", _cfg("05_reentry_scan", (4,10,2),  (12,16,2), (6.5,8,0.5),  (5,15,5),   (0,30,3)))    # R 0-30
    _do(6, "06_atrl_test",    _cfg("06_atrl_test",    (4,10,2),  (6,30,3),  (6.5,8,0.5),  (5,15,5),   (10,16,3)))   # ATRLength sweep
    _do(7, "07_len_mult_2d",  _cfg("07_len_mult_2d",  (2,20,2),  (12,16,2), (5,12,0.5),   (5,15,5),   (12,14,2)))   # L x M plane
    _do(8, "08_very_wide_m",  _cfg("08_very_wide_m",  (4,10,2),  (12,16,2), (8,16,0.5),   (5,15,5),   (10,16,3)))   # M up to 16
    _do(9, "09_short_len_re", _cfg("09_short_len_re", (2,12,1),  (12,16,2), (6.5,8,0.5),  (5,25,10),  (5,25,4)))    # short L x R
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
