"""
search_bnb_donchian_v2_hourly.py -- SFJ_DonchianATR_crypto_v2 on BNBUSDT HOT Hourly, R1 (IS 2022/01-2026/01)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchian_v2_hourly3_search")
...
SEED_LEN, SEED_ATRL, SEED_MULT, SEED_TR, SEED_RE = 4.0, 16.0, 7.0, 5.0, 17.0

PREFIX = "BNBDONV23_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchian_v2_hourly3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_donchian_v2_hourly3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 on BNBUSDT HOT Hourly -- Round 3 ceiling confirm (IS 2022/01-2026/01)")
    log.info("  Params: Length, ATRLength, ATRMult, TrendLen, ReentryBars.  Obj=NP^2/|MDD|.  Target %.0f", TARGET_NP)
    log.info("  R2: Obj-max L4/ATRL16/M7/T5/R17 NP$20.8K Obj92.5K (210tr); NP-max L4/M16 NP$23.3K (67tr).")
    log.info("  R3 zoom M=16 low-freq regime + Length floor + ATRLength.  Run start: %s", datetime.now().isoformat())
    log.info("==============================================================")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (3, 5, 1), (14, 18, 2),
                                                 (6.5, 7.5, 0.5), (5, 15, 5), (15, 19, 2)))
    ...
    # static attempts (5D R3 ceiling confirm: M=7 (210tr) and M=16 (67tr) regimes)
    _do(1, "01_rule5_obj",    _cfg("01_rule5_obj",    (2,8,1),   (14,18,2), (6,8,0.5),    (5,15,5),   (14,20,3)))   # 945 retest M7 obj-max
    _do(2, "02_rule5_np_wm",  _cfg("02_rule5_np_wm",  (2,8,1),   (10,16,2), (12,20,1),    (5,25,10),  (6,14,4)))    # retest M16 np-max
    _do(3, "03_widem_zoom",   _cfg("03_widem_zoom",   (2,8,2),   (10,16,2), (13,20,0.5),  (10,20,5),  (8,14,3)))    # zoom M=16 regime
    _do(4, "04_len_floor_m7", _cfg("04_len_floor_m7", (2,10,1),  (12,16,2), (6.5,8,0.5),  (5,15,5),   (14,20,3)))   # 972 L floor (M7)
    _do(5, "05_len_floor_wm", _cfg("05_len_floor_wm", (2,10,1),  (10,14,2), (14,18,0.5),  (10,20,5),  (8,12,2)))    # L floor (M16)
    _do(6, "06_atrl_fine",    _cfg("06_atrl_fine",    (3,5,1),   (8,20,1),  (6.5,8,0.5),  (5,15,5),   (14,18,2)))   # 1404 ATRL fine
    _do(7, "07_ultra_wide_m", _cfg("07_ultra_wide_m", (2,8,2),   (10,16,2), (16,24,1),    (10,20,5),  (8,12,2)))    # push M beyond 16
    _do(8, "08_reentry_obj",  _cfg("08_reentry_obj",  (3,5,1),   (14,18,2), (6.5,8,0.5),  (5,15,5),   (10,26,2)))   # 2916 R fine (M7)
    _do(9, "09_reentry_np",   _cfg("09_reentry_np",   (3,5,1),   (10,14,2), (15,17,0.5),  (10,20,5),  (2,22,2)))    # R fine (M16)

    # adaptive zooms around the running Obj-max
    for A, nm_, (rL, rA, rM, rT, rR, nL, nA, nM, nT, nR) in [
        (10, "10_adaptive_zoom1", (4, 4, 2.0, 40, 8, 7, 3, 9, 5, 5)),
        (11, "11_adaptive_zoom2", (3, 2, 1.0, 20, 5, 7, 3, 9, 5, 5)),
        (12, "12_adaptive_zoom3", (2, 2, 0.5, 10, 3, 7, 3, 7, 5, 5)),
    ]:
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 BNBUSDT Hourly Round-3 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_DonchianATR_crypto_v2 BNBUSDT HOT Hourly R1 search")
