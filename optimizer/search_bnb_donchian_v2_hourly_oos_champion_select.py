"""
search_bnb_donchian_v2_hourly_oos_champion_select.py -- OOS selection among the converged
BNB Hourly SFJ_DonchianATR_crypto_v2 champions (R1-R3, results/bnb_donchian_v2_hourly{,2,3}_search/).
...
trailing stop, long+short, _Crypto1MUSD. R3 ceiling = 2 regimes:
  Obj-max  M=7 hi-freq  Length=3 ATRLength=16 ATRMult=7  TrendLen=10 ReentryBars=17 (NP 19141 MDD-3661 212tr)
  NP-max   M=16 lo-freq Length=2 ATRLength=12 ATRMult=16 TrendLen=15 ReentryBars=10 (NP 24426 MDD-6394 66tr)
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_DonchianATR_AI.wsp open, BNBUSDT HOT HOURLY chart tab ACTIVE,
    SFJ_DonchianATR_crypto_v2 signal ON. Full data to 2026/06/10. CLOSE the Study Editor.

CLI:
  py search_bnb_donchian_v2_hourly_oos_champion_select.py                  # merged verdict
  py search_bnb_donchian_v2_hourly_oos_champion_select.py --candidate DN1  # smoke one
  py search_bnb_donchian_v2_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchian_v2_hourly_oos_champion_select_search")
PREFIX      = "BNBDONSEL_"
...
# Converged BNB Hourly v2 candidates (bnb_donchian_v2_hourly{,2,3}_search).
# id, regime, Length, ATRLength, ATRMult, TrendLen, ReentryBars, prior IS NP, prior IS MDD.
CANDIDATES = [
    ("DN1", "R3 Obj-max M7 hi-freq ***", 3.0, 16.0, 7.0,  10.0, 17.0, 19141.0, -3661.0),  # 212tr lowest MDD
    ("DN2", "R3 NP-max M16 lo-freq",     2.0, 12.0, 16.0, 15.0, 10.0, 24426.0, -6394.0),  # 66tr highest NP
    ("DN3", "R2 Obj-max M7 alt",         4.0, 16.0, 7.0,  5.0,  17.0, 20808.0, -4678.0),  # 210tr
    ("DN4", "M14 mid-wide",              4.0, 12.0, 14.0, 10.0, 11.0, 21229.0, -5854.0),  # 77tr distinct
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchian_v2_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_donchian_v2_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_donchian_v2_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("DN1", "is", *CANDIDATES[0][2:7]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly DonchianATR v2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly SFJ_DonchianATR_crypto_v2 OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="DNn", default=None)
