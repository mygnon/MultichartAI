"""
search_bnb_donchianasym_hourly.py -- SFJ_DonchianAsym_crypto on BNBUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_DonchianAsym_crypto.txt): asymmetric Donchian channel breakout + ATR chandelier
...
WORKSPACE  = r"C:\Users\Tim\Downloads\Multichart64\Tim\20260622_SFJ_DonchianAsym_crypto_AI.wsp"
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "SFJ_DonchianAsym_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchianasym_hourly_search")
...
SEED_LEN, SEED_ATRL, SEED_MULT = 20.0, 20.0, 3.0  # LongLen, ShortLen, ATRMult

PREFIX = "BNBDA_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchianasym_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "Asymmetric Donchian breakout: HHl=Highest(High,LongLen)[1] LLs=Lowest(Low,ShortLen)[1]; flat -> Buy STOP@HHl / SellShort STOP@LLs; ATR(14) chandelier trailing exits; flat-only; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "LongLen (long-side channel int), ShortLen (short-side channel int), ATRMult (trail frac); defaults 20/20/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_donchianasym_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianAsym_crypto on BNBUSDT HOT Hourly -- Round 1 (IS 2022/01-2026/01)")
    log.info("  Params: LongLen, ShortLen, ATRMult.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_atrl, seed_mult = om["LongLen"], om["ShortLen"], om["ATRMult"]
        ...
        log.info("  [A%02d %-16s] obj_max EMA=%.4g Band=%.4g Mult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["LongLen"], om["ShortLen"], om["ATRMult"],
    ...
    # static attempts (3D)
    _do(1, "01_global",       _cfg("01_global",       (5,150,15), (5,150,15),  (2,10,1)))      # LongLen x ShortLen x ATRMult
    _do(2, "02_short_both",   _cfg("02_short_both",   (5,80,5),   (5,80,5),    (3,6,1)))
    _do(3, "03_classic",      _cfg("03_classic",      (10,60,5),  (10,60,5),   (2.5,5,0.5)))
    _do(4, "04_wide_trail",   _cfg("04_wide_trail",   (10,100,10),(10,100,10), (4,16,1)))
    _do(5, "05_long_asym",    _cfg("05_long_asym",    (20,300,20),(5,60,5),    (3,7,1)))      # long LongLen, short ShortLen
    _do(6, "06_short_asym",   _cfg("06_short_asym",   (5,60,5),   (20,300,20), (3,7,1)))      # mirror asym
    _do(7, "07_trail_fine",   _cfg("07_trail_fine",   (15,45,5),  (15,45,5),   (1,14,0.5)))
    _do(8, "08_dense_low",    _cfg("08_dense_low",    (2,40,2),   (2,40,2),    (3,6,1)))
    _do(9, "09_long_both",    _cfg("09_long_both",    (60,300,15),(60,300,15), (2,8,1)))

    # adaptive zooms around the running Obj-max (LongLen/ShortLen integer; ATRMult fractional)
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianAsym_crypto BNBUSDT Hourly Round-1 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["LongLen"], obj_best["ShortLen"], obj_best["ATRMult"],
    ...
    if np_best:
        log.info("  NP-max: LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f Obj=%.0f",
                 np_best["LongLen"], np_best["ShortLen"], np_best["ATRMult"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_DonchianAsym_crypto BNBUSDT HOT Hourly R1 search")
