"""
search_bnb_donchianasym_hourly.py -- SFJ_DonchianAsymV2_crypto on BNBUSDT HOT Hourly, Round 2 boundary push (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_DonchianAsymV2_crypto.txt): asymmetric Donchian breakout + ReentryBars cooldown + ATR chandelier
trailing stop, long+short, STOP entries, _Crypto1MUSD sizing.
  HHl=Highest(High,LongLen)[1]; LLs=Lowest(Low,ShortLen)[1] (rolling, non-lagged)
...
3 params: LongLen (long-side channel bars, int), ShortLen (short-side channel bars, int), ATRMult (trail, frac).
...
WORKSPACE  = r"C:\Users\Tim\Downloads\Multichart64\Tim\20260622_SFJ_DonchianAsymV2_crypto_AI.wsp"
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "SFJ_DonchianAsymV2_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_donchianasymv2_hourly_search")
...
LEN_LO,  LEN_HI  = 2.0,  500.0   # LongLen
ATRL_LO, ATRL_HI = 2.0,  500.0  # ShortLen (int)
MULT_LO, MULT_HI = 0.5,  20.0   # ATRMult
RE_LO,   RE_HI   = 0.0,  300.0   # ReentryBars (int)

SEED_LEN, SEED_ATRL, SEED_MULT, SEED_RE = 6.0, 300.0, 7.0, 15.0  # LongLen, ShortLen, ATRMult, ReentryBars (R1 A11 best)

PREFIX = "BNBDV_R2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_donchianasymv2_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, atrl, mult, reentry) -> StrategyConfig:
    ...
    atrl    = _safe(atrl,    ATRL_LO, ATRL_HI, 1.0)   # ShortLen integer
    ...
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[ParamAxis("LongLen",     *length),
                ParamAxis("ShortLen",    *atrl),
...
def _row(df, idx):
    b = df.loc[idx]
    return {"LongLen": float(b["LongLen"]), "ShortLen": float(b["ShortLen"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "Asymmetric Donchian breakout + ReentryBars cooldown: HHl=Highest(High,LongLen)[1] LLs=Lowest(Low,ShortLen)[1]; flat & flatBars>=ReentryBars -> Buy STOP@HHl / SellShort STOP@LLs; ATR(14) chandelier trailing exits; flat-only; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "LongLen (long-side channel int), ShortLen (short-side channel int), ATRMult (trail frac), ReentryBars (flat cooldown int); defaults 20/20/3.0/0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_donchianasymv2_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianAsymV2_crypto on BNBUSDT HOT Hourly -- Round 2 boundary push (IS 2022/01-2026/01; seed R1 6/300/7/15)")
    log.info("  Params: LongLen, ShortLen, ATRMult, ReentryBars.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (18, 22, 2), (18, 22, 2), (2.5, 3.5, 0.5), (0, 4, 2)))
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_atrl, seed_mult, seed_re = om["LongLen"], om["ShortLen"], om["ATRMult"], om["ReentryBars"]
        ...
        log.info("  [A%02d %-16s] obj_max Long=%.4g Short=%.4g Mult=%.4g Re=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["LongLen"], om["ShortLen"], om["ATRMult"], om["ReentryBars"],
    ...
    # R2 boundary push around R1 A11 best: LongLen=6 ShortLen=300 ATRMult=7 ReentryBars=15
    _do(1, "01_retest_seed",  _cfg("01_retest_seed",  (4,8,2),    (290,310,10),(6.5,7.5,0.5),(13,17,2)))   # Rule 5 in-session baseline
    _do(2, "02_short_push",   _cfg("02_short_push",   (4,10,2),   (300,500,25),(6,8,0.5),   (12,18,3)))    # PUSH ShortLen to cap 500
    _do(3, "03_short_push2",  _cfg("03_short_push2",  (5,7,1),    (300,500,20),(7,7,0.5),   (15,15,3)))    # fine ShortLen push (LongLen/Re fixed-ish)
    _do(4, "04_long_fine",    _cfg("04_long_fine",    (2,20,1),   (300,400,25),(6.5,7.5,0.5),(12,18,3)))   # nail LongLen 2-20
    _do(5, "05_atrmult_fine", _cfg("05_atrmult_fine", (4,10,2),   (300,400,50),(5,9,0.25),  (13,17,2)))    # ATRMult fine 5-9
    _do(6, "06_reentry_fine", _cfg("06_reentry_fine", (4,10,2),   (300,360,30),(6.5,7.5,0.5),(4,40,2)))    # ReentryBars fine 4-40
    _do(7, "07_vlong_short",  _cfg("07_vlong_short",  (5,9,2),    (350,500,15),(6.5,7.5,0.5),(13,17,2)))   # very long ShortLen zone
    _do(8, "08_wide_trail",   _cfg("08_wide_trail",   (4,12,2),   (300,400,50),(8,16,1),    (12,18,3)))    # confirm wide trail worse
    _do(9, "09_combo",        _cfg("09_combo",        (4,10,1),   (300,420,20),(6.5,8,0.5), (14,16,2)))    # tight combo at peak

    # adaptive zooms around the running Obj-max (LongLen/ShortLen/ReentryBars integer; ATRMult fractional)
    for A, nm_, (rl_mul, rl_min, ra_mul, ra_min, rm_mul, rm_min, rr_mul, rr_min, nL, nA, nM, nR) in [
        (10, "10_adaptive_zoom1", (0.40, 8.0, 0.40, 30.0, 0.40, 1.5, 0.60, 6.0, 7, 7, 7, 5)),
        (11, "11_adaptive_zoom2", (0.22, 4.0, 0.22, 18.0, 0.22, 0.75, 0.40, 4.0, 7, 7, 7, 5)),
        (12, "12_adaptive_zoom3", (0.12, 2.0, 0.12, 10.0, 0.12, 0.5, 0.25, 3.0, 7, 5, 5, 5)),
    ]:
        ...
        if start_attempt <= A:
            ...
            _atrl = zoom_fixed(seed_atrl, max(ra_min, seed_atrl * ra_mul),  nA, 1.0,  ATRL_LO, ATRL_HI)
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianAsymV2_crypto BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): Long=%.4g Short=%.4g Mult=%.4g Re=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["LongLen"], obj_best["ShortLen"], obj_best["ATRMult"], obj_best["ReentryBars"],
    ...
    if np_best:
        log.info("  NP-max: Long=%.4g Short=%.4g Mult=%.4g Re=%.4g NP=%.0f Obj=%.0f",
                 np_best["LongLen"], np_best["ShortLen"], np_best["ATRMult"], np_best["ReentryBars"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_DonchianAsymV2_crypto BNBUSDT HOT Hourly R2 boundary-push")
