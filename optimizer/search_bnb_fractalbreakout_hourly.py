"""
search_bnb_fractalbreakout_hourly.py — SFJ_FractalBreakout_crypto on BNBUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_FractalBreakout_crypto.txt): Williams-fractal swing-pivot breakout +
ATR(14) chandelier trailing exit, long+short, STOP entries, _Crypto1MUSD.
  Williams fractal: up=High[FracLen] highest of 2*FracLen+1 bars; lastHi/lastLo = last confirmed pivot
  Long  : flat & lastHi>0 -> Buy STOP@lastHi ; Short: flat & lastLo>0 -> SellShort STOP@lastLo
  Exit  : ATR chandelier trail (ATRMult x ATR(14))
2 params: FracLen (fractal strength, bars each side, int), ATRMult (trail multiple fractional).
Target NP>100,000. Objective = NetProfit^2/|MaxDrawdown|. <=5000 combos/attempt. NP-max + Obj-max.
Attempt schedule (8 static 2D + 3 zoom): FracLen x ATRMult grids.
...
WORKSPACE  = r"C:\Users\Tim\Downloads\Multichart64\Tim\20260622_SFJ_FractalBreakout_crypto_AI.wsp"
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "SFJ_FractalBreakout_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_fractalbreakout_hourly_search")
...
LEN_LO, LEN_HI = 1.0, 50.0     # FracLen
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_fractalbreakout_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, gap) -> StrategyConfig:
    ...
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[ParamAxis("FracLen", *length), ParamAxis("ATRMult", *gap)],
...
def _row(df, idx):
    b = df.loc[idx]
    return {"FracLen": float(b["FracLen"]), "ATRMult": float(b["ATRMult"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "Williams-fractal swing-pivot breakout: lastHi/lastLo = last confirmed 2*FracLen+1 fractal; flat -> Buy STOP@lastHi / SellShort STOP@lastLo; ATR(14) chandelier trailing exit; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "FracLen (fractal strength bars-each-side int), ATRMult (trail frac); defaults 2/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_fractalbreakout_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_FractalBreakout_crypto on BNBUSDT HOT Hourly -- Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    log.info("  Params: FracLen, ATRMult.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_gap = om["FracLen"], om["ATRMult"]
        if not np_best or nm["net_profit"] > np_best.get("net_profit", -1):
            ...
            if nm["net_profit"] > TARGET_NP:
                target_met = True
        log.info("  [A%02d %-16s] obj_max FracLen=%.4g ATRMult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["FracLen"], om["ATRMult"], om["net_profit"], om["max_drawdown"],
    ...
    # static attempts
    _do(1, "01_global",      _cfg("01_global",      (1,40,2),  (1.5,12,0.5)))    # FracLen x ATRMult
    _do(2, "02_fine_low",    _cfg("02_fine_low",    (1,20,1),  (2,8,0.25)))
    _do(3, "03_classic",     _cfg("03_classic",     (2,15,1),  (2,6,0.25)))
    _do(4, "04_wide_trail",  _cfg("04_wide_trail",  (2,20,2),  (4,18,0.5)))
    _do(5, "05_frac_scan",   _cfg("05_frac_scan",   (1,50,2),  (3,6,0.5)))
    _do(6, "06_trail_fine",  _cfg("06_trail_fine",  (2,12,1),  (1,14,0.25)))
    _do(7, "07_long_frac",   _cfg("07_long_frac",   (15,50,2), (2,10,0.5)))
    _do(8, "08_dense_low",   _cfg("08_dense_low",   (1,15,1),  (2,9,0.5)))
    ...
    log.info("==============================================================")
    log.info("  SFJ_FractalBreakout_crypto BNBUSDT Hourly Round-1 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): FracLen=%.4g ATRMult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["FracLen"], obj_best["ATRMult"], obj_best["net_profit"],
                 obj_best["max_drawdown"], obj_best["objective"], obj_best["total_trades"])
    if np_best:
        log.info("  NP-max: FracLen=%.4g ATRMult=%.4g NP=%.0f Obj=%.0f",
                 np_best["FracLen"], np_best["ATRMult"], np_best["net_profit"], np_best["objective"])
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_FractalBreakout_crypto BNBUSDT HOT Hourly R1 search")
