"""
search_bnb_qpatrsi_hourly.py — SFJ_HeikinAshi_crypto on BNBUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_HeikinAshi_crypto.txt): smoothed Heikin-Ashi trend-flip entry +
ATR(14) chandelier trailing exit, long+short, MARKET entries, _Crypto1MUSD.
  sO/sH/sL/sC=XAverage(O/H/L/C,HASmooth); HAClose=(sO+sH+sL+sC)/4; HAOpen=(HAOpen[1]+HAClose[1])/2
  Long  : flat & HAClose cross over  HAOpen -> Buy market ; Short: cross under -> SellShort
  Exit  : ATR chandelier trail (ATRMult x ATR(14))
2 params: HASmooth (OHLC EMA smoothing int), ATRMult (trail multiple fractional).
Target NP>100,000. Objective = NetProfit^2/|MaxDrawdown|. <=5000 combos/attempt. NP-max + Obj-max.
Attempt schedule (8 static 2D + 3 zoom): HASmooth x ATRMult grids.
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_heikinashi_hourly_search")
...
LEN_LO, LEN_HI = 1.0, 60.0     # HASmooth
GAP_LO, GAP_HI = 0.5, 20.0     # ATRMult
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_heikinashi_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, gap) -> StrategyConfig:
    ...
    gap    = _safe(gap,    GAP_LO, GAP_HI, 0.25)  # ATRMult frac
    ...
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[ParamAxis("HASmooth", *length), ParamAxis("ATRMult", *gap)],
...
def _row(df, idx):
    b = df.loc[idx]
    return {"HASmooth": float(b["HASmooth"]), "ATRMult": float(b["ATRMult"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "Smoothed Heikin-Ashi: sOHLC=XAverage(OHLC,HASmooth); HAClose vs HAOpen flip -> long on cross over / short on cross under (flat-only, market); ATR(14) chandelier trailing exit; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "HASmooth (OHLC EMA smoothing int), ATRMult (trail frac); defaults 5/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_heikinashi_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_HeikinAshi_crypto on BNBUSDT HOT Hourly -- Round 2 boundary confirm (IS 2022/01-2026/01 chart-trimmed; seed R1 3/8.25)")
    log.info("  Params: HASmooth, ATRMult.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_gap = om["HASmooth"], om["ATRMult"]
        if not np_best or nm["net_profit"] > np_best.get("net_profit", -1):
            ...
            if nm["net_profit"] > TARGET_NP:
                target_met = True
        log.info("  [A%02d %-16s] obj_max HASmooth=%.4g ATRMult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["HASmooth"], om["ATRMult"], om["net_profit"], om["max_drawdown"],
    ...
    # static attempts -- R2 boundary confirmation around R1 champion HASmooth=3 ATRMult=8.25
    _do(1, "01_retest_seed",   _cfg("01_retest_seed",   (2,4,1),   (8.0,8.5,0.25)))   # Rule 5 in-session baseline
    _do(2, "02_hasm_floor",    _cfg("02_hasm_floor",    (1,2,1),   (6,12,0.25)))      # HASmooth 1-2 at wide trail (gap)
    _do(3, "03_atrmult_fine",  _cfg("03_atrmult_fine",  (2,4,1),   (7,12,0.1)))       # ATRMult fine around 8.25
    _do(4, "04_atrmult_high",  _cfg("04_atrmult_high",  (2,5,1),   (8,20,0.5)))       # fill 8.25-16-20, confirm worse
    _do(5, "05_low_smooth",    _cfg("05_low_smooth",    (1,6,1),   (6,12,0.5)))       # short-smooth x wide-trail regime
    _do(6, "06_micro_bridge",  _cfg("06_micro_bridge",  (1,5,1),   (7.5,9,0.1)))      # micro grid at the peak
    _do(7, "07_wider_smooth",  _cfg("07_wider_smooth",  (1,10,1),  (7,10,0.5)))
    _do(8, "08_dense_low2",    _cfg("08_dense_low2",    (1,8,1),   (6,11,0.5)))
    ...
    for A, nm_, (rl_mul, rl_min, rg_mul, rg_min, nL, nG) in [
        ...
        if start_attempt <= A:
            ...
            _gap = zoom_fixed(seed_gap, max(rg_min, seed_gap * rg_mul), nG, 0.25, GAP_LO, GAP_HI)
    ...
    log.info("==============================================================")
    log.info("  SFJ_HeikinAshi_crypto BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): HASmooth=%.4g ATRMult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["HASmooth"], obj_best["ATRMult"], obj_best["net_profit"],
                 obj_best["max_drawdown"], obj_best["objective"], obj_best["total_trades"])
    if np_best:
        log.info("  NP-max: HASmooth=%.4g ATRMult=%.4g NP=%.0f Obj=%.0f",
                 np_best["HASmooth"], np_best["ATRMult"], np_best["net_profit"], np_best["objective"])
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_HeikinAshi_crypto BNBUSDT HOT Hourly R2 boundary-confirm")
