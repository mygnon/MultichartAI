"""
search_bnb_heikinashi_hourly_oos_champion_select.py — OOS selection among the converged BNB Hourly
SFJ_HeikinAshi_crypto champions (rounds 1-2, results/bnb_heikinashi_hourly{,2}_search/).
...
METHOD — MERGED single-run, AUTO data-range (proven route; set_instrument_data_range per pass):
  ...
  Each candidate = a 9-combo micro-grid (2 params HASmooth/ATRMult, all vary +/-1 step, Critical Rule 1);
  exact candidate row picked. Metric = NetProfit + Max INTRADAY Drawdown.

PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260622_SFJ_SFJ_HeikinAshi_crypto_AI.wsp open, BNBUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
  - SFJ_HeikinAshi_crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_bnb_heikinashi_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_bnb_heikinashi_hourly_oos_champion_select.py --period is
  py search_bnb_heikinashi_hourly_oos_champion_select.py --candidate HA1
  py search_bnb_heikinashi_hourly_oos_champion_select.py --from-csv
  py search_bnb_heikinashi_hourly_oos_champion_select.py --probe-instrument
"""
...
WORKSPACE   = r"C:\Users\Tim\Downloads\Multichart64\Tim\20260622_SFJ_SFJ_HeikinAshi_crypto_AI.wsp"
SYMBOL      = "BNBUSDT HOT"
SIGNAL      = "SFJ_HeikinAshi_crypto"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_heikinashi_hourly_oos_champion_select_search")
PREFIX      = "BNBHASEL_"
...
STEP = {"HASmooth": 1.0, "ATRMult": 0.25}
LO   = {"HASmooth": 1.0, "ATRMult": 0.5}
HI   = {"HASmooth": 60.0, "ATRMult": 20.0}

# 4 DISTINCT converged BNB Hourly HeikinAshi regimes (from bnb_heikinashi_hourly{,2}_search).
# id, regime, HASmooth, ATRMult, prior IS NP, prior IS MDD (intraday)  (reference only)
CANDIDATES = [
    ("HA1", "wide-trail champion ***", 3.0, 8.25, 20688.8, -6347.95),  # R1=R2 NP/Obj-max, 196tr
    ("HA2", "raw-HA wide-trail",       1.0, 9.0,  18879.4, -6563.44),  # HASmooth floor, lowest-MDD of HASmooth=1, 169tr
    ("HA3", "ultra-wide low-freq",     4.0, 16.0, 15473.0, -7115.0),   # 71tr, distinct low-freq
    ("HA4", "tight-trail high-freq",   3.0, 7.0,   8036.0, -9567.0),   # 244tr, denser generalizer candidate
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_heikinashi_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, at, ml) -> StrategyConfig:
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[_axis("HASmooth", at), _axis("ATRMult", ml)],
...
def _pick(df, at, ml):
    m = df
    for nm, v in (("HASmooth", at), ("ATRMult", ml)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_heikinashi_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_heikinashi_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("HA1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly HeikinAshi — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
    ...
    for (cid, regime, at, ml, pnp, pmdd) in CANDIDATES:
        ...
        c = {"id": cid, "regime": regime, "params": {"HASmooth": at, "ATRMult": ml},
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly HeikinAshi OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="HAn", default=None)
