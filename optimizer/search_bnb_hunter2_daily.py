"""
search_bnb_hunter2_daily.py — SFJ_HUNTER2_crypto on BNBUSDT HOT Daily, Round 1 (IS 2022/01-2026/01)
...
BNB HUNTER2 Hourly converged to a MID-LEN regime (LEN_L=136 LEN_S=187 ATR_L=3.75 ATR_S=2.75).
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_hunter2_daily_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_hunter2_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met, obj_best):
    ...
    out = OUTPUT_DIR / "final_params_bnb_hunter2_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto on BNBUSDT HOT Daily — Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto BNBUSDT HOT Daily Round-1 COMPLETE  (%.1f min total)",
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto BNBUSDT HOT Daily NP>100K USD Round-1 search")
