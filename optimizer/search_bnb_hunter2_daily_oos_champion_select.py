"""
search_bnb_hunter2_daily_oos_champion_select.py — Out-of-sample (OOS) selection among the converged
BNB Daily SFJ_HUNTER2_crypto champions (rounds 1-2, results/bnb_hunter2_daily{,2}_search/).
...
BNB HUNTER2 Daily converged to an ULTRA-SHORT LEN_L regime (R2 ceiling Obj-max=NP-max LEN_L=2 LEN_S=68
ATR_L=1.0 ATR_S=0.9; LEN_L pinned at the 2 floor). Candidates span the ultra-short-LEN_L core
(champion + low-ATR_S variant) plus short-LEN_S and high-ATR low-freq lower-MDD contrasts so the
OOS test reveals which holds its Max Drawdown out-of-sample.

TASK: re-test each previously-converged BNB Daily HUNTER2 champion on this workspace and pick the
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_HUNTER_AI.wsp open, BNBUSDT HOT **Daily** chart tab ACTIVE, the
    SFJ_HUNTER2_crypto signal applied (Status ON)
  - Full data available to 2026/06/10 (the script sets the range itself)

CLI:
  py search_bnb_hunter2_daily_oos_champion_select.py                 # merged: IS then FULL, then verdict
  py search_bnb_hunter2_daily_oos_champion_select.py --period is     # one pass only
  py search_bnb_hunter2_daily_oos_champion_select.py --candidate BND1  # smoke one candidate
  py search_bnb_hunter2_daily_oos_champion_select.py --from-csv      # recompute from existing CSVs
  py search_bnb_hunter2_daily_oos_champion_select.py --probe-instrument  # verify auto data-range
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_hunter2_daily_oos_champion_select_search")
PREFIX      = "BNBH2DSEL_"
...
STEP = {"LEN_L": 1.0, "LEN_S": 1.0, "ATR_multiplier_L": 0.25, "ATR_multiplier_S": 0.05}
...
# 4 converged BNB Daily HUNTER2 candidates (bnb_hunter2_daily{,2}_search). BNB Daily regime = ULTRA-SHORT LEN_L.
# id, regime, LEN_L, LEN_S, ATR_multiplier_L, ATR_multiplier_S, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("BND1", "R2 Obj-max ultra-short ***", 2.0,  68.0, 1.0, 0.9,  31730.9, -8424.0),  # 56tr, R2 ceiling Obj=119,521 highest NP
    ("BND2", "low-ATR_S variant",          3.0,  70.0, 1.0, 0.75, 29454.0, -8424.0),  # 60tr, R2 A08 near-champion
    ("BND3", "short-LEN_S",                2.0,   6.0, 1.0, 2.5,  25529.0, -7451.0),  # 24tr, R2 A06 lower-MDD sub-regime
    ("BND4", "high-ATR low-freq",         22.0,   6.0, 2.5, 2.5,  25208.0, -7207.0),  # 11tr, R1 A03 lowest-MDD contrast
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_hunter2_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_hunter2_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_hunter2_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BND1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Daily HUNTER2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Daily HUNTER2 OOS champion selection (auto-range merged)")
