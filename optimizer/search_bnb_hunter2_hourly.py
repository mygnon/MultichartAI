"""
search_bnb_hunter2_hourly.py — SFJ_HUNTER2_crypto on BNBUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)
...
BTC HUNTER2 Hourly converged to an asymmetric ultra-long regime (LEN_L=225 LEN_S=825 ATR_L=4.25
ATR_S=1.75, Obj-max). A07 here pushes into that asym ultra-long region; adaptive zooms refine.
...
Attempt schedule (11 attempts):
  ...
  A07 ultra_long   : LEN_L(150-600 s50)×LEN_S(400-1000 s75)×ATR_L(1-5 s1)×ATR_S(1-5 s1) = 10×9×5×5=2250
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_hunter2_hourly_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_hunter2_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met, obj_best):
    ...
    out = OUTPUT_DIR / "final_params_bnb_hunter2_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto on BNBUSDT HOT Hourly — Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    A = 5
    _n = "05_asym_ma"
    _c = _cfg(_n, (5, 95, 10), (100, 400, 50), (0.5, 5.5, 1), (0.5, 5.5, 1))
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  ultra_long — asym ultra-long region where BTC HUNTER2 champion lived
    #      LEN_L(150-600 s50)×LEN_S(400-1000 s75)×ATR_L(1-5 s1)×ATR_S(1-5 s1) = 10×9×5×5=2250
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_ultra_long"
    _c = _cfg(_n, (150, 600, 50), (400, 1000, 75), (1, 5, 1), (1, 5, 1))
    log.info("A07  LEN_L(150-600 s50)×LEN_S(400-1000 s75)×ATR_L(1-5 s1)×ATR_S(1-5 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _t = time.time(); _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs(), _t)
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto BNBUSDT HOT Hourly Round-1 COMPLETE  (%.1f min total)",
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto BNBUSDT HOT Hourly NP>100K USD Round-1 search")
