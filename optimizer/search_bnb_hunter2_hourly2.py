"""
search_bnb_hunter2_hourly2.py — SFJ_HUNTER2_crypto on BNBUSDT HOT Hourly, Round 2 (IS 2022/01-2026/01)

R1 found BNB's regime = MID-LEN (distinct from BTC/ETH ultra-long), but 5/11 attempts FAILED
(MC64 wizard didn't open in 30s — automation flakiness, not data) and the zoom combos blew past 5000:

  R1 Obj-max ⭐ (A10): LEN_L=136 LEN_S=187 ATR_L=3.75 ATR_S=2.75  NP=$20,118 MDD=-$5,687 Obj=71,169 271tr
  R1 NP-max   (A10) : LEN_L=136 LEN_S=198 ATR_L=3.75 ATR_S=3.0   NP=$21,392 MDD=-$6,761 Obj=67,681 256tr
  (BNB/BTC ~7.4x, BNB/ETH ~5.4x; $100K unreachable -79%)

R2 PURPOSE: (1) confirm the MID-LEN Obj-max regime via zoom; (2) RE-TEST the long-LEN / ultra-long
regions that FAILED in R1 (never measured); (3) robust run_or_load (cleanup strays + retry +
re-focus chart each attempt) so wizard-open flakiness no longer silently zeroes an attempt; (4)
zoom radius GUARANTEED <=5000 (final tiny-radius fallback). Zoom seed = Obj-max (user criterion).

11 attempts, each <=5000 combos. Objective = NetProfit² / |MaxDrawdown|.

Attempt schedule:
  A01 retest_center : LEN_L(126-146 s5)×LEN_S(177-197 s5)×ATR_L(3.25-4.25 s0.25)×ATR_S(2.25-3.25 s0.25) = 5×5×5×5=625  (Rule 5 drift)
  A02 mid_wide      : LEN_L(80-260 s20)×LEN_S(100-300 s20)×ATR_L(2.5-4.5 s0.5)×ATR_S(2-4 s0.5) = 10×11×5×5=2750
  A03 mid_fine      : LEN_L(120-160 s10)×LEN_S(170-210 s10)×ATR_L(3-4.5 s0.25)×ATR_S(2-3.5 s0.25) = 5×5×7×7=1225
  A04 lenL_sweep    : LEN_L(50-300 s25)×LEN_S(150-230 s20)×ATR_L(3.5-4.0 s0.25)×ATR_S(2.5-3.0 s0.25) = 11×5×3×3=495
  A05 lenS_sweep    : LEN_L(126-146 s10)×LEN_S(80-320 s20)×ATR_L(3.5-4.0 s0.25)×ATR_S(2.5-3.0 s0.25) = 3×13×3×3=351
  A06 long_LEN_retest  : LEN_L(100-500 s50)×LEN_S(100-500 s50)×ATR_L(2-4 s0.5)×ATR_S(1.5-3.5 s0.5) = 9×9×5×5=2025  (R1 A02/A05 failed)
  A07 ultra_long_retest: LEN_L(150-600 s50)×LEN_S(400-1000 s75)×ATR_L(1-5 s1)×ATR_S(1-5 s1) = 10×9×5×5=2250  (R1 A07 failed)
  A08 high_atr      : LEN_L(80-260 s40)×LEN_S(100-300 s40)×ATR_L(3-7 s0.5)×ATR_S(3-7 s0.5) = 5×6×9×9=2430
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_hunter2_hourly2_search")
...
# R2 seed = R1 MID-LEN Obj-max champion (A10)
SEED_LL   = 136.0
SEED_LS   = 187.0
SEED_ATRL = 3.75
SEED_ATRS = 2.75
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_hunter2_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r1_obj_max": {"LEN_L": 136, "LEN_S": 187, "ATR_multiplier_L": 3.75, "ATR_multiplier_S": 2.75,
                       "net_profit": 20117.5, "max_drawdown": -5686.69, "objective": 71169, "total_trades": 271},
        "notes": {
    ...
    out = OUTPUT_DIR / "final_params_bnb_hunter2_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    obj_best: Dict = {"LEN_L": SEED_LL, "LEN_S": SEED_LS, "ATR_multiplier_L": SEED_ATRL,
                      ...
                      "max_drawdown": -5687.0, "objective": SEED_OBJ, "total_trades": 271}
    np_best: Dict = {}
    target_met = False
    attempt_log: List[dict] = []

    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto BNBUSDT HOT Hourly — Round 2 (confirm MID-LEN Obj-max + retest failed regions)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (126, 146, 10), (177, 197, 10),
                                                 (3.5, 4.0, 0.25), (2.5, 3.0, 0.25)))
    ...
    # static attempts
    _do(1, "01_retest_center",     _cfg("01_retest_center",     (126,146,5),   (177,197,5),   (3.25,4.25,0.25),(2.25,3.25,0.25)))
    _do(2, "02_mid_wide",          _cfg("02_mid_wide",          (80,260,20),   (100,300,20),  (2.5,4.5,0.5),   (2,4,0.5)))
    _do(3, "03_mid_fine",          _cfg("03_mid_fine",          (120,160,10),  (170,210,10),  (3,4.5,0.25),    (2,3.5,0.25)))
    _do(4, "04_lenL_sweep",        _cfg("04_lenL_sweep",        (50,300,25),   (150,230,20),  (3.5,4.0,0.25),  (2.5,3.0,0.25)))
    _do(5, "05_lenS_sweep",        _cfg("05_lenS_sweep",        (126,146,10),  (80,320,20),   (3.5,4.0,0.25),  (2.5,3.0,0.25)))
    _do(6, "06_long_LEN_retest",   _cfg("06_long_LEN_retest",   (100,500,50),  (100,500,50),  (2,4,0.5),       (1.5,3.5,0.5)))
    _do(7, "07_ultra_long_retest", _cfg("07_ultra_long_retest", (150,600,50),  (400,1000,75), (1,5,1),         (1,5,1)))
    _do(8, "08_high_atr",          _cfg("08_high_atr",          (80,260,40),   (100,300,40),  (3,7,0.5),       (3,7,0.5)))
    ...
    # summary
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  BNB HUNTER2 Hourly Round-2 COMPLETE  (%.1f min total)", (time.time()-_RUN_T0)/60)
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto BNBUSDT HOT Hourly Round-2 (confirm mid-LEN Obj-max + retest)")
