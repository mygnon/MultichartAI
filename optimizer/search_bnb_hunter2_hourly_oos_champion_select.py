"""
search_bnb_hunter2_hourly_oos_champion_select.py — Out-of-sample (OOS) selection among the converged
BNB Hourly SFJ_HUNTER2_crypto champions (rounds 1-2, results/bnb_hunter2_hourly{,2}_search/).
...
BNB HUNTER2 Hourly converged to a MID-LEN regime (R2 ceiling Obj-max LEN_L=136 LEN_S=187 ATR_L=3.75
ATR_S=2.75, distinct from BTC/ETH ultra-long). Candidates span the mid-LEN core (lowest-MDD vs
highest-NP) plus a longer-LEN and a high-ATR low-freq contrast so the OOS test reveals which holds
its Max Drawdown out-of-sample.

TASK: re-test each previously-converged BNB Hourly HUNTER2 champion on this workspace and pick the
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_HUNTER_AI.wsp open, BNBUSDT HOT **60-Minute** chart tab ACTIVE, the
    SFJ_HUNTER2_crypto signal applied (Status ON)
  - Full data available to 2026/06/10 (the script sets the range itself)

CLI:
  py search_bnb_hunter2_hourly_oos_champion_select.py                 # merged: IS then FULL, then verdict
  py search_bnb_hunter2_hourly_oos_champion_select.py --period is     # one pass only
  py search_bnb_hunter2_hourly_oos_champion_select.py --candidate BNH1  # smoke one candidate
  py search_bnb_hunter2_hourly_oos_champion_select.py --from-csv      # recompute from existing CSVs
  py search_bnb_hunter2_hourly_oos_champion_select.py --probe-instrument  # verify auto data-range
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_hunter2_hourly_oos_champion_select_search")
PREFIX      = "BNBH2SEL_"
...
# 4 converged BNB Hourly HUNTER2 candidates (bnb_hunter2_hourly{,2}_search). BNB regime = MID-LEN.
# id, regime, LEN_L, LEN_S, ATR_multiplier_L, ATR_multiplier_S, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("BNH1", "R2 Obj-max mid-LEN ***",  136.0, 187.0, 3.75, 2.75, 20117.5, -5687.0),  # 271tr, R2 ceiling Obj=71,169 lowest MDD
    ("BNH2", "R2 NP-max mid-LEN",       140.0, 195.0, 3.7,  3.0,  21434.7, -6716.0),  # 254tr, highest IS NP
    ("BNH3", "long-LEN variant",        150.0, 200.0, 3.5,  3.0,  13662.0, -7104.0),  # 268tr, R2 A06 longer-LEN
    ("BNH4", "high-ATR low-freq",       260.0, 260.0, 6.0,  6.5,  13182.0, -6374.0),  # 74tr, R2 A08 contrast regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_hunter2_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_hunter2_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_hunter2_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BNH1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly HUNTER2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly HUNTER2 OOS champion selection (auto-range merged)")
