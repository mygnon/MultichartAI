"""
search_bnb_keltner_hourly.py -- SFJ_KeltnerTrend_crypto on BNBUSDT HOT Hourly, Round 2 integer-grid + boundary confirm

Strategy (Strategy/SFJ_KeltnerTrend_crypto.txt): Keltner volatility breakout + ATR chandelier
trailing stop, long+short, STOP entries, _Crypto1MUSD sizing.
  Mid=XAverage(C,EMALen); UpBand=Mid+BandMult*ATR(EMALen); DnBand=Mid-BandMult*ATR(EMALen)
  Long  entry : Buy       next bar at UpBand STOP (flat-only)
  Short entry : SellShort  next bar at DnBand STOP (flat-only)
  Exits : ATR chandelier trail (ATRMult x ATR(EMALen)) under/over the running extreme.

3 params: EMALen (EMA+ATR period, int), BandMult (band width, fractional), ATRMult (trail, fractional).
...
WORKSPACE  = r"C:\Users\Tim\Downloads\Multichart64\Tim\20260622_SFJ_KeltnerTrend_crypto_AI.wsp"
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "SFJ_KeltnerTrend_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_keltner_hourly2_search")
...
ATRL_LO, ATRL_HI = 0.5,  10.0   # BandMult
MULT_LO, MULT_HI = 0.5,  20.0

SEED_LEN, SEED_ATRL, SEED_MULT = 107.0, 1.0, 5.4  # EMALen, BandMult, ATRMult (R1 champion)

PREFIX = "BNBKEL2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_keltner_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, atrl, mult) -> StrategyConfig:
    ...
    atrl   = _safe(atrl,   ATRL_LO, ATRL_HI, 0.25)  # BandMult fractional
    ...
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[ParamAxis("EMALen",    *length),
                ParamAxis("BandMult", *atrl),
...
def _row(df, idx):
    b = df.loc[idx]
    return {"EMALen": float(b["EMALen"]), "BandMult": float(b["BandMult"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "Keltner STOP-breakout (UpBand/DnBand = EMA+/-BandMult*ATR(EMALen)); ATR chandelier trailing-stop exits (ATRMult x ATR(EMALen)); flat-only; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "EMALen (EMA+ATR period int), BandMult (band width frac), ATRMult (trail frac); defaults 20/2.0/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_keltner_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_KeltnerTrend_crypto on BNBUSDT HOT Hourly -- Round 2 integer-grid + boundary confirm (IS 2022/01-2026/01)")
    log.info("  Params: EMALen, BandMult, ATRMult.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_atrl, seed_mult = om["EMALen"], om["BandMult"], om["ATRMult"]
        ...
        log.info("  [A%02d %-16s] obj_max EMA=%.4g Band=%.4g Mult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["EMALen"], om["BandMult"], om["ATRMult"],
    ...
    # static attempts (3D)
    # static attempts (INTEGER EMALen, long-EMA regime focus)
    _do(1, "01_rule5_retest", _cfg("01_rule5_retest", (90,125,5),  (0.5,1.5,0.25),(4,7,0.5)))     # 280
    _do(2, "02_ema_fine",     _cfg("02_ema_fine",     (60,160,2),  (0.75,1.25,0.25),(5,6,0.5)))   # int EMA fine
    _do(3, "03_band_scan",    _cfg("03_band_scan",    (90,125,5),  (0.25,3,0.25), (4.5,6.5,0.5))) # Band 0.25-3
    _do(4, "04_mult_scan",    _cfg("04_mult_scan",    (95,120,5),  (0.75,1.5,0.25),(2,10,0.25)))  # Mult 2-10
    _do(5, "05_short_ema",    _cfg("05_short_ema",    (20,80,2),   (0.5,2,0.5),   (4,7,0.5)))      # does short EMA work?
    _do(6, "06_vlong_ema",    _cfg("06_vlong_ema",    (120,260,5), (0.5,1.5,0.25),(4.5,6.5,0.5))) # push EMA longer
    _do(7, "07_narrow_band",  _cfg("07_narrow_band",  (90,140,5),  (0.25,1.25,0.25),(4,10,0.5)))  # narrow band x wide trail
    _do(8, "08_ema_dense",    _cfg("08_ema_dense",    (95,120,1),  (0.5,1.5,0.25),(5,6,0.5)))      # EMA s1 dense core
    _do(9, "09_global_int",   _cfg("09_global_int",   (30,200,10), (0.5,3,0.5),   (3,9,1)))        # global integer
    ...
    log.info("==============================================================")
    log.info("  SFJ_KeltnerTrend_crypto BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["EMALen"], obj_best["BandMult"], obj_best["ATRMult"],
    ...
    if np_best:
        log.info("  NP-max: LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f Obj=%.0f",
                 np_best["EMALen"], np_best["BandMult"], np_best["ATRMult"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_KeltnerTrend_crypto BNBUSDT HOT Hourly R1 search")
