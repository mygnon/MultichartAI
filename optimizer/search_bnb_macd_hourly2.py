...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_macd_hourly2_search")
...
SEED_FAST, SEED_SLOW, SEED_MACD = 60.0, 116.0, 34.0

PREFIX = "BNBMACDH2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_macd_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def zoom_int(center, radius, n_target, lo, hi):
    c = round(center)
    lo_val = max(round(lo), c - round(radius))
    hi_val = min(round(hi), c + round(radius))
    if lo_val >= hi_val:
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_macd_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_MACD_Strategy03_crypto on BNBUSDT HOT Hourly -- Round 2 integer-grid + boundary confirm (IS 2022/01-2026/01)")
    log.info("  Params: FastLength, SlowLength, MACDLength.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    log.info("  R1: short/mid MACD ALL-LOSING; only long regime works ~F60/S116/M34 NP~$8.5K MDD/NP~99%%.")
    log.info("  Integer grids only.  Run start: %s", datetime.now().isoformat())
    log.info("==============================================================")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (58, 62, 2), (112, 120, 4), (32, 36, 2)))
    ...
    # static attempts (3D INTEGER grids in the long-period regime)
    _do(1, "01_rule5_core",   _cfg("01_rule5_core",   (50,70,2),  (105,130,5),  (28,40,2)))   # 11x6x7=462
    _do(2, "02_fast_fine",    _cfg("02_fast_fine",    (40,80,2),  (110,125,5),  (32,38,2)))   # 21x4x4=336
    _do(3, "03_slow_push",    _cfg("03_slow_push",    (52,66,2),  (100,180,10), (30,40,2)))   # 8x9x6=432
    _do(4, "04_macd_push",    _cfg("04_macd_push",    (54,64,2),  (110,130,5),  (20,60,4)))   # 6x5x11=330
    _do(5, "05_longer_all",   _cfg("05_longer_all",   (50,100,10),(120,250,15), (30,70,8)))   # 6x9x6=324 boundary
    _do(6, "06_dense_core",   _cfg("06_dense_core",   (54,66,1),  (108,128,4),  (30,40,1)))   # 13x6x11=858
    _do(7, "07_midlong_check",_cfg("07_midlong_check",(20,50,2),  (60,110,5),   (15,35,2)))   # 16x11x11=1936 (does mid-long work?)
    _do(8, "08_fast_low_slow",_cfg("08_fast_low_slow",(40,70,2),  (80,110,3),   (24,40,2)))   # 16x11x9=1584
    _do(9, "09_macd_lt_slow", _cfg("09_macd_lt_slow", (45,75,3),  (110,160,5),  (40,80,4)))   # 11x11x11=1331

    # adaptive zooms around the running Obj-max (INTEGER grids)
    for A, nm_, (rF, rS, rM, nF, nS, nM) in [
        (10, "10_adaptive_zoom1", (10, 22, 8, 11, 11, 9)),
        (11, "11_adaptive_zoom2", (6,  14, 5, 13, 11, 11)),
        (12, "12_adaptive_zoom3", (3,  8,  3, 7,  9,  7)),
    ]:
        log.info("A%02d  %s -- center F=%.4g S=%.4g M=%.4g Obj=%.0f",
                 A, nm_, seed_fast, seed_slow, seed_macd, obj_best.get("objective", 0))
        if start_attempt <= A:
            _fast = zoom_int(seed_fast, rF, nF, FAST_LO, FAST_HI)
            _slow = zoom_int(seed_slow, rS, nS, SLOW_LO, SLOW_HI)
            _macd = zoom_int(seed_macd, rM, nM, MACD_LO, MACD_HI)
    ...
    log.info("==============================================================")
    log.info("  SFJ_MACD_Strategy03_crypto BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
