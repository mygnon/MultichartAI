"""
search_bnb_osc_hourly2.py -- _2021Basic_Osc_crypto on BNBUSDT HOT Hourly, Round 2 (boundary confirm)

R1 ceiling: Obj-max $13,992 (LEN=13 LE=-0.5 SE=3 STP=1 LMT=13.5, MDD=-1905, 178tr, Obj=102787, A10=A11=A12);
            NP-max  $14,139 (LEN=15 LE=0 SE=3 STP=2 LMT=20, MDD=-6506, 292tr; worse Obj).
BNB = ASYMMETRIC regime (LE~0 long near MA, SE=3 short far above) -- a 3rd distinct regime vs ETH deep-LE
mean-reversion and BTC LE>0 momentum. R1 SE=3 sits on the grid boundary; R2 confirms by pushing SE higher
(3-6) + LE confirm (-2..+1) + LEN fine (8-20) + LMT (10-22) + Rule-5 retest.
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_osc_hourly2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_osc_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r1_champion": {"obj_max": "LEN=13 LE=-0.5 SE=3 STP=1 LMT=13.5 NP=13992 MDD=-1905 178tr Obj=102787 (A10=A11=A12)",
                        "np_max": "LEN=15 LE=0 SE=3 STP=2 LMT=20 NP=14139 MDD=-6506 292tr"},
        "notes": {
            ...
            "r2_goal": "confirm ceiling: push SE above 3, LE confirm -2..+1, fine LEN 8-20, LMT 10-22, Rule-5 retest",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_osc_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  _2021Basic_Osc_crypto BNBUSDT HOT Hourly -- Round 2 boundary confirm (IS 2022/01-2026/01)")
    log.info("  R1 ceiling Obj $102,787 / NP $14,139 (LEN=13 LE=-0.5 SE=3 STP=1 LMT=13.5 ASYM)")
    log.info("  Pushing SE>3, LE confirm, LEN 8-20.  Obj=NP^2/|MDD|.  Target NP %.0f", TARGET_NP)
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (12, 14, 1), (-1, 0, 0.5),
                                                 (2.5, 3.5, 0.5), (0.5, 1.5, 0.5), (12, 15, 1)))
    ...
    _do(1, "01_rule5_retest",   _cfg("01_rule5_retest",   (11,15,1),  (-1.5,0.5,0.5),  (2.5,3.5,0.5), (0.5,1.5,0.5),    (12,15,1)))
    _do(2, "02_SE_push_high",   _cfg("02_SE_push_high",   (10,16,2),  (-1.5,0.5,0.5),  (3,6,0.5),     (0.5,1.5,0.5),    (11,15,2)))
    _do(3, "03_LE_confirm",     _cfg("03_LE_confirm",     (10,16,2),  (-2,1,0.5),      (2.5,3.5,0.5), (0.5,1.5,0.5),    (11,15,2)))
    _do(4, "04_LEN_fine",       _cfg("04_LEN_fine",       (8,20,1),   (-1,0,0.5),      (2.5,3.5,0.5), (0.5,1.5,0.5),    (12,15,1)))
    _do(5, "05_LMT_STP_refine", _cfg("05_LMT_STP_refine", (11,15,2),  (-1,0,0.5),      (2.5,3.5,0.5), (0.5,2.0,0.25),   (10,22,1)))
    _do(6, "06_highSE_corner",  _cfg("06_highSE_corner",  (9,17,2),   (-1.5,0.5,0.5),  (3.5,6,0.5),   (0.5,1.5,0.5),    (11,15,2)))
    ...
    log.info("==============================================================")
    log.info("  _2021Basic_Osc_crypto BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="_2021Basic_Osc_crypto BNBUSDT HOT Hourly R2 boundary-confirm")
