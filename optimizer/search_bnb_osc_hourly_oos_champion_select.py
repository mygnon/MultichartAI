"""
search_bnb_osc_hourly_oos_champion_select.py -- OOS selection among the converged
BNB Hourly _2021Basic_Osc_crypto champions (R1-R2, results/bnb_osc_hourly{,2}_search/).

_2021Basic_Osc_crypto (5 params LEN, LE, SE, STP, LMT; BB-oscillator STOP entries, ATR exits,
_Crypto1MUSD).  BNB HOURLY IS ceiling (ASYMMETRIC regime LE~0 long / SE=3 short, a 3rd regime vs ETH mean-reversion & BTC
momentum): Obj-max=NP-max LEN=13 LE=-0.5 SE=3 STP=1 LMT=19.5 ($15,113 MDD-1919, 166tr, R2 B07=B08=B09);
strongest crypto Osc (ETH 9.1x, BTC 16x). $100K unreachable -84.9%.
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_BASIC_OSC_AI.wsp open, BNBUSDT HOT HOURLY chart tab ACTIVE, _2021Basic_Osc_crypto ON
  - Full data to 2026/06/10 (the script sets the range itself). CLOSE the Study Editor window.

CLI:
  py search_bnb_osc_hourly_oos_champion_select.py                  # merged: IS then FULL, verdict
  py search_bnb_osc_hourly_oos_champion_select.py --candidate BNO1  # smoke one candidate
  py search_bnb_osc_hourly_oos_champion_select.py --from-csv       # recompute from existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_osc_hourly_oos_champion_select_search")
PREFIX      = "BNBOSCSEL_"
...
STEP = {"LEN": 1.0, "LE": 0.5, "SE": 0.5, "STP": 0.25, "LMT": 0.5}
LO   = {"LEN": 2.0, "LE": -10.0, "SE": -10.0, "STP": 0.1, "LMT": 0.5}
HI   = {"LEN": 200.0, "LE": 10.0, "SE": 10.0, "STP": 50.0, "LMT": 200.0}

# Converged BNB Hourly _2021Basic_Osc_crypto candidates (bnb_osc_hourly{,2}_search).
# id, regime, LEN, LE, SE, STP, LMT, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("BNO1", "R2 Obj-max=NP-max ***", 13.0, -0.5, 3.0,  1.0, 19.5, 15113.0, -1919.0),  # 166tr, R2 converged
    ("BNO2", "R1 Obj-max low-LMT",    13.0, -0.5, 3.0,  1.0, 13.5, 13992.0, -1905.0),  # 178tr, R1 champion
    ("BNO3", "R1 NP-max hi-freq",     15.0,  0.0, 3.0,  2.0, 20.0, 14139.0, -6506.0),  # 292tr, deep MDD
    ("BNO4", "R1 alt deep-MR hi-freq",10.0, -2.0, 2.5,  1.0, 12.0,  8467.0, -3663.0),  # 495tr, distinct regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_osc_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, len_, le, se, stp, lmt) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="hourly",
        bar_period=60,
        params=[_axis("LEN", len_), _axis("LE", le), _axis("SE", se),
                _axis("STP", stp), _axis("LMT", lmt)],
...
def _pick(df, len_, le, se, stp, lmt):
    m = df
    for nm, v in (("LEN", len_), ("LE", le), ("SE", se), ("STP", stp), ("LMT", lmt)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_osc_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_osc_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BNO1", "is", *CANDIDATES[0][2:7]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly Osc -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
        ...
        for (cid, regime, len_, le, se, stp, lmt, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, len_, le, se, stp, lmt)
            log.info("--- [%s] %s %s LEN=%g LE=%g SE=%g STP=%g LMT=%g [%s] ---",
                     period, cid, regime, len_, le, se, stp, lmt, datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"LEN": len_, "LE": le, "SE": se, "STP": stp, "LMT": lmt},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, len_, le, se, stp, lmt) if (df is not None and not df.empty) else None
            if row is not None:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly _2021Basic_Osc_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="BNOn", default=None)
