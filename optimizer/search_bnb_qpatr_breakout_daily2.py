"""
search_bnb_qpatr_breakout_daily2.py  QuantPassATR_Breakout on BNBUSDT HOT Daily, Round 2

R1 Summary (11 attempts):
  NP-max + Obj-max ⭐⭐⭐ A09=A10=A11 triple-conv: Len=18 Mult=0.74 NP=$20,317 MDD=-$5,066 114tr Obj=81,486
  A02 sub:  Len=17 Mult=0.79 NP=$20,160 98tr Obj=80,230
  A04 same: Len=18 Mult=0.75 NP=$19,902 112tr Obj=78,196
  A07 same: Len=17 Mult=0.78 NP=$18,718 104tr Obj=69,166
  A01 long-Len: Len=70 Mult=0.74 NP=$17,186 115tr (same Mult, different Len)
  A03/A05 UI fails (rows truncated)

R2 Plan (11 attempts, <=5000 combos each, 2D grids):
  Goal: confirm Len=18 Mult=0.74 ultra-fine + explore A01 long-Len regime + redo R1 failures + untested Mult 1.5-3.

  A01 A09_ultrafine        Len(15-22 s1)   xMult(0.70-0.80 s0.005)  [8x21=168]
  A02 A02_region_ultrafine Len(14-20 s1)   xMult(0.75-0.85 s0.005)  [7x21=147]
  A03 bridge_18_70         Len(18-70 s2)   xMult(0.70-0.80 s0.01)   [27x11=297]
  A04 redo_wide_multi      Len(20-220 s5)  xMult(2.0-6.0 s0.1)      [41x41=1681]
  A05 redo_long_len_lowM   Len(80-200 s5)  xMult(0.5-1.0 s0.02)     [25x26=650]
  A06 Mult_below_1_fine    Len(15-30 s1)   xMult(0.60-1.00 s0.005)  [16x81=1296]
  A07 Mult_1.5_to_3        Len(10-50 s1)   xMult(1.5-3.0 s0.025)    [41x61=2501]
  A08 global_recheck       Len(5-300 s10)  xMult(0.5-5.0 s0.05)     [30x91=2730]
  A09-A11 adaptive zoom (ultra-tight from Len=18 Mult=0.74)
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_qpatr_breakout_daily2_search")
...
# R1 NP-max champion as seed (triple-conv)
SEED_LEN  = 18.0
SEED_MULT = 0.74
SEED_NP   = 20317.0
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_qpatr_breakout_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result":"R1 NP+Obj-max triple-conv A09=A10=A11: Len=18 Mult=0.74 NP=$20,317 MDD=-$5,066 114tr Obj=81,486",
            "r2_plan":  "Ultra-fine A09 + A02 region + bridge 18-70 + redo R1 failures + Mult<1 ultra-fine + Mult 1.5-3 + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_qpatr_breakout_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout on BNBUSDT HOT Daily -- Round 2")
    log.info("  Target: %.0f USD", TARGET_NP)
    log.info("  R1 NP+Obj-max triple-conv: Len=18 Mult=0.74 NP=$20,317 Obj=81,486")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_A09_ultrafine",        (15, 22, 1),      (0.70, 0.80, 0.005)),
        ("02_A02_region_ultrafine", (14, 20, 1),      (0.75, 0.85, 0.005)),
        ("03_bridge_18_70",         (18, 70, 2),      (0.70, 0.80, 0.01)),
        ("04_redo_wide_multi",      (20, 220, 5),     (2.0, 6.0, 0.1)),
        ("05_redo_long_len_lowM",   (80, 200, 5),     (0.5, 1.0, 0.02)),
        ("06_Mult_below_1_fine",    (15, 30, 1),      (0.60, 1.00, 0.005)),
        ("07_Mult_1.5_to_3",        (10, 50, 1),      (1.5, 3.0, 0.025)),
    ...
    # Adaptive zooms (ultra-tight from R1 champion Len=18 Mult=0.74)
    zoom_configs = [
        (9,  "09_adaptive_zoom1", 0.15, 0.10, 25, 33),
        (10, "10_adaptive_zoom2", 0.08, 0.05, 17, 21),
        (11, "11_adaptive_zoom3", 0.04, 0.025, 11, 15),
    ]
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout BNBUSDT Daily Round-2 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATR_Breakout BNBUSDT HOT Daily R2 parameter search")
