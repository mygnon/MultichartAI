"""
search_bnb_qpatr_breakout_hourly.py  QuantPassATR_Breakout on BNBUSDT HOT Hourly, Round 1
...
Target: NP > 100,000 USD (10x BTC/ETH target -- BNB is original instrument)
Insample: 2019/01/01 - 2026/01/01

Reference ceilings:
  QPATR_Breakout BTC Hourly: $2,748 (Len=212 Mult=3.27)
  QPATR_Breakout ETH Hourly: $4,444 (Len=32 Mult=3.37, 8-conv)
  QPATRex BNB Hourly:        $39,921 (Len=235 Su=0.586 Ni=2.128) -- strategy's strongest

BNB is strategy's ORIGINAL test instrument per docx (Binance BNBUSDT 60min).
QPATRex BNB/BTC ratio = 12.1x. If similar for QPATR_Breakout:
  Expected ~$33K (2748 * 12.1)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_qpatr_breakout_hourly_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_qpatr_breakout_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "note":     "BNB is strategy's original instrument. QPATRex BNB Hourly $39,921 (strategy strongest).",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_qpatr_breakout_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout on BNBUSDT HOT Hourly -- Round 1")
    log.info("  Target: %.0f USD (10x BTC/ETH target)", TARGET_NP)
    log.info("  BNB is strategy's ORIGINAL test instrument per docx")
    log.info("  Reference: QPATRex BNB Hourly $39,921; QPATR_Breakout BTC $2,748 / ETH $4,444")
    log.info("==============================================================")
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout BNBUSDT Hourly Round-1 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATR_Breakout BNBUSDT HOT Hourly R1 parameter search")
