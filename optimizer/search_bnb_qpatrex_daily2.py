"""
search_bnb_qpatrex_daily2.py  QuantPassATRex on BNBUSDT HOT Daily, Round 2

R1 Summary:
  NP-max A09=A10=A11 triple-conv: Len=25 Su=0.946 Ni=1.1055 NP=$26,945 MDD=-$8,424 21tr Obj=86,185
  Obj-max A05: Len=100 Su=0.39 Ni=0.5 NP=$25,724 MDD=-$5,815 87tr Obj=113,788
  A03 wide_multi UI failed (same as BTC Daily R1)
  BNB Daily -32.5% vs BNB Hourly $39,921 (follows ETH pattern); still 7.7x BTC Daily.

R2 Plan (11 attempts, <=5000 combos each):
  Priority: fine-zoom A05 Obj regime (never zoomed) + A09 NP ultra-fine + bridges.

  A01 A05_finezoom     Len(80-130 s5) xSu(0.30-0.50 s0.02) xNi(0.40-0.65 s0.02)   [11x11x13=1573]
  A02 A09_ultrafine    Len(20-30 s1)  xSu(0.85-1.05 s0.02) xNi(1.0-1.25 s0.015)   [11x11x17=2057]
  A03 bridge_25_100    Len(28-95 s7)  xSu(0.4-1.0 s0.05)   xNi(0.5-1.2 s0.05)     [11x13x15=2145]
  A04 highFreq         Len(50-150 s10)xSu(0.15-0.45 s0.025)xNi(0.2-0.55 s0.025)   [11x13x15=2145]
  A05 wide_retry       Len(20-220 s20)xSu(1.0-6.0 s0.5)    xNi(2.0-12.0 s1.0)     [11x11x11=1331]
  A06 longLen          Len(150-300 s15)xSu(0.2-0.7 s0.04)  xNi(0.5-1.5 s0.08)     [11x13x14=2002]
  A07 A02_region_zoom  Len(10-22 s1)  xSu(1.0-1.4 s0.025)  xNi(1.1-1.4 s0.02)     [13x17x16=3536]
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_qpatrex_daily2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_qpatrex_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result": "R1 NP-max triple-conv: Len=25 Su=0.946 Ni=1.1055 NP=$26,945; Obj-max A05: Len=100 Su=0.39 Ni=0.5 NP=$25,724 87tr Obj=113,788",
            "r2_plan":   "Fine-zoom A05 Obj regime + A09 NP ultra-fine + bridges + high-freq + longLen + A02 region + wide retry",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_qpatrex_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on BNBUSDT HOT Daily -- Round 2")
    ...
    log.info("  R1 Obj-max A05: Len=100 Su=0.39 Ni=0.5 NP=$25,724 87tr Obj=113,788")
    log.info("  Goal: fine-zoom A05 + A09 ultra-fine + bridges + high-freq")
    log.info("==============================================================")
    ...
    # A01  A05_finezoom (R1 A05 Obj champion regime)
    A = 1
    _n = "01_A05_finezoom"
    _c = _cfg(_n, (80, 130, 5), (0.30, 0.50, 0.02), (0.40, 0.65, 0.02))
    log.info("A01  Len(80-130 s5)xSu(0.30-0.50 s0.02)xNi(0.40-0.65 s0.02)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A01 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A02  A09_ultrafine (R1 NP-max regime)
    A = 2
    _n = "02_A09_ultrafine"
    _c = _cfg(_n, (20, 30, 1), (0.85, 1.05, 0.02), (1.0, 1.25, 0.015))
    log.info("A02  Len(20-30 s1)xSu(0.85-1.05 s0.02)xNi(1.0-1.25 s0.015)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    A = 3
    _n = "03_bridge_25_100"
    _c = _cfg(_n, (28, 95, 7), (0.4, 1.0, 0.05), (0.5, 1.2, 0.05))
    log.info("A03  Len(28-95 s7)xSu(0.4-1.0 s0.05)xNi(0.5-1.2 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    A = 4
    _n = "04_highFreq"
    _c = _cfg(_n, (50, 150, 10), (0.15, 0.45, 0.025), (0.2, 0.55, 0.025))
    log.info("A04  Len(50-150 s10)xSu(0.15-0.45 s0.025)xNi(0.2-0.55 s0.025)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A04 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A05  wide_retry (R1 A03 UI failed)
    A = 5
    _n = "05_wide_retry"
    ...
    # A06  longLen (BNB Hourly champion was Len=235!)
    A = 6
    _n = "06_longLen"
    _c = _cfg(_n, (150, 300, 15), (0.2, 0.7, 0.04), (0.5, 1.5, 0.08))
    log.info("A06  Len(150-300 s15)xSu(0.2-0.7 s0.04)xNi(0.5-1.5 s0.08)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A06 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A07  A02_region_zoom (R1 A02 had NP=$26,043 at Len=15)
    A = 7
    _n = "07_A02_region_zoom"
    _c = _cfg(_n, (10, 22, 1), (1.0, 1.4, 0.025), (1.1, 1.4, 0.02))
    log.info("A07  Len(10-22 s1)xSu(1.0-1.4 s0.025)xNi(1.1-1.4 s0.02)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    if start_attempt <= A:
        r_len = max(5.0,   best_LEN * 0.15)
        r_su  = max(0.10,  best_SU  * 0.20)
        r_ni  = max(0.15,  best_NI  * 0.20)
    ...
    log.info("A10  adaptive_zoom2 -- center: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f",
             best_LEN, best_SU, best_NI, best_np)
    if start_attempt <= A:
        r_len = max(3.0,   best_LEN * 0.07)
        r_su  = max(0.05,  best_SU  * 0.10)
        r_ni  = max(0.08,  best_NI  * 0.10)
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex BNBUSDT Daily Round-2 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex BNBUSDT HOT Daily R2 parameter search")
