"""
search_bnb_qpatrex_hourly.py  QuantPassATRex on BNBUSDT HOT Hourly, Round 1
...
Params (MC64 input order): Len, Su_Multiple, Ni_Multiple
  Defaults: Len=55, Su_Multiple=0.75, Ni_Multiple=1.5

Target: NP > 100,000 USD  (10x BTC/ETH targets -- BNB was strategy's original test instrument)
Insample: 2019/01/01 - 2026/01/01

Note from QuantPassATRex.docx: strategy was DESIGNED for "Binance BNBUSDT 60min".
Therefore BNB may perform much better than BTC/ETH (which we found capped at $3,293/$5,198).
BNB price ~$600 -> ~1667 contracts/trade (vs BTC's 20, ETH's 333).

BTC/ETH reference ceilings (for intuition only):
  BTC R3: $3,293 (Len=125 long-Len, Len=13 v-short-Len with Ni=3)
  ETH R3: $5,198 (Len=24, Su=1.48, Ni=2.47); high-Ni Ni=5.4 best risk-adjusted
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_qpatrex_hourly_search")
INSAMPLE   = DateRange("2019/01/01", "2026/01/01")

TARGET_NP  = 100_000.0   # USD (10x BTC/ETH target -- BNB is strategy's original instrument)

LEN_LO,  LEN_HI  = 1.0,   2000.0
SU_LO,   SU_HI   = 0.01,  50.0
NI_LO,   NI_HI   = 0.01,  100.0

# Strategy defaults as nominal seed (no prior BNB results)
SEED_LEN = 55.0
SEED_SU  = 0.75
SEED_NI  = 1.5
SEED_NP  = 0.0
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_qpatrex_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "note":     "BNB is strategy's original instrument per docx -- target 100K USD (10x BTC/ETH)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_qpatrex_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on BNBUSDT HOT Hourly -- Round 1")
    ...
    log.info("  Note: BNB is strategy's ORIGINAL test instrument (per docx)")
    log.info("  Goal: broad regime discovery on BNB (no prior knowledge)")
    log.info("==============================================================")
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex BNBUSDT Hourly Round-1 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex BNBUSDT HOT Hourly R1 parameter search")
