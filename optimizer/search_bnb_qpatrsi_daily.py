"""
search_bnb_qpatrsi_daily.py — QuantPassRSI on BNBUSDT HOT Daily, Round 1 (IS 2022/01-2026/01)
...
Attempt schedule (11 attempts, 2D grids, <=5000):
  A01 global       Len(2-100 s2)   x RSI_Gap(2-50 s1)  = 50x49=2450
  A02 short_len    Len(2-50 s1)    x RSI_Gap(5-45 s1)  = 49x41=2009
  A03 mid_len      Len(30-150 s2)  x RSI_Gap(5-45 s1)  = 61x41=2501
  A04 long_len     Len(100-400 s5) x RSI_Gap(5-45 s1)  = 61x41=2501
  A05 low_gap      Len(2-100 s2)   x RSI_Gap(1-20 s1)  = 50x20=1000  (strong-momentum extreme)
  A06 high_gap     Len(2-100 s2)   x RSI_Gap(25-49 s1) = 50x25=1250  (weak filter near 50)
  A07 ultra_short  Len(2-30 s1)    x RSI_Gap(3-49 s1)  = 29x47=1363
  A08 vlong_len    Len(200-800 s15)x RSI_Gap(5-45 s1)  = 41x41=1681
...
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "QuantPassRSI"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_qpatrsi_daily_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_qpatrsi_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_qpatrsi_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI on BNBUSDT HOT Daily -- Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    # static attempts
    _do(1, "01_global",      _cfg("01_global",      (2,100,2),   (2,50,1)))
    _do(2, "02_short_len",   _cfg("02_short_len",   (2,50,1),    (5,45,1)))
    _do(3, "03_mid_len",     _cfg("03_mid_len",     (30,150,2),  (5,45,1)))
    _do(4, "04_long_len",    _cfg("04_long_len",    (100,400,5), (5,45,1)))
    _do(5, "05_low_gap",     _cfg("05_low_gap",     (2,100,2),   (1,20,1)))
    _do(6, "06_high_gap",    _cfg("06_high_gap",    (2,100,2),   (25,49,1)))
    _do(7, "07_ultra_short", _cfg("07_ultra_short", (2,30,1),    (3,49,1)))
    _do(8, "08_vlong_len",   _cfg("08_vlong_len",   (200,800,15),(5,45,1)))
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI BNBUSDT Daily Round-1 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="QuantPassRSI BNBUSDT HOT Daily R1 search")
