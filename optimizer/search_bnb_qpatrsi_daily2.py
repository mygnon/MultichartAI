"""
search_bnb_qpatrsi_daily2.py — QuantPassRSI on BNBUSDT HOT Daily, Round 2 (IS 2022/01-2026/01)
...
R1 Obj-max = NP-max Len=20 RSI_Gap=45 NP=$6,062 MDD=-$8,122 Obj=4,524 11tr (7x reproduced; short-Len
high-gap near-degenerate). 4 attempts garbled (sparse mid/long-Len). R2 uses COARSE grids in sparse
regions + integer confirm Gap=45 + denser mid-gap check. BNB Daily<Hourly ($6K vs $17.4K). $100K -94%.
...
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "QuantPassRSI"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_qpatrsi_daily2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_qpatrsi_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_qpatrsi_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI on BNBUSDT HOT Daily -- Round 2 (confirm Obj-max, coarse sparse; IS 2022/01-2026/01)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (18, 22, 2), (43, 47, 2)))
    ...
    # static attempts
    _do(1, "01_retest_center",  _cfg("01_retest_center",  (12,28,1),   (38,49,1)))
    _do(2, "02_fine_lengap",    _cfg("02_fine_lengap",    (5,45,1),    (35,49,1)))
    _do(3, "03_gap_fine",       _cfg("03_gap_fine",       (16,24,1),   (20,49,1)))
    _do(4, "04_mid_len_coarse", _cfg("04_mid_len_coarse", (30,200,10), (30,49,1)))
    _do(5, "05_long_len_coarse",_cfg("05_long_len_coarse",(100,500,20),(35,49,1)))
    _do(6, "06_mid_gap_dense",  _cfg("06_mid_gap_dense",  (5,60,2),    (20,44,2)))
    _do(7, "07_short_len_np",   _cfg("07_short_len_np",   (2,30,1),    (2,15,1)))
    _do(8, "08_broad_int",      _cfg("08_broad_int",      (2,120,2),   (35,49,1)))
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI BNBUSDT Daily Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="QuantPassRSI BNBUSDT HOT Daily R2 (confirm) search")
