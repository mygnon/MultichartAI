"""
search_bnb_qpatrsi_hourly_oos_champion_select.py — OOS selection among the converged
BNB Hourly QuantPassRSI champions (R1-R2, results/bnb_qpatrsi_hourly{,2}_search/).

QuantPassRSI (2 params Len, RSI_Gap; momentum: RSI>100-Gap -> BUY market, RSI<Gap -> SHORT market,
reversal exits, _Crypto1MUSD). IS converged to Obj-max Len=56 RSI_Gap=34 (sparse 16tr, deep MDD -12,161; strongest QuantPassRSI).
...
PREREQUISITES (MC64, elevated):
  - 20260101_QuantPassRSI_AI.wsp open, BNBUSDT HOT 60-Minute chart tab ACTIVE, QuantPassRSI signal ON
  - Full data to 2026/06/10 (the script sets the range itself). CLOSE the Study Editor window.

CLI:
  py search_bnb_qpatrsi_hourly_oos_champion_select.py                 # merged: IS then FULL, verdict
  py search_bnb_qpatrsi_hourly_oos_champion_select.py --candidate BS1 # smoke one candidate
  py search_bnb_qpatrsi_hourly_oos_champion_select.py --from-csv      # recompute from existing CSVs
"""
...
SYMBOL      = "BNBUSDT HOT"
SIGNAL      = "QuantPassRSI"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_qpatrsi_hourly_oos_champion_select_search")
PREFIX      = "BNBRSISEL_"
...
# 4 converged BNB Hourly QuantPassRSI candidates (bnb_qpatrsi_hourly{,2}_search).
# id, regime, Len, RSI_Gap, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("BS1", "R2 Obj-max mid-len ***", 56.0, 34.0, 17397.0, -12161.0),  # 16tr, R2 ceiling Obj=24,887
    ("BS2", "short-len",              12.0, 15.0,  9825.0,  -7227.0),  # 26tr, lower MDD
    ("BS3", "high-gap DENSE",         48.0, 46.0,  7237.0,  -7096.0),  # 297tr, lowest MDD, most active
    ("BS4", "very-long-len",         230.0, 44.0, 11771.0, -11456.0),  # 12tr, long-RSI contrast
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_qpatrsi_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_qpatrsi_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_qpatrsi_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BS1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly QuantPassRSI -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly QuantPassRSI OOS champion selection")
