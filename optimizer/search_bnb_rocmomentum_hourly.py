"""
search_bnb_rocmomentum_hourly.py -- SFJ_ROCmomentum_crypto on BNBUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_ROCmomentum_crypto.txt): Rate-of-Change momentum-magnitude breakout +
ATR(14) chandelier trailing exit, long+short, MARKET entries, _Crypto1MUSD sizing.
  ROCv = (Close - Close[ROCLen]) / Close[ROCLen] * 100
  Long  entry : ROCv cross over  ( ROCThresh)  -> Buy       market (flat-only)
  Short entry : ROCv cross under (-ROCThresh)  -> SellShort market (flat-only)
  Exits : ATR chandelier trail (ATRMult x ATR(14)) under/over the running extreme.

3 params: ROCLen (lookback, int), ROCThresh (percent, fractional), ATRMult (trail, fractional).
...
WORKSPACE  = r"C:\Users\Tim\Downloads\Multichart64\Tim\20260622_SFJ_ROCmomentum_crypto_AI.wsp"
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "SFJ_ROCmomentum_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_rocmomentum_hourly_search")
...
LEN_LO,  LEN_HI  = 2.0,  200.0   # ROCLen
ATRL_LO, ATRL_HI = 0.25, 30.0   # ROCThresh (percent)
MULT_LO, MULT_HI = 0.5,  20.0

SEED_LEN, SEED_ATRL, SEED_MULT = 20.0, 5.0, 3.0  # ROCLen, ROCThresh, ATRMult

PREFIX = "BNBROC_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_rocmomentum_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, atrl, mult) -> StrategyConfig:
    ...
    atrl   = _safe(atrl,   ATRL_LO, ATRL_HI, 0.25)  # ROCThresh fractional
    ...
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[ParamAxis("ROCLen",    *length),
                ParamAxis("ROCThresh", *atrl),
...
def _row(df, idx):
    b = df.loc[idx]
    return {"ROCLen": float(b["ROCLen"]), "ROCThresh": float(b["ROCThresh"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "ROC momentum breakout: ROCv=(C-C[ROCLen])/C[ROCLen]*100; long ROCv cross over +ROCThresh, short cross under -ROCThresh; ATR(14) chandelier trailing exit; flat-only; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "ROCLen (lookback int), ROCThresh (percent frac), ATRMult (trail frac); defaults 20/5.0/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_rocmomentum_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_ROCmomentum_crypto on BNBUSDT HOT Hourly -- Round 1 (IS 2022/01-2026/01)")
    log.info("  Params: ROCLen, ROCThresh, ATRMult.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_atrl, seed_mult = om["ROCLen"], om["ROCThresh"], om["ATRMult"]
        if not np_best or nm["net_profit"] > np_best.get("net_profit", -1):
            ...
            if nm["net_profit"] > TARGET_NP:
                target_met = True
        log.info("  [A%02d %-16s] obj_max ROCLen=%.4g Thr=%.4g M=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["ROCLen"], om["ROCThresh"], om["ATRMult"],
    ...
    # static attempts (3D)
    _do(1, "01_global_wide",  _cfg("01_global_wide",  (5,100,5),  (1,15,2),     (1.5,6.0,0.5)))   # ROCLen x ROCThresh% x Trail
    _do(2, "02_default_zone", _cfg("02_default_zone", (10,40,2),  (2,10,1),     (2.0,4.0,0.5)))
    _do(3, "03_roclen_fine",  _cfg("03_roclen_fine",  (2,50,2),   (3,7,1),      (2.5,4.5,0.5)))
    _do(4, "04_thresh_fine",  _cfg("04_thresh_fine",  (15,35,5),  (0.5,15,0.5), (2.5,4.0,0.5)))
    _do(5, "05_mult_fine",    _cfg("05_mult_fine",    (15,35,5),  (3,8,1),      (1.0,8.0,0.25)))
    _do(6, "06_short_roclen", _cfg("06_short_roclen", (2,20,1),   (2,10,2),     (2.0,5.0,0.5)))
    _do(7, "07_long_roclen",  _cfg("07_long_roclen",  (40,150,5), (2,8,2),      (2.0,5.0,0.5)))
    _do(8, "08_high_thresh",  _cfg("08_high_thresh",  (5,50,5),   (8,30,2),     (2.0,6.0,0.5)))
    _do(9, "09_global_bound", _cfg("09_global_bound", (2,150,12), (1,25,3),     (1.0,9.0,1.0)))
    ...
    log.info("==============================================================")
    log.info("  SFJ_ROCmomentum_crypto BNBUSDT Hourly Round-1 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["ROCLen"], obj_best["ROCThresh"], obj_best["ATRMult"],
    ...
    if np_best:
        log.info("  NP-max: LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f Obj=%.0f",
                 np_best["ROCLen"], np_best["ROCThresh"], np_best["ATRMult"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_ROCmomentum_crypto BNBUSDT HOT Hourly R1 search")
