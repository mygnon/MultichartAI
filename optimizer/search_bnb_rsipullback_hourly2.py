"""
search_bnb_rsipullback_hourly.py -- SFJ_RSIPullback_crypto on BNBUSDT HOT Hourly, R2 integer-grid + boundary confirm

Strategy (Strategy/SFJ_RSIPullback_crypto.txt): trend-filtered RSI pullback, long+short, MARKET
entries, ATR(14) chandelier trailing exits, _Crypto1MUSD. Buy dips in an uptrend / sell rips in a
downtrend.
  TrendMA=Average(C,TrendLen); RSIv=RSI(C,RSILen)
  Long  : Close>TrendMA and RSIv cross over RSIThresh        -> Buy market (flat-only)
  Short : Close<TrendMA and RSIv cross under (100-RSIThresh) -> SellShort market (flat-only)
  Exit  : ATR chandelier trail (ATRMult x ATR(14))

4 params: TrendLen (int), RSILen (int), RSIThresh (int), ATRMult (fractional).
...
Attempt schedule (12 attempts, 4D grids, <=5000):
  A01 global       Trend(20-200 s20) RSILen(7-21 s7)  Thresh(20-60 s10) Mult(2-5 s1)    = 600
  A02 default      Trend(50-150 s20) RSILen(10-20 s2) Thresh(30-50 s5)  Mult(2-4 s0.5)  = 900
  A03 trend_fine   Trend(20-300 s20) RSILen(10-18 s4) Thresh(35-45 s5)  Mult(2.5-4 s0.5)= 540
  A04 rsilen_fine  Trend(80-120 s20) RSILen(4-30 s2)  Thresh(30-50 s5)  Mult(2.5-4 s0.5)= 840
  A05 thresh_fine  Trend(80-120 s20) RSILen(10-18 s4) Thresh(15-55 s2)  Mult(2.5-4 s0.5)= 756
  A06 mult_fine    Trend(60-140 s20) RSILen(10-18 s4) Thresh(30-50 s10) Mult(1-8 s0.25) = 1305
  A07 short_trend  Trend(10-60 s5)   RSILen(8-16 s4)  Thresh(25-50 s5)  Mult(2-5 s1)    = 792
  A08 deep_pull    Trend(50-150 s25) RSILen(10-20 s5) Thresh(10-40 s5)  Mult(2-6 s1)    = 525
  A09 global_bound Trend(20-400 s40) RSILen(5-35 s10) Thresh(20-60 s10) Mult(1-9 s2)    = 1000
...
WORKSPACE  = r"C:\Users\Tim\Downloads\Multichart64\Tim\20260622_SFJ_RSIPullback_crypto_AI.wsp"
SYMBOL     = "BNBUSDT HOT"
SIGNAL     = "SFJ_RSIPullback_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_rsipullback_hourly2_search")
...
TR_LO,   TR_HI   = 5.0,  500.0
RL_LO,   RL_HI   = 2.0,  100.0
TH_LO,   TH_HI   = 2.0,  98.0
MULT_LO, MULT_HI = 0.5,  20.0

SEED_TR, SEED_RL, SEED_TH, SEED_MULT = 5.0, 6.0, 54.0, 9.0  # R1 champion

PREFIX = "BNBRSIP2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_rsipullback_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, trend, rsilen, thresh, mult) -> StrategyConfig:
    ...
    rsilen = _safe(rsilen, RL_LO,   RL_HI,   1.0)
    ...
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[ParamAxis("TrendLen",  *trend),
                ParamAxis("RSILen",    *rsilen),
                ParamAxis("RSIThresh", *thresh),
...
def _row(df, idx):
    b = df.loc[idx]
    return {"TrendLen": float(b["TrendLen"]), "RSILen": float(b["RSILen"]),
            "RSIThresh": float(b["RSIThresh"]), "ATRMult": float(b["ATRMult"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic": "Trend-filtered RSI pullback: long Close>MA(TrendLen) & RSI(RSILen) cross over RSIThresh; short mirror; ATR(14) chandelier trailing exit; flat-only; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "TrendLen (MA filter int), RSILen (RSI period int), RSIThresh (long cross level int), ATRMult (trail frac); defaults 100/14/40/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_rsipullback_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_RSIPullback_crypto on BNBUSDT HOT Hourly -- Round 2 integer-grid + boundary confirm (IS 2022/01-2026/01)")
    log.info("  Params: TrendLen, RSILen, RSIThresh, ATRMult.  Obj=NP^2/|MDD|.  Target %.0f", TARGET_NP)
    log.info("  R1: NP $20,831 (T5/RL6/Th54/M9 -- TrendLen->floor, wide-trail momentum, NOT pullback).")
    log.info("  R2 integer grids + confirm regime + push ATRMult.  Run start: %s", datetime.now().isoformat())
    log.info("==============================================================")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (3, 7, 2), (5, 7, 1),
                                                 (52, 56, 2), (8.5, 9.5, 0.5)))
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            sT, sR, sH, sM = om["TrendLen"], om["RSILen"], om["RSIThresh"], om["ATRMult"]
        if not np_best or nm["net_profit"] > np_best.get("net_profit", -1):
            ...
            if nm["net_profit"] > TARGET_NP:
                target_met = True
        log.info("  [A%02d %-16s] obj_max T=%.4g RL=%.4g Th=%.4g M=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["TrendLen"], om["RSILen"], om["RSIThresh"], om["ATRMult"],
    ...
    # static attempts (4D)
    # static attempts (INTEGER TrendLen/RSILen/RSIThresh; ATRMult fractional)
    _do(1, "01_rule5_retest", _cfg("01_rule5_retest", (2,12,2),  (4,10,1),  (48,60,3), (7,11,0.5)))    # 1890
    _do(2, "02_trend_floor",  _cfg("02_trend_floor",  (2,30,2),  (4,10,2),  (50,58,2), (8,10,0.5)))    # confirm TrendLen floor
    _do(3, "03_rsilen_scan",  _cfg("03_rsilen_scan",  (3,7,1),   (2,20,1),  (50,58,2), (8,10,0.5)))    # RSILen 2-20
    _do(4, "04_thresh_scan",  _cfg("04_thresh_scan",  (3,7,1),   (5,8,1),   (35,70,2), (8,10,0.5)))    # Thresh 35-70
    _do(5, "05_mult_push",    _cfg("05_mult_push",    (3,7,1),   (5,8,1),   (50,58,2), (6,16,0.5)))    # ATRMult 6-16
    _do(6, "06_trend_active", _cfg("06_trend_active", (20,200,10),(5,8,1),  (35,50,5), (7,10,1)))      # does real trend filter help?
    _do(7, "07_core_dense",   _cfg("07_core_dense",   (2,10,1),  (4,9,1),   (50,58,2), (8,10,0.5)))    # dense core
    _do(8, "08_ultra_wide_m", _cfg("08_ultra_wide_m", (3,7,1),   (5,8,1),   (50,58,2), (11,20,0.5)))   # M beyond 9
    _do(9, "09_deep_dip",     _cfg("09_deep_dip",     (3,9,2),   (5,10,1),  (20,50,2), (7,11,1)))      # deeper-dip Thresh
    ...
    for A, nm_, (rT_mul, rT_min, rR_mul, rR_min, rH_mul, rH_min, rM_mul, rM_min, nT, nR, nH, nM) in [
        ...
        if start_attempt <= A:
            _T = zoom_int(sT, max(rT_min, sT * rT_mul), nT, TR_LO, TR_HI)
            _R = zoom_int(sR, max(rR_min, sR * rR_mul), nR, RL_LO, RL_HI)
            _H = zoom_int(sH, max(rH_min, sH * rH_mul), nH, TH_LO, TH_HI)
    ...
    log.info("==============================================================")
    log.info("  SFJ_RSIPullback_crypto BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): T=%.4g RL=%.4g Th=%.4g M=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["TrendLen"], obj_best["RSILen"], obj_best["RSIThresh"], obj_best["ATRMult"],
    ...
    if np_best:
        log.info("  NP-max: T=%.4g RL=%.4g Th=%.4g M=%.4g NP=%.0f Obj=%.0f",
                 np_best["TrendLen"], np_best["RSILen"], np_best["RSIThresh"], np_best["ATRMult"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_RSIPullback_crypto BNBUSDT HOT Hourly R1 search")
