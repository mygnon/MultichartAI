"""
search_bnb_rsipullback_hourly_oos_champion_select.py -- OOS selection among the converged
BNB Hourly SFJ_RSIPullback_crypto champions (R1-R2, results/bnb_rsipullback_hourly{,2}_search/).

4 params TrendLen, RSILen, RSIThresh, ATRMult; trend-filtered RSI pullback, long+short, MARKET
entries, ATR(14) chandelier trailing exit, _Crypto1MUSD. R2 ceiling = strongest non-CT BNB strategy:
  best (B11)  TrendLen=7 RSILen=6 RSIThresh=55 ATRMult=9.02 (NP 25303 MDD-5800 179tr Obj 110393)
  stable      TrendLen=8 RSILen=6 RSIThresh=54 ATRMult=9.0  (NP 21025 MDD-5858 188tr, 3-conv)
"Pullback" degenerated to momentum + wide trail (TrendLen->floor, RSIThresh~55 not a deep dip).

TASK: re-test each converged champion and pick the one that earns the MOST out-of-sample profit
...
METHOD -- MERGED single-run, AUTO data-range:
  ...
  Each candidate = a 4D micro-grid (all params +/-1 step, Critical Rule 1); exact center row picked.
  Metric = Max Intraday Drawdown. Winner = max OOS_NP among PASS.

PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_RSIPullback_crypto_AI.wsp open, BNBUSDT HOT HOURLY chart tab ACTIVE,
    SFJ_RSIPullback_crypto signal ON. Full data to 2026/06/10. CLOSE the Study Editor.

CLI:
  py search_bnb_rsipullback_hourly_oos_champion_select.py                  # merged verdict
  py search_bnb_rsipullback_hourly_oos_champion_select.py --candidate RP1  # smoke one
  py search_bnb_rsipullback_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_rsipullback_hourly_oos_champion_select_search")
PREFIX      = "BNBRSIPSEL_"
...
STEP = {"TrendLen": 1.0, "RSILen": 1.0, "RSIThresh": 1.0, "ATRMult": 0.5}
LO   = {"TrendLen": 2.0, "RSILen": 2.0, "RSIThresh": 2.0,  "ATRMult": 0.5}
HI   = {"TrendLen": 500.0, "RSILen": 100.0, "RSIThresh": 98.0, "ATRMult": 20.0}

# Converged BNB Hourly RSIPullback candidates (bnb_rsipullback_hourly{,2}_search).
# id, regime, TrendLen, RSILen, RSIThresh, ATRMult, prior IS NP, prior IS MDD.
CANDIDATES = [
    ("RP1", "R2 best momentum ***",   7.0,  6.0, 55.0, 9.02, 25303.0, -5800.0),  # 179tr highest NP/Obj
    ("RP2", "R2 stable plateau",      8.0,  6.0, 54.0, 9.0,  21025.0, -5858.0),  # 188tr 3-conv robust
    ("RP3", "ultra-wide-trail lo-fq", 4.0,  5.0, 58.0, 17.0, 20199.0, -6808.0),  # 67tr distinct M=17
    ("RP4", "real-trend-filter",     30.0,  5.0, 50.0, 7.0,  12362.0, -6142.0),  # 253tr actual trend filter
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_rsipullback_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, trend, rsilen, thresh, mult) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="hourly",
        bar_period=60,
        params=[_axis("TrendLen", trend), _axis("RSILen", rsilen),
                _axis("RSIThresh", thresh), _axis("ATRMult", mult)],
...
def _pick(df, trend, rsilen, thresh, mult):
    m = df
    for nm, v in (("TrendLen", trend), ("RSILen", rsilen), ("RSIThresh", thresh), ("ATRMult", mult)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_rsipullback_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_rsipullback_hourly_oos_champion_select.json"
    periods = [] if from_csv else ([only_period] if only_period else PERIOD_ORDER)

    payload = {
        "symbol": SYMBOL, "signal": SIGNAL, "timeframe": "Hourly (60 min)",
        "method": "MERGED auto-range; set_instrument_data_range per pass; 4D micro-grid MCReport CSV; "
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("RP1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly RSIPullback -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
            mc.ensure_chart_ready(conn, _cfg(CANDIDATES[0][0], period, *CANDIDATES[0][2:6]))
        ...
        for (cid, regime, trend, rsilen, thresh, mult, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, trend, rsilen, thresh, mult)
            log.info("--- [%s] %s %s T=%g RL=%g Th=%g M=%g [%s] ---",
                     period, cid, regime, trend, rsilen, thresh, mult,
                     datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"TrendLen": trend, "RSILen": rsilen, "RSIThresh": thresh, "ATRMult": mult},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, trend, rsilen, thresh, mult) if (df is not None and not df.empty) else None
            if row is not None:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly SFJ_RSIPullback_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="RPn", default=None)
