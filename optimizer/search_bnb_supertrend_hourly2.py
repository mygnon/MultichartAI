"""
search_bnb_supertrend_hourly2.py — SFJ_SuperTrend_crypto on BNBUSDT HOT Hourly, Round 2
...
Priors (no SuperTrend crypto prior; 2-param ATR-breakout analog = QPATR_Breakout):
  BNB QPATR_Breakout Hourly: NP-max Len=3 Mult=2.965; Obj-max Len=145 Mult=2.91 ($35,634).
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_supertrend_hourly2_search")
...
SEED_AT, SEED_ML = 79.0, 6.625   # R1 champion
SEED_NP = 0.0

PREFIX = "BNBSTH2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_supertrend_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "priors":   "QPATR_Breakout BNB Hourly Len=3 Mult=2.965 (NP-max) / Len=145 Mult=2.91 (Obj-max $35,634). SuperTrend default ATRLength=10 Mult=3",
            "r1_plan":  "global + short/mid/long/vlong ATR + low/high mult + fine-default + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_bnb_supertrend_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly SFJ_SuperTrend_crypto NP>100K -- Round 2")
    log.info("  IS window %s ~ %s  (chart-trimmed)", IS_RANGE[0], IS_RANGE[1])
    log.info("  R1 champ ATR=79 Mult=6.625 NP=17453 (wide-band ~220tr; confirming/pushing Mult)", )
    log.info("==============================================================")

    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (74, 84, 2), (6.0, 7.0, 0.25)))
    ...
    attempts_config = [
        ("01_Mult_sweep",      (72, 88, 2),    (4.5, 10.0, 0.125)),
        ("02_ATR_sweep",       (50, 130, 2),   (6.0, 7.5, 0.125)),
        ("03_high_mult_push",  (60, 100, 4),   (7.0, 14.0, 0.25)),
        ("04_fine_zoom",       (74, 84, 1),    (6.0, 7.25, 0.05)),
        ("05_ATR_bridge_low",  (20, 80, 2),    (5.0, 8.0, 0.25)),
        ("06_ATR_bridge_high", (80, 220, 5),   (4.0, 8.0, 0.25)),
        ("07_vhigh_mult",      (50, 120, 5),   (8.0, 18.0, 0.5)),
        ("08_mid_mult_wideATR",(40, 160, 4),   (5.0, 7.5, 0.25)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly SuperTrend_crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Hourly SFJ_SuperTrend_crypto NP>100K Round-2 search")
