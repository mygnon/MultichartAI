"""
search_bnb_supertrend_hourly_oos_champion_select.py — OOS selection among the converged BNB Hourly
SFJ_SuperTrend_crypto champions (rounds 1-2, results/bnb_supertrend_hourly{,2}_search/).
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_SuperTrend_AI.wsp open, BNBUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
  - SFJ_SuperTrend_crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_bnb_supertrend_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_bnb_supertrend_hourly_oos_champion_select.py --period is
  py search_bnb_supertrend_hourly_oos_champion_select.py --candidate SS1
  py search_bnb_supertrend_hourly_oos_champion_select.py --from-csv
  py search_bnb_supertrend_hourly_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_supertrend_hourly_oos_champion_select_search")
PREFIX      = "BNBSTHSEL_"
...
# 4 DISTINCT converged BNB Hourly SuperTrend regimes (from bnb_supertrend_hourly{,2}_search).
# id, regime, ATRLength, Multiplier, prior IS NP, prior IS MDD  (reference only)
CANDIDATES = [
    ("SS1", "wide-band champion ***", 79.0,  6.625, 17453.0, -7614.0),  # R1=R2 peak, 220tr; NP/Obj-max
    ("SS2", "mid-ATR low-MDD",        52.0,  6.0,   11525.0, -5255.0),  # 268tr, lowest MDD, high-freq
    ("SS3", "high-mult low-freq",    105.0, 12.0,    9368.0, -6824.0),  # 80tr, distinct low-freq
    ("SS4", "long-ATR vhigh-freq",   190.0,  4.25,   9229.0, -7964.0),  # 489tr, distinct long-ATR
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_supertrend_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_supertrend_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_supertrend_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("SS1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly SuperTrend — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly SuperTrend OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="SSn", default=None)
