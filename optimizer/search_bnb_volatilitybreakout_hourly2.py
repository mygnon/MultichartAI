"""
search_bnb_volatilitybreakout_hourly.py -- SFJ_VolatilityBreakout_crypto on BNBUSDT HOT Hourly, Round 2 boundary confirm (IS 2022/01-2026/01)
...
ATRL_LO, ATRL_HI = 0.05, 5.0    # EntryMult (test below R1 floor 0.1)
MULT_LO, MULT_HI = 0.5,  20.0   # TrailMult

SEED_LEN, SEED_ATRL, SEED_MULT = 63.0, 0.1, 17.15  # ATRLen, EntryMult, TrailMult (R1 A10 best)

PREFIX = "BNBVB_R2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_volatilitybreakout_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, atrl, mult) -> StrategyConfig:
    ...
    atrl   = _safe(atrl,   ATRL_LO, ATRL_HI, 0.1)   # EntryMult fractional
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_volatilitybreakout_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_VolatilityBreakout_crypto on BNBUSDT HOT Hourly -- Round 2 boundary confirm (IS 2022/01-2026/01; seed R1 63/0.1/17.15)")
    ...
    # static attempts (3D)
    # R2 boundary confirmation around R1 A10 best: ATRLen=63 EntryMult=0.1 TrailMult=17.15
    _do(1, "01_retest_seed",  _cfg("01_retest_seed",  (60,66,3),   (0.05,0.15,0.05),(16.5,17.5,0.25)))  # Rule 5 in-session baseline
    _do(2, "02_entry_floor",  _cfg("02_entry_floor",  (55,70,5),   (0.05,0.5,0.05), (15,18,0.5)))       # confirm EntryMult floor
    _do(3, "03_trail_push",   _cfg("03_trail_push",   (55,70,5),   (0.05,0.2,0.05), (14,20,0.25)))      # push TrailMult to cap 20
    _do(4, "04_atrlen_sweep", _cfg("04_atrlen_sweep", (40,130,5),  (0.05,0.2,0.05), (15,19,0.5)))       # nail ATRLen
    _do(5, "05_atrlen_fine",  _cfg("05_atrlen_fine",  (50,80,2),   (0.1,0.1,0.05),  (16,18,0.25)))      # ATRLen fine at peak
    _do(6, "06_low_entry_2d", _cfg("06_low_entry_2d", (40,120,5),  (0.05,0.3,0.05), (16,18,0.5)))       # ATRLen x EntryMult low band
    _do(7, "07_wide_combo",   _cfg("07_wide_combo",   (45,100,5),  (0.05,0.25,0.05),(17,20,0.5)))       # ultra-wide trail combo
    _do(8, "08_short_atrlen", _cfg("08_short_atrlen", (10,55,3),   (0.05,0.3,0.05), (15,19,0.5)))       # confirm short-ATRLen worse
    _do(9, "09_micro_peak",   _cfg("09_micro_peak",   (58,70,1),   (0.05,0.2,0.05), (16.5,18,0.25)))    # micro grid at the peak
    ...
    for A, nm_, (rl_mul, rl_min, ra_mul, ra_min, rm_mul, rm_min, nL, nA, nM) in [
        ...
        if start_attempt <= A:
            ...
            _atrl = zoom_fixed(seed_atrl, max(ra_min, seed_atrl * ra_mul),  nA, 0.1,  ATRL_LO, ATRL_HI)
    ...
    log.info("==============================================================")
    log.info("  SFJ_VolatilityBreakout_crypto BNBUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_VolatilityBreakout_crypto BNBUSDT HOT Hourly R2 boundary-confirm")
