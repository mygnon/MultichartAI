"""
search_bnb_volatilitybreakout_hourly.py -- SFJ_VolatilityBreakout_crypto on BNBUSDT HOT Hourly, Round 3 final confirm (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_VolatilityBreakout_crypto.txt): volatility-breakout STOP entry + ATR chandelier
trailing stop, long+short, STOP entries, _Crypto1MUSD sizing.
  ATRv=AvgTrueRange(ATRLen); BuyLevel=Close+EntryMult*ATRv; SellLevel=Close-EntryMult*ATRv
  Long  entry : Buy       next bar at UpBand STOP (flat-only)
  Short entry : SellShort  next bar at DnBand STOP (flat-only)
  Exits : ATR chandelier trail (TrailMult x ATR(ATRLen)) under/over the running extreme.

3 params: ATRLen (ATR period, int), EntryMult (breakout buffer in ATR mults, frac), TrailMult (trail, frac).
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_volatilitybreakout_hourly_search")
...
LEN_LO,  LEN_HI  = 2.0,  200.0   # ATRLen
ATRL_LO, ATRL_HI = 0.01, 5.0    # EntryMult (test down to 0.01 to confirm ->0 floor)
MULT_LO, MULT_HI = 0.5,  20.0   # TrailMult

SEED_LEN, SEED_ATRL, SEED_MULT = 45.0, 0.05, 16.5  # ATRLen, EntryMult, TrailMult (R2 A04 best)

PREFIX = "BNBVB_R3_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_volatilitybreakout_hourly3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, atrl, mult) -> StrategyConfig:
    ...
    atrl   = _safe(atrl,   ATRL_LO, ATRL_HI, 0.025) # EntryMult fractional
    ...
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[ParamAxis("ATRLen",    *length),
                ParamAxis("EntryMult", *atrl),
                ParamAxis("TrailMult", *mult)],
...
def _row(df, idx):
    b = df.loc[idx]
    return {"ATRLen": float(b["ATRLen"]), "EntryMult": float(b["EntryMult"]),
            "TrailMult": float(b["TrailMult"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        "round": 3, "strategy": SIGNAL, "symbol": SYMBOL, "timeframe": "Hourly (60 min)",
        ...
        "notes": {
            "logic": "Volatility-breakout STOP entry (BuyLevel=Close+EntryMult*ATR(ATRLen); SellLevel=Close-EntryMult*ATR); ATR chandelier trailing-stop exits (TrailMult x ATR(ATRLen)); flat-only; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "ATRLen (ATR period int), EntryMult (breakout buffer ATR-mult frac), TrailMult (trail frac); defaults 14/1.0/3.0",
        },
    ...
    out = OUTPUT_DIR / "final_params_bnb_volatilitybreakout_hourly3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_VolatilityBreakout_crypto on BNBUSDT HOT Hourly -- Round 3 final confirm (IS 2022/01-2026/01; seed R2 45/0.05/16.5)")
    log.info("  Params: ATRLen, EntryMult, TrailMult.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (18, 22, 2), (12, 16, 2), (2.5, 3.5, 0.5)))
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_atrl, seed_mult = om["ATRLen"], om["EntryMult"], om["TrailMult"]
        if not np_best or nm["net_profit"] > np_best.get("net_profit", -1):
            ...
            if nm["net_profit"] > TARGET_NP:
                target_met = True
        log.info("  [A%02d %-16s] obj_max EMA=%.4g Band=%.4g Mult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["ATRLen"], om["EntryMult"], om["TrailMult"],
    ...
    # static attempts (3D)
    # R3 final confirmation around R2 A04 best: ATRLen=45 EntryMult=0.05 TrailMult=16.5
    _do(1, "01_retest_seed",  _cfg("01_retest_seed",  (43,47,1),   (0.025,0.075,0.025),(16,17,0.25)))   # Rule 5 in-session baseline
    _do(2, "02_atrlen_fine",  _cfg("02_atrlen_fine",  (30,55,1),   (0.025,0.1,0.025),  (16,17,0.5)))    # nail ATRLen 30-55 step1
    _do(3, "03_entry_floor",  _cfg("03_entry_floor",  (42,48,2),   (0.01,0.15,0.01),   (16,17,0.5)))    # confirm EntryMult ->0 floor
    _do(4, "04_trail_fine",   _cfg("04_trail_fine",   (42,48,2),   (0.025,0.075,0.025),(15,18,0.25)))   # TrailMult fine at peak
    _do(5, "05_atrlen_low",   _cfg("05_atrlen_low",   (18,45,1),   (0.025,0.1,0.025),  (16,17,0.5)))    # check ATRLen below 45
    _do(6, "06_micro_peak",   _cfg("06_micro_peak",   (40,50,1),   (0.025,0.1,0.025),  (16,17,0.25)))   # micro grid at the peak
    _do(7, "07_combo_scan",   _cfg("07_combo_scan",   (35,60,1),   (0.025,0.075,0.025),(16,17,0.5)))    # ATRLen x trail combo
    _do(8, "08_trail_high",   _cfg("08_trail_high",   (42,48,2),   (0.025,0.075,0.025),(17,20,0.25)))   # confirm TrailMult>17 worse
    _do(9, "09_entry_micro",  _cfg("09_entry_micro",  (43,47,1),   (0.01,0.1,0.01),    (16,17,0.5)))    # EntryMult micro at peak

    # adaptive zooms around the running Obj-max (ATRLen integer; EntryMult/TrailMult fractional)
    for A, nm_, (rl_mul, rl_min, ra_mul, ra_min, rm_mul, rm_min, nL, nA, nM) in [
        ...
        if start_attempt <= A:
            ...
            _atrl = zoom_fixed(seed_atrl, max(ra_min, seed_atrl * ra_mul),  nA, 0.025, ATRL_LO, ATRL_HI)
    ...
    log.info("==============================================================")
    log.info("  SFJ_VolatilityBreakout_crypto BNBUSDT Hourly Round-3 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["ATRLen"], obj_best["EntryMult"], obj_best["TrailMult"],
    ...
    if np_best:
        log.info("  NP-max: LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f Obj=%.0f",
                 np_best["ATRLen"], np_best["EntryMult"], np_best["TrailMult"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_VolatilityBreakout_crypto BNBUSDT HOT Hourly R3 final-confirm")
