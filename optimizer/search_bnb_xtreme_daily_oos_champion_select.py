"""
search_bnb_xtreme_hourly_oos_champion_select.py — OOS selection among the converged BNB Hourly
SFJ_XtremeStop_Crypto Daily champions (R1/R2, results/bnb_xtreme_daily{,2}_search/).
...
METHOD — MERGED single-run, AUTO data-range (proven route; set_instrument_data_range per pass):
  ...
  Each candidate = a 27-combo micro-grid (3 params X/LY/SY, all vary +/-1 step, Critical Rule 1);
  exact candidate row picked. Metric = NetProfit + Max INTRADAY Drawdown.

PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_XtremeStop_AI.wsp open, BNBUSDT HOT **Daily** chart tab ACTIVE and VISIBLE
  - SFJ_XtremeStop_Crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_bnb_xtreme_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_bnb_xtreme_hourly_oos_champion_select.py --period is
  py search_bnb_xtreme_hourly_oos_champion_select.py --candidate BD1
  py search_bnb_xtreme_hourly_oos_champion_select.py --from-csv
  py search_bnb_xtreme_hourly_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_xtreme_daily_oos_champion_select_search")
PREFIX      = "BNBXTDSEL_"
...
STEP = {"X": 1.0, "LY": 0.05, "SY": 0.05}
LO   = {"X": 1.0, "LY": 0.1, "SY": 0.1}
HI   = {"X": 2000.0, "LY": 50.0, "SY": 50.0}

# 4 DISTINCT converged BNB Daily XtremeStop regimes (from bnb_xtreme_daily{,2}_search R1/R2).
# id, regime, X, LY, SY, prior IS NP, prior IS MDD  (NP data-vintage-dependent, Rule 5 — reference only)
CANDIDATES = [
    ("BD1", "mid-X asym champion ***",  70.0, 0.5,  2.2,  23605.2, -8952.55),  # R2 ceiling 9-conv; NP/Obj-max 43tr
    ("BD2", "short-X high-freq",         2.0, 5.0,  5.0,  14611.3, -7767.4),   # R1 A02 short_X 105tr
    ("BD3", "high-pct low-freq",        30.0, 9.0, 12.5,  21241.0, -9265.4),   # R1 A05 high_pct 15tr
    ("BD4", "low-pct mid-X high-freq",  46.0, 1.2,  3.0,  15102.9, -13707.4),  # R1 A06 low_pct 61tr
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_xtreme_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, x, ly, sy) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="daily",
        bar_period=1440,
        params=[_axis("X", x), _axis("LY", ly), _axis("SY", sy)],
...
def _pick(df, x, ly, sy):
    m = df
    for nm, v in (("X", x), ("LY", ly), ("SY", sy)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_xtreme_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_xtreme_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BX1", "is", *CANDIDATES[0][2:5]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Daily XtremeStop — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
        ...
        for (cid, regime, x, ly, sy, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, x, ly, sy)
            log.info("--- [%s] %s %s X=%g LY=%g SY=%g [%s] ---",
                     period, cid, regime, x, ly, sy, datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"X": x, "LY": ly, "SY": sy},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, x, ly, sy) if (df is not None and not df.empty) else None
            if row is not None:
    ...
    for (cid, regime, x, ly, sy, pnp, pmdd) in CANDIDATES:
        ...
        c = {"id": cid, "regime": regime, "params": {"X": x, "LY": ly, "SY": sy},
...
def main():
    ap = argparse.ArgumentParser(description="BNB Daily XtremeStop OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="BDn", default=None)
