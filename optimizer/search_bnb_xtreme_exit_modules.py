"""
search_bnb_xtreme_exit_modules.py -- Exit-module independent optimization on top of the
SFJ_XtremeStop_Crypto BNB Hourly main (OOS-selected BX3).

Main signal FIXED at the user-selected BX3 (X=14 LY=6.5 SY=8.0; short-X high-freq, the
de-facto OOS winner on the 2026/06/10 full-period re-test), Status ON. Six exit modules
tested INDEPENDENTLY (one enabled at a time), exhaustive optimization over its input range,
max-NetProfit row selected, vs the same-day baseline.
...
IS window 2022/01/01-2026/01/01: the script CHART-TRIMS to IS first (a prior OOS run leaves the
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_XtremeStop_AI.wsp open, BNBUSDT HOT HOURLY tab active
  - Main signal SFJ_XtremeStop_Crypto inputs set to X=14 LY=6.5 SY=8.0 (BX3), Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved
  - CLOSE the Study Editor window

CLI:
  py search_bnb_xtreme_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_bnb_xtreme_exit_modules.py --module 3       # only module 3
  py search_bnb_xtreme_exit_modules.py --module 0       # baseline A00 only
  py search_bnb_xtreme_exit_modules.py --smoke --module 1
  py search_bnb_xtreme_exit_modules.py --manual-status  # user toggles Status by hand
  py search_bnb_xtreme_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_bnb_xtreme_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
SYMBOL      = "BNBUSDT HOT"
MAIN_SIGNAL = "SFJ_XtremeStop_Crypto"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_xtreme_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # signal date no-op; chart-trim is the control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# XtremeStop BNB main = OOS-selected BX3 (short-X high-freq, de-facto OOS winner) — set in MC64 (Format Objects, Status ON)
CHAMPION = {
    "X":  14.0,
    "LY": 6.5,
    "SY": 8.0,
}
# Reference baseline = XtremeStop main-only IS (BX3 row, bnb_xtreme_hourly_oos_champion_select 2026-06-25)
BASELINE_REF = {"net_profit": 19614.2, "max_drawdown": -8392.14, "total_trades": 60}
BASELINE_ABORT_PCT = 12.0   # abort modules if same-day baseline drifts > this (BX3 IS drifted ~6% on refresh)
...
_LOG_FILE = OUTPUT_DIR / f"search_bnb_xtreme_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    # 27-combo micro-grid around BX3: all 3 params vary (Critical Rule 1);
    # grid points include the EXACT main X=14 LY=6.5 SY=8.0 so A00 reads main-only at BX3.
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("X",  12.0, 16.0, 2.0),
            ParamAxis("LY", 6.25, 6.75, 0.25),
            ParamAxis("SY", 7.75, 8.25, 0.25),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs X=14 LY=6.5 SY=8.0)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_bnb_xtreme_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_bnb_xtreme_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly XtremeStop exit-module optimization%s", " [SMOKE]" if smoke else "")
    log.info("  Main fixed: X=14 LY=6.5 SY=8.0 (BX3, Status ON)")
    log.info("  Baseline ref (XtremeStop IS BX3): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
...
def main():
    ap = argparse.ArgumentParser(description="BNB XtremeStop exit-module independent optimization")
