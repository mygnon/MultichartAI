"""
search_bnb_xtreme_hourly3.py — SFJ_XtremeStop_Crypto on BNBUSDT HOT Hourly, Round 3
...
Priors (XtremeStop on futures; no crypto prior):
  NQ Hourly X=10 LY=1.48 SY=2.18 ($624K); GC Hourly X=7 LY=0.67 SY=1.80; TXF Hourly X=63 LY=5.155 SY=5.79.
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\bnb_xtreme_hourly3_search")
...
SEED_X, SEED_LY, SEED_SY = 909.0, 2.9, 10.65
SEED_NP = 0.0

PREFIX = "BNBXTH3_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_xtreme_hourly3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_bnb_xtreme_hourly3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly SFJ_XtremeStop_Crypto NP>100K -- Round 3")
    log.info("  IS window %s ~ %s  (chart-trimmed)", IS_RANGE[0], IS_RANGE[1])
    log.info("  R2 champ X=909 LY=2.9 SY=10.65 NP=27991 (confirming peak; X bounded, SY widening)", )
    log.info("==============================================================")

    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (890, 930, 10), (2.5, 3.25, 0.25), (10.0, 11.5, 0.25)))
    ...
    attempts_config = [
        ("01_X_bridge",      (820, 1000, 10), (2.0, 3.5, 0.25), (9.5, 11.5, 0.25)),
        ("02_SY_high_sweep", (880, 940, 10),  (2.5, 3.25, 0.25),(9.0, 15.0, 0.25)),
        ("03_LY_sweep",      (880, 940, 10),  (0.5, 6.0, 0.25), (10.0, 11.0, 0.25)),
        ("04_X_1000_1500",   (1000, 1500, 30),(1.5, 4.0, 0.5),  (8.0, 13.0, 0.5)),
        ("05_asym_fine",     (890, 930, 5),   (2.5, 3.5, 0.1),  (10.0, 11.5, 0.1)),
        ("06_X_750_900",     (750, 900, 10),  (2.0, 3.5, 0.25), (9.0, 12.0, 0.5)),
        ("07_SY_vhigh",      (880, 940, 15),  (2.5, 3.25, 0.25),(13.0, 20.0, 0.5)),
        ("08_sym_recheck",   (850, 1050, 25), (5.0, 12.0, 0.5), (5.0, 12.0, 0.5)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BNB Hourly XtremeStop_Crypto Round-3 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BNB Hourly SFJ_XtremeStop_Crypto NP>100K Round-3 search")
