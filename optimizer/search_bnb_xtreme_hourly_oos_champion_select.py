...
SFJ_XtremeStop_Crypto champions (rounds 1-3, results/bnb_xtreme_hourly{,2,3}_search/).
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_XtremeStop_AI.wsp open, BNBUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
...
CLI:
  ...
  py search_bnb_xtreme_hourly_oos_champion_select.py --candidate BX1
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\bnb_xtreme_hourly_oos_champion_select_search")
PREFIX      = "BNBXTHSEL_"
...
HI   = {"X": 5000.0, "LY": 50.0, "SY": 50.0}

# 4 DISTINCT converged BNB Hourly XtremeStop regimes (from bnb_xtreme_hourly{,2,3}_search).
# id, regime, X, LY, SY, prior IS NP, prior IS MDD  (NP data-vintage-dependent, Rule 5 — reference only)
CANDIDATES = [
    ("BX1", "long-X asym champion ***", 909.0, 2.9,  10.65, 27991.0, -9724.0),  # R2 peak, 25tr; NP/Obj-max
    ("BX2", "symmetric high-pct",      1000.0, 9.5,  11.0,  24913.0, -8945.0),  # 20tr, distinct sym regime
    ("BX3", "short-X high-freq",         14.0, 6.5,   8.0,  20921.0, -8392.0),  # 57tr, R2 short_X_redo
    ("BX4", "asym mid-X high-freq",      70.0, 5.5,   4.5,  11688.0, -7625.0),  # 166tr, R1 asym_LY_high
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_bnb_xtreme_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_bnb_xtreme_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_bnb_xtreme_hourly_oos_champion_select.json"
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BNB Hourly XtremeStop — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly XtremeStop OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="BXn", default=None)
