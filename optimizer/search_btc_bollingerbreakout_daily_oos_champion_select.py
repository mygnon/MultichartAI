"""
search_btc_bollingerbreakout_daily_oos_champion_select.py -- OOS selection among the converged
BTC Daily SFJ_BollingerBreakout_crypto champions (R1-R2, results/btc_bollingerbreakout_daily{,2}_search/).

4 params BBLen, BBmult, ATRMult, ReentryBars; pure Bollinger-band breakout STOP + cooldown + ATR chandelier exit,
long+short, _Crypto1MUSD. BTC DAILY: weakest cell, sparse 10-30tr (does not converge cleanly).
Obj-max ~BBLen=3 BBmult=3.5 ATRMult=4 Re=4 NP$1,531/19tr; NP-max $1,647/10tr. $100K unreachable.
BTC Hourly + ETH Hourly OOS all-broke; ETH Daily PASSED (ED4). BTC Daily = check (sparse).
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, BTCUSDT HOT DAILY chart tab ACTIVE,
...
CLI:
  py search_btc_bollingerbreakout_daily_oos_champion_select.py                  # merged verdict
  py search_btc_bollingerbreakout_daily_oos_champion_select.py --candidate CD1  # smoke one
  py search_btc_bollingerbreakout_daily_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_bollingerbreakout_daily_oos_champion_select_search")
PREFIX      = "BTCBODSEL_"
...
# BTC Daily BollingerBreakout candidates (btc_bollingerbreakout_daily{,2}; SPARSE 10-30tr, 1-trade degenerate pts EXCLUDED).
# id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday).
CANDIDATES = [
    # id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (1-trade degenerate EXCLUDED)
    ("CD1", "Obj-max robust (19tr) ***", 3.0, 3.5, 4.0, 4.0, 1531.0, -541.0),  # R2 Obj-max, most trades+lowest MDD
    ("CD2", "NP-max (10tr)",             4.0, 2.8, 5.5, 2.0, 1647.0, -758.0),  # R2 NP-max, sparse
    ("CD3", "high-freq (30tr)",          2.0, 2.6, 3.5, 4.0, 1168.0, -701.0),  # densest = best OOS-law candidate
    ("CD4", "R1 NP-max (10tr)",          4.0, 2.3, 5.6, 3.0, 1610.0, -765.0),  # alt sparse regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_bollingerbreakout_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_bollingerbreakout_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_bollingerbreakout_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("CD1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Daily BollingerBreakout -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Daily SFJ_BollingerBreakout_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="CDn", default=None)
