"""
search_btc_bollingerbreakout_hourly_oos_champion_select.py -- OOS selection among the converged
BTC Hourly SFJ_BollingerBreakout_crypto champions (R1-R2, results/btc_bollingerbreakout_hourly{,2}_search/).

4 params BBLen, BBmult, ATRMult, ReentryBars; pure Bollinger-band breakout STOP + cooldown + ATR chandelier exit,
long+short, _Crypto1MUSD. BTC R2 ceiling: Obj-max BBLen=8 BBmult=1.5 ATRMult=15 ReentryBars=22
(NP $2,236 MDD -$425 MDD/NP 19%); BTC wants wider trail (15 vs BNB 7), shorter BBLen, higher cooldown.
BTC/BNB ~9.5x; $100K unreachable. Same family as BNB BollingerBreakout (2nd OOS PASS).
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, BTCUSDT HOT HOURLY chart tab ACTIVE,
...
CLI:
  py search_btc_bollingerbreakout_hourly_oos_champion_select.py                  # merged verdict
  py search_btc_bollingerbreakout_hourly_oos_champion_select.py --candidate CO1  # smoke one
  py search_btc_bollingerbreakout_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_bollingerbreakout_hourly_oos_champion_select_search")
PREFIX      = "BTCBOSEL_"
...
# Converged BTC Hourly BollingerBreakout candidates (btc_bollingerbreakout_hourly{,2}_search).
# id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday).
CANDIDATES = [
    # id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday)
    ("CO1", "Obj-max (lowest MDD) ***", 8.0,  1.5,  15.0, 22.0, 2236.0, -425.0),  # R2 Obj-max 98tr
    ("CO2", "NP-max short-BBLen",        4.0,  1.5,  14.0, 21.0, 2308.0, -859.0),  # 113tr, highest NP, deeper MDD
    ("CO3", "wide-band tight-trail",     8.0,  2.25, 9.0,  23.0, 2091.0, -409.0),  # 173tr, distinct hi-freq
    ("CO4", "long-BBLen hi-freq",        13.0, 1.8,  7.8,  21.0, 1703.0, -529.0),  # 216tr, high-freq generalizer
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_bollingerbreakout_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_bollingerbreakout_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_bollingerbreakout_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("CO1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Hourly BollingerBreakout -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Hourly SFJ_BollingerBreakout_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="COn", default=None)
