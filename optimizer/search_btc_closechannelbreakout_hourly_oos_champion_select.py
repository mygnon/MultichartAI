"""
search_btc_closechannelbreakout_hourly_oos_champion_select.py -- OOS selection among the converged
...
CLI:
  py search_btc_closechannelbreakout_hourly_oos_champion_select.py                  # merged verdict
  py search_btc_closechannelbreakout_hourly_oos_champion_select.py --candidate VB1  # smoke one
  py search_btc_closechannelbreakout_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_closechannelbreakout_hourly_oos_champion_select_search")
PREFIX      = "BTCCCBSEL_"
...
CANDIDATES = [
    ("CC1", "Obj-max champion ***",  7.0,  8.0,   19.0, 1866.3, -363.7),  # R1/R2 8-conv Obj-max, 223tr
    ("CC2", "NP-max high-trail",     5.0,  10.25, 19.0, 1974.9, -586.8),  # R2 A03 NP-max, 162tr
    ("CC3", "short-len high-freq",   2.0,  9.0,   22.0, 1722.0, -571.0),  # R2 A07, 214tr
    ("CC4", "wide-trail low-freq",  11.0,  14.5,  10.0, 1658.0, -493.0),  # R2 A06, 106tr
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_closechannelbreakout_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_closechannelbreakout_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_closechannelbreakout_hourly_oos_champion_select.json"
