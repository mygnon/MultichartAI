"""
search_btc_ct_daily.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BTCUSDT HOT Daily, Round 1

Priors from completed searches:
  BTC Hourly CT (R1-R5, ceiling $4,155): LL=104 SL=4.05-4.15 LS=68-165 SS=4.65-4.95
    rare-extreme regime, 22-25tr/7yr.  LL=104 hours ~ 4.3 days -> time-mapped
    daily analog would be ultra-short LL~4-5; daily may also have its own regime.
  ETH Hourly CT $5,005 (LL=111 LS=115); BNB Hourly CT $36,703 (ASYMMETRIC LL=229 LS=22).
  Futures CT Daily regimes: NQ Daily LL=8 SL=0.7 LS=47 SS=1.86; GC Daily LL=3 SL=1.28 LS=50 SS=2.3.
  Daily vs Hourly on BTC: QPATRex +6.6% (Daily $3,511 > Hourly $3,293);
    QPATR_Breakout -7.4% (Daily $2,544 < Hourly $2,748) — direction uncertain.
  Daily bars ~2,557 over 7yr — long warmup matters for large LL/LS.

R1 Plan (11 attempts, <=5000 combos each):
  A01 global_coarse     LL(10-130 s20)xSL(0.5-4.5 s0.5)xLS(10-130 s20)xSS(1.0-4.5 s0.5)  = 3528
  A02 hourly_analog     LL(2-12 s1)xSL(2.5-5.0 s0.25)xLS(2-12 s2)xSS(2.5-5.0 s0.5)       = 4356
  A03 long_period       LL(100-250 s25)xSL(2.0-5.0 s0.5)xLS(100-250 s25)xSS(2.0-5.0 s0.5)= 2401
  A04 futures_daily     LL(2-14 s2)xSL(0.5-2.0 s0.25)xLS(35-60 s5)xSS(1.0-3.0 s0.25)     = 2646
  A05 mid_period        LL(15-60 s5)xSL(1.0-4.0 s0.5)xLS(15-60 s5)xSS(1.0-4.0 s0.5)      = 4900
  A06 high_stddev       LL(10-90 s10)xSL(4.5-7.0 s0.5)xLS(10-90 s10)xSS(4.5-7.0 s0.5)    = 2916
  A07 asym_longLL_shortLS LL(60-130 s10)xSL(3.0-5.0 s0.25)xLS(2-20 s2)xSS(3.0-5.0 s0.5)  = 3600
  A08 tight_SL          LL(2-20 s3)xSL(0.1-1.0 s0.15)xLS(20-60 s5)xSS(0.2-2.0 s0.3)      = 3087
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_ct_daily_search")
...
# No daily prior — neutral seed at signal defaults (LENGTH=46, STDDEV=3.8)
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_ct_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "priors":   "BTC Hourly CT ceiling $4,155 (LL=104 SL=4.05-4.15, 22-25tr); futures CT Daily NQ LL=8 LS=47 / GC LL=3 LS=50; BTC Daily vs Hourly mixed on other strategies (QPATRex +6.6%, Breakout -7.4%)",
            "r1_plan":  "Global coarse + hourly time-mapped analog (ultra-short LL) + long period + futures-daily regime + mid period + high stddev + BNB-style asym + tight SL + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_ct_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BTC Daily countertrend_LS_crypto NP>10K -- Round 1")
    log.info("  Priors: BTC Hourly CT $4,155 (LL=104); futures daily LL 3-8 LS 47-50")
    log.info("==============================================================")
    ...
    attempts_config = [
        ...
        ("02_hourly_analog",        (2, 12, 1),     (2.5, 5.0, 0.25),  (2, 12, 2),     (2.5, 5.0, 0.5)),
        ("03_long_period",          (100, 250, 25), (2.0, 5.0, 0.5),   (100, 250, 25), (2.0, 5.0, 0.5)),
    ...
    log.info("==============================================================")
    log.info("  BTC Daily countertrend_LS_crypto Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BTC Daily countertrend_LS_crypto NP>10K Round-1 search")
