"""
search_btc_ct_daily_oos_champion_select.py — Out-of-sample (OOS) selection among the converged
BTC *Daily* CT champions (rounds 1-3, results/btc_ct_daily{,2,3}_search/).
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_Bollinger_AI.wsp open, BTCUSDT HOT **DAILY** chart tab ACTIVE, the
...
CLI:
  py search_btc_ct_daily_oos_champion_select.py                 # merged: IS then FULL, then verdict
  py search_btc_ct_daily_oos_champion_select.py --period is     # one pass only
  py search_btc_ct_daily_oos_champion_select.py --candidate D1  # smoke one candidate
  py search_btc_ct_daily_oos_champion_select.py --from-csv      # recompute from existing CSVs
  py search_btc_ct_daily_oos_champion_select.py --probe-instrument  # verify auto data-range
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_ct_daily_oos_champion_select_search")
PREFIX      = "BTCCTDSEL_"
...
# 4 DISTINCT converged BTC Daily regimes (verified vs btc_ct_daily{,2,3}_search JSONs).
...
CANDIDATES = [
    ("D1", "NP/Obj-max 7conv ***", 49.0, 2.40, 123.0, 1.20, 3593.0, -399.0),  # the ceiling champion, 16tr
    ("D2", "LL30-70 mid-freq",     40.0, 2.20, 130.0, 1.00, 3519.0, -442.0),  # 20tr, near-champion
    ("D3", "futures high-freq",     4.0, 1.25,  45.0, 1.75, 2687.0, -365.0),  # 47tr, distinct regime
    ("D4", "tight-SL high-freq",   14.0, 0.85,  60.0, 0.50, 2482.0, -536.0),  # 49tr, distinct regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_ct_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_ct_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_ct_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("D1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Daily CT — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Daily CT OOS champion selection (auto-range merged)")
