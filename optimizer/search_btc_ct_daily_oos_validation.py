"""
search_btc_ct_daily_oos_validation.py — Full-period / out-of-sample (OOS) validation of the
already-optimized exit modules on top of the BTC DAILY CT champion.
...
Main signal FIXED at Obj-max champion LL=49 SL=2.4 LS=123 SS=1.2 (Status ON).
Module params FIXED at their IS max-NP values (from btc_ct_daily_exit_modules_search):
  M1 ATRstop STP=0.4 | M2 TrailingStop ATRSTP=15.8 | M3 EntryBarsAfterExit EXITBAR=76
  M4 high_volatility DAYRANGE=4.68 | M5 PT_Exit PT_Base=0.672 | M6 RescueTeamExit Length=20 std=3.0
...
PREREQUISITES (one-time manual setup in MC64):
  ...
  - BTCUSDT HOT DAILY tab active
  - Main signal inputs set to 49 / 2.4 / 123 / 1.2, Status ON
  - All 6 module signals inserted on the chart, Status OFF
  - Workspace saved

CLI:
  py search_btc_ct_daily_oos_validation.py                 # IS + FULL, baseline + M1-M6 + teardown
  py search_btc_ct_daily_oos_validation.py --module 0      # baseline only (both periods)
  py search_btc_ct_daily_oos_validation.py --period full    # only the full period
  py search_btc_ct_daily_oos_validation.py --manual-status  # toggle Status by hand
  py search_btc_ct_daily_oos_validation.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_ct_daily_oos_validation_search")

# BTC Daily champion — set in MC64 manually, Status ON
CHAMPION = {
    "LENGTH_LONG":  49.0,
    "STDDEV_LONG":  2.4,
    "LENGTH_SHORT": 123.0,
...
MODULES: List[Tuple[int, str, List[Tuple[str, float, float]]]] = [
    (1, "SFJ_15Dworkshop_lesson4_ATRstop",                 [("STP",      0.4,   0.1)]),
    (2, "SFJ_15Dworkshop_lesson9_1_TrailingStop",          [("ATRSTP",   15.8,  0.1)]),
    (3, "SFJ_15Dworkshop_lesson10_3_EntryBarsAfterExit",   [("EXITBAR",  1.0,   1.0)]),
    (4, "SFJ_15Dworkshop_lesson11_3_high_volatility_exit", [("DAYRANGE", 4.77,  0.01)]),
    (5, "QuantPass_PT_Exit",                               [("PT_Base",  0.672, 0.001)]),
    (6, "RescueTeamExit",                                  [("Length",   20.0,  20.0),
                                                            ("std",      3.0,   0.1)]),
]
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_ct_daily_oos_validation_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg(period: str) -> StrategyConfig:
    return StrategyConfig(
        ...
        params=[
            ...
            ParamAxis("STDDEV_LONG",  2.35,  2.45,  0.025),
            ParamAxis("LENGTH_SHORT", 122.0, 124.0, 1.0),
            ParamAxis("STDDEV_SHORT", 1.15,  1.25,  0.025),
        ],
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_ct_daily_oos_validation.json"
...
def run(conn, from_csv, only_module, only_period, manual_status, smoke):
    ...
    out_json = OUTPUT_DIR / "final_params_btc_ct_daily_oos_validation.json"
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  BTC Daily CT — OOS / FULL-PERIOD module validation%s [SPEED]",
             " [SMOKE]" if smoke else "")
    ...
    log.info("==============================================================")
    log.info("  BTC DAILY OOS VALIDATION SUMMARY  (total %.1f min)", (time.time() - run_t0) / 60)
...
def main():
    ap = argparse.ArgumentParser(description="BTC Daily CT OOS/full-period module validation")
