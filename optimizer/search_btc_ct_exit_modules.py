"""
search_btc_ct_exit_modules.py — Exit-module independent optimization on top of the
SFJ_15Dworkshop_lesson5_countertrend_LS_crypto BTC Hourly champion.
...
the same-day baseline (A00) and the reference baseline NP=$4,092.62 / MDD=-$431.10.

Prior art: the SAME experiment on BNB (search_bnb_ct_exit_modules.py, 2026-06-13)
found ALL 6 modules hurt (-75% to -99.95%) — early exits truncate rare-extreme
holds and trigger re-entries.  BTC shares the regime (23tr/7yr), so the same
outcome is expected; this run verifies it on BTC per user request.
...
CLI:
  py search_btc_ct_exit_modules.py                  # A00 baseline + M1-M6 + teardown
  py search_btc_ct_exit_modules.py --module 3       # only module 3 (status set for it)
  py search_btc_ct_exit_modules.py --module 0       # baseline A00 only
  py search_btc_ct_exit_modules.py --smoke --module 1   # tiny end-to-end smoke test
  py search_btc_ct_exit_modules.py --manual-status  # user toggles Status by hand
  py search_btc_ct_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_ct_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2026/01/01")

# Obj-max champion (R3 A01 = R4 A02 = R5 A02 exact, R1-R5) — set in MC64 manually
CHAMPION = {
    "LENGTH_LONG":  104.0,
    "STDDEV_LONG":  4.15,
    "LENGTH_SHORT": 68.0,
...
# Reference baseline (R5 A02 retest, 2026-06-11)
BASELINE_REF = {"net_profit": 4092.62, "max_drawdown": -431.10, "total_trades": 23}
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_ct_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_btc_ct_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_btc_ct_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  BTC Hourly CT exit-module optimization%s", " [SMOKE]" if smoke else "")
...
def main():
    ap = argparse.ArgumentParser(
        description="BTC CT exit-module independent optimization")
