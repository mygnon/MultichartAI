"""
search_btc_ct_hourly2.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BTCUSDT HOT Hourly, Round 2

R1 Summary (12 attempts):
  NP-max ⭐ A10: LL=107 SL=4.1 LS=139 SS=4.7 NP=$4,104 MDD=-$799 25tr Obj=21,094
  Obj-max ⭐⭐⭐ A12: LL=110 SL=4.0 LS=140 SS=5.0 NP=$3,754 MDD=-$504 22tr Obj=27,942 (lowest MDD)
  A11 near A12: LL=120 SL=4.0 LS=140 SS=5.0 NP=$3,733 MDD=-$504 22tr Obj=27,628
  A04 long_period: LL=110 SL=4.0 LS=140 SS=4.5 NP=$3,306 MDD=-$800 28tr Obj=13,661
  BTC Hourly = LONG-PERIOD RARE-EXTREME regime (22-25 trades) -- opposite of futures (1614-1984 trades)
  A07/A11/A12 had UI truncation issues
  Cross BTC Hourly: SFJ_ct wins NP, Obj, MDD/NP across all 3 strategies (QPATRex $3,293, QPATR_Breakout $2,748)

R2 Plan (11 attempts, <=5000 combos each):
  Goal: confirm both regimes ultra-fine + very-very long Len + redo R1 truncated.

  A01 NPmax_ultrafine   LL(100-114 s2)xSL(3.9-4.3 s0.05)xLS(130-150 s5)xSS(4.5-5.0 s0.1)   = 2160
  A02 Objmax_ultrafine  LL(105-125 s2)xSL(3.85-4.20 s0.05)xLS(135-145 s2)xSS(4.7-5.2 s0.1) = 3168
  A03 very_very_long    LL(150-400 s25)xSL(3.5-5.5 s0.5)xLS(150-400 s25)xSS(3.5-5.5 s0.5)  = 3025
  A04 bridge_60_100     LL(60-100 s5)xSL(3.5-4.5 s0.25)xLS(60-150 s10)xSS(4.0-5.0 s0.25)   = 2250
  A05 high_stddev_fine  LL(80-180 s10)xSL(3.5-6.0 s0.5)xLS(80-180 s10)xSS(3.5-6.0 s0.5)    = 4356
  A06 asym_longLL_shortLS LL(80-150 s10)xSL(3.5-4.5 s0.25)xLS(5-50 s5)xSS(3.5-5.0 s0.5)    = 1600
  A07 redo_global_med   LL(50-200 s15)xSL(3.0-5.5 s0.5)xLS(50-200 s15)xSS(3.0-5.5 s0.5)    = 4356
  A08 micro_zoom_A10    LL(105-115 s1)xSL(4.0-4.2 s0.05)xLS(138-145 s1)xSS(4.6-5.1 s0.1)   = 2640
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_ct_hourly2_search")
...
# R1 NP-max as seed
SEED_LL, SEED_SL = 107.0, 4.1
SEED_LS, SEED_SS = 139.0, 4.7
SEED_NP          = 4104.0

PREFIX = "BTCCT2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_ct_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result":"R1 NP-max A10: LL=107 SL=4.1 LS=139 SS=4.7 NP=$4,104 MDD=-$799 25tr Obj=21,094; Obj-max A12: LL=110 SL=4.0 LS=140 SS=5.0 NP=$3,754 MDD=-$504 Obj=27,942",
            "r2_plan":  "Ultra-fine NP+Obj champions + very_very_long + asymmetric + redo truncated + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_ct_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BTC Hourly countertrend_LS_crypto NP>10K -- Round 2")
    log.info("  R1 NP-max: LL=107 SL=4.1 LS=139 SS=4.7 NP=$4,104 Obj=21,094")
    log.info("  R1 Obj-max: LL=110 SL=4.0 LS=140 SS=5.0 NP=$3,754 Obj=27,942 (MDD=-$504)")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_NPmax_ultrafine",    (100, 114, 2),  (3.9, 4.3, 0.05),  (130, 150, 5),  (4.5, 5.0, 0.1)),
        ("02_Objmax_ultrafine",   (105, 125, 2),  (3.85, 4.20, 0.05),(135, 145, 2),  (4.7, 5.2, 0.1)),
        ("03_very_very_long",     (150, 400, 25), (3.5, 5.5, 0.5),   (150, 400, 25), (3.5, 5.5, 0.5)),
        ("04_bridge_60_100",      (60, 100, 5),   (3.5, 4.5, 0.25),  (60, 150, 10),  (4.0, 5.0, 0.25)),
        ("05_high_stddev_fine",   (80, 180, 10),  (3.5, 6.0, 0.5),   (80, 180, 10),  (3.5, 6.0, 0.5)),
        ("06_asym_longLL_shortLS",(80, 150, 10),  (3.5, 4.5, 0.25),  (5, 50, 5),     (3.5, 5.0, 0.5)),
        ("07_redo_global_med",    (50, 200, 15),  (3.0, 5.5, 0.5),   (50, 200, 15),  (3.0, 5.5, 0.5)),
        ("08_micro_zoom_A10",     (105, 115, 1),  (4.0, 4.2, 0.05),  (138, 145, 1),  (4.6, 5.1, 0.1)),
    ]
    ...
    # Adaptive zooms (ultra-tight from R1 champion)
    A = 9
    ...
    if start_attempt <= A:
        for r_ll, r_sl, r_ls, r_ss in [(8, 0.3, 8, 0.3), (5, 0.2, 5, 0.2), (3, 0.15, 3, 0.15)]:
    ...
    log.info("A10  adaptive_zoom2 -- center: LL=%.4g SL=%.4g LS=%.4g SS=%.4g  NP=%.0f",
             best_ll, best_sl, best_ls, best_ss, best_np)
    if start_attempt <= A:
        for r_ll, r_sl, r_ls, r_ss in [(4, 0.15, 4, 0.15), (3, 0.1, 3, 0.1), (2, 0.075, 2, 0.075)]:
            _ll = zoom(best_ll, r_ll, 1.0,  LL_LO, LL_HI)
    ...
    log.info("==============================================================")
    log.info("  BTC Hourly countertrend_LS_crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BTC Hourly countertrend_LS_crypto NP>10K Round-2 search")
