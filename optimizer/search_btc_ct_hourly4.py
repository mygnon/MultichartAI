"""
search_btc_ct_hourly4.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on BTCUSDT HOT Hourly, Round 4

R1+R2+R3 Summary (34 attempts):
  NP-max ⭐ 11-conv (R2 A01/A02/A08/A09/A10/A11 + R3 A04/A06/A09/A10/A11):
    LL=104 SL=4.05 LS=139 SS=4.65 NP=$4,125 MDD=-$799 25tr Obj=21,303
  Obj-max ⭐⭐⭐ R3 A01 BREAKTHROUGH: LL=105 SL=4.2 LS=68 SS=4.9
    NP=$3,991 MDD=-$464 (lowest!) 23tr Obj=34,346 (highest of all BTC searches)
  R3 A08 regime confirm: LL=94 SL=4.5 LS=69 SS=4.9 NP=$3,984 Obj=32,388
  NP gain R2->R3 +0.00%; Obj gain R2->R3 +16.6% (Obj still climbing!)

R4 Plan (11 attempts, <=5000 combos each) — FINAL VERIFICATION ROUND:
  Goal: (1) same-day retest of both champions (data-drift baseline),
        (2) push Obj-max region finer (R3 just broke +16.6%),
        (3) close remaining unexplored territory.

  A01 Objmax_microfine    LL(101-109 s1)xSL(4.1-4.3 s0.05)xLS(64-72 s1)xSS(4.8-5.0 s0.05)   = 2025
  A02 NPmax_retest        LL(102-106 s1)xSL(4.0-4.1 s0.025)xLS(137-141 s1)xSS(4.6-4.7 s0.025)= 625
  A03 SS_above_5p5        LL(90-115 s5)xSL(4.0-5.0 s0.2)xLS(60-80 s4)xSS(5.5-8.0 s0.25)     = 2376
  A04 SL_above_5          LL(85-115 s5)xSL(5.0-7.0 s0.25)xLS(60-150 s10)xSS(4.5-5.5 s0.25)  = 3150
  A05 ultra_long_LL       LL(300-500 s25)xSL(4.0-5.5 s0.25)xLS(100-300 s25)xSS(4.0-5.5 s0.25)= 3969
  A06 shortLS_longLL      LL(95-125 s5)xSL(3.8-4.8 s0.2)xLS(20-60 s4)xSS(4.4-5.4 s0.2)      = 2772
  A07 low_SS_longLS       LL(95-115 s4)xSL(3.8-4.3 s0.1)xLS(125-155 s5)xSS(3.5-4.5 s0.1)    = 2772
  A08 LS_145_220_bridge   LL(95-115 s4)xSL(3.9-4.4 s0.1)xLS(145-220 s5)xSS(4.4-5.0 s0.15)   = 2880
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_ct_hourly4_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_ct_hourly4_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r3_result":"R3 NP-max 11-conv: LL=104 SL=4.05 LS=139 SS=4.65 NP=$4,125 Obj=21,303; Obj-max A01 BREAKTHROUGH: LL=105 SL=4.2 LS=68 SS=4.9 NP=$3,991 MDD=-$464 Obj=34,346",
            "r4_plan":  "Final verification: same-day champion retests + Obj-max microfine + remaining unexplored (SS>5.5, SL>5, LL 300-500, LS 20-60 @ longLL, SS<4.5 @ longLS, LS 145-220)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_ct_hourly4.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BTC Hourly countertrend_LS_crypto NP>10K -- Round 4 (FINAL VERIFY)")
    log.info("  NP-max 11-conv: LL=104 SL=4.05 LS=139 SS=4.65 NP=$4,125")
    log.info("  Obj-max R3 A01: LL=105 SL=4.2 LS=68 SS=4.9 Obj=34,346 MDD=-$464")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_Objmax_microfine",   (101, 109, 1),   (4.1, 4.3, 0.05),   (64, 72, 1),     (4.8, 5.0, 0.05)),
        ("02_NPmax_retest",       (102, 106, 1),   (4.0, 4.1, 0.025),  (137, 141, 1),   (4.6, 4.7, 0.025)),
        ("03_SS_above_5p5",       (90, 115, 5),    (4.0, 5.0, 0.2),    (60, 80, 4),     (5.5, 8.0, 0.25)),
        ("04_SL_above_5",         (85, 115, 5),    (5.0, 7.0, 0.25),   (60, 150, 10),   (4.5, 5.5, 0.25)),
        ("05_ultra_long_LL",      (300, 500, 25),  (4.0, 5.5, 0.25),   (100, 300, 25),  (4.0, 5.5, 0.25)),
        ("06_shortLS_longLL",     (95, 125, 5),    (3.8, 4.8, 0.2),    (20, 60, 4),     (4.4, 5.4, 0.2)),
        ("07_low_SS_longLS",      (95, 115, 4),    (3.8, 4.3, 0.1),    (125, 155, 5),   (3.5, 4.5, 0.1)),
        ("08_LS_145_220_bridge",  (95, 115, 4),    (3.9, 4.4, 0.1),    (145, 220, 5),   (4.4, 5.0, 0.15)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BTC Hourly countertrend_LS_crypto Round-4 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BTC Hourly countertrend_LS_crypto NP>10K Round-4 search")
