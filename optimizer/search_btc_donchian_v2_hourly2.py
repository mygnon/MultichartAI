"""
search_btc_donchian_v2_hourly.py -- SFJ_DonchianATR_crypto_v2 on BTCUSDT HOT Hourly, R1 (IS 2022/01-2026/01)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_donchian_v2_hourly2_search")
...
SEED_LEN, SEED_ATRL, SEED_MULT, SEED_TR, SEED_RE = 37.0, 16.0, 8.75, 91.0, 2.0

PREFIX = "BTCDONV22_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_donchian_v2_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_btc_donchian_v2_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 on BTCUSDT HOT Hourly -- Round 2 boundary confirm (IS 2022/01-2026/01)")
    log.info("  Params: Length, ATRLength, ATRMult, TrendLen, ReentryBars.  Obj=NP^2/|MDD|.  Target %.0f", TARGET_NP)
    log.info("  R1: NP-max $1,418 (L39/M8.5/T75/R0); MID-Length + ACTIVE TrendLen + R0 (regime != BNB).")
    log.info("  R2 zoom mid-L/M~8.5/TrendLen~75-91 + push M.  Run start: %s", datetime.now().isoformat())
    log.info("==============================================================")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (35, 39, 2), (14, 18, 2),
                                                 (8.5, 9.0, 0.5), (80, 100, 10), (0, 4, 2)))
    ...
    # static attempts (5D R2 boundary confirm: mid-L + M~8.5 + active TrendLen regime)
    _do(1, "01_rule5_retest", _cfg("01_rule5_retest", (33,43,2),  (14,18,2), (8,9.5,0.5),  (75,105,15), (0,4,2)))    # 1944
    _do(2, "02_mult_push",    _cfg("02_mult_push",    (35,41,2),  (14,18,2), (7,12,0.5),   (75,105,15), (0,4,2)))    # push M>8.75
    _do(3, "03_trend_zoom",   _cfg("03_trend_zoom",   (35,41,2),  (14,18,2), (8,9.5,0.5),  (50,130,10), (0,4,2)))    # TrendLen fine
    _do(4, "04_len_fine",     _cfg("04_len_fine",     (28,52,2),  (14,18,2), (8,9.5,0.5),  (80,100,10), (0,4,2)))    # 1404
    _do(5, "05_reentry_scan", _cfg("05_reentry_scan", (35,41,2),  (14,18,2), (8,9.5,0.5),  (75,105,15), (0,16,2)))   # R 0-16
    _do(6, "06_atrl_fine",    _cfg("06_atrl_fine",    (35,41,2),  (8,24,2),  (8,9.5,0.5),  (80,100,10), (0,4,2)))    # ATRL fine
    _do(7, "07_ultra_wide_m", _cfg("07_ultra_wide_m", (33,43,2),  (14,18,2), (9.5,16,0.5), (75,105,15), (0,2,1)))    # M up to 16
    _do(8, "08_trend_low",    _cfg("08_trend_low",    (35,41,2),  (14,18,2), (8,9.5,0.5),  (20,80,10),  (0,4,2)))    # lower TrendLen
    _do(9, "09_len_mult_2d",  _cfg("09_len_mult_2d",  (25,55,3),  (14,18,2), (7,11,0.5),   (80,100,20), (0,2,2)))    # L x M plane
    ...
    log.info("==============================================================")
    log.info("  SFJ_DonchianATR_crypto_v2 BTCUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_DonchianATR_crypto_v2 BTCUSDT HOT Hourly R1 search")
