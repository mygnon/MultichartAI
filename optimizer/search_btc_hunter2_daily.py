"""
search_btc_hunter2_daily.py — SFJ_HUNTER2_crypto on BTCUSDT HOT Daily, Round 1 (IS 2022/01-2026/01)
...
BTC HUNTER2 Hourly converged to an ULTRA-LONG regime (LEN_L=225 LEN_S=825 ATR_L=4.25 ATR_S=1.75);
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_hunter2_daily_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_hunter2_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met, obj_best):
    ...
    out = OUTPUT_DIR / "final_params_btc_hunter2_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto on BTCUSDT HOT Daily — Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  long_ma — asym ultra-long region where BTC HUNTER2 champion lived
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto BTCUSDT HOT Daily Round-1 COMPLETE  (%.1f min total)",
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto BTCUSDT HOT Daily NP>100K USD Round-1 search")
