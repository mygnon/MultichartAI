"""
search_btc_hunter2_daily2.py — SFJ_HUNTER2_crypto on BTCUSDT HOT Daily, Round 2 (IS 2022/01-2026/01)

R1 found BTC Daily's regime = ULTRA-SHORT LEN_L (LEN_L pinned at the 2 floor, same as BNB Daily) and
Obj rose to A10 (NOT converged): A01 LEN_L=5 Obj 9,849 -> A09 13,326 -> A10 13,418 (peak) -> A11 12,467.

  R1 Obj-max = NP-max ⭐ (A10): LEN_L=2 LEN_S=116 ATR_L=0.25 ATR_S=0.75  NP=$2,934 MDD=-$642 Obj=13,418 90tr
  (BTC Daily $2,934 ~ BTC Hourly $2,720 +7.9%; BNB Daily/BTC Daily 10.8x; $100K unreachable -97.1%)

R2 PURPOSE: (1) confirm LEN_L=2 floor (test LEN_L 2-12); (2) fine-sweep LEN_S~116, ATR_L~0.25 (low,
near 0.1 floor), ATR_S~0.75; (3) RE-TEST high-ATR (R1 A08 failed); (4) check short-LEN_S sub-regime;
robust run_or_load (cleanup+retry+re-focus); zoom GUARANTEED <=5000. Zoom seed = Obj-max (user criterion).

11 attempts, each <=5000 combos. Objective = NetProfit² / |MaxDrawdown|.

Attempt schedule (daily, LEN_L floor=2):
  A01 retest_center : LEN_L(2-6 s1)×LEN_S(112-120 s2)×ATR_L(0.1-0.5 s0.1)×ATR_S(0.55-0.95 s0.1) = 5×5×5×5=625  (Rule 5 drift)
  A02 atrl_fine     : LEN_L(2-6 s1)×LEN_S(108-124 s2)×ATR_L(0.1-0.6 s0.05)×ATR_S(0.55-0.95 s0.1) = 5×9×11×5=2475
  A03 lenS_sweep    : LEN_L(2-6 s1)×LEN_S(60-200 s10)×ATR_L(0.15-0.35 s0.1)×ATR_S(0.55-0.95 s0.2) = 5×15×3×3=675
  A04 atrs_fine     : LEN_L(2-6 s1)×LEN_S(108-124 s4)×ATR_L(0.15-0.35 s0.05)×ATR_S(0.3-1.5 s0.1) = 5×5×5×13=1625
  A05 lenL_confirm  : LEN_L(2-12 s1)×LEN_S(108-124 s4)×ATR_L(0.15-0.35 s0.1)×ATR_S(0.55-0.95 s0.2) = 11×5×3×3=495
  A06 short_lenS    : LEN_L(2-10 s2)×LEN_S(20-110 s10)×ATR_L(0.25-1.5 s0.25)×ATR_S(0.5-1.0 s0.25) = 5×10×6×3=900
  A07 high_atr_retest: LEN_L(10-130 s30)×LEN_S(10-130 s30)×ATR_L(3-12 s1)×ATR_S(3-12 s1) = 5×5×10×10=2500  (R1 A08 failed)
  A08 mid_lenS      : LEN_L(2-6 s1)×LEN_S(80-200 s10)×ATR_L(0.15-0.5 s0.05)×ATR_S(0.5-1.0 s0.25) = 5×13×8×3=1560
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_hunter2_daily2_search")
...
# R2 seed = R1 Daily Obj-max champion (A10, LEN_L at the 2 floor)
SEED_LL   = 2.0
SEED_LS   = 116.0
SEED_ATRL = 0.25
SEED_ATRS = 0.75
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_hunter2_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r1_obj_max": {"LEN_L": 2, "LEN_S": 116, "ATR_multiplier_L": 0.25, "ATR_multiplier_S": 0.75,
                       "net_profit": 2934.34, "max_drawdown": -641.72, "objective": 13418, "total_trades": 90},
        "notes": {
    ...
    out = OUTPUT_DIR / "final_params_btc_hunter2_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    obj_best: Dict = {"LEN_L": SEED_LL, "LEN_S": SEED_LS, "ATR_multiplier_L": SEED_ATRL,
                      ...
                      "max_drawdown": -642.0, "objective": SEED_OBJ, "total_trades": 90}
    np_best: Dict = {}
    target_met = False
    attempt_log: List[dict] = []

    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto BTCUSDT HOT Daily — Round 2 (confirm ULTRA-SHORT LEN_L + fine-sweep)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (2, 6, 1), (112, 120, 2),
                                                 (0.1, 0.5, 0.1), (0.55, 0.95, 0.1)))
    ...
    # static attempts
    _do(1, "01_retest_center",  _cfg("01_retest_center",  (2,6,1),    (112,120,2),  (0.1,0.5,0.1),   (0.55,0.95,0.1)))
    _do(2, "02_atrl_fine",      _cfg("02_atrl_fine",      (2,6,1),    (108,124,2),  (0.1,0.6,0.05),  (0.55,0.95,0.1)))
    _do(3, "03_lenS_sweep",     _cfg("03_lenS_sweep",     (2,6,1),    (60,200,10),  (0.15,0.35,0.1), (0.55,0.95,0.2)))
    _do(4, "04_atrs_fine",      _cfg("04_atrs_fine",      (2,6,1),    (108,124,4),  (0.15,0.35,0.05),(0.3,1.5,0.1)))
    _do(5, "05_lenL_confirm",   _cfg("05_lenL_confirm",   (2,12,1),   (108,124,4),  (0.15,0.35,0.1), (0.55,0.95,0.2)))
    _do(6, "06_short_lenS",     _cfg("06_short_lenS",     (2,10,2),   (20,110,10),  (0.25,1.5,0.25), (0.5,1.0,0.25)))
    _do(7, "07_high_atr_retest",_cfg("07_high_atr_retest",(10,130,30),(10,130,30),  (3,12,1),        (3,12,1)))
    _do(8, "08_mid_lenS",       _cfg("08_mid_lenS",       (2,6,1),    (80,200,10),  (0.15,0.5,0.05), (0.5,1.0,0.25)))
    ...
    # summary
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  BTC HUNTER2 Daily Round-2 COMPLETE  (%.1f min total)", (time.time()-_RUN_T0)/60)
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto BTCUSDT HOT Daily Round-2 (confirm ultra-short LEN_L + fine-sweep)")
