"""
search_btc_hunter2_daily_exit_modules.py — Exit-module independent optimization on top of the
SFJ_HUNTER2_crypto BTC Daily champion.

Main signal FIXED at Obj-max champion (LEN_L=4 LEN_S=112 ATR_multiplier_L=0.55 ATR_multiplier_S=0.75), Status ON.
...
the same-day baseline (A00) and the reference baseline NP=$2,948 / MDD=-$642 (R2 Obj-max BTC Daily HUNTER2 champion).
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_HUNTER_AI.wsp open, BTCUSDT HOT DAILY tab active
  - Main signal SFJ_HUNTER2_crypto inputs set to LEN_L=4 LEN_S=112 ATR_multiplier_L=0.55 ATR_multiplier_S=0.75, Status ON
  - All 6 module signals inserted on the chart, Status OFF
  - Workspace saved

CLI:
  py search_btc_hunter2_daily_exit_modules.py                  # A00 baseline + M1-M6 + teardown
  py search_btc_hunter2_daily_exit_modules.py --module 3       # only module 3 (status set for it)
  py search_btc_hunter2_daily_exit_modules.py --module 0       # baseline A00 only
  py search_btc_hunter2_daily_exit_modules.py --smoke --module 1   # tiny end-to-end smoke test
  py search_btc_hunter2_daily_exit_modules.py --manual-status  # user toggles Status by hand
  py search_btc_hunter2_daily_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_hunter2_daily_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # WIDE no-op; chart-trim is the real control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# BTC Daily HUNTER2 Obj-max champion (R2 ceiling) — main signal must be set to these in MC64 (Format Objects)
CHAMPION = {
    "LEN_L":            4.0,
    "LEN_S":            112.0,
    "ATR_multiplier_L": 0.55,
    "ATR_multiplier_S": 0.75,
}
# Reference baseline (R2 Obj-max BTC Daily HUNTER2 champion, IS 2022/01-2026/01)
BASELINE_REF = {"net_profit": 2948.0, "max_drawdown": -642.0, "total_trades": 70}
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_hunter2_daily_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    ...
    return StrategyConfig(
        ...
        bar_period=1440,
        params=[
            ParamAxis("LEN_L",            3.0,   5.0,   1.0),
            ParamAxis("LEN_S",            111.0, 113.0, 1.0),
            ParamAxis("ATR_multiplier_L", 0.45,  0.65,  0.1),
            ParamAxis("ATR_multiplier_S", 0.65,  0.85,  0.1),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (LEN_L=4 LEN_S=112 ATR_L=0.55 ATR_S=0.75)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_btc_hunter2_daily_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_btc_hunter2_daily_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  BTC Daily HUNTER2 exit-module optimization%s", " [SMOKE]" if smoke else "")
    log.info("  Main fixed: LEN_L=4 LEN_S=112 ATR_L=0.55 ATR_S=0.75 (ON); IS 2022/01-2026/01 (chart-trimmed)")
...
def main():
    ap = argparse.ArgumentParser(
        description="BTC Daily HUNTER2 exit-module independent optimization")
