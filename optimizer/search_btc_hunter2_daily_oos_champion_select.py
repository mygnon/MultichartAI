"""
search_btc_hunter2_daily_oos_champion_select.py — Out-of-sample (OOS) selection among the converged
BTC Daily SFJ_HUNTER2_crypto champions (rounds 1-2, results/btc_hunter2_daily{,2}_search/).
...
BTC HUNTER2 Daily converged to an ULTRA-SHORT LEN_L regime (R2 ceiling Obj-max=NP-max LEN_L=4 LEN_S=112
ATR_L=0.55 ATR_S=0.75; flat plateau MDD locked at -$642). Candidates span the short-LEN_L core
(champion + R1 LEN_L=2 variant) plus an ultra-high-freq and a longer-LEN low-freq contrast so the
OOS test reveals which holds its Max Drawdown out-of-sample.

TASK: re-test each previously-converged BTC Daily HUNTER2 champion on this workspace and pick the
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_HUNTER_AI.wsp open, BTCUSDT HOT **Daily** chart tab ACTIVE, the
    SFJ_HUNTER2_crypto signal applied (Status ON)
  - Full data available to 2026/06/10 (the script sets the range itself)

CLI:
  py search_btc_hunter2_daily_oos_champion_select.py                 # merged: IS then FULL, then verdict
  py search_btc_hunter2_daily_oos_champion_select.py --period is     # one pass only
  py search_btc_hunter2_daily_oos_champion_select.py --candidate BTD1  # smoke one candidate
  py search_btc_hunter2_daily_oos_champion_select.py --from-csv      # recompute from existing CSVs
  py search_btc_hunter2_daily_oos_champion_select.py --probe-instrument  # verify auto data-range
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_hunter2_daily_oos_champion_select_search")
PREFIX      = "BTCH2DSEL_"
...
# 4 converged BTC Daily HUNTER2 candidates (btc_hunter2_daily{,2}_search). BTC Daily regime = ULTRA-SHORT LEN_L.
# id, regime, LEN_L, LEN_S, ATR_multiplier_L, ATR_multiplier_S, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("BTD1", "R2 Obj-max short-LEN ***", 4.0,  112.0, 0.55, 0.75, 2948.0, -642.0),  # 70tr, R2 ceiling Obj=13,547 highest NP
    ("BTD2", "R1 LEN_L=2 variant",       2.0,  116.0, 0.25, 0.75, 2934.0, -642.0),  # 90tr, R1 plateau point
    ("BTD3", "ultra-high-freq low-ATR_S",5.0,  116.0, 0.2,  0.3,  2894.0, -642.0),  # 97tr, R2 A04 highest-freq
    ("BTD4", "longer-LEN low-freq",     27.0,  110.0, 0.35, 0.4,  2862.0, -642.0),  # 47tr, R2 A10 contrast
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_hunter2_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_hunter2_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_hunter2_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BTD1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Daily HUNTER2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Daily HUNTER2 OOS champion selection (auto-range merged)")
