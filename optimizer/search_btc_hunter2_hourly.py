"""
search_btc_hunter2_hourly.py — SFJ_HUNTER2_crypto on BTCUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_hunter2_hourly_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_hunter2_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_btc_hunter2_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto on BTCUSDT HOT Hourly — Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    A = 7
    _n = "07_ultra_long"
    _c = _cfg(_n, (200, 600, 50), (200, 600, 50), (0.5, 4.5, 1), (0.5, 4.5, 1))
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto BTCUSDT HOT Hourly Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto BTCUSDT HOT Hourly NP>100K USD Round-1 search")
