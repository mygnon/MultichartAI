"""
search_btc_osc_hourly_oos_champion_select.py -- OOS selection among the converged
BTC Hourly _2021Basic_Osc_crypto champions (R1-R2, results/btc_osc_hourly{,2}_search/).

_2021Basic_Osc_crypto (5 params LEN, LE, SE, STP, LMT; BB-oscillator STOP entries, ATR exits,
_Crypto1MUSD).  BTC HOURLY IS ceiling (MOMENTUM regime, OPPOSITE of ETH's mean-reversion): Obj-max LEN=6 LE=2.25 SE=-0.5
STP=2 LMT=9 ($946 MDD-187, 66tr, 8-attempt R2 convergence, Rule-5 0.0% drift); NP-max LE=2.75 ($990).
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_BASIC_OSC_AI.wsp open, BTCUSDT HOT HOURLY chart tab ACTIVE, _2021Basic_Osc_crypto ON
  - Full data to 2026/06/10 (the script sets the range itself). CLOSE the Study Editor window.

CLI:
  py search_btc_osc_hourly_oos_champion_select.py                  # merged: IS then FULL, verdict
  py search_btc_osc_hourly_oos_champion_select.py --candidate BO1  # smoke one candidate
  py search_btc_osc_hourly_oos_champion_select.py --from-csv       # recompute from existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_osc_hourly_oos_champion_select_search")
PREFIX      = "BTCOSCSEL_"
...
# Converged BTC Hourly _2021Basic_Osc_crypto candidates (btc_osc_hourly{,2}_search).
# id, regime, LEN, LE, SE, STP, LMT, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("BO1", "R2 Obj-max momentum ***", 6.0, 2.25, -0.5, 2.0,  9.0, 946.0, -187.0),  # 66tr, 8-attempt converged
    ("BO2", "R2 NP-max momentum",      8.0, 2.75,  0.0, 1.5,  9.0, 990.0, -235.0),  # 82tr, max IS NP
    ("BO3", "R1 NP-max momentum",      7.0, 2.5,  -0.5, 2.0,  9.0, 950.0, -227.0),  # 104tr, denser momentum
    ("BO4", "R1 alt mean-reversion",   5.0,-3.0,   0.0, 1.5,  9.0, 603.0, -290.0),  # 62tr, distinct regime (LE<0)
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_osc_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_osc_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_osc_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BO1", "is", *CANDIDATES[0][2:7]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Hourly Osc -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Hourly _2021Basic_Osc_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="BOn", default=None)
