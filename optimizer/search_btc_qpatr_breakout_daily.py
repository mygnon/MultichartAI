"""
search_btc_qpatr_breakout_daily.py  QuantPassATR_Breakout on BTCUSDT HOT Daily, Round 1
...
Strategy: QuantPassATR_Breakout (long+short reversal, no STP/LMT)
  ...
  2-param: Len, Multiple (simpler than QPATRex's 3-param)

Timeframe: Daily (1440-min bars)
Target: NP > 10,000 USD

Reference ceilings (all related searches confirmed):
  QPATR_Breakout BTC Hourly: $2,748 (4-conv, Len=212, Multiple=3.27)
  QPATR_Breakout ETH Hourly: $4,444 (8-conv, Len=32, Multiple=3.37)
  QPATR_Breakout BNB Hourly: $35,634 (13-conv, Len=3, Multiple=2.965)
  QPATRex BTC Daily: $3,511 (R3, Len=24, Su=0.75, Ni=3.5)
  QPATRex BTC Hourly: $3,293 (R3, Len=125)

Predictions:
  BTC QPATRex Daily / Hourly = 3,511/3,293 = +6.6% (slight Daily bump)
  BTC QPATR_Breakout / QPATRex Hourly = 2,748/3,293 = -16.5%
  Expected BTC QPATR_Breakout Daily ~ $2,930 (Hourly +6.6%) or ~$2,985 (QPATRex Daily x 0.85)
  Gap to $10K: ~-70% (likely structurally unreachable)
...
Attempt schedule (11 attempts, <=5000 combos each, 2D grids):
  ...
  A07 high_multi     Len(5-205 s10)  x Mult(4.0-15.0 s0.13) [21x86=1806]
  A08 verylong       Len(200-800 s30)x Mult(0.3-3.5 s0.04)  [21x81=1701]
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatr_breakout_daily_search")
...
# Strategy defaults as nominal seed (Hourly R2 champion as starting reference)
SEED_LEN  = 212.0
SEED_MULT = 3.27
SEED_NP   = 0.0
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatr_breakout_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "reference": "BTC QPATR_Breakout Hourly ceiling $2,748 (Len=212, Multiple=3.27); BTC QPATRex Daily $3,511",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_qpatr_breakout_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout on BTCUSDT HOT Daily -- Round 1")
    log.info("  Target: %.0f USD", TARGET_NP)
    log.info("  Hourly reference: Len=212 Mult=3.27 NP=$2,748 (4-conv)")
    log.info("  BTC QPATRex Daily: $3,511 (Len=24, Su=0.75, Ni=3.5)")
    log.info("==============================================================")
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout BTCUSDT Daily Round-1 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATR_Breakout BTCUSDT HOT Daily R1 parameter search")
