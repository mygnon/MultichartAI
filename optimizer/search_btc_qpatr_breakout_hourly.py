"""
search_btc_qpatr_breakout_hourly.py  QuantPassATR_Breakout on BTCUSDT HOT Hourly, Round 1

Strategy: QuantPassATR_Breakout (long+short pure breakout, reversal exits, no STP/LMT)
...
Params (MC64 input order, 2 params only):
  Len (int, default=45): ATR period
  Multiple (float, default=2.8): breakout threshold multiplier

Differences vs QuantPassATRex (which we exhaustively tested across BTC/ETH/BNB):
  - QPATR_Breakout: pure ATR, single Multiple, MARKET-only entries
  - QPATRex: ATR+StdDev, separate Su (STOP) and Ni (MARKET) multipliers
  - Search space is 2D vs 3D (much smaller; allows much finer grids)

Target: NP > 10,000 USD
Insample: 2019/01/01 - 2026/01/01
Original docx: ETHUSDT 15min. Adapting to BTCUSDT Hourly here.
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatr_breakout_hourly_search")
...
# Strategy defaults as nominal seed
SEED_LEN  = 45.0
SEED_MULT = 2.8
SEED_NP   = 0.0
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatr_breakout_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            "logic":    "ATR=AvgTrueRange(Len); LE: C>C[1]+ATR[1]*Mult -> market; SE: C<C[1]-ATR[1]*Mult -> market; both long+short, reversal exits",
            "contract": "_Crypto1MUSD=Round(1000000/C,0) -- ~$1M notional per trade",
            "r1_plan":  "8 broad sweeps (global, tight/wide multi, short/long Len, near-default, high-multi, verylong) + 3 adaptive zooms",
            "note":     "2-param strategy (Len, Multiple). Simpler than QPATRex's 3-param (Len, Su, Ni). Original docx: ETHUSDT 15min.",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_qpatr_breakout_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout on BTCUSDT HOT Hourly -- Round 1")
    log.info("  Signal: %s  Symbol: %s", SIGNAL, SYMBOL)
    log.info("  Target: %.0f USD", TARGET_NP)
    log.info("  Strategy defaults: Len=45 Multiple=2.8")
    log.info("  2-param search (Len, Multiple) -- simpler than QPATRex 3-param")
    log.info("==============================================================")
    ...
    # A01  global_fine -- 2D fine grid covering whole space
    A = 1
    _n = "01_global_fine"
    _c = _cfg(_n, (5, 205, 5), (0.2, 5.0, 0.06))
    log.info("A01  Len(5-205 s5)xMultiple(0.2-5.0 s0.06)  %d combos", _c.total_runs())
    if start_attempt <= A:
    ...
    A = 2
    _n = "02_tight_multi"
    _c = _cfg(_n, (5, 105, 2), (0.1, 3.0, 0.03))
    log.info("A02  Len(5-105 s2)xMultiple(0.1-3.0 s0.03)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    A = 3
    _n = "03_wide_multi"
    _c = _cfg(_n, (20, 220, 5), (2.0, 8.0, 0.07))
    log.info("A03  Len(20-220 s5)xMultiple(2.0-8.0 s0.07)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A03 -- best NP=%.0f  (Len=%.4g Mult=%.4g)", best_np, best_LEN, best_MULT)

    # A04  short_len (Len=2-52 step=1)
    A = 4
    _n = "04_short_len"
    _c = _cfg(_n, (2, 52, 1), (0.5, 5.0, 0.05))
    log.info("A04  Len(2-52 s1)xMultiple(0.5-5.0 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    A = 5
    _n = "05_long_len"
    _c = _cfg(_n, (100, 300, 5), (0.5, 5.0, 0.05))
    log.info("A05  Len(100-300 s5)xMultiple(0.5-5.0 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A05 -- best NP=%.0f  (Len=%.4g Mult=%.4g)", best_np, best_LEN, best_MULT)

    # A06  near_default (around Len=45 Multi=2.8)
    A = 6
    _n = "06_near_default"
    _c = _cfg(_n, (20, 90, 2), (1.5, 4.0, 0.025))
    log.info("A06  Len(20-90 s2)xMultiple(1.5-4.0 s0.025)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    A = 7
    _n = "07_high_multi"
    _c = _cfg(_n, (5, 205, 10), (4.0, 15.0, 0.13))
    log.info("A07  Len(5-205 s10)xMultiple(4.0-15.0 s0.13)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    A = 8
    _n = "08_verylong"
    _c = _cfg(_n, (200, 800, 30), (0.3, 3.5, 0.04))
    log.info("A08  Len(200-800 s30)xMultiple(0.3-3.5 s0.04)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A08 -- best NP=%.0f  (Len=%.4g Mult=%.4g)", best_np, best_LEN, best_MULT)

    # A09  adaptive_zoom1 (2D)
    A = 9
    ...
    if start_attempt <= A:
        ...
        r_mult = max(0.30,  best_MULT * 0.30)
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout BTCUSDT Hourly Round-1 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATR_Breakout BTCUSDT HOT Hourly R1 parameter search")
