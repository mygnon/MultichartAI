"""
search_btc_qpatr_breakout_hourly_oos_champion_select.py — OOS selection among the converged BTC Hourly
QuantPassATR_Breakout champions (results/btc_qpatr_breakout_hourly{,2}_search/).
...
METHOD — MERGED single-run, AUTO data-range (proven route; set_instrument_data_range per pass):
  ...
  Each candidate = a 9-combo micro-grid (2 params Len/Multiple, all vary +/-1 step, Critical Rule 1);
  exact candidate row picked. Metric = NetProfit + Max INTRADAY Drawdown.

PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_QuantPassATR_Breakout_AI.wsp open, BTCUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
  - QuantPassATR_Breakout signal applied (Status ON), data to 2026/06/10

CLI:
  py search_btc_qpatr_breakout_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_btc_qpatr_breakout_hourly_oos_champion_select.py --period is
  py search_btc_qpatr_breakout_hourly_oos_champion_select.py --candidate QB1
  py search_btc_qpatr_breakout_hourly_oos_champion_select.py --from-csv
  py search_btc_qpatr_breakout_hourly_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatr_breakout_hourly_oos_champion_select_search")
PREFIX      = "BTCQBSEL_"
...
STEP = {"Len": 1.0, "Multiple": 0.05}
LO   = {"Len": 1.0, "Multiple": 0.1}
HI   = {"Len": 1000.0, "Multiple": 50.0}

# 4 DISTINCT converged BTC Hourly QPATR_Breakout regimes (from btc_qpatr_breakout_hourly{,2}_search).
# id, regime, Len, Multiple, prior IS NP, prior IS MDD  (reference only)
CANDIDATES = [
    # id, regime, Len, Multiple, prior IS NP, prior IS MDD (intraday). Reversal-exit ATR breakout, _Crypto1MUSD.
    ("QB1", "Obj-max ceiling (90tr) ***", 212.0, 3.27, 2748.0, -689.0),  # documented ceiling, long-Len low-freq
    ("QB2", "alt low-MDD (94tr)",         210.0, 3.24, 2690.0, -660.0),  # near-tied Obj, lowest MDD
    ("QB3", "short-Len high-freq (190tr)",  7.0, 2.56, 1592.0, -755.0),  # densest = OOS-law generalizer
    ("QB4", "short-Len alt (186tr)",        8.0, 2.60, 1522.0, -880.0),  # alt high-freq regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatr_breakout_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, ln, ml) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="hourly",
        bar_period=60,
        params=[_axis("Len", ln), _axis("Multiple", ml)],
...
def _pick(df, ln, ml):
    m = df
    for nm, v in (("Len", ln), ("Multiple", ml)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_qpatr_breakout_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_qpatr_breakout_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("QB1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Hourly QPATR_Breakout — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
        ...
        for (cid, regime, ln, ml, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, ln, ml)
            log.info("--- [%s] %s %s Len=%g Mult=%g [%s] ---",
                     period, cid, regime, ln, ml, datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"Len": ln, "Multiple": ml},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, ln, ml) if (df is not None and not df.empty) else None
            if row is not None:
    ...
    for (cid, regime, ln, ml, pnp, pmdd) in CANDIDATES:
        ...
        c = {"id": cid, "regime": regime, "params": {"Len": ln, "Multiple": ml},
...
def main():
    ap = argparse.ArgumentParser(description="BTC Hourly QPATR_Breakout OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="QBn", default=None)
