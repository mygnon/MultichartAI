"""
search_btc_qpatrex_daily.py  QuantPassATRex on BTCUSDT HOT Daily, Round 1
...
Hourly reference ceilings (BTC/ETH/BNB completed):
  BTC R3 Hourly: $3,293 (Len=125)
  ETH R3 Hourly: $5,198 (Len=24, Su=1.48, Ni=2.47)
  BNB R5 Hourly: $39,921 (Len=235, Su=0.58575, Ni=2.1275)

Daily vs Hourly: ~24x fewer bars, but per-trade NP may be larger (positions held longer).
Daily Len=55 = ~55 days = ~2 months effective period (vs ~2.3 days for hourly Len=55).
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrex_daily_search")
...
# Strategy defaults as nominal seed (no prior BTC daily results)
SEED_LEN = 55.0
SEED_SU  = 0.75
SEED_NI  = 1.5
SEED_NP  = 0.0
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrex_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "note":     "Daily timeframe (1440 min). ~2,555 bars over 7 years.",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_qpatrex_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on BTCUSDT HOT Daily -- Round 1")
    ...
    log.info("  Note: Daily = ~2,555 bars over 7 years (vs ~61K hourly bars)")
    log.info("  BTC Hourly ceiling reference: $3,293 (Len=125)")
    log.info("==============================================================")
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex BTCUSDT Daily Round-1 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex BTCUSDT HOT Daily R1 parameter search")
