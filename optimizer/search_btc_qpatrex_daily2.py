"""
search_btc_qpatrex_daily2.py  QuantPassATRex on BTCUSDT HOT Daily, Round 2

R1 Summary (best NP=$3,387):
  A10=A11 convergence: Len=48 Su=0.728 Ni=2.142 NP=$3,387 MDD=-$1,125 11tr Obj=10,199
  A08 boundary regime: Len=45 Su=0.5(bdy) Ni=0.5(bdy) NP=$1,766 MDD=-$429 127tr (highest freq)
  A03 (wide_multi) UI FAILED -- 0 rows, needs retry
  BTC Daily ceiling ~$3,387 ≈ BTC Hourly ceiling $3,293 (+2.9%, structural)

R2 target REDUCED: $100K -> $10K (gap -66.1%, more achievable but still ambitious).

R2 Plan (11 attempts, <=5000 combos each):
  Goal: tight NP-max zoom + break A08's Su/Ni<0.5 boundary + retry A03 + bridges.

  A01 A10_ultrafine    Len(42-55 s1)  xSu(0.65-0.80 s0.01) xNi(1.9-2.4 s0.025)   [14x16x21=4704]
  A02 short_finer      Len(18-58 s2)  xSu(0.6-1.1 s0.04)   xNi(2.0-4.5 s0.2)     [21x13x13=3549]
  A03 below_05_bdy     Len(35-60 s2)  xSu(0.1-0.55 s0.025) xNi(0.1-0.6 s0.03)    [13x19x17=4199]
  A04 midlong_finer    Len(60-110 s4) xSu(0.4-0.85 s0.025) xNi(1.2-2.0 s0.05)    [13x19x17=4199]
  A05 wide_retry       Len(20-220 s20)xSu(1.0-6.0 s0.5)    xNi(2.0-12.0 s1.0)    [11x11x11=1331]
  A06 vshort_len       Len(2-15 s1)   xSu(0.5-2.0 s0.1)    xNi(1.0-4.0 s0.2)     [14x16x16=3584]
  A07 vlong_len        Len(200-500 s30)xSu(0.2-1.5 s0.1)   xNi(1.0-5.0 s0.3)     [11x14x14=2156]
  A08 global_recheck   Len(5-300 s30) xSu(0.3-3.0 s0.25)   xNi(0.5-6.0 s0.5)     [11x12x12=1584]
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrex_daily2_search")
INSAMPLE   = DateRange("2019/01/01", "2026/01/01")

TARGET_NP  = 10_000.0   # USD  (reduced from R1's 100K)
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrex_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result": "R1 NP-max A10=A11: Len=48 Su=0.728 Ni=2.142 NP=$3,387 MDD=-$1,125 11tr Obj=10,199; A08 boundary regime Su=Ni=0.5 NP=$1,766 MDD=-$429 127tr; A03 UI failed",
            "r2_plan":   "Tight A10/A11 zoom + break A08 boundary (Su/Ni<0.5) + A03 retry + bridges + adaptive zoom",
            "target_change": "Target reduced from R1's $100K to $10K (gap -66%)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_qpatrex_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on BTCUSDT HOT Daily -- Round 2")
    log.info("  Signal: %s  Symbol: %s  Timeframe: Daily (1440min)", SIGNAL, SYMBOL)
    log.info("  Target: %.0f USD  (REDUCED from R1's $100K)", TARGET_NP)
    log.info("  R1 NP-max: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f",
             SEED_LEN, SEED_SU, SEED_NI, SEED_NP)
    log.info("  R1 high-freq (boundary): Len=45 Su=Ni=0.5 NP=$1,766 MDD=-$429 127tr")
    log.info("  Goal: tight zoom + break Su/Ni<0.5 boundary + A03 retry + bridges")
    log.info("==============================================================")
    ...
    # A01  A10_ultrafine -- ultra-fine zoom of R1 A10=A11 champion
    A = 1
    _n = "01_A10_ultrafine"
    _c = _cfg(_n, (42, 55, 1), (0.65, 0.80, 0.01), (1.9, 2.4, 0.025))
    log.info("A01  Len(42-55 s1)xSu(0.65-0.80 s0.01)xNi(1.9-2.4 s0.025)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A01 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A02  short_finer -- fine zoom around A06/A04 (Len 20-52)
    A = 2
    _n = "02_short_finer"
    _c = _cfg(_n, (18, 58, 2), (0.6, 1.1, 0.04), (2.0, 4.5, 0.2))
    log.info("A02  Len(18-58 s2)xSu(0.6-1.1 s0.04)xNi(2.0-4.5 s0.2)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A02 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A03  below_05_bdy -- BREAK A08's Su/Ni<0.5 boundary (high-freq regime)
    A = 3
    _n = "03_below_05_bdy"
    _c = _cfg(_n, (35, 60, 2), (0.1, 0.55, 0.025), (0.1, 0.6, 0.03))
    log.info("A03  Len(35-60 s2)xSu(0.1-0.55 s0.025)xNi(0.1-0.6 s0.03)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A03 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A04  midlong_finer -- A05/A09 mid-long Len zoom
    A = 4
    _n = "04_midlong_finer"
    _c = _cfg(_n, (60, 110, 4), (0.4, 0.85, 0.025), (1.2, 2.0, 0.05))
    log.info("A04  Len(60-110 s4)xSu(0.4-0.85 s0.025)xNi(1.2-2.0 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A04 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A05  wide_retry -- retry R1 A03 (UI failed) with same wide_multi range
    A = 5
    _n = "05_wide_retry"
    _c = _cfg(_n, (20, 220, 20), (1.0, 6.0, 0.5), (2.0, 12.0, 1.0))
    log.info("A05  Len(20-220 s20)xSu(1.0-6.0 s0.5)xNi(2.0-12.0 s1.0)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A05 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A06  vshort_len -- very short Len (Len=2-15)
    A = 6
    _n = "06_vshort_len"
    _c = _cfg(_n, (2, 15, 1), (0.5, 2.0, 0.1), (1.0, 4.0, 0.2))
    log.info("A06  Len(2-15 s1)xSu(0.5-2.0 s0.1)xNi(1.0-4.0 s0.2)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A06 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A07  vlong_len -- very long Len (Len=200-500)
    A = 7
    _n = "07_vlong_len"
    _c = _cfg(_n, (200, 500, 30), (0.2, 1.5, 0.1), (1.0, 5.0, 0.3))
    log.info("A07  Len(200-500 s30)xSu(0.2-1.5 s0.1)xNi(1.0-5.0 s0.3)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    _c = _cfg(_n, (5, 300, 30), (0.3, 3.0, 0.25), (0.5, 6.0, 0.5))
    log.info("A08  Len(5-300 s30)xSu(0.3-3.0 s0.25)xNi(0.5-6.0 s0.5)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    if start_attempt <= A:
        ...
        r_su  = max(0.15,  best_SU  * 0.30)
        r_ni  = max(0.25,  best_NI  * 0.30)
    ...
    if start_attempt <= A:
        ...
        r_su  = max(0.06,  best_SU  * 0.12)
        r_ni  = max(0.10,  best_NI  * 0.12)
    ...
    if start_attempt <= A:
        ...
        r_su  = max(0.03,  best_SU  * 0.06)
        r_ni  = max(0.05,  best_NI  * 0.06)
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex BTCUSDT Daily Round-2 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex BTCUSDT HOT Daily R2 parameter search")
