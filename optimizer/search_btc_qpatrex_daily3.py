"""
search_btc_qpatrex_daily3.py  QuantPassATRex on BTCUSDT HOT Daily, Round 3

R2 Summary:
  A02 NP-max: Len=24 Su=0.76 Ni=3.6 NP=$3,511 MDD=-$925 13tr Obj=13,329 (R1->R2 +3.66%)
  A03 risk-adj: Len=39 Su=0.45 Ni=0.55 NP=$2,312 MDD=-$446 105tr (Su/Ni at upper boundary)
  R1 NP-max: Len=48 Su=0.728 Ni=2.142 NP=$3,387 11tr (still valid, different regime)
  A05 wide_retry UI failed again (R1 A03 also failed) -- this zone untestable
  A09-A11 adaptive zoom DROPPED to $2,529 (A02 sharp local peak, zooms missed exact point)

R3 Plan (11 attempts, <=5000 combos each):
  Goal: ultra-fine A02 + push A03 boundary deeper + bridges + R1 regime confirm.

  A01 A02_ultrafine     Len(20-30 s1) xSu(0.65-0.90 s0.02) xNi(3.2-4.2 s0.05)   [11x13x21=3003]
  A02 A03_deeper        Len(30-55 s2) xSu(0.10-0.45 s0.025)xNi(0.10-0.55 s0.03) [13x15x16=3120]
  A03 bridge_24_50      Len(24-50 s2) xSu(0.65-0.85 s0.02) xNi(2.5-3.8 s0.07)   [14x11x19=2926]
  A04 highNi_shortLen   Len(20-32 s1) xSu(0.5-1.0 s0.04)   xNi(4.0-6.0 s0.1)    [13x13x21=3549]
  A05 A02_Su_lateral    Len(24-30 s1) xSu(0.4-1.5 s0.05)   xNi(3.0-4.0 s0.05)   [7x23x21=3381]
  A06 midLen_zoom       Len(30-58 s2) xSu(0.50-0.90 s0.03) xNi(1.5-3.0 s0.075)  [15x14x21=4410]
  A07 A02_Ni_higher     Len(22-28 s1) xSu(0.7-0.85 s0.01)  xNi(3.5-5.5 s0.1)    [7x16x21=2352]
  A08 R1_regime_confirm Len(44-54 s1) xSu(0.65-0.80 s0.01) xNi(1.9-2.4 s0.025)  [11x16x21=3696]
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrex_daily3_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrex_daily3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result": "R1 NP-max Len=48 Su=0.728 Ni=2.142 NP=$3,387",
            "r2_result": "R2 NP-max A02 Len=24 Su=0.76 Ni=3.6 NP=$3,511 (+3.66%, new short-Len regime); A03 Len=39 Su=0.45 Ni=0.55 NP=$2,312 105tr",
            "r3_plan":   "Ultra-fine A02 + push A03 deeper + Len 24-50 bridge + high-Ni + lateral Su + R1 regime confirm",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_btc_qpatrex_daily3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on BTCUSDT HOT Daily -- Round 3")
    log.info("  Signal: %s  Symbol: %s  Timeframe: Daily (1440min)", SIGNAL, SYMBOL)
    log.info("  Target: %.0f USD", TARGET_NP)
    log.info("  R2 NP-max A02: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f",
             SEED_LEN, SEED_SU, SEED_NI, SEED_NP)
    log.info("  R2 risk-adj A03: Len=39 Su=0.45 Ni=0.55 NP=$2,312 MDD=-$446 105tr")
    log.info("  Goal: ultra-fine A02 + push A03 deeper + Len bridges + high-Ni")
    log.info("==============================================================")
    ...
    # A01  A02_ultrafine -- ultra-fine zoom of R2 A02 champion
    A = 1
    _n = "01_A02_ultrafine"
    _c = _cfg(_n, (20, 30, 1), (0.65, 0.90, 0.02), (3.2, 4.2, 0.05))
    log.info("A01  Len(20-30 s1)xSu(0.65-0.90 s0.02)xNi(3.2-4.2 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A01 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A02  A03_deeper -- push R2 A03 boundary deeper (Su/Ni<0.45 untested)
    A = 2
    _n = "02_A03_deeper"
    _c = _cfg(_n, (30, 55, 2), (0.10, 0.45, 0.025), (0.10, 0.55, 0.03))
    log.info("A02  Len(30-55 s2)xSu(0.10-0.45 s0.025)xNi(0.10-0.55 s0.03)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A02 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A03  bridge_24_50 -- bridge between R2 A02 (Len=24) and R1 NP-max (Len=48)
    A = 3
    _n = "03_bridge_24_50"
    _c = _cfg(_n, (24, 50, 2), (0.65, 0.85, 0.02), (2.5, 3.8, 0.07))
    log.info("A03  Len(24-50 s2)xSu(0.65-0.85 s0.02)xNi(2.5-3.8 s0.07)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A03 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A04  highNi_shortLen -- explore Ni > 4.0 at Len 20-32 (untested)
    A = 4
    _n = "04_highNi_shortLen"
    _c = _cfg(_n, (20, 32, 1), (0.5, 1.0, 0.04), (4.0, 6.0, 0.1))
    log.info("A04  Len(20-32 s1)xSu(0.5-1.0 s0.04)xNi(4.0-6.0 s0.1)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A04 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A05  A02_Su_lateral -- wider Su exploration around A02 (Su 0.4-1.5)
    A = 5
    _n = "05_A02_Su_lateral"
    _c = _cfg(_n, (24, 30, 1), (0.4, 1.5, 0.05), (3.0, 4.0, 0.05))
    log.info("A05  Len(24-30 s1)xSu(0.4-1.5 s0.05)xNi(3.0-4.0 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A05 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A06  midLen_zoom -- explore Len 30-58 with finer resolution
    A = 6
    _n = "06_midLen_zoom"
    _c = _cfg(_n, (30, 58, 2), (0.50, 0.90, 0.03), (1.5, 3.0, 0.075))
    log.info("A06  Len(30-58 s2)xSu(0.50-0.90 s0.03)xNi(1.5-3.0 s0.075)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A06 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A07  A02_Ni_higher -- Su tight + Ni higher (3.5-5.5)
    A = 7
    _n = "07_A02_Ni_higher"
    _c = _cfg(_n, (22, 28, 1), (0.7, 0.85, 0.01), (3.5, 5.5, 0.1))
    log.info("A07  Len(22-28 s1)xSu(0.7-0.85 s0.01)xNi(3.5-5.5 s0.1)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A07 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A08  R1_regime_confirm -- confirm R1 NP-max regime at fine resolution
    A = 8
    _n = "08_R1_regime_confirm"
    _c = _cfg(_n, (44, 54, 1), (0.65, 0.80, 0.01), (1.9, 2.4, 0.025))
    log.info("A08  Len(44-54 s1)xSu(0.65-0.80 s0.01)xNi(1.9-2.4 s0.025)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A08 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A09  adaptive_zoom1 -- TIGHTER ranges than R2 (which missed A02 exact peak)
    A = 9
    _n = "09_adaptive_zoom1"
    log.info("A09  adaptive_zoom1 -- center: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f",
             best_LEN, best_SU, best_NI, best_np)
    if start_attempt <= A:
        r_len = max(4.0,   best_LEN * 0.10)
        r_su  = max(0.08,  best_SU  * 0.15)
        r_ni  = max(0.20,  best_NI  * 0.15)
    ...
    log.info("A10  adaptive_zoom2 -- center: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f",
             best_LEN, best_SU, best_NI, best_np)
    if start_attempt <= A:
        r_len = max(2.0,   best_LEN * 0.05)
        r_su  = max(0.04,  best_SU  * 0.07)
        r_ni  = max(0.10,  best_NI  * 0.07)
    ...
    log.info("A11  adaptive_zoom3 -- center: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f",
             best_LEN, best_SU, best_NI, best_np)
    if start_attempt <= A:
        r_len = max(1.0,   best_LEN * 0.025)
        r_su  = max(0.02,  best_SU  * 0.04)
        r_ni  = max(0.05,  best_NI  * 0.04)
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex BTCUSDT Daily Round-3 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex BTCUSDT HOT Daily R3 parameter search")
