"""
search_btc_qpatrex_hourly.py  QuantPassATRex on BTCUSDT HOT Hourly, Round 1
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrex_hourly_search")
...
# Strategy defaults as nominal seed (no prior results)
SEED_LEN = 55.0
SEED_SU  = 0.75
SEED_NI  = 1.5
SEED_NP  = 0.0
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrex_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_btc_qpatrex_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on BTCUSDT HOT Hourly -- Round 1")
    ...
    # -----------------------------------------------------------------------
    # A01  global_coarse
    #      Wide landscape: Len 5-205, Su 0.1-5.0, Ni 0.2-10.0
    ...
    # -----------------------------------------------------------------------
    # A02  tight_multiples (both Su and Ni small, high frequency)
    #      Tests frequently-triggered regime with small moves
    ...
    # -----------------------------------------------------------------------
    # A03  wide_multiples (both Su and Ni large, low frequency, big moves)
    #      Tests rare-trigger regime: only enter on very large ATRex moves
    ...
    # -----------------------------------------------------------------------
    # A04  short_len (fast ATRex, quick adaptation)
    #      Short Len: ATRex adapts quickly to recent volatility
    ...
    # -----------------------------------------------------------------------
    # A05  long_len (slow ATRex, stable baseline)
    #      Long Len: ATRex is smoother, fewer entries on short-term noise
    ...
    # -----------------------------------------------------------------------
    # A06  near_default (original design center: Len=55, Su=0.75, Ni=1.5)
    #      Fine-grained scan around strategy default parameters
    ...
    # -----------------------------------------------------------------------
    # A07  high_ni (Ni >> Su: Ni entries dominate over Su entries)
    #      Tests regime where market entries (Ni) trigger much more easily than stop entries
    ...
    # -----------------------------------------------------------------------
    # A08  equal_multiples (Su == Ni: both triggers equal strength)
    #      Tests balanced regime where stop and market entries are equally spaced
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex BTCUSDT Hourly Round-1 COMPLETE")
    ...
    print(f"\nDone -- results at: {out}")
    print(f"Best NP={best_np:.0f}  ({pct:.1f}% of {TARGET_NP:.0f} target)")
    print(f"Target NP>10K USD: {'MET' if target_met else 'NOT MET -- best NP=%.0f' % best_np}")
    return 0
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex BTCUSDT HOT Hourly R1 parameter search")
