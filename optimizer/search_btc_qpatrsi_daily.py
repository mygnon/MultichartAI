"""
search_btc_qpatrsi_daily.py — QuantPassRSI on BTCUSDT HOT Daily, Round 1 (IS 2022/01-2026/01)
...
SYMBOL     = "BTCUSDT HOT"
SIGNAL     = "QuantPassRSI"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrsi_daily_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrsi_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_btc_qpatrsi_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI on BTCUSDT HOT Daily -- Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI BTCUSDT Daily Round-1 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="QuantPassRSI BTCUSDT HOT Daily R1 search")
