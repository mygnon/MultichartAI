"""
search_btc_qpatrsi_daily2.py — QuantPassRSI on BTCUSDT HOT Daily, Round 2 (IS 2022/01-2026/01)
...
R1 Obj-max = NP-max Len=63 RSI_Gap=48 NP=$1,605 MDD=-$839 Obj=3,072 11tr (sparse, high-gap near-degenerate;
zoom Obj UNSTABLE 1,846-3,072). 2 attempts garbled. R2 = INTEGER confirm Len=63 Gap=48 + RULE-5 DRIFT
retest (does same-param NP hold or drift like BNB Daily?) + COARSE sparse re-tests + dense low-gap check.
BTC Daily<Hourly ($1.6K vs $2.0K). $100K -98.4%.
...
SYMBOL     = "BTCUSDT HOT"
SIGNAL     = "QuantPassRSI"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrsi_daily2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrsi_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_btc_qpatrsi_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI on BTCUSDT HOT Daily -- Round 2 (integer confirm + Rule-5 drift; IS 2022/01-2026/01)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (61, 65, 2), (46, 50, 2)))
    ...
    # static attempts
    _do(1, "01_retest_center",  _cfg("01_retest_center",  (55,71,1),   (42,49,1)))
    _do(2, "02_fine_lengap",    _cfg("02_fine_lengap",    (40,90,1),   (40,49,1)))
    _do(3, "03_gap_fine",       _cfg("03_gap_fine",       (58,68,1),   (20,49,1)))
    _do(4, "04_mid_len_coarse", _cfg("04_mid_len_coarse", (20,120,5),  (40,49,1)))
    _do(5, "05_long_len_coarse",_cfg("05_long_len_coarse",(100,500,20),(40,49,1)))
    _do(6, "06_dense_lowgap",   _cfg("06_dense_lowgap",   (2,40,2),    (20,44,2)))
    _do(7, "07_short_len_np",   _cfg("07_short_len_np",   (2,30,1),    (20,49,1)))
    _do(8, "08_broad_int",      _cfg("08_broad_int",      (2,140,2),   (40,49,1)))
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI BTCUSDT Daily Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="QuantPassRSI BTCUSDT HOT Daily R2 (confirm) search")
