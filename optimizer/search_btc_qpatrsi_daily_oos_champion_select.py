"""
search_btc_qpatrsi_daily_oos_champion_select.py — OOS selection among the converged
BTC Daily QuantPassRSI champions (R1-R2, results/btc_qpatrsi_daily{,2}_search/).

QuantPassRSI (2 params Len, RSI_Gap; momentum; reversal exits, _Crypto1MUSD). BTC DAILY: IS data-STABLE
(Rule-5 retest PASS, Len=60 Gap=48=$1,503 R1=R2) but weak ($1.5K, 11tr, Gap~48 near-degenerate).
Candidates span the mid-len champion, a denser short-len, a dense low-gap, and a mid-len contrast.
...
PREREQUISITES (MC64, elevated):
  - 20260101_QuantPassRSI_AI.wsp open, BTCUSDT HOT DAILY chart tab ACTIVE, QuantPassRSI signal ON
  - Full data to 2026/06/10 (the script sets the range itself). CLOSE the Study Editor window.

CLI:
  py search_btc_qpatrsi_daily_oos_champion_select.py                 # merged: IS then FULL, verdict
  py search_btc_qpatrsi_daily_oos_champion_select.py --candidate BSD1 # smoke one candidate
  py search_btc_qpatrsi_daily_oos_champion_select.py --from-csv      # recompute from existing CSVs
"""
...
SYMBOL      = "BTCUSDT HOT"
SIGNAL      = "QuantPassRSI"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrsi_daily_oos_champion_select_search")
PREFIX      = "BTCRSIDSEL_"
...
# 4 converged BTC Daily QuantPassRSI candidates (btc_qpatrsi_daily{,2}_search).
# id, regime, Len, RSI_Gap, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("BSD1", "R2 Obj-max mid-len ***", 59.0, 48.0, 1517.0, -858.0),  # 11tr, R2 ceiling Obj=2,680 (stable)
    ("BSD2", "short-len DENSE",        20.0, 46.0, 1172.0, -571.0),  # 29tr, lowest MDD, most active
    ("BSD3", "dense low-gap",          24.0, 40.0, 1123.0, -646.0),  # 10tr, low-gap
    ("BSD4", "mid-len contrast",       42.0, 45.0, 1178.0, -948.0),  # 11tr, mid Gap
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrsi_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_qpatrsi_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_qpatrsi_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("BSD1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Daily QuantPassRSI -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Daily QuantPassRSI OOS champion selection")
