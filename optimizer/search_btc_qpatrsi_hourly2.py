"""
search_btc_qpatrsi_hourly2.py — QuantPassRSI on BTCUSDT HOT Hourly, Round 2 (IS 2022/01-2026/01)
...
R1 Obj-max ⭐ Len~58-62 RSI_Gap~33 (~= default 32) NP=$2,020 MDD=-$624 Obj=6,537 15tr (sparse, low-MDD);
R1 NP-max Len=7 Gap=5 NP=$2,207 MDD=-$1,156 (deeper MDD, lower Obj). VERY WEAK ($2K), $100K -98%.
...
SYMBOL     = "BTCUSDT HOT"
SIGNAL     = "QuantPassRSI"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_qpatrsi_hourly2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_qpatrsi_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_btc_qpatrsi_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI on BTCUSDT HOT Hourly -- Round 2 (confirm Obj-max, integer; IS 2022/01-2026/01)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (56, 60, 2), (31, 35, 2)))
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI BTCUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="QuantPassRSI BTCUSDT HOT Hourly R2 (confirm) search")
