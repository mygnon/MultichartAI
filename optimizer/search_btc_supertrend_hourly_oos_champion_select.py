"""
search_btc_supertrend_hourly_oos_champion_select.py — OOS selection among the converged BTC Hourly
SFJ_SuperTrend_crypto champions (rounds 1-2, results/btc_supertrend_hourly{,2}_search/).
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_SuperTrend_AI.wsp open, BTCUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
  - SFJ_SuperTrend_crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_btc_supertrend_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_btc_supertrend_hourly_oos_champion_select.py --period is
  py search_btc_supertrend_hourly_oos_champion_select.py --candidate TS1
  py search_btc_supertrend_hourly_oos_champion_select.py --from-csv
  py search_btc_supertrend_hourly_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_supertrend_hourly_oos_champion_select_search")
PREFIX      = "BTCSTHSEL_"
...
# 4 DISTINCT converged BTC Hourly SuperTrend regimes (from btc_supertrend_hourly{,2}_search).
# id, regime, ATRLength, Multiplier, prior IS NP, prior IS MDD  (reference only)
CANDIDATES = [
    ("TS1", "long-ATR wide-band ***", 151.0, 9.15, 1986.0, -586.0),  # R1=R2 peak, 127tr; NP/Obj-max
    ("TS2", "mid-ATR high-freq",       68.0, 6.0,  1228.0, -618.0),  # 277tr, distinct high-freq
    ("TS3", "short-ATR high-freq",     23.0, 7.5,  1641.0, -582.0),  # 187tr, distinct short-ATR
    ("TS4", "vhigh-mult low-freq",    200.0, 15.5, 1525.0, -710.0),  # 53tr, distinct low-freq
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_supertrend_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_supertrend_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_supertrend_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("TS1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Hourly SuperTrend — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Hourly SuperTrend OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="TSn", default=None)
