"""
search_btc_supertrend_oos_validation.py — FULL-PERIOD (incl. OOS) validation of the IS-optimized
SuperTrend exit modules on BTCUSDT HOT Hourly.

Main signal SFJ_SuperTrend_crypto FIXED at champion (ATRLength=151 Multiplier=9.15), Status ON.
Each of the 6 exit modules is FIXED at its IS-best param (from
results/btc_supertrend_exit_modules_search) and re-tested over the FULL range 2021/03/01-2026/06/10
...
IS-best module params (from the exit-module optimization, IS 2022/01-2026/01):
  M1 ATRstop            STP=25.7
  M2 TrailingStop       ATRSTP=51.7
  M3 EntryBarsAfterExit EXITBAR=425
  M4 high_volatility    DAYRANGE=6.21
  M5 QuantPass_PT_Exit  PT_Base=0.65
  M6 RescueTeamExit     Length=140, std=3.0
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  ...
  - Main signal SFJ_SuperTrend_crypto inputs = ATRLength 151 Multiplier 9.15, Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved

CLI:
  py search_btc_supertrend_oos_validation.py                 # A00 full baseline + M1-M6 + teardown
  py search_btc_supertrend_oos_validation.py --module 6      # only module 6
  py search_btc_supertrend_oos_validation.py --manual-status # toggle Status by hand
  py search_btc_supertrend_oos_validation.py --from-csv      # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_supertrend_oos_validation_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # WIDE no-op; chart-trim is the real control
FULL_RANGE  = ("2021/03/01", "2026/06/10")

CHAMPION = {"ATRLength": 151.0, "Multiplier": 9.15}
...
MODULES: List[Tuple[int, str, List[Tuple[str, float, float, float, float]]]] = [
    (1, "SFJ_15Dworkshop_lesson4_ATRstop",                 [("STP",      25.7,  0.1,   0.1,   100.0)]),
    (2, "SFJ_15Dworkshop_lesson9_1_TrailingStop",          [("ATRSTP",   51.7,  0.1,   0.1,   100.0)]),
    (3, "SFJ_15Dworkshop_lesson10_3_EntryBarsAfterExit",   [("EXITBAR",  425.0, 1.0,   1.0,   1000.0)]),
    (4, "SFJ_15Dworkshop_lesson11_3_high_volatility_exit", [("DAYRANGE", 6.21,  0.01,  0.01,  10.0)]),
    (5, "QuantPass_PT_Exit",                               [("PT_Base",  0.65,  0.001, 0.001, 1.0)]),
    (6, "RescueTeamExit",                                  [("Length",   140.0, 20.0,  20.0,  600.0),
                                                            ("std",      3.0,   0.1,   3.0,   6.0)]),
]
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_supertrend_oos_validation_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("ATRLength",  150.0, 152.0, 1.0),
            ParamAxis("Multiplier", 9.10,  9.20,  0.05),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (ATRLength=151 Multiplier=9.15)")
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_supertrend_oos_validation.json"
...
def run_search(conn, from_csv: bool, only_module: Optional[int], manual_status: bool):
    ...
    out_json = OUTPUT_DIR / "final_params_btc_supertrend_oos_validation.json"
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  BTC SuperTrend exit-module FULL-PERIOD validation")
    log.info("  Main fixed ATRLength=151 Multiplier=9.15 (ON); range %s ~ %s",
    ...
    if only_module is None or only_module == 0:
        ...
        # the exact champion (151/9.15) row below.  That check is for MODULE runs only.
...
def main():
    ap = argparse.ArgumentParser(description="BTC SuperTrend exit-module full-period OOS validation")
