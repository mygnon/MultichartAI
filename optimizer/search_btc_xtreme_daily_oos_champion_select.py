...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_XtremeStop_AI.wsp open, BTCUSDT HOT **Daily** chart tab ACTIVE and VISIBLE
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_xtreme_daily_oos_champion_select_search")
PREFIX      = "BTCXTDSEL_"
...
# 4 DISTINCT converged BTC Daily XtremeStop regimes (from btc_xtreme_daily{,2}_search R1/R2).
# id, regime, X, LY, SY, prior IS NP, prior IS MDD  (NP data-vintage-dependent, Rule 5 — reference only)
CANDIDATES = [
    ("CD1", "long-X asym champion ***",  268.0, 2.3, 0.1,  4020.6, -1741.61),  # R2 ceiling 7-conv; NP/Obj-max 11tr
    ("CD2", "dense low-MDD high-freq",     42.0, 0.5, 0.2,  1796.3, -831.3),    # R2 A06 dense_recheck 105tr
    ("CD3", "short-X high-freq",           29.0, 0.5, 6.0,  1852.3, -674.2),    # R1 A02 short_X 48tr
    ("CD4", "mid-X high-pct",              55.0, 8.0, 6.5,  1964.4, -864.7),    # R1 A03/A05 13tr
]
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_xtreme_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_xtreme_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_xtreme_daily_oos_champion_select.json"
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Daily XtremeStop — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Daily XtremeStop OOS champion selection (auto-range merged)")
