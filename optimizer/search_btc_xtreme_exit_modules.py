"""
search_btc_xtreme_exit_modules.py -- Exit-module independent optimization on top of the
SFJ_XtremeStop_Crypto BTC Hourly champion.

Main signal FIXED at the XtremeStop champion (X=60 LY=12.75 SY=12.4; IS ceiling = OOS
de-facto winner), Status ON. Six exit modules tested INDEPENDENTLY (one enabled at a time),
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_XtremeStop_AI.wsp open, BTCUSDT HOT HOURLY tab active
  - Main signal SFJ_XtremeStop_Crypto inputs set to X=60 LY=12.75 SY=12.4 (champion), Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved
  - CLOSE the Study Editor window

CLI:
  py search_btc_xtreme_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_btc_xtreme_exit_modules.py --module 3       # only module 3
  py search_btc_xtreme_exit_modules.py --module 0       # baseline A00 only
  py search_btc_xtreme_exit_modules.py --smoke --module 1
  py search_btc_xtreme_exit_modules.py --manual-status  # user toggles Status by hand
  py search_btc_xtreme_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_btc_xtreme_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_xtreme_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # signal date no-op; chart-trim is the control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# XtremeStop champion (IS ceiling = OOS de-facto winner) — main signal set in MC64 (Format Objects, Status ON)
CHAMPION = {
    "X":  60.0,
    "LY": 12.75,
    "SY": 12.4,
}
# Reference baseline = XtremeStop main-only IS ceiling (btc_xtreme_hourly{,2}_search)
BASELINE_REF = {"net_profit": 2606.0, "max_drawdown": -582.0, "total_trades": 13}
...
_LOG_FILE = OUTPUT_DIR / f"search_btc_xtreme_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    # 27-combo micro-grid around champion: all 3 params vary (Critical Rule 1);
    # grid points include the EXACT champion X=60 LY=12.75 SY=12.4 so A00 reads main-only at champion.
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("X",  58.0,  62.0,  2.0),
            ParamAxis("LY", 12.5,  13.0,  0.25),
            ParamAxis("SY", 12.15, 12.65, 0.25),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs X=60 LY=12.75 SY=12.4)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_btc_xtreme_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_btc_xtreme_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("  Main fixed: X=60 LY=12.75 SY=12.4 (champion, Status ON)")
    log.info("  Baseline ref (XtremeStop IS): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
