"""
search_btc_xtreme_hourly2.py — SFJ_XtremeStop_Crypto on BTCUSDT HOT Hourly, Round 2
...
Priors (XtremeStop on futures; no crypto prior):
  NQ Hourly X=10 LY=1.48 SY=2.18; GC X=7 LY=0.67 SY=1.80; TXF X=63 LY=5.155 SY=5.79; BNB crypto X=905 LY=3.0 SY=10.65 (asym long-X).
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\btc_xtreme_hourly2_search")
...
SEED_X, SEED_LY, SEED_SY = 60.0, 12.75, 12.4
SEED_NP = 0.0

PREFIX = "BTCXTH2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_xtreme_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_btc_xtreme_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  BTC Hourly SFJ_XtremeStop_Crypto NP>100K -- Round 2")
    log.info("  IS window %s ~ %s  (chart-trimmed)", IS_RANGE[0], IS_RANGE[1])
    log.info("  R1 champ X=60 LY=12.75 SY=12.4 NP=2606 (sym high-pct 13tr; confirming peak)", )
    log.info("==============================================================")

    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (56, 64, 2), (12.0, 13.5, 0.25), (11.9, 12.9, 0.25)))
    ...
    attempts_config = [
        ("01_pct_sweep",     (55, 65, 2),    (9.0, 16.0, 0.5),  (9.0, 16.0, 0.5)),
        ("02_X_sweep",       (20, 150, 5),   (11.0, 14.0, 0.5), (11.0, 14.0, 0.5)),
        ("03_LY_sweep",      (55, 65, 2),    (8.0, 20.0, 0.5),  (12.0, 13.0, 0.25)),
        ("04_SY_sweep",      (55, 65, 2),    (12.0, 13.0, 0.25),(8.0, 20.0, 0.5)),
        ("05_high_pct_push", (50, 70, 5),    (13.0, 25.0, 0.5), (13.0, 25.0, 0.5)),
        ("06_X_bridge_long", (60, 400, 20),  (10.0, 15.0, 0.5), (10.0, 15.0, 0.5)),
        ("07_asym_check",    (55, 65, 2),    (2.0, 13.0, 0.5),  (10.0, 15.0, 0.5)),
        ("08_fine_zoom",     (56, 64, 1),    (12.0, 13.5, 0.1), (11.9, 12.9, 0.1)),
    ]
    ...
    log.info("==============================================================")
    log.info("  BTC Hourly XtremeStop_Crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="BTC Hourly SFJ_XtremeStop_Crypto NP>100K Round-2 search")
