"""
search_btc_xtreme_hourly_oos_champion_select.py — OOS selection among the converged BTC Hourly
SFJ_XtremeStop_Crypto champions (rounds 1-2, results/btc_xtreme_hourly{,2}_search/).
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_XtremeStop_AI.wsp open, BTCUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
  - SFJ_XtremeStop_Crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_btc_xtreme_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_btc_xtreme_hourly_oos_champion_select.py --period is
  py search_btc_xtreme_hourly_oos_champion_select.py --candidate CX1
  py search_btc_xtreme_hourly_oos_champion_select.py --from-csv
  py search_btc_xtreme_hourly_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\btc_xtreme_hourly_oos_champion_select_search")
PREFIX      = "BTCXTHSEL_"
...
# 4 DISTINCT converged BTC Hourly XtremeStop regimes (from btc_xtreme_hourly{,2}_search).
# id, regime, X, LY, SY, prior IS NP, prior IS MDD  (NP data-vintage-dependent, Rule 5 — reference only)
CANDIDATES = [
    ("CX1", "sym high-pct champion ***", 60.0,  12.75, 12.4, 2606.0, -582.0),  # R1=R2 peak, 13tr; NP/Obj-max
    ("CX2", "asym mid-X high-freq",      70.0,  2.5,   7.5,  2345.0, -599.0),  # 94tr, distinct
    ("CX3", "long-X low-MDD",           800.0,  1.0,   5.0,  2100.0, -494.0),  # 69tr, lowest MDD
    ("CX4", "short-pct high-freq",       65.0,  3.0,   6.5,  1962.0, -500.0),  # 121tr, distinct high-freq
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_btc_xtreme_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_btc_xtreme_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_btc_xtreme_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("CX1", "is", *CANDIDATES[0][2:5]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  BTC Hourly XtremeStop — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="BTC Hourly XtremeStop OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="CXn", default=None)
