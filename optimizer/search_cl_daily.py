"""
search_cl_daily.py — Breakout Daily NP > 700,000 on CME.CL HOT, Round 1

No prior knowledge — first exploration of CL daily bars.
Target: NP > 700,000 USD on CME.CL HOT daily bars, _2021Basic_Break_NQ
Insample: 2019/01/01 – 2026/01/01
Objective: NP² / |MaxDrawdown|

CL daily context:
  1 point = $1,000 / contract (1,000 bbl × $1/bbl)
  Typical daily range: 1–3 points = $1K–3K / contract
  Expected trade frequency: much lower than hourly (~20–100 trades / 7yr)

Parameter design based on daily strategy experience (GC/NQ/TWF):
  LE: 1–8   (entry bar-count lookback)
  SE: 10–120 (breakout signal length in bars)
  STP: 0.2–10 (stop loss in points; min step 0.2 to avoid CSV garbling)
  LMT: 2–30  (profit target in points; integer start)

Attempt schedule (12 attempts, ≤5,000 combos each):
  A01 coarse_survey:  LE(1-5 s2) × SE(10-80 s10) × STP(1-5 s2) × LMT(2-20 s3)   3×8×3×7=504
  A02 mid_se:         LE(1-5 s2) × SE(25-65 s5) × STP(0.5-3 s0.5) × LMT(3-15 s3)  3×9×6×5=810
  A03 low_se:         LE(1-5 s1) × SE(5-20 s3) × STP(1-5 s2) × LMT(2-15 s3)      5×6×3×5=450
  A04 high_se:        LE(1-4 s1) × SE(70-150 s10) × STP(1-5 s2) × LMT(3-20 s3)   4×9×3×6=648
  A05 tight_stp:      LE(1-5 s2) × SE(20-60 s5) × STP(0.2-1.0 s0.2) × LMT(3-15 s3) 3×9×5×5=675
  A06 high_lmt:       LE(1-4 s1) × SE(25-65 s5) × STP(1-5 s2) × LMT(20-60 s5)    4×9×3×9=972
  A07 zoom_best:      adaptive zoom from best NP so far
  A08 le_fine:        LE(1-8 s1) × SE(30-60 s5) × STP(1-3 s1) × LMT(5-20 s3)     8×7×3×6=1008
  A09 stp_fine:       LE(1-3 s1) × SE(30-60 s5) × STP(0.3-3.0 s0.3) × LMT(5-20 s5) 3×7×10×4=840
  A10 dense_zoom:     adaptive dense zoom
  A11 lmt_fine:       LE(1-4 s1) × SE(30-60 s5) × STP(1-3 s1) × LMT(1-15 s1)     4×7×3×15=1260
  A12 wide_boundary:  LE(1-5 s1) × SE(10-100 s10) × STP(0.5-5 s0.5) × LMT(2-25 s3) 5×10×10×8=4000
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\cl_daily_search")
...
SEED_LE,  SEED_SE  = 3.0, 50.0
SEED_STP, SEED_LMT = 2.0, 10.0
SEED_NP   = 0.0
SEED_OBJ  = 0.0

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_cl_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def csv_for(name: str) -> Path:
    return OUTPUT_DIR / f"CLD1_{name}_raw.csv"
...
def run_or_load(name, cfg, conn, from_csv):
    ...
    if from_csv or csv_path.exists():
        ...
        return None
    log.info("=== Starting CLD1_%s (%d combos) ===", name, cfg.total_runs())
    t0 = time.time()
    try:
...
def save_json(best, log_, met):
    ...
    payload = {
        "strategy":           "Breakout_CL_Daily  (target NP>700K round-1)",
        "symbol":             SYMBOL,
    ...
    out = OUTPUT_DIR / "final_params_cl_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  CL Daily Breakout NP>700K Round-1 — initial exploration")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A01  Coarse survey  (3×8×3×7=504)
    # ──────────────────────────────────────────────────────────────────────
    A = 1
    _n = "01_coarse_survey"
    _c = _cfg(_n, (1, 5, 2), (10, 80, 10), (1.0, 5.0, 2.0), (2, 20, 3))
    log.info("A01  LE(1-5 s2) × SE(10-80 s10) × STP(1-5 s2) × LMT(2-20 s3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  Mid SE range  (3×9×6×5=810)
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_mid_se"
    _c = _cfg(_n, (1, 5, 2), (25, 65, 5), (0.5, 3.0, 0.5), (3, 15, 3))
    log.info("A02  LE(1-5 s2) × SE(25-65 s5) × STP(0.5-3 s0.5) × LMT(3-15 s3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  Low SE (fast signals)  (5×6×3×5=450)
    # ──────────────────────────────────────────────────────────────────────
    A = 3
    _n = "03_low_se"
    _c = _cfg(_n, (1, 5, 1), (5, 20, 3), (1.0, 5.0, 2.0), (2, 15, 3))
    log.info("A03  LE(1-5 s1) × SE(5-20 s3) × STP(1-5 s2) × LMT(2-15 s3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  High SE (slow signals)  (4×9×3×6=648)
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_high_se"
    _c = _cfg(_n, (1, 4, 1), (70, 150, 10), (1.0, 5.0, 2.0), (3, 20, 3))
    log.info("A04  LE(1-4 s1) × SE(70-150 s10) × STP(1-5 s2) × LMT(3-20 s3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  Tight STP 0.2-1.0  (3×9×5×5=675)
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_tight_stp"
    _c = _cfg(_n, (1, 5, 2), (20, 60, 5), (0.2, 1.0, 0.2), (3, 15, 3))
    log.info("A05  LE(1-5 s2) × SE(20-60 s5) × STP(0.2-1.0 s0.2) × LMT(3-15 s3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  High LMT 20-60  (4×9×3×9=972)
    # ──────────────────────────────────────────────────────────────────────
    A = 6
    _n = "06_high_lmt"
    _c = _cfg(_n, (1, 4, 1), (25, 65, 5), (1.0, 5.0, 2.0), (20, 60, 5))
    log.info("A06  LE(1-4 s1) × SE(25-65 s5) × STP(1-5 s2) × LMT(20-60 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  Adaptive zoom from best so far
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_zoom_best"
    for _r_le, _r_se, _r_stp, _r_lmt, _le_step, _se_step, _stp_step, _lmt_step in [
        (3, 15, 1.0, 10, 1, 3, 0.2, 2),
        (2, 10, 0.8,  7, 1, 2, 0.2, 2),
        (2,  8, 0.6,  5, 1, 2, 0.2, 1),
        (1,  5, 0.4,  3, 1, 1, 0.2, 1),
    ]:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  LE fine survey  (8×7×3×6=1008)
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_le_fine"
    _c = _cfg(_n, (1, 8, 1), (30, 60, 5), (1.0, 3.0, 1.0), (5, 20, 3))
    log.info("A08  LE(1-8 s1) × SE(30-60 s5) × STP(1-3 s1) × LMT(5-20 s3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A09  STP fine scan  (3×7×10×4=840)
    # ──────────────────────────────────────────────────────────────────────
    A = 9
    _n = "09_stp_fine"
    _c = _cfg(_n, (1, 3, 1), (30, 60, 5), (0.3, 3.0, 0.3), (5, 20, 5))
    log.info("A09  LE(1-3 s1) × SE(30-60 s5) × STP(0.3-3.0 s0.3) × LMT(5-20 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A11  LMT fine 1-15  (4×7×3×15=1260)
    # ──────────────────────────────────────────────────────────────────────
    A = 11
    _n = "11_lmt_fine"
    _c = _cfg(_n, (1, 4, 1), (30, 60, 5), (1.0, 3.0, 1.0), (1, 15, 1))
    log.info("A11  LE(1-4 s1) × SE(30-60 s5) × STP(1-3 s1) × LMT(1-15 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A12  Wide boundary  (5×10×10×8=4000)
    # ──────────────────────────────────────────────────────────────────────
    A = 12
    _n = "12_wide_boundary"
    _c = _cfg(_n, (1, 5, 1), (10, 100, 10), (0.5, 5.0, 0.5), (2, 25, 3))
    log.info("A12  LE(1-5 s1) × SE(10-100 s10) × STP(0.5-5 s0.5) × LMT(2-25 s3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    log.info("")
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  FINAL — CL Daily Breakout NP>700K Round-1")
    log.info("══════════════════════════════════════════════════════════════")
...
def main():
    ap = argparse.ArgumentParser(description="CL Daily Breakout NP>700K Round-1 search")
