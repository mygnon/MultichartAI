"""
search_eth_bollingerbreakout_daily_oos_champion_select.py -- OOS selection among the converged
ETH Daily SFJ_BollingerBreakout_crypto champions (R1-R3, results/eth_bollingerbreakout_daily{,2,3}_search/).

4 params BBLen, BBmult, ATRMult, ReentryBars; pure Bollinger-band breakout STOP + cooldown + ATR chandelier exit,
long+short, _Crypto1MUSD. ETH DAILY R3 ceiling: Obj-max=NP-max BBLen=10 BBmult=1.4 ATRMult=3.0 ReentryBars=14
(NP $2,291 MDD -$429 21tr MDD/NP 18.7%); Daily<Hourly -56%. Daily sparse (21tr). $100K unreachable.
ETH Hourly OOS all-broke; Daily = check.
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, ETHUSDT HOT DAILY chart tab ACTIVE,
...
CLI:
  py search_eth_bollingerbreakout_daily_oos_champion_select.py                  # merged verdict
  py search_eth_bollingerbreakout_daily_oos_champion_select.py --candidate ED1  # smoke one
  py search_eth_bollingerbreakout_daily_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_bollingerbreakout_daily_oos_champion_select_search")
PREFIX      = "ETHBODSEL_"
...
# Converged ETH Daily BollingerBreakout candidates (eth_bollingerbreakout_daily{,2,3}_search).
# id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday).
CANDIDATES = [
    # id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday)
    ("ED1", "Obj-max=NP-max ***",     10.0, 1.4, 3.0,  14.0, 2291.0, -429.0),  # R3 champion, 21tr low-freq
    ("ED2", "wide-BBmult low-MDD",    10.0, 2.5, 2.75, 15.0, 2036.0, -396.0),  # 23tr, lowest IS-MDD, distinct
    ("ED3", "tight-ATR higher-freq",  10.0, 2.6, 1.5,  14.0, 1503.0, -377.0),  # 40tr, lowest MDD, distinct regime
    ("ED4", "ultra-short hi-freq",    2.0,  3.0, 2.0,  11.0, 1609.0, -500.0),  # 45tr, high-freq generalizer
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_bollingerbreakout_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_bollingerbreakout_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_bollingerbreakout_daily_oos_champion_select.json"
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Daily BollingerBreakout -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Daily SFJ_BollingerBreakout_crypto OOS champion selection")
