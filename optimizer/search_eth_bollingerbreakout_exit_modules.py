"""
search_eth_bollingerbreakout_exit_modules.py -- Exit-module independent optimization on top of the
SFJ_BollingerBreakout_crypto ETH Hourly champion.

Main signal FIXED at EO3 (ETH OOS-best regime: BBLen=6 BBmult=2.0 ATRMult=7
ReentryBars=15), Status ON. Six exit modules tested INDEPENDENTLY (one enabled at a time),
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, ETHUSDT HOT HOURLY tab active
  - Main signal SFJ_BollingerBreakout_crypto inputs set to BBLen=6 BBmult=2.0 ATRMult=7 ReentryBars=15 (EO3), Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved
  - CLOSE the Study Editor window

CLI:
  py search_eth_bollingerbreakout_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_eth_bollingerbreakout_exit_modules.py --module 3       # only module 3
  py search_eth_bollingerbreakout_exit_modules.py --module 0       # baseline A00 only
  py search_eth_bollingerbreakout_exit_modules.py --smoke --module 1
  py search_eth_bollingerbreakout_exit_modules.py --manual-status  # user toggles Status by hand
  py search_eth_bollingerbreakout_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_eth_bollingerbreakout_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_bollingerbreakout_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # signal date no-op; chart-trim is the control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# EO3 (ETH OOS-best regime) — main signal set to these in MC64 (Format Objects, Status ON)
CHAMPION = {
    "BBLen":       6.0,
    "BBmult":      2.0,
    "ATRMult":     7.0,
...
# Reference baseline = EO3 main-only IS (eth_bollingerbreakout_hourly_oos_champion_select, IS pass)
BASELINE_REF = {"net_profit": 2623.0, "max_drawdown": -535.0, "total_trades": 257}
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_bollingerbreakout_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    # 81-combo micro-grid around EO3: all 4 params vary (Critical Rule 1);
    # grid points include the EXACT EO3 6/2.0/7/15 so A00 reads main-only at EO3.
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("BBLen",       5.0,  7.0,  1.0),
            ParamAxis("BBmult",      1.9,  2.1,  0.05),
            ParamAxis("ATRMult",     6.5,  7.5,  0.5),
            ParamAxis("ReentryBars", 14.0, 16.0, 1.0),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs BBLen=6 BBmult=2.0 ATRMult=7 ReentryBars=15)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_eth_bollingerbreakout_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_eth_bollingerbreakout_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("  Main fixed: BBLen=6 BBmult=2.0 ATRMult=7 ReentryBars=15 (EO3, Status ON)")
    log.info("  Baseline ref (EO3 IS): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
