"""
search_eth_bollingerbreakout_hourly_oos_champion_select.py -- OOS selection among the converged
ETH Hourly SFJ_BollingerBreakout_crypto champions (R1-R3, results/eth_bollingerbreakout_hourly{,2,3}_search/).

4 params BBLen, BBmult, ATRMult, ReentryBars; pure Bollinger-band breakout STOP + cooldown + ATR chandelier exit,
long+short, _Crypto1MUSD. ETH R3 ceiling: Obj-max BBLen=4 BBmult=0.775 ATRMult=15.5 ReentryBars=6
(NP $5,267 MDD -$473 MDD/NP 9.0% = best of 3 coins); shortest BBLen + narrowest BBmult + wide trail + lowest cooldown.
ETH/BTC ~2.36x; $100K unreachable. BNB OOS-robust(2 PASS), BTC OOS-fragile(all broke) — ETH = open question.
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_BollingerBreakout_crypto_AI.wsp open, ETHUSDT HOT HOURLY chart tab ACTIVE,
...
CLI:
  py search_eth_bollingerbreakout_hourly_oos_champion_select.py                  # merged verdict
  py search_eth_bollingerbreakout_hourly_oos_champion_select.py --candidate EO1  # smoke one
  py search_eth_bollingerbreakout_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_bollingerbreakout_hourly_oos_champion_select_search")
PREFIX      = "ETHBOSEL_"
...
# Converged ETH Hourly BollingerBreakout candidates (eth_bollingerbreakout_hourly{,2,3}_search).
# id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday).
CANDIDATES = [
    # id, regime, BBLen, BBmult, ATRMult, ReentryBars, prior IS NP, prior IS MDD (intraday)
    ("EO1", "Obj-max (lowest MDD) ***", 4.0,  0.775, 15.5, 6.0,  5267.0, -473.0),  # R3 NP/Obj-max, 68tr wide-trail low-freq
    ("EO2", "long-BBLen mid-freq",      15.0, 2.25,  8.0,  15.0, 2909.0, -460.0),  # 195tr, low MDD, distinct
    ("EO3", "BNB-like tight-trail",     6.0,  2.0,   7.0,  15.0, 2623.0, -535.0),  # 257tr, high-freq generalizer
    ("EO4", "wide-band hi-freq",        5.0,  3.0,   8.0,  10.0, 2678.0, -785.0),  # 208tr, distinct wide-band
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_bollingerbreakout_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_bollingerbreakout_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_bollingerbreakout_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("EO1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Hourly BollingerBreakout -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly SFJ_BollingerBreakout_crypto OOS champion selection")
