"""
search_eth_closechannelbreakout_daily2.py -- SFJ_CloseChannelBreakout_crypto on ETHUSDT HOT Daily, Round 1 (IS 2022/01-2026/01)

Strategy (Strategy/SFJ_CloseChannelBreakout_crypto.txt): CLOSE-channel breakout STOP entry + ATR(14)
chandelier trailing stop + ReentryBars cooldown, long+short, _Crypto1MUSD sizing.
  UpChan=Highest(Close,Length)[1]; DnChan=Lowest(Close,Length)[1]
  Long  entry : flat & cooldown -> Buy       next bar at UpChan STOP
  Short entry : flat & cooldown -> SellShort  next bar at DnChan STOP
  Exits : ATR(14) chandelier trail (ATRMult x ATR) under/over the running extreme.

3 params: Length (close-channel lookback, int), ATRMult (chandelier trail, frac), ReentryBars (cooldown, int).
...
SYMBOL     = "ETHUSDT HOT"
SIGNAL     = "SFJ_CloseChannelBreakout_crypto"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_closechannelbreakout_daily2_search")
...
LEN_LO,  LEN_HI  = 2.0,  300.0   # Length (close-channel lookback)
ATRL_LO, ATRL_HI = 0.5,  20.0   # ATRMult (chandelier trail)
MULT_LO, MULT_HI = 0.0,  50.0   # ReentryBars (cooldown, int)

SEED_LEN, SEED_ATRL, SEED_MULT = 24.0, 3.0, 12.0  # R2 seed = R1 Obj-max (Daily narrow-trail)

PREFIX = "ETHCCBD2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_closechannelbreakout_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(name, length, atrl, mult) -> StrategyConfig:
    ...
    atrl   = _safe(atrl,   ATRL_LO, ATRL_HI, 0.5)   # ATRMult fractional
    mult   = _safe(mult,   MULT_LO, MULT_HI, 1.0)
    ...
    return StrategyConfig(
        ...
        params=[ParamAxis("Length",    *length),
                ParamAxis("ATRMult", *atrl),
                ParamAxis("ReentryBars", *mult)],
...
def _row(df, idx):
    b = df.loc[idx]
    return {"Length": float(b["Length"]), "ATRMult": float(b["ATRMult"]),
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        "round": 2, "strategy": SIGNAL, "symbol": SYMBOL, "timeframe": "Daily (1440 min)",
        ...
        "notes": {
            "logic": "Close-channel breakout STOP entry (Buy at Highest(Close,Length)[1]; SellShort at Lowest(Close,Length)[1]); ATR(14) chandelier trailing-stop exit (ATRMult x ATR); flat-only + ReentryBars cooldown; long+short",
            "contract": "_Crypto1MUSD = Round(1000000/C, 0) ~ $1M notional/trade",
            "params": "Length (close-channel lookback int), ATRMult (chandelier trail frac), ReentryBars (cooldown int); defaults 20/3.0/0",
        },
    ...
    out = OUTPUT_DIR / "final_params_eth_closechannelbreakout_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  SFJ_VolatilityBreakout_crypto on ETHUSDT HOT Daily -- Round 1 (IS 2022/01-2026/01)")
    log.info("  Params: Length, ATRMult, ReentryBars.  Obj=NP^2/|MDD|.  Target NP %.0f USD", TARGET_NP)
    ...
    def _update(df, cfg, name, attempt_num, combos, t_attempt):
        ...
        if not obj_best or om["objective"] > obj_best.get("objective", -1):
            ...
            seed_len, seed_atrl, seed_mult = om["Length"], om["ATRMult"], om["ReentryBars"]
        ...
        log.info("  [A%02d %-16s] obj_max EMA=%.4g Band=%.4g Mult=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d (%.1fs)",
                 attempt_num, name, om["Length"], om["ATRMult"], om["ReentryBars"],
    ...
    # static attempts (3D)
    _do(1, "01_retest_seed",  _cfg("01_retest_seed",  (22,26,1),  (2.5,3.5,0.5),  (10,14,1)))   # Rule-5 + tight box (incl 24/3/12)
    _do(2, "02_length_fine",  _cfg("02_length_fine",  (15,35,1),  (2.5,3.5,0.5),  (11,13,1)))
    _do(3, "03_atrmult_fine", _cfg("03_atrmult_fine", (22,26,1),  (1.0,6.0,0.25), (11,13,1)))
    _do(4, "04_reentry_fine", _cfg("04_reentry_fine", (22,26,1),  (2.5,3.5,0.5),  (0,30,1)))
    _do(5, "05_npmax_check",  _cfg("05_npmax_check",  (2,8,1),    (3.0,6.0,0.5),  (3,9,1)))     # R1 NP-max short-len regime
    _do(6, "06_wide_trail",   _cfg("06_wide_trail",   (10,40,2),  (5.0,12.0,0.5), (8,16,2)))    # re-cover R1 garbled A06
    _do(7, "07_long_len",     _cfg("07_long_len",     (30,80,2),  (2.0,4.0,0.5),  (8,14,2)))
    _do(8, "08_combo_2d",     _cfg("08_combo_2d",     (18,30,1),  (2.0,4.0,0.25), (8,16,1)))
    _do(9, "09_atrmult_low",  _cfg("09_atrmult_low",  (22,26,1),  (0.5,3.0,0.25), (10,14,1)))   # verify <3 worse
    ...
    for A, nm_, (rl_mul, rl_min, ra_mul, ra_min, rm_mul, rm_min, nL, nA, nM) in [
        ...
        if start_attempt <= A:
            ...
            _atrl = zoom_fixed(seed_atrl, max(ra_min, seed_atrl * ra_mul),  nA, 0.5,  ATRL_LO, ATRL_HI)
            _mult = zoom_fixed(seed_mult, max(rm_min, seed_mult * rm_mul),  nM, 1.0,  MULT_LO, MULT_HI)
    ...
    log.info("==============================================================")
    log.info("  SFJ_CloseChannelBreakout_crypto ETHUSDT Daily Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
        log.info("  Obj-max (your criterion): LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f MDD=%.0f Obj=%.0f tr=%d",
                 obj_best["Length"], obj_best["ATRMult"], obj_best["ReentryBars"],
    ...
    if np_best:
        log.info("  NP-max: LEN=%.4g ATRL=%.4g MULT=%.4g NP=%.0f Obj=%.0f",
                 np_best["Length"], np_best["ATRMult"], np_best["ReentryBars"],
...
def main():
    ap = argparse.ArgumentParser(description="SFJ_VolatilityBreakout_crypto ETHUSDT HOT Daily R1 search")
