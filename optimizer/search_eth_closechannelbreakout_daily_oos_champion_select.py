"""
search_eth_closechannelbreakout_daily_oos_champion_select.py -- OOS selection among the converged
...
CLI:
  py search_eth_closechannelbreakout_daily_oos_champion_select.py                  # merged verdict
  py search_eth_closechannelbreakout_daily_oos_champion_select.py --candidate VB1  # smoke one
  py search_eth_closechannelbreakout_daily_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_closechannelbreakout_daily_oos_champion_select_search")
PREFIX      = "ETHCCBDSEL_"
...
CANDIDATES = [
    ("CC1", "R3 Obj-max",          9.0,  2.5, 10.0, 1894.1, -461.5),  # R3 zoom 29tr
    ("CC2", "R2 short-len",        3.0,  3.0, 5.0,  1739.0, -454.5),  # R2 35tr
    ("CC3", "R1 mid-len",         24.0,  3.0, 12.0, 1586.0, -455.7),  # R1 21tr
    ("CC4", "high-freq NP-max",    2.0,  3.0, 6.0,  1914.2, -646.7),  # R2 A05 np-max 37tr
]



OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_closechannelbreakout_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_closechannelbreakout_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_closechannelbreakout_daily_oos_champion_select.json"
