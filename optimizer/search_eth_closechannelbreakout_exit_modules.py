"""
search_eth_closechannelbreakout_exit_modules.py -- Exit-module independent optimization on top of the
...
CLI:
  py search_eth_closechannelbreakout_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_eth_closechannelbreakout_exit_modules.py --module 3       # only module 3
  py search_eth_closechannelbreakout_exit_modules.py --module 0       # baseline A00 only
  py search_eth_closechannelbreakout_exit_modules.py --smoke --module 1
  py search_eth_closechannelbreakout_exit_modules.py --manual-status  # user toggles Status by hand
  py search_eth_closechannelbreakout_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_eth_closechannelbreakout_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_closechannelbreakout_exit_modules_search")
...
# Reference baseline = ETH CC4 main-only IS (narrow-trail OOS de-facto winner)
BASELINE_REF = {"net_profit": 2676.52, "max_drawdown": -610.231, "total_trades": 233}
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_closechannelbreakout_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs Length=6 ATRMult=7.5 ReentryBars=12)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_eth_closechannelbreakout_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_eth_closechannelbreakout_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("  Baseline ref (CC4 IS): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
