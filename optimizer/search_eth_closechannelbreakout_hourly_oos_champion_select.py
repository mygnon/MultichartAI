"""
search_eth_closechannelbreakout_hourly_oos_champion_select.py -- OOS selection among the converged
BNB Hourly SFJ_CloseChannelBreakout_crypto champions (R1-R3, results/bnb_volatilitybreakout_hourly{,2,3}_search/).
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_CloseChannelBreakout_crypto_AI.wsp open, BNBUSDT HOT HOURLY chart tab ACTIVE,
    SFJ_CloseChannelBreakout_crypto signal ON. Full data to 2026/06/10. Binance source connected.
    CLOSE the Study Editor. (Uses the mc_automation Settings-tab retry fix.)

CLI:
  py search_eth_closechannelbreakout_hourly_oos_champion_select.py                  # merged verdict
  py search_eth_closechannelbreakout_hourly_oos_champion_select.py --candidate VB1  # smoke one
  py search_eth_closechannelbreakout_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_closechannelbreakout_hourly_oos_champion_select_search")
PREFIX      = "ETHCCBSEL_"
...
STEP = {"Length": 1.0, "ATRMult": 0.5, "ReentryBars": 1.0}
LO   = {"Length": 2.0, "ATRMult": 0.5, "ReentryBars": 0.0}
HI   = {"Length": 300.0, "ATRMult": 20.0, "ReentryBars": 50.0}
...
CANDIDATES = [
    ("CC1", "Obj-max wide-trail ***", 2.0,  12.0, 6.0,  4137.7, -475.9),  # R2 A10 Obj-max, 138tr
    ("CC2", "stable wide-trail",      7.0,  15.0, 7.0,  4133.5, -495.8),  # R2 6-conv, 78tr
    ("CC3", "NP-max wide-trail",      4.0,  15.5, 6.0,  4469.3, -591.0),  # R2 A07 NP-max, 72tr
    ("CC4", "narrow-trail high-freq", 6.0,  7.5,  12.0, 2676.5, -610.2),  # R2 A05 narrow (BNB/BTC style), 233tr
]


OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_closechannelbreakout_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, atrlen, entry, mult) -> StrategyConfig:
    return StrategyConfig(
        ...
        timeframe="hourly",
        bar_period=60,
        params=[_axis("Length", atrlen), _axis("ATRMult", entry), _axis("ReentryBars", mult)],
...
def _pick(df, atrlen, entry, mult):
    m = df
    for nm, v in (("Length", atrlen), ("ATRMult", entry), ("ReentryBars", mult)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_closechannelbreakout_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_closechannelbreakout_hourly_oos_champion_select.json"
...
def main():
    ap = argparse.ArgumentParser(description="BNB Hourly SFJ_CloseChannelBreakout_crypto OOS champion selection")
