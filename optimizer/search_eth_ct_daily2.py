"""
search_eth_ct_daily2.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on ETHUSDT HOT Daily, Round 2

R1 (search_eth_ct_daily.py) champion: LL=13 SL=1.525 LS=36 SS=1.375 NP=$3,318 MDD=-$860 41tr
  (HIGH-FREQ regime; target $10K not met -66.8%).  R1 was NOT converged — the adaptive zoom was
  still rising (A09 $3,163 -> A10 $3,203 -> A11 $3,318).  ETH-hourly-analog / high-stddev / asym
  regions returned 0 valid rows (BB bands too wide -> ~no trades; genuinely barren on ETH daily).

IN-SAMPLE WINDOW = 2022/01/01 - 2026/01/01 (chart trimmed via set_instrument_data_range).
Objective = NetProfit^2 / |MaxDrawdown| ; target NP > 10,000 USD.

R2 Plan (11 attempts, <=5000 combos each) — confirm/refine the high-freq peak + map the basin:
  A01 LS_sweep        LL(11-15 s1)   SL(1.4-1.7 s0.05) LS(20-60 s2)  SS(1.2-1.6 s0.1)   = 3675
  A02 SS_sweep        LL(11-15 s1)   SL(1.4-1.7 s0.05) LS(34-38 s1)  SS(0.8-2.2 s0.1)   = 2625
  A03 SL_sweep        LL(11-15 s1)   SL(0.5-2.5 s0.1)  LS(34-38 s1)  SS(1.3-1.5 s0.05)  = 2625
  A04 LL_sweep        LL(4-30 s1)    SL(1.4-1.7 s0.1)  LS(34-38 s1)  SS(1.3-1.45 s0.05) = 2160
  A05 mid_freq_alt    LL(40-60 s5)   SL(1.5-3.0 s0.25) LS(90-140 s10) SS(0.8-1.6 s0.2)  = 1050
  A06 ultra_short_LL  LL(2-12 s1)    SL(1.0-2.0 s0.2)  LS(30-46 s2)  SS(1.2-1.6 s0.1)   = 2970
  A07 longer_LS_check LL(11-15 s1)   SL(1.4-1.7 s0.1)  LS(45-90 s5)  SS(1.2-1.6 s0.1)   = 1000
  A08 tight_SS_lowSL  LL(8-20 s3)    SL(0.3-1.2 s0.1)  LS(30-50 s4)  SS(0.3-1.2 s0.1)   = 3000
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_ct_daily2_search")
...
# Seed = R1 champion (re-measured fresh; SEED_NP=0 so the first valid result re-baselines, Rule 5)
SEED_LL, SEED_SL = 13.0, 1.525
SEED_LS, SEED_SS = 36.0, 1.375
SEED_NP          = 0.0

PREFIX = "ETHCTD2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_ct_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_champion": "LL=13 SL=1.525 LS=36 SS=1.375 NP=3318 MDD=-860 41tr (HIGH-FREQ; not converged)",
            "r2_plan":  "Confirm high-freq peak: LS/SS/SL/LL sweeps + ultra-short-LL + longer-LS check + mid-freq alt + tight-SS-lowSL + adaptive zoom. (Long-LL/high-SS/asym confirmed barren in R1.)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_eth_ct_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  ETH Daily countertrend_LS_crypto NP>10K -- Round 2")
    log.info("  IS window %s ~ %s  (chart-trimmed)", IS_RANGE[0], IS_RANGE[1])
    log.info("  R1 champion LL=13 SL=1.525 LS=36 SS=1.375 NP=3318 (HIGH-FREQ, confirming)")
    log.info("==============================================================")

    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (11, 15, 1), (1.4, 1.7, 0.05),
                                         (34, 38, 1), (1.3, 1.5, 0.05)))
    ...
    attempts_config = [
        ("01_LS_sweep",        (11, 15, 1),    (1.4, 1.7, 0.05),  (20, 60, 2),    (1.2, 1.6, 0.1)),
        ("02_SS_sweep",        (11, 15, 1),    (1.4, 1.7, 0.05),  (34, 38, 1),    (0.8, 2.2, 0.1)),
        ("03_SL_sweep",        (11, 15, 1),    (0.5, 2.5, 0.1),   (34, 38, 1),    (1.3, 1.5, 0.05)),
        ("04_LL_sweep",        (4, 30, 1),     (1.4, 1.7, 0.1),   (34, 38, 1),    (1.3, 1.45, 0.05)),
        ("05_mid_freq_alt",    (40, 60, 5),    (1.5, 3.0, 0.25),  (90, 140, 10),  (0.8, 1.6, 0.2)),
        ("06_ultra_short_LL",  (2, 12, 1),     (1.0, 2.0, 0.2),   (30, 46, 2),    (1.2, 1.6, 0.1)),
        ("07_longer_LS_check", (11, 15, 1),    (1.4, 1.7, 0.1),   (45, 90, 5),    (1.2, 1.6, 0.1)),
        ("08_tight_SS_lowSL",  (8, 20, 3),     (0.3, 1.2, 0.1),   (30, 50, 4),    (0.3, 1.2, 0.1)),
    ]
    ...
    if start_attempt <= A:
        for r_ll, r_sl, r_ls, r_ss in [(6, 0.3, 8, 0.3), (4, 0.2, 5, 0.2), (3, 0.15, 4, 0.15)]:
    ...
    log.info("A10  adaptive_zoom2 -- center: LL=%.4g SL=%.4g LS=%.4g SS=%.4g  NP=%.0f",
             best_ll, best_sl, best_ls, best_ss, best_np)
    if start_attempt <= A:
        for r_ll, r_sl, r_ls, r_ss in [(3, 0.15, 4, 0.15), (2, 0.1, 3, 0.1), (2, 0.05, 2, 0.05)]:
            _ll = zoom(best_ll, r_ll, 1.0,   LL_LO, LL_HI)
    ...
    log.info("==============================================================")
    log.info("  ETH Daily countertrend_LS_crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH Daily countertrend_LS_crypto NP>10K Round-2 search")
