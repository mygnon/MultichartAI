"""
search_eth_ct_daily_oos_champion_select.py — OOS selection among the converged ETH *Daily* CT
champions (rounds 1-2, results/eth_ct_daily{,2}_search/).

Workspace 20260101_SFJ_Bollinger_AI.wsp has full data 2021/03/01 – 2026/06/10, extending past
the IS cutoff 2026/01/01 — so OOS is measurable.

TASK: re-test each converged ETH Daily champion and pick the parameter set that earns the MOST
out-of-sample profit WITHOUT its drawdown breaking the established Max Drawdown.

METHOD — MERGED single-run, AUTO data-range (the proven route):
  set Format Instruments -> Settings -> Data Range itself, per pass:
  ...
  Each candidate = an 81-combo micro-grid centred on its exact params (all 4 axes vary +/-1 step,
  Critical Rule 1); the exact candidate row is picked.  Metric = NetProfit + Max INTRADAY Drawdown.
  Winner = max OOS_NP among PASS candidates; also report overall max-OOS_NP and lowest MDD_full.

PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_Bollinger_AI.wsp open, ETHUSDT HOT **DAILY** chart tab ACTIVE and VISIBLE
  - SFJ_15Dworkshop_lesson5_countertrend_LS_crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_eth_ct_daily_oos_champion_select.py                # merged: IS then FULL, then verdict
  py search_eth_ct_daily_oos_champion_select.py --period is    # one pass only
  py search_eth_ct_daily_oos_champion_select.py --candidate ED1
  py search_eth_ct_daily_oos_champion_select.py --from-csv
  py search_eth_ct_daily_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_ct_daily_oos_champion_select_search")
PREFIX      = "ETHCTDSEL_"
...
STEP = {"LENGTH_LONG": 1.0, "STDDEV_LONG": 0.05, "LENGTH_SHORT": 1.0, "STDDEV_SHORT": 0.05}
...
# 4 DISTINCT converged ETH Daily regimes (from eth_ct_daily{,2}_search; R2 A10=A11 + R2 alternates).
# id, regime, LL, SL, LS, SS, prior IS NP, prior IS MDD (= Max Intraday DD; fresh IS re-measure expected)
CANDIDATES = [
    ("ED1", "NP/Obj-max hi-freq ***", 5.0,  1.75, 37.0,  1.375, 3755.0, -863.0),  # 47tr R2 champion
    ("ED2", "low-MDD tight-SS",       11.0, 1.00, 46.0,  1.20,  2980.0, -689.0),  # 51tr, lowest MDD
    ("ED3", "longer-LS low-MDD",      12.0, 1.70, 45.0,  1.20,  2833.0, -703.0),  # 35tr
    ("ED4", "low-freq BTC-style",     45.0, 2.25, 100.0, 1.00,  2447.0, -1002.0), # 18tr, distinct regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_ct_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_ct_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_ct_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("ED1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Daily CT — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
    ...
    log.info("  %-4s %-22s %9s %9s %9s %9s %4s", "id", "regime", "NP_is", "NP_full", "OOS_NP", "MDD_full", "pass")
    for c in sorted(valid, key=lambda c: c["oos_np"], reverse=True):
        log.info("  %-4s %-22s %9.0f %9.0f %9.0f %9.0f %4s", c["id"], c["regime"],
...
def main():
    ap = argparse.ArgumentParser(description="ETH Daily CT OOS champion selection (auto-range merged)")
