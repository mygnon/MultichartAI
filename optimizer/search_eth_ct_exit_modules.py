"""
search_eth_ct_exit_modules.py — Exit-module independent optimization on top of the
SFJ_15Dworkshop_lesson5_countertrend_LS_crypto ETH Hourly champion.

Main signal FIXED at Obj-max champion (LL=110 SL=4.3 LS=110 SS=4.7 — the measured
plateau point, R2 A07 = R3 A02), Status ON.  Six exit modules tested INDEPENDENTLY:
one module enabled at a time, exhaustive optimization over its input range,
max-NetProfit row selected, compared against the same-day baseline (A00) and the
reference baseline NP=$4,847.50 / MDD=-$579.56 / 20tr.

Prior art: the SAME experiment on BNB and BTC (2026-06-13) found ALL 6 modules
hurt (BNB -75..-99.95%; BTC -78..-94%, strictly worse on both NP and MDD).
ETH shares the rare-extreme regime (20tr/7yr) — same outcome expected.
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_Bollinger_AI.wsp open, ETHUSDT HOT HOURLY tab active
  - Main signal inputs set to 110 / 4.3 / 110 / 4.7, Status ON
  - All 6 module signals inserted on the chart, Status OFF
  - Workspace saved

CLI:
  py search_eth_ct_exit_modules.py                  # A00 baseline + M1-M6 + teardown
  py search_eth_ct_exit_modules.py --module 3       # only module 3
  py search_eth_ct_exit_modules.py --module 0       # baseline A00 only
  py search_eth_ct_exit_modules.py --smoke --module 1   # tiny end-to-end smoke test
  py search_eth_ct_exit_modules.py --verify-status  # re-enable reopen-verify (slower)
  py search_eth_ct_exit_modules.py --manual-status  # user toggles Status by hand
  py search_eth_ct_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_ct_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2026/01/01")

# Obj-max champion — measured plateau point (R2 A07 = R3 A02 identical trade set)
CHAMPION = {
    "LENGTH_LONG":  110.0,
    "STDDEV_LONG":  4.3,
    "LENGTH_SHORT": 110.0,
...
# Reference baseline (R2 A07 / R3 A02, 2026-06-11/12)
BASELINE_REF = {"net_profit": 4847.50, "max_drawdown": -579.56, "total_trades": 20}
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_ct_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    ...
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("LENGTH_LONG",  109.0, 111.0, 1.0),
            ParamAxis("STDDEV_LONG",  4.275, 4.325, 0.025),
            ParamAxis("LENGTH_SHORT", 109.0, 111.0, 1.0),
            ParamAxis("STDDEV_SHORT", 4.675, 4.725, 0.025),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool, verify: bool) -> float:
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs 110/4.3/110/4.7)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_eth_ct_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_eth_ct_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly CT exit-module optimization%s [SPEED MODE]",
             " [SMOKE]" if smoke else "")
    log.info("  Main fixed: LL=110 SL=4.3 LS=110 SS=4.7 (Status ON)")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH CT exit-module independent optimization (speed mode)")
