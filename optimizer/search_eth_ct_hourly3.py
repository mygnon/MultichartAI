"""
search_eth_ct_hourly3.py — SFJ_15Dworkshop_lesson5_countertrend_LS_crypto on ETHUSDT HOT Hourly, Round 3

R1+R2 Summary (22 attempts):
  NP-max ⭐ R2 A09=A10=A11 conv: LL=111 SL=4.025 LS=115 SS=4.725
    NP=$5,004.84 MDD=-$748.14 25tr Obj=33,481
  Obj-max ⭐⭐ R2 A07 (SL 4.2-5.2 sweep): LL=110 SL=4.3 LS=110 SS=4.7
    NP=$4,847.50 MDD=-$579.56 20tr Obj=40,545 (MDD/NP 12.0%)
  LS bridge moved champion AGAIN (99 -> 113 -> 115) — bridge rule 3rd payoff.
  R2 A01 microfine = R1 champion exactly ($4,868.49) — no drift.
  R1->R2 gain +2.80% — NOT converged.
  Worse: LS 74-96 ($4,507), SS 4.8-5.5 ($4,806), SS 3.8-4.5 ($4,772),
         SL<3.9 ($4,245 MDD -1154), LL 116-160 ($3,998).

R3 Plan (11 attempts, <=5000 combos each):
  Converge NP-max + Obj-max + bridge the two regimes + recheck long-LS with fine SS.

  A01 NPmax_microfine    LL(108-114 s1)xSL(3.975-4.075 s0.025)xLS(111-119 s1)xSS(4.675-4.775 s0.025) = 1575
  A02 Objmax_fine        LL(105-115 s2)xSL(4.2-4.45 s0.025)xLS(105-115 s2)xSS(4.6-4.8 s0.05)         = 1980
  A03 SL_4p075_4p2_bridge LL(106-116 s2)xSL(4.075-4.2 s0.025)xLS(108-120 s2)xSS(4.65-4.8 s0.05)      = 1008
  A04 LS_116_135         LL(106-116 s2)xSL(3.95-4.15 s0.05)xLS(116-135 s1)xSS(4.65-4.8 s0.05)        = 2400
  A05 LS_135_165_SSfine  LL(105-115 s5)xSL(3.9-4.2 s0.05)xLS(135-165 s2)xSS(4.6-4.9 s0.05)           = 2352
  A06 SL_4p5_5p5_redo    LL(105-115 s2)xSL(4.5-5.5 s0.1)xLS(105-120 s5)xSS(4.6-4.8 s0.1)             = 792
  A07 SS_4p75_5p0        LL(108-114 s2)xSL(3.95-4.1 s0.025)xLS(110-120 s2)xSS(4.75-5.0 s0.025)       = 1848
  A08 global_verify      LL(95-130 s5)xSL(3.9-4.4 s0.1)xLS(95-135 s5)xSS(4.5-4.9 s0.1)               = 2160
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_ct_hourly3_search")
...
# R2 NP-max as seed (A09=A10=A11 convergence)
SEED_LL, SEED_SL = 111.0, 4.025
SEED_LS, SEED_SS = 115.0, 4.725
SEED_NP          = 5004.84

PREFIX = "ETHCT3_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_ct_hourly3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r2_result":"R2 NP-max A09=A10=A11: LL=111 SL=4.025 LS=115 SS=4.725 NP=$5,005 Obj=33,481; Obj-max A07: LL=110 SL=4.3 LS=110 SS=4.7 NP=$4,848 MDD=-$580 Obj=40,545; R1->R2 +2.8%",
            "r3_plan":  "Converge NP-max + Obj-max + SL bridge between regimes + LS 116-165 fine + SL 4.5-5.5 redo + SS 4.75-5.0 + global verify + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_eth_ct_hourly3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly countertrend_LS_crypto NP>10K -- Round 3")
    log.info("  R2 NP-max: LL=111 SL=4.025 LS=115 SS=4.725 NP=$5,005")
    log.info("  R2 Obj-max: LL=110 SL=4.3 LS=110 SS=4.7 Obj=40,545 MDD=-$580")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_NPmax_microfine",     (108, 114, 1),  (3.975, 4.075, 0.025), (111, 119, 1),  (4.675, 4.775, 0.025)),
        ("02_Objmax_fine",         (105, 115, 2),  (4.2, 4.45, 0.025),    (105, 115, 2),  (4.6, 4.8, 0.05)),
        ("03_SL_4p075_4p2_bridge", (106, 116, 2),  (4.075, 4.2, 0.025),   (108, 120, 2),  (4.65, 4.8, 0.05)),
        ("04_LS_116_135",          (106, 116, 2),  (3.95, 4.15, 0.05),    (116, 135, 1),  (4.65, 4.8, 0.05)),
        ("05_LS_135_165_SSfine",   (105, 115, 5),  (3.9, 4.2, 0.05),      (135, 165, 2),  (4.6, 4.9, 0.05)),
        ("06_SL_4p5_5p5_redo",     (105, 115, 2),  (4.5, 5.5, 0.1),       (105, 120, 5),  (4.6, 4.8, 0.1)),
        ("07_SS_4p75_5p0",         (108, 114, 2),  (3.95, 4.1, 0.025),    (110, 120, 2),  (4.75, 5.0, 0.025)),
        ("08_global_verify",       (95, 130, 5),   (3.9, 4.4, 0.1),       (95, 135, 5),   (4.5, 4.9, 0.1)),
    ]
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly countertrend_LS_crypto Round-3 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH Hourly countertrend_LS_crypto NP>10K Round-3 search")
