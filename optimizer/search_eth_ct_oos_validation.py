"""
search_eth_ct_oos_validation.py — Full-period / out-of-sample (OOS) validation of the
already-optimized exit modules on top of the ETH Hourly CT champion.
...
Main signal FIXED at Obj-max champion LL=110 SL=4.3 LS=110 SS=4.7 (Status ON).
Module params FIXED at their IS max-NP values (from eth_ct_exit_modules_search):
  M1 ATRstop STP=7.2 | M2 TrailingStop ATRSTP=7.8 | M3 EntryBarsAfterExit EXITBAR=76
  M4 high_volatility DAYRANGE=4.68 | M5 PT_Exit PT_Base=0.311 | M6 RescueTeamExit Length=320 std=3.2
...
PREREQUISITES (one-time manual setup in MC64):
  ...
  - ETHUSDT HOT HOURLY tab active
  - Main signal inputs set to 110 / 4.3 / 110 / 4.7, Status ON
  - All 6 module signals inserted on the chart, Status OFF
  - Workspace saved

CLI:
  py search_eth_ct_oos_validation.py                 # IS + FULL, baseline + M1-M6 + teardown
  py search_eth_ct_oos_validation.py --module 0      # baseline only (both periods)
  py search_eth_ct_oos_validation.py --period full    # only the full period
  py search_eth_ct_oos_validation.py --manual-status  # toggle Status by hand
  py search_eth_ct_oos_validation.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_ct_oos_validation_search")

# ETH Hourly Obj-max champion — set in MC64 manually, Status ON
CHAMPION = {
    "LENGTH_LONG":  110.0,
    "STDDEV_LONG":  4.3,
    "LENGTH_SHORT": 110.0,
...
MODULES: List[Tuple[int, str, List[Tuple[str, float, float]]]] = [
    (1, "SFJ_15Dworkshop_lesson4_ATRstop",                 [("STP",      7.2,   0.1)]),
    (2, "SFJ_15Dworkshop_lesson9_1_TrailingStop",          [("ATRSTP",   7.8,   0.1)]),
    (3, "SFJ_15Dworkshop_lesson10_3_EntryBarsAfterExit",   [("EXITBAR",  76.0,  1.0)]),
    (4, "SFJ_15Dworkshop_lesson11_3_high_volatility_exit", [("DAYRANGE", 4.68,  0.01)]),
    (5, "QuantPass_PT_Exit",                               [("PT_Base",  0.311, 0.001)]),
    (6, "RescueTeamExit",                                  [("Length",   320.0, 20.0),
                                                            ("std",      3.2,   0.1)]),
]
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_ct_oos_validation_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_ct_oos_validation.json"
...
def run(conn, from_csv, only_module, only_period, manual_status, smoke):
    ...
    out_json = OUTPUT_DIR / "final_params_eth_ct_oos_validation.json"
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly CT — OOS / FULL-PERIOD module validation%s [SPEED]",
             " [SMOKE]" if smoke else "")
    ...
    log.info("==============================================================")
    log.info("  ETH OOS VALIDATION SUMMARY  (total %.1f min)", (time.time() - run_t0) / 60)
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly CT OOS/full-period module validation")
