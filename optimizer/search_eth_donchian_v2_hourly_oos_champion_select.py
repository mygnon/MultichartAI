"""
search_eth_donchian_v2_hourly_oos_champion_select.py -- OOS selection among the converged
ETH Hourly SFJ_DonchianATR_crypto_v2 champions (R1-R3, results/eth_donchian_v2_hourly{,2}_search/).
...
trailing stop, long+short, _Crypto1MUSD. R2 ceiling = 2 regimes (DISTINCT from BNB/BTC):
  Obj-max  ultra-long tight-M Length=167 ATRLength=22 ATRMult=2 TrendLen=107 ReentryBars=0 (NP1593 MDD-275 402tr)
  NP-max   short-L wide-M     Length=20  ATRLength=24 ATRMult=6 TrendLen=40  ReentryBars=0 (NP1844 MDD-995 321tr)
...
PREREQUISITES (MC64, elevated):
  - 20260622_SFJ_DonchianATR_AI.wsp open, ETHUSDT HOT HOURLY chart tab ACTIVE,
    SFJ_DonchianATR_crypto_v2 signal ON. Full data to 2026/06/10. CLOSE the Study Editor.

CLI:
  py search_eth_donchian_v2_hourly_oos_champion_select.py                  # merged verdict
  py search_eth_donchian_v2_hourly_oos_champion_select.py --candidate ED1  # smoke one
  py search_eth_donchian_v2_hourly_oos_champion_select.py --from-csv       # recompute
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_donchian_v2_hourly_oos_champion_select_search")
PREFIX      = "ETHDONSEL_"
...
# Converged ETH Hourly v2 candidates (eth_donchian_v2_hourly{,2}_search).
# id, regime, Length, ATRLength, ATRMult, TrendLen, ReentryBars, prior IS NP, prior IS MDD.
CANDIDATES = [
    ("ED1", "R2 Obj-max ultra-long M2 ***", 167.0, 22.0, 2.0, 107.0, 0.0, 1593.0, -275.0),  # 402tr lowest MDD
    ("ED2", "R1 Obj-max ultra-long T145",   160.0, 22.0, 2.0, 145.0, 2.0, 1513.0, -277.0),  # 412tr more filter
    ("ED3", "NP-max short-L wide-M",         20.0, 24.0, 6.0,  40.0, 0.0, 1844.0, -995.0),  # 321tr highest NP
    ("ED4", "vlong tight-M",                180.0, 22.0, 2.0, 200.0, 0.0, 1449.0, -326.0),  # 386tr longest-L
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_donchian_v2_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_donchian_v2_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_donchian_v2_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("ED1", "is", *CANDIDATES[0][2:7]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Hourly DonchianATR v2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly SFJ_DonchianATR_crypto_v2 OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="EDn", default=None)
