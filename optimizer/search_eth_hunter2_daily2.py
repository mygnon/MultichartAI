"""
search_eth_hunter2_daily2.py — SFJ_HUNTER2_crypto on ETHUSDT HOT Daily, Round 2 (IS 2022/01-2026/01)

R1 found ETH Daily's regime = MID-SHORT LEN_L=34 (NOT the ultra-short LEN_L=2-4 floor of BTC/BNB Daily)
and A09=A10=A11 converged: A01 Obj 7,054 -> A06 9,554 -> A09/A10/A11 ~10,485.

  R1 Obj-max = NP-max ⭐ (A11): LEN_L=34 LEN_S=116 ATR_L=0.3 ATR_S=1.0  NP=$3,052 MDD=-$888 Obj=10,488 31tr
  (ETH Daily $3,052 < ETH Hourly $3,699 -17.5% — Daily<Hourly unlike BNB; ~BTC Daily; $100K unreachable -96.9%)

R2 PURPOSE: (1) confirm LEN_L=34 (test LEN_L 10-70, interior not floor); (2) fine-sweep ATR_L~0.3 (low,
near 0.1 floor), LEN_S~116, ATR_S~1.0; (3) RE-TEST high-ATR + short-LEN_S sub-regime; robust run_or_load
(cleanup+retry+re-focus); zoom step-5 LEN GUARANTEED <=5000 (R1 A11 hit 6930). Zoom seed = Obj-max.

11 attempts, each <=5000 combos. Objective = NetProfit² / |MaxDrawdown|.

Attempt schedule (daily, ETH MID-SHORT LEN_L):
  A01 retest_center : LEN_L(30-38 s2)×LEN_S(112-120 s2)×ATR_L(0.2-0.4 s0.05)×ATR_S(0.8-1.2 s0.1) = 5×5×5×5=625  (Rule 5 drift)
  A02 atrl_fine     : LEN_L(28-40 s2)×LEN_S(108-124 s4)×ATR_L(0.1-0.6 s0.05)×ATR_S(0.8-1.2 s0.1) = 7×5×11×5=1925
  A03 lenL_sweep    : LEN_L(10-70 s5)×LEN_S(112-120 s4)×ATR_L(0.25-0.35 s0.05)×ATR_S(0.8-1.2 s0.2) = 13×3×3×3=351
  A04 lenS_sweep    : LEN_L(30-38 s2)×LEN_S(60-200 s10)×ATR_L(0.25-0.35 s0.05)×ATR_S(0.8-1.2 s0.2) = 5×15×3×3=675
  A05 atrs_fine     : LEN_L(30-38 s2)×LEN_S(108-124 s4)×ATR_L(0.25-0.35 s0.05)×ATR_S(0.3-1.5 s0.1) = 5×5×3×13=975
  A06 short_lenS    : LEN_L(10-30 s5)×LEN_S(20-60 s5)×ATR_L(1-2.5 s0.25)×ATR_S(1.5-3 s0.5) = 5×9×7×4=1260
  A07 high_atr_retest: LEN_L(10-130 s30)×LEN_S(10-130 s30)×ATR_L(3-12 s1)×ATR_S(3-12 s1) = 5×5×10×10=2500
  A08 mid_lenS      : LEN_L(30-38 s2)×LEN_S(80-200 s10)×ATR_L(0.2-0.5 s0.05)×ATR_S(0.8-1.2 s0.2) = 5×13×7×3=1365
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_daily2_search")
...
# R2 seed = R1 Daily Obj-max champion (A11, MID-SHORT LEN_L)
SEED_LL   = 34.0
SEED_LS   = 116.0
SEED_ATRL = 0.3
SEED_ATRS = 1.0
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r1_obj_max": {"LEN_L": 34, "LEN_S": 116, "ATR_multiplier_L": 0.3, "ATR_multiplier_S": 1.0,
                       "net_profit": 3051.58, "max_drawdown": -887.87, "objective": 10488, "total_trades": 31},
        "notes": {
    ...
    out = OUTPUT_DIR / "final_params_eth_hunter2_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    obj_best: Dict = {"LEN_L": SEED_LL, "LEN_S": SEED_LS, "ATR_multiplier_L": SEED_ATRL,
                      ...
                      "max_drawdown": -888.0, "objective": SEED_OBJ, "total_trades": 31}
    np_best: Dict = {}
    target_met = False
    attempt_log: List[dict] = []

    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto ETHUSDT HOT Daily — Round 2 (confirm MID-SHORT LEN_L + fine-sweep)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (30, 38, 2), (112, 120, 2),
                                                 (0.2, 0.4, 0.05), (0.8, 1.2, 0.1)))
    ...
    # static attempts
    _do(1, "01_retest_center",  _cfg("01_retest_center",  (30,38,2),  (112,120,2),  (0.2,0.4,0.05),  (0.8,1.2,0.1)))
    _do(2, "02_atrl_fine",      _cfg("02_atrl_fine",      (28,40,2),  (108,124,4),  (0.1,0.6,0.05),  (0.8,1.2,0.1)))
    _do(3, "03_lenL_sweep",     _cfg("03_lenL_sweep",     (10,70,5),  (112,120,4),  (0.25,0.35,0.05),(0.8,1.2,0.2)))
    _do(4, "04_lenS_sweep",     _cfg("04_lenS_sweep",     (30,38,2),  (60,200,10),  (0.25,0.35,0.05),(0.8,1.2,0.2)))
    _do(5, "05_atrs_fine",      _cfg("05_atrs_fine",      (30,38,2),  (108,124,4),  (0.25,0.35,0.05),(0.3,1.5,0.1)))
    _do(6, "06_short_lenS",     _cfg("06_short_lenS",     (10,30,5),  (20,60,5),    (1,2.5,0.25),    (1.5,3,0.5)))
    _do(7, "07_high_atr_retest",_cfg("07_high_atr_retest",(10,130,30),(10,130,30),  (3,12,1),        (3,12,1)))
    _do(8, "08_mid_lenS",       _cfg("08_mid_lenS",       (30,38,2),  (80,200,10),  (0.2,0.5,0.05),  (0.8,1.2,0.2)))
    ...
    # summary
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  ETH HUNTER2 Daily Round-2 COMPLETE  (%.1f min total)", (time.time()-_RUN_T0)/60)
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto ETHUSDT HOT Daily Round-2 (confirm mid-short LEN_L + fine-sweep)")
