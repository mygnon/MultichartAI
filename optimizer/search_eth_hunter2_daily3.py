"""
search_eth_hunter2_daily3.py — SFJ_HUNTER2_crypto on ETHUSDT HOT Daily, Round 3 (IS 2022/01-2026/01)

R2 OVERTURNED R1: the R1 LEN_L=34 was a FALSE local peak. R2's lenL_sweep + zoom found the true
ULTRA-SHORT LEN_L=2 regime (same family as BTC/BNB Daily): Obj 10,488 -> 14,578 (+39%).

  R2 Obj-max = NP-max ⭐: LEN_L=2 LEN_S=130 ATR_L=0.1 ATR_S=0.8  NP=$3,688 MDD=-$933 Obj=14,578 82tr
  (A09=A10=A11 converged; LEN_L=2 AND ATR_L=0.1 BOTH at floor; ETH Daily ~ ETH Hourly $3,699; $100K -96.3%)

R3 PURPOSE: confirm the ULTRA-SHORT regime (R2 found it late at A09, never fine-swept its LEN_S/ATR_S):
  (1) confirm LEN_L=2 + ATR_L=0.1 floors (test LEN_L 2-12, ATR_L 0.1-0.5); (2) fine-sweep LEN_S~130,
  ATR_S~0.8 at this regime; (3) re-check R1 mid-LEN + high-ATR are worse; (4) Rule-5 drift retest.
  robust run_or_load; zoom step-5 LEN <=5000. Zoom seed = Obj-max. Ceiling if R3 gain <=~0.1%.

11 attempts, each <=5000 combos. Objective = NetProfit² / |MaxDrawdown|.

Attempt schedule (daily, ULTRA-SHORT LEN_L floor):
  A01 retest_center : LEN_L(2-6 s1)×LEN_S(126-134 s2)×ATR_L(0.1-0.3 s0.05)×ATR_S(0.6-1.0 s0.1) = 5×5×5×5=625  (Rule 5 drift)
  A02 lenS_fine     : LEN_L(2-6 s1)×LEN_S(110-150 s2)×ATR_L(0.1-0.2 s0.05)×ATR_S(0.6-1.0 s0.1) = 5×21×3×5=1575
  A03 atrs_fine     : LEN_L(2-6 s1)×LEN_S(120-140 s4)×ATR_L(0.1-0.2 s0.05)×ATR_S(0.3-1.5 s0.1) = 5×6×3×13=1170
  A04 lenL_confirm  : LEN_L(2-12 s1)×LEN_S(120-140 s5)×ATR_L(0.1-0.2 s0.05)×ATR_S(0.7-0.9 s0.1) = 11×5×3×3=495
  A05 atrl_confirm  : LEN_L(2-6 s1)×LEN_S(124-136 s4)×ATR_L(0.1-0.5 s0.05)×ATR_S(0.7-0.9 s0.1) = 5×4×9×3=540
  A06 lenS_wide     : LEN_L(2-6 s1)×LEN_S(60-260 s10)×ATR_L(0.1-0.2 s0.05)×ATR_S(0.7-0.9 s0.2) = 5×21×3×2=630
  A07 high_atr_retest: LEN_L(10-130 s30)×LEN_S(10-130 s30)×ATR_L(3-12 s1)×ATR_S(3-12 s1) = 5×5×10×10=2500
  A08 mid_lenL_recheck: LEN_L(10-50 s5)×LEN_S(110-150 s10)×ATR_L(0.2-0.5 s0.05)×ATR_S(0.6-1.0 s0.2) = 9×5×7×3=945
  A09 adaptive_zoom1: dynamic around Obj-max best (step-5 LEN, guaranteed <=5000)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_daily3_search")
...
# R3 seed = R2 Daily Obj-max champion (ULTRA-SHORT LEN_L=2, ATR_L=0.1 floor)
SEED_LL   = 2.0
SEED_LS   = 130.0
SEED_ATRL = 0.1
SEED_ATRS = 0.8
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_daily3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r2_obj_max": {"LEN_L": 2, "LEN_S": 130, "ATR_multiplier_L": 0.1, "ATR_multiplier_S": 0.8,
                       "net_profit": 3688.04, "max_drawdown": -933.04, "objective": 14578, "total_trades": 82},
        "notes": {
    ...
    out = OUTPUT_DIR / "final_params_eth_hunter2_daily3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    obj_best: Dict = {"LEN_L": SEED_LL, "LEN_S": SEED_LS, "ATR_multiplier_L": SEED_ATRL,
                      ...
                      "max_drawdown": -933.0, "objective": SEED_OBJ, "total_trades": 82}
    np_best: Dict = {}
    target_met = False
    attempt_log: List[dict] = []

    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto ETHUSDT HOT Daily — Round 3 (confirm ULTRA-SHORT LEN_L ceiling)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (2, 6, 1), (126, 134, 2),
                                                 (0.1, 0.3, 0.05), (0.6, 1.0, 0.1)))
    ...
    # static attempts
    _do(1, "01_retest_center",   _cfg("01_retest_center",   (2,6,1),    (126,134,2),  (0.1,0.3,0.05),  (0.6,1.0,0.1)))
    _do(2, "02_lenS_fine",       _cfg("02_lenS_fine",       (2,6,1),    (110,150,2),  (0.1,0.2,0.05),  (0.6,1.0,0.1)))
    _do(3, "03_atrs_fine",       _cfg("03_atrs_fine",       (2,6,1),    (120,140,4),  (0.1,0.2,0.05),  (0.3,1.5,0.1)))
    _do(4, "04_lenL_confirm",    _cfg("04_lenL_confirm",    (2,12,1),   (120,140,5),  (0.1,0.2,0.05),  (0.7,0.9,0.1)))
    _do(5, "05_atrl_confirm",    _cfg("05_atrl_confirm",    (2,6,1),    (124,136,4),  (0.1,0.5,0.05),  (0.7,0.9,0.1)))
    _do(6, "06_lenS_wide",       _cfg("06_lenS_wide",       (2,6,1),    (60,260,10),  (0.1,0.2,0.05),  (0.7,0.9,0.2)))
    _do(7, "07_high_atr_retest", _cfg("07_high_atr_retest", (10,130,30),(10,130,30),  (3,12,1),        (3,12,1)))
    _do(8, "08_mid_lenL_recheck",_cfg("08_mid_lenL_recheck",(10,50,5),  (110,150,10), (0.2,0.5,0.05),  (0.6,1.0,0.2)))
    ...
    # summary
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  ETH HUNTER2 Daily Round-3 COMPLETE  (%.1f min total)", (time.time()-_RUN_T0)/60)
    ...
    log.info("  R2 Obj-max=14,578  →  R3 Obj-max=%.0f  (%+.1f%%)  [ceiling if <=~0.1%%]",
             obj_best["objective"], (obj_best["objective"]/SEED_OBJ - 1) * 100)
    log.info("══════════════════════════════════════════════════════════════")

    out = save_json(obj_best, np_best, attempt_log, target_met)
    print(f"\nDone — results at: {out}")
    print(f"Obj-max champion Obj={obj_best['objective']:.0f}  (R2 14,578)")
    return 0
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto ETHUSDT HOT Daily Round-3 (confirm ultra-short LEN_L ceiling)")
