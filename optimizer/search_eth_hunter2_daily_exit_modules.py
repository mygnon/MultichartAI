"""
search_eth_hunter2_daily_exit_modules.py — Exit-module independent optimization on top of the
SFJ_HUNTER2_crypto ETH Daily champion.

Main signal FIXED at Obj-max champion (LEN_L=2 LEN_S=116 ATR_multiplier_L=0.1 ATR_multiplier_S=0.8), Status ON.
...
the same-day baseline (A00) and the reference baseline NP=$3,697 / MDD=-$933 (R3 Obj-max ETH Daily HUNTER2 champion).
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_HUNTER_AI.wsp open, ETHUSDT HOT DAILY tab active
  - Main signal SFJ_HUNTER2_crypto inputs set to LEN_L=2 LEN_S=116 ATR_multiplier_L=0.1 ATR_multiplier_S=0.8, Status ON
  - All 6 module signals inserted on the chart, Status OFF
  - Workspace saved

CLI:
  py search_eth_hunter2_daily_exit_modules.py                  # A00 baseline + M1-M6 + teardown
  py search_eth_hunter2_daily_exit_modules.py --module 3       # only module 3 (status set for it)
  py search_eth_hunter2_daily_exit_modules.py --module 0       # baseline A00 only
  py search_eth_hunter2_daily_exit_modules.py --smoke --module 1   # tiny end-to-end smoke test
  py search_eth_hunter2_daily_exit_modules.py --manual-status  # user toggles Status by hand
  py search_eth_hunter2_daily_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_daily_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # WIDE no-op; chart-trim is the real control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# ETH Daily HUNTER2 Obj-max champion (R3 ceiling) — main signal must be set to these in MC64 (Format Objects)
CHAMPION = {
    "LEN_L":            2.0,
    "LEN_S":            116.0,
    "ATR_multiplier_L": 0.1,
    "ATR_multiplier_S": 0.8,
}
# Reference baseline (R3 Obj-max ETH Daily HUNTER2 champion, IS 2022/01-2026/01)
BASELINE_REF = {"net_profit": 3696.68, "max_drawdown": -933.0, "total_trades": 90}
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_daily_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    ...
    return StrategyConfig(
        ...
        bar_period=1440,
        params=[
            ParamAxis("LEN_L",            2.0,   4.0,   1.0),
            ParamAxis("LEN_S",            115.0, 117.0, 1.0),
            ParamAxis("ATR_multiplier_L", 0.1,   0.2,   0.05),
            ParamAxis("ATR_multiplier_S", 0.7,   0.9,   0.1),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (LEN_L=2 LEN_S=116 ATR_L=0.1 ATR_S=0.8)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_eth_hunter2_daily_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_eth_hunter2_daily_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH Daily HUNTER2 exit-module optimization%s", " [SMOKE]" if smoke else "")
    log.info("  Main fixed: LEN_L=2 LEN_S=116 ATR_L=0.1 ATR_S=0.8 (ON); IS 2022/01-2026/01 (chart-trimmed)")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH Daily HUNTER2 exit-module independent optimization")
