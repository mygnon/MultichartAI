"""
search_eth_hunter2_daily_oos_champion_select.py — Out-of-sample (OOS) selection among the converged
ETH Daily SFJ_HUNTER2_crypto champions (rounds 1-3, results/eth_hunter2_daily{,2,3}_search/).

Workspace 20260101_SFJ_HUNTER_AI.wsp now has full data 2021/03/01 – 2026/06/10, which
extends past the IS cutoff 2026/01/01 used in all prior rounds — so OOS is measurable.

ETH HUNTER2 Daily converged to an ULTRA-SHORT LEN_L regime (R3 ceiling Obj-max=NP-max LEN_L=2 LEN_S=116
ATR_L=0.1 ATR_S=0.8; LEN_L=2 AND ATR_L=0.1 both at floor; R1 LEN_L=34 was a false peak). Candidates span
the ultra-short core (R3 champion + R2 LEN_S=130 variant) plus a high-freq low-LEN_S lower-MDD and the
R1 mid-LEN low-freq contrast so the OOS test reveals which holds its Max Drawdown out-of-sample.

TASK: re-test each previously-converged ETH Daily HUNTER2 champion on this workspace and pick the
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_HUNTER_AI.wsp open, ETHUSDT HOT **Daily** chart tab ACTIVE, the
    SFJ_HUNTER2_crypto signal applied (Status ON)
  - Full data available to 2026/06/10 (the script sets the range itself)

CLI:
  py search_eth_hunter2_daily_oos_champion_select.py                 # merged: IS then FULL, then verdict
  py search_eth_hunter2_daily_oos_champion_select.py --period is     # one pass only
  py search_eth_hunter2_daily_oos_champion_select.py --candidate EHD1  # smoke one candidate
  py search_eth_hunter2_daily_oos_champion_select.py --from-csv      # recompute from existing CSVs
  py search_eth_hunter2_daily_oos_champion_select.py --probe-instrument  # verify auto data-range
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_daily_oos_champion_select_search")
PREFIX      = "ETHH2DSEL_"
...
# ATR_S step = 0.05 (NOT 0.25): ETH champions have tiny ATR_S (0.25/0.3); a 0.25 step clamped at
# LO=0.1 would shift the grid off the candidate value. 0.05 keeps the exact value on-grid.
STEP = {"LEN_L": 1.0, "LEN_S": 1.0, "ATR_multiplier_L": 0.05, "ATR_multiplier_S": 0.05}
LO   = {"LEN_L": 2.0, "LEN_S": 2.0, "ATR_multiplier_L": 0.1, "ATR_multiplier_S": 0.05}
HI   = {"LEN_L": 5000.0, "LEN_S": 5000.0, "ATR_multiplier_L": 30.0, "ATR_multiplier_S": 30.0}

# 4 converged ETH Daily HUNTER2 candidates (eth_hunter2_daily{,2,3}_search). ETH Daily regime = ULTRA-SHORT LEN_L.
# id, regime, LEN_L, LEN_S, ATR_multiplier_L, ATR_multiplier_S, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("EHD1", "R3 Obj-max ultra-short ***", 2.0,  116.0, 0.1, 0.8, 3696.68, -933.0),  # 90tr, R3 ceiling Obj=14,646 highest NP
    ("EHD2", "R2 LEN_S=130 variant",       2.0,  130.0, 0.1, 0.8, 3688.04, -933.0),  # 82tr, R2 plateau point
    ("EHD3", "high-freq low-LEN_S",        2.0,   60.0, 0.1, 0.9, 2629.0,  -540.0),  # 118tr, R3 A06 lowest-MDD
    ("EHD4", "R1 mid-LEN low-freq",       34.0,  116.0, 0.3, 1.0, 3052.0,  -888.0),  # 31tr, R1 false-peak regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def run_or_load(cfg, conn, from_csv):
    ...
    for attempt in (1, 2):
        ...
        try:
            mc.ensure_chart_ready(conn, cfg)   # re-focus the CHART (not Study Editor) each attempt
        except Exception as e:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_hunter2_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_hunter2_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("EHD1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Daily HUNTER2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Daily HUNTER2 OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="EHn", default=None)
