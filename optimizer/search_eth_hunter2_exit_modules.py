"""
search_eth_hunter2_exit_modules.py — Exit-module independent optimization on top of the
SFJ_HUNTER2_crypto ETH Hourly champion.

Main signal FIXED at Obj-max champion (LEN_L=495 LEN_S=635 ATR_multiplier_L=2.8 ATR_multiplier_S=0.25), Status ON.
...
the same-day baseline (A00) and the reference baseline NP=$3,699 / MDD=-$437 (R3 Obj-max ETH HUNTER2 champion).
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_HUNTER_AI.wsp open, ETHUSDT HOT 60-Minute tab active
  - Main signal SFJ_HUNTER2_crypto inputs set to LEN_L=495 LEN_S=635 ATR_multiplier_L=2.8 ATR_multiplier_S=0.25, Status ON
  - All 6 module signals inserted on the chart, Status OFF
  - Workspace saved

CLI:
  py search_eth_hunter2_exit_modules.py                  # A00 baseline + M1-M6 + teardown
  py search_eth_hunter2_exit_modules.py --module 3       # only module 3 (status set for it)
  py search_eth_hunter2_exit_modules.py --module 0       # baseline A00 only
  py search_eth_hunter2_exit_modules.py --smoke --module 1   # tiny end-to-end smoke test
  py search_eth_hunter2_exit_modules.py --manual-status  # user toggles Status by hand
  py search_eth_hunter2_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # WIDE no-op; chart-trim is the real control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# ETH HUNTER2 Obj-max champion (R3 ceiling) — main signal must be set to these in MC64 (Format Objects)
CHAMPION = {
    "LEN_L":            495.0,
    "LEN_S":            635.0,
    "ATR_multiplier_L": 2.8,
    "ATR_multiplier_S": 0.25,
}
# Reference baseline (R3 Obj-max ETH HUNTER2 champion, IS 2022/01-2026/01)
BASELINE_REF = {"net_profit": 3699.0, "max_drawdown": -437.0, "total_trades": 182}
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_eth_hunter2_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_eth_hunter2_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly HUNTER2 exit-module optimization%s", " [SMOKE]" if smoke else "")
    log.info("  Main fixed: LEN_L=495 LEN_S=635 ATR_L=2.8 ATR_S=0.25 (ON); IS 2022/01-2026/01 (chart-trimmed)")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH HUNTER2 exit-module independent optimization")
