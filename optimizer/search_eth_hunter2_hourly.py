"""
search_eth_hunter2_hourly.py — SFJ_HUNTER2_crypto on ETHUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_hourly_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met, obj_best):
    ...
    out = OUTPUT_DIR / "final_params_eth_hunter2_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto on ETHUSDT HOT Hourly — Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto ETHUSDT HOT Hourly Round-1 COMPLETE  (%.1f min total)",
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto ETHUSDT HOT Hourly NP>100K USD Round-1 search")
