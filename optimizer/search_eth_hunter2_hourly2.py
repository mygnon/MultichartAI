"""
search_eth_hunter2_hourly2.py — SFJ_HUNTER2_crypto on ETHUSDT HOT Hourly, Round 2 (IS 2022/01-2026/01)

R1 found TWO regimes; the user's criterion (Obj = NP²/|MDD| largest) favors the ULTRA-LONG one,
which R1's adaptive zooms never refined (R1 zoom followed NP-max, Rule 6):

  R1 Obj-max ⭐ (A07 ultra_long): LEN_L=500 LEN_S=625 ATR_L=3 ATR_S=1  NP=$3,424 MDD=-$453 Obj=25,882 170tr
  R1 NP-max   (A11 zoom)        : LEN_L=24  LEN_S=5   ATR_L=3.8 ATR_S=5.5 NP=$3,861 MDD=-$615 Obj=24,227 204tr
  (BTC HUNTER2 Obj-max was the same family: LEN_L=225 LEN_S=825 ATR_L=4.25 ATR_S=1.75 Obj=23,082)

R2 PURPOSE: refine + confirm the ULTRA-LONG Obj-max regime.
  - ATR_S=1 sat on the A07 lower grid edge → push ATR_S below 1.
  - LEN_L / LEN_S unswept above 600/700 → push toward 1000.
  - Zoom seed = Obj-max row (NOT NP-max), since target NP>100K is structurally unreachable and
    the user's true objective is NP²/|MDD|.
  - One fine-res cross-check of the short-MA NP-max regime to confirm it doesn't out-Obj the long one.

11 attempts, each ≤5000 combos. Objective = NetProfit² / |MaxDrawdown|.

Attempt schedule:
  A01 retest_center : LEN_L(450-550 s25)×LEN_S(575-675 s25)×ATR_L(2.5-3.5 s0.25)×ATR_S(0.5-1.5 s0.25) = 5×5×5×5=625  (Rule 5 drift check)
  A02 ultra_wide    : LEN_L(300-1000 s100)×LEN_S(400-1000 s100)×ATR_L(1-5 s1)×ATR_S(0.5-4.5 s1) = 8×7×5×5=1400
  A03 atrs_low      : LEN_L(400-700 s50)×LEN_S(500-800 s50)×ATR_L(2-4 s0.5)×ATR_S(0.1-1.5 s0.1) = 7×7×5×15=3675
  A04 atrl_sweep    : LEN_L(400-700 s50)×LEN_S(500-800 s50)×ATR_L(1-6 s0.5)×ATR_S(0.5-1.5 s0.25) = 7×7×11×5=2695
  A05 lenL_long     : LEN_L(500-1000 s50)×LEN_S(500-1000 s50)×ATR_L(2-4 s0.5)×ATR_S(0.5-1.5 s0.5) = 11×11×5×3=1815
  A06 lenS_sweep    : LEN_L(450-550 s50)×LEN_S(300-1000 s50)×ATR_L(2.5-3.5 s0.5)×ATR_S(0.5-1.5 s0.5) = 3×15×3×3=405
  A07 fine_mid      : LEN_L(475-525 s5)×LEN_S(600-650 s5)×ATR_L(2.75-3.25 s0.25)×ATR_S(0.75-1.25 s0.25) = 11×11×3×3=1089
  A08 short_ma_xchk : LEN_L(15-35 s5)×LEN_S(2-12 s2)×ATR_L(3-4.5 s0.25)×ATR_S(4.5-6 s0.25) = 5×6×7×7=1470  (confirm NP-max regime Obj)
  A09 adaptive_zoom1: dynamic around Obj-max best (moderate)
  A10 adaptive_zoom2: dynamic (tighter)
  A11 adaptive_zoom3: dynamic (finest, step=5 LEN / 0.1 ATR)
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_hourly2_search")
...
# R2 seed = R1 ultra-long Obj-max champion
SEED_LL   = 500.0
SEED_LS   = 625.0
SEED_ATRL = 3.0
SEED_ATRS = 1.0
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r1_obj_max": {"LEN_L": 500, "LEN_S": 625, "ATR_multiplier_L": 3, "ATR_multiplier_S": 1,
                       "net_profit": 3424.48, "max_drawdown": -453.091, "objective": 25882, "total_trades": 170},
        "notes": {
            ...
            "purpose": "Refine the ultra-long Obj-max regime R1 left unzoomed; zoom seed = Obj-max",
        },
    ...
    out = OUTPUT_DIR / "final_params_eth_hunter2_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    obj_best: Dict = {"LEN_L": SEED_LL, "LEN_S": SEED_LS, "ATR_multiplier_L": SEED_ATRL,
                      ...
                      "max_drawdown": -453.0, "objective": SEED_OBJ, "total_trades": 170}
    np_best: Dict = {}
    target_met = False
    attempt_log: List[dict] = []

    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto ETHUSDT HOT Hourly — Round 2 (refine ULTRA-LONG Obj-max)")
    ...
    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (480, 520, 20), (600, 650, 25),
                                         (2.5, 3.5, 0.5), (0.5, 1.5, 0.5)))
    ...
    # static attempts
    _do(1, "01_retest_center", _cfg("01_retest_center", (450,550,25), (575,675,25), (2.5,3.5,0.25), (0.5,1.5,0.25)))
    _do(2, "02_ultra_wide",    _cfg("02_ultra_wide",    (300,1000,100),(400,1000,100),(1,5,1),       (0.5,4.5,1)))
    _do(3, "03_atrs_low",      _cfg("03_atrs_low",      (400,700,50),  (500,800,50),  (2,4,0.5),     (0.1,1.5,0.1)))
    _do(4, "04_atrl_sweep",    _cfg("04_atrl_sweep",    (400,700,50),  (500,800,50),  (1,6,0.5),     (0.5,1.5,0.25)))
    _do(5, "05_lenL_long",     _cfg("05_lenL_long",     (500,1000,50), (500,1000,50), (2,4,0.5),     (0.5,1.5,0.5)))
    _do(6, "06_lenS_sweep",    _cfg("06_lenS_sweep",    (450,550,50),  (300,1000,50), (2.5,3.5,0.5), (0.5,1.5,0.5)))
    _do(7, "07_fine_mid",      _cfg("07_fine_mid",      (475,525,5),   (600,650,5),   (2.75,3.25,0.25),(0.75,1.25,0.25)))
    _do(8, "08_short_ma_xchk", _cfg("08_short_ma_xchk", (15,35,5),     (2,12,2),      (3,4.5,0.25),  (4.5,6,0.25)))
    ...
    # summary
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  ETH HUNTER2 Hourly Round-2 COMPLETE  (%.1f min total)", (time.time()-_RUN_T0)/60)
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto ETHUSDT HOT Hourly Round-2 (refine ultra-long Obj-max)")
