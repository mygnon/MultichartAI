"""
search_eth_hunter2_hourly3.py — SFJ_HUNTER2_crypto on ETHUSDT HOT Hourly, Round 3 (IS 2022/01-2026/01)

R2 confirmed the ULTRA-LONG Obj-max regime and lifted Obj 25,882 → 30,979 (+19.7%):
  R2 Obj-max ⭐: LEN_L=495 LEN_S=635 ATR_L=2.8 ATR_S=0.3  NP=$3,685 MDD=-$438 Obj=30,979 182tr
  R2 NP-max   : LEN_L=490 LEN_S=665 ATR_L=2.75 ATR_S=0.4 NP=$3,691 MDD=-$455 Obj=29,949 186tr (same regime)
  A09→A10→A11 converged; all 4 axes bounded; short-MA cross-check only Obj 19,301.

R3 PURPOSE: CONFIRM the ceiling (Rule 7 — need ≤~0.1% round gain + convergence).
  - Rule 5 drift retest of the champion in this session.
  - Fine ATR_S 0.1-0.6 step 0.05 (R2 settled at 0.3 — verify true peak, not coarse-step artifact).
  - Fine LEN_L / LEN_S / ATR_L micro-sweeps.
  - Check ATR_S below 0.3 does NOT keep improving (boundary safety).
  - Wide re-confirm + boundary push (LEN→1000) to rule out a higher Obj elsewhere.
  - Zoom seed = Obj-max row (user criterion NP²/|MDD|).

11 attempts, each ≤5000 combos. Objective = NetProfit² / |MaxDrawdown|.

Attempt schedule:
  A01 retest_center : LEN_L(485-505 s5)×LEN_S(625-645 s5)×ATR_L(2.7-2.9 s0.05)×ATR_S(0.2-0.4 s0.05) = 5×5×5×5=625  (Rule 5 drift)
  A02 atrs_fine     : LEN_L(470-520 s10)×LEN_S(615-665 s10)×ATR_L(2.6-3.0 s0.1)×ATR_S(0.1-0.6 s0.05) = 6×6×5×11=1980
  A03 lenL_fine     : LEN_L(450-540 s10)×LEN_S(610-660 s10)×ATR_L(2.6-3.0 s0.1)×ATR_S(0.25-0.35 s0.05) = 10×6×5×3=900
  A04 lenS_fine     : LEN_L(480-510 s10)×LEN_S(580-700 s10)×ATR_L(2.7-2.9 s0.1)×ATR_S(0.25-0.35 s0.05) = 4×13×3×3=468
  A05 atrl_fine     : LEN_L(480-510 s10)×LEN_S(620-650 s10)×ATR_L(2.4-3.4 s0.1)×ATR_S(0.25-0.35 s0.05) = 4×4×11×3=528
  A06 atrs_verylow  : LEN_L(470-520 s10)×LEN_S(615-665 s10)×ATR_L(2.7-2.9 s0.1)×ATR_S(0.05-0.35 s0.05) = 6×6×3×7=756
  A07 wide_confirm  : LEN_L(300-700 s50)×LEN_S(400-900 s50)×ATR_L(2-4 s0.5)×ATR_S(0.2-1.2 s0.2) = 9×11×5×6=2970
  A08 boundary_push : LEN_L(500-1000 s100)×LEN_S(600-1000 s100)×ATR_L(2-4 s0.5)×ATR_S(0.1-0.5 s0.1) = 6×5×5×5=750
  A09 adaptive_zoom1: dynamic around Obj-max best (moderate)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_hourly3_search")
...
# R3 seed = R2 ultra-long Obj-max champion
SEED_LL   = 495.0
SEED_LS   = 635.0
SEED_ATRL = 2.8
SEED_ATRS = 0.3
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_hourly3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r2_obj_max": {"LEN_L": 495, "LEN_S": 635, "ATR_multiplier_L": 2.8, "ATR_multiplier_S": 0.3,
                       "net_profit": 3684.91, "max_drawdown": -438.318, "objective": 30979, "total_trades": 182},
        "notes": {
            ...
            "purpose": "Confirm the ultra-long Obj-max ceiling (Rule 7); zoom seed = Obj-max",
        },
    ...
    out = OUTPUT_DIR / "final_params_eth_hunter2_hourly3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    obj_best: Dict = {"LEN_L": SEED_LL, "LEN_S": SEED_LS, "ATR_multiplier_L": SEED_ATRL,
                      ...
                      "max_drawdown": -438.0, "objective": SEED_OBJ, "total_trades": 182}
    np_best: Dict = {}
    target_met = False
    attempt_log: List[dict] = []

    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_crypto ETHUSDT HOT Hourly — Round 3 (CONFIRM ULTRA-LONG Obj-max ceiling)")
    log.info("  Seed (R2 Obj-max): LEN_L=%.0f LEN_S=%.0f ATR_L=%.2f ATR_S=%.2f  Obj=%.0f",
             SEED_LL, SEED_LS, SEED_ATRL, SEED_ATRS, SEED_OBJ)
    log.info("  Objective: NP^2/|MDD| (bigger=better). Zoom seed = Obj-max row.")
    log.info("  Run start: %s", datetime.now().isoformat())
    log.info("══════════════════════════════════════════════════════════════")

    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (485, 505, 10), (625, 645, 10),
                                         (2.7, 2.9, 0.1), (0.2, 0.4, 0.1)))
    ...
    # static attempts
    _do(1, "01_retest_center", _cfg("01_retest_center", (485,505,5),   (625,645,5),   (2.7,2.9,0.05),(0.2,0.4,0.05)))
    _do(2, "02_atrs_fine",     _cfg("02_atrs_fine",     (470,520,10),  (615,665,10),  (2.6,3.0,0.1), (0.1,0.6,0.05)))
    _do(3, "03_lenL_fine",     _cfg("03_lenL_fine",     (450,540,10),  (610,660,10),  (2.6,3.0,0.1), (0.25,0.35,0.05)))
    _do(4, "04_lenS_fine",     _cfg("04_lenS_fine",     (480,510,10),  (580,700,10),  (2.7,2.9,0.1), (0.25,0.35,0.05)))
    _do(5, "05_atrl_fine",     _cfg("05_atrl_fine",     (480,510,10),  (620,650,10),  (2.4,3.4,0.1), (0.25,0.35,0.05)))
    _do(6, "06_atrs_verylow",  _cfg("06_atrs_verylow",  (470,520,10),  (615,665,10),  (2.7,2.9,0.1), (0.05,0.35,0.05)))
    _do(7, "07_wide_confirm",  _cfg("07_wide_confirm",  (300,700,50),  (400,900,50),  (2,4,0.5),     (0.2,1.2,0.2)))
    _do(8, "08_boundary_push", _cfg("08_boundary_push", (500,1000,100),(600,1000,100),(2,4,0.5),     (0.1,0.5,0.1)))
    ...
    # summary
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  ETH HUNTER2 Hourly Round-3 COMPLETE  (%.1f min total)", (time.time()-_RUN_T0)/60)
    ...
    log.info("  R2 Obj-max=30,979  →  R3 Obj-max=%.0f  (%+.1f%%)  [ceiling if <=~0.1%%]",
             obj_best["objective"], (obj_best["objective"]/SEED_OBJ - 1) * 100)
    log.info("══════════════════════════════════════════════════════════════")

    out = save_json(obj_best, np_best, attempt_log, target_met)
    print(f"\nDone — results at: {out}")
    print(f"Obj-max champion Obj={obj_best['objective']:.0f}  (R2 30,979)")
    return 0
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_crypto ETHUSDT HOT Hourly Round-3 (confirm ultra-long Obj-max ceiling)")
