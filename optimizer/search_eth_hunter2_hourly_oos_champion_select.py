"""
search_eth_hunter2_hourly_oos_champion_select.py — Out-of-sample (OOS) selection among the converged
ETH Hourly SFJ_HUNTER2_crypto champions (rounds 1-3, results/eth_hunter2_hourly{,2,3}_search/).
...
ETH HUNTER2 Hourly converged to a ULTRA-LONG asym regime (R3 ceiling Obj-max LEN_L=495 LEN_S=635
ATR_L=2.8 ATR_S=0.25) plus a competing SHORT-MA high-freq regime (R1 LEN_L=24 LEN_S=5). Candidates
span both so the OOS test reveals which regime holds its Max Drawdown out-of-sample.

TASK: re-test each previously-converged ETH Hourly HUNTER2 champion on this workspace and pick the
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_HUNTER_AI.wsp open, ETHUSDT HOT **60-Minute** chart tab ACTIVE, the
    SFJ_HUNTER2_crypto signal applied (Status ON)
  - Full data available to 2026/06/10 (the script sets the range itself)

CLI:
  py search_eth_hunter2_hourly_oos_champion_select.py                 # merged: IS then FULL, then verdict
  py search_eth_hunter2_hourly_oos_champion_select.py --period is     # one pass only
  py search_eth_hunter2_hourly_oos_champion_select.py --candidate EH1  # smoke one candidate
  py search_eth_hunter2_hourly_oos_champion_select.py --from-csv      # recompute from existing CSVs
  py search_eth_hunter2_hourly_oos_champion_select.py --probe-instrument  # verify auto data-range
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_hourly_oos_champion_select_search")
PREFIX      = "ETHH2SEL_"
...
# 4 converged ETH Hourly HUNTER2 candidates spanning both regimes (eth_hunter2_hourly{,2,3}_search).
# id, regime, LEN_L, LEN_S, ATR_multiplier_L, ATR_multiplier_S, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("EH1", "R3 Obj-max ultra-long ***", 495.0, 635.0, 2.8,  0.25, 3699.0, -437.0),  # 182tr, R3 ceiling Obj=31,283 lowest MDD
    ("EH2", "R3 NP-max ultra-long",      490.0, 665.0, 2.75, 0.3,  3724.0, -453.0),  # 186tr, highest IS NP
    ("EH3", "R1 ultra-long wide-ATR_S",  500.0, 625.0, 3.0,  1.0,  3424.0, -453.0),  # 170tr, R1 Obj=25,882 (ATR_S=1.0)
    ("EH4", "R1 short-MA high-freq",      24.0,   5.0, 3.8,  5.5,  3861.0, -615.0),  # 204tr, opposite regime (NP-max R1)
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_hunter2_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_hunter2_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("EH1", "is", *CANDIDATES[0][2:6]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Hourly HUNTER2 -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly HUNTER2 OOS champion selection (auto-range merged)")
