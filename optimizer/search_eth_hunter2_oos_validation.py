"""
search_eth_hunter2_oos_validation.py — FULL-PERIOD (incl. OOS) validation of the IS-optimized
HUNTER2 exit modules on ETHUSDT HOT Hourly.

Main signal SFJ_HUNTER2_crypto FIXED at Obj-max champion (LEN_L=495 LEN_S=635 ATR_multiplier_L=2.8 ATR_multiplier_S=0.25), Status ON.
Each of the 6 exit modules is FIXED at its IS-best param (from
results/eth_hunter2_exit_modules_search) and re-tested over the FULL range 2021/03/01-2026/06/10
...
IS-best module params (from the exit-module optimization, IS 2022/01-2026/01):
  M1 ATRstop            STP=11.0
  M2 TrailingStop       ATRSTP=36.9
  M3 EntryBarsAfterExit EXITBAR=48
  M4 high_volatility    DAYRANGE=6.09
  M5 QuantPass_PT_Exit  PT_Base=0.677
  M6 RescueTeamExit     Length=80, std=3.4
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  ...
  - ETHUSDT HOT 60-Minute chart tab ACTIVE and VISIBLE
  - Main signal SFJ_HUNTER2_crypto inputs = LEN_L 495 LEN_S 635 ATR_multiplier_L 2.8 ATR_multiplier_S 0.25, Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved

CLI:
  py search_eth_hunter2_oos_validation.py                 # A00 full baseline + M1-M6 + teardown
  py search_eth_hunter2_oos_validation.py --module 6      # only module 6
  py search_eth_hunter2_oos_validation.py --manual-status # toggle Status by hand
  py search_eth_hunter2_oos_validation.py --from-csv      # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_hunter2_oos_validation_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # WIDE no-op; chart-trim is the real control
FULL_RANGE  = ("2021/03/01", "2026/06/10")

CHAMPION = {"LEN_L": 495.0, "LEN_S": 635.0, "ATR_multiplier_L": 2.8, "ATR_multiplier_S": 0.25}
...
MODULES: List[Tuple[int, str, List[Tuple[str, float, float, float, float]]]] = [
    (1, "SFJ_15Dworkshop_lesson4_ATRstop",                 [("STP",      11.0,  0.1,   0.1,   100.0)]),
    (2, "SFJ_15Dworkshop_lesson9_1_TrailingStop",          [("ATRSTP",   36.9,  0.1,   0.1,   100.0)]),
    (3, "SFJ_15Dworkshop_lesson10_3_EntryBarsAfterExit",   [("EXITBAR",  48.0,  1.0,   1.0,   1000.0)]),
    (4, "SFJ_15Dworkshop_lesson11_3_high_volatility_exit", [("DAYRANGE", 6.09,  0.01,  0.01,  10.0)]),
    (5, "QuantPass_PT_Exit",                               [("PT_Base",  0.677, 0.001, 0.001, 1.0)]),
    (6, "RescueTeamExit",                                  [("Length",   80.0,  20.0,  20.0,  600.0),
                                                            ("std",      3.4,   0.1,   3.0,   6.0)]),
]
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_hunter2_oos_validation_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("LEN_L",            494.0, 496.0, 1.0),
            ParamAxis("LEN_S",            634.0, 636.0, 1.0),
            ParamAxis("ATR_multiplier_L", 2.55,  3.05,  0.25),
            ParamAxis("ATR_multiplier_S", 0.2,   0.3,   0.05),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (LEN_L=495 LEN_S=635 ATR_L=2.8 ATR_S=0.25)")
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_hunter2_oos_validation.json"
...
def run_search(conn, from_csv: bool, only_module: Optional[int], manual_status: bool):
    ...
    out_json = OUTPUT_DIR / "final_params_eth_hunter2_oos_validation.json"
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH HUNTER2 exit-module FULL-PERIOD validation")
    log.info("  Main fixed LEN_L=495 LEN_S=635 ATR_L=2.8 ATR_S=0.25 (ON); range %s ~ %s",
             FULL_RANGE[0], FULL_RANGE[1])
    log.info("==============================================================")

    # ── Switch to the CHART window, then trim to FULL range (3-attempt retry) ──
    #    ALWAYS ensure_chart_ready first: connect() picks the largest MC window, which can be
    #    the Study Editor when open — ensure_chart_ready matches the workspace stem in the CHART
    #    title (not the Study Editor) and focuses it before the Format-Instruments right-click.
    if not from_csv and conn is not None:
        _trim_ok = False
        for _att in (1, 2, 3):
            try:
                log.info("Switching to chart + trimming to FULL range %s ~ %s (attempt %d) ...",
                         *FULL_RANGE, _att)
                try:
    ...
    if only_module is None or only_module == 0:
        ...
        # intentionally VARIES the main params (LEN_L 494-496, LEN_S 634-636, ATR_L 2.55-3.05, ATR_S 0.2-0.3);
        # we pick the exact champion (495/635/2.8/0.25) row below.  That check is for MODULE runs only.
...
def main():
    ap = argparse.ArgumentParser(description="ETH HUNTER2 exit-module full-period OOS validation")
