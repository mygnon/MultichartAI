"""
search_eth_osc_hourly2.py -- _2021Basic_Osc_crypto on ETHUSDT HOT Hourly, Round 2 (boundary confirm)

R1 ceiling: NP-max $1,657 (LEN=45 LE=-3 SE=2 STP=1 LMT=16.5, 397tr);
            Obj-max $1,387 (LEN=46 LE=-3 SE=2 STP=1 LMT=16.5, MDD=-271, 383tr, Obj=7098).
R1 left two axes ON their swept boundaries: LE=-3 (boundary of A06) and LMT crept 13->15->16.5.
R2 confirms the ceiling by pushing both boundaries + Rule-5 retest of the champion neighborhood.
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_osc_hourly2_search")
...
SEED_LEN, SEED_LE  = 45.0, -3.0
SEED_SE,  SEED_STP = 2.0,   1.0
SEED_LMT           = 16.5

PREFIX = "ETHOSCH2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_osc_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    payload = {
        ...
        "r1_champion": {"np_max": "LEN=45 LE=-3 SE=2 STP=1 LMT=16.5 NP=1657 MDD=-393 397tr",
                        "obj_max": "LEN=46 LE=-3 SE=2 STP=1 LMT=16.5 NP=1387 MDD=-271 383tr Obj=7098"},
        "notes": {
            ...
            "r2_goal": "confirm ceiling: push LE below -3, LMT above 16.5, fine LEN 36-56, Rule-5 retest",
        },
    ...
    out = OUTPUT_DIR / "final_params_eth_osc_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  _2021Basic_Osc_crypto ETHUSDT HOT Hourly -- Round 2 boundary confirm (IS 2022/01-2026/01)")
    log.info("  R1 ceiling NP $1,657 / Obj $7,098 (LEN~45 LE=-3 SE=2 STP=1 LMT=16.5)")
    log.info("  Pushing LE<-3, LMT>16.5, LEN 36-56.  Obj=NP^2/|MDD|.  Target NP %.0f", TARGET_NP)
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (44, 46, 1), (-3.5, -2.5, 0.5),
                                                 (1.5, 2.5, 0.5), (0.75, 1.25, 0.25), (15, 18, 1)))
    ...
    _do(1, "01_rule5_retest",   _cfg("01_rule5_retest",   (43,47,1),  (-3.5,-2.5,0.5), (1.5,2.5,0.5), (0.75,1.25,0.25), (15,18,1)))
    _do(2, "02_LE_push_deep",   _cfg("02_LE_push_deep",   (42,50,2),  (-6,-2.5,0.5),   (1.5,2.5,0.5), (0.75,1.25,0.25), (14,20,2)))
    _do(3, "03_LMT_push_high",  _cfg("03_LMT_push_high",  (42,50,2),  (-4,-2.5,0.5),   (1.5,2.5,0.5), (0.75,1.25,0.25), (12,28,2)))
    _do(4, "04_LEN_fine",       _cfg("04_LEN_fine",       (36,56,1),  (-3.5,-2.5,0.5), (1.5,2.5,0.5), (0.75,1.25,0.25), (15,18,1)))
    _do(5, "05_deepLE_highLMT", _cfg("05_deepLE_highLMT", (42,50,4),  (-6,-3,0.5),     (1.5,2.5,0.5), (0.75,1.25,0.25), (14,24,2)))
    _do(6, "06_SE_STP_refine",  _cfg("06_SE_STP_refine",  (43,49,2),  (-3.5,-2.5,0.5), (1,3.5,0.5),   (0.5,2.0,0.25),   (15,18,1)))
    ...
    log.info("==============================================================")
    log.info("  _2021Basic_Osc_crypto ETHUSDT Hourly Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="_2021Basic_Osc_crypto ETHUSDT HOT Hourly R2 boundary-confirm")
