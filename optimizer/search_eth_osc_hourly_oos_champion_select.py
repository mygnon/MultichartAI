"""
search_eth_osc_hourly_oos_champion_select.py -- OOS selection among the converged
ETH Hourly _2021Basic_Osc_crypto champions (R1-R2, results/eth_osc_hourly{,2}_search/).

_2021Basic_Osc_crypto (5 params LEN, LE, SE, STP, LMT; BB-oscillator STOP entries, ATR exits,
_Crypto1MUSD).  ETH HOURLY IS ceiling: Obj-max LEN=45 LE=-3 SE=2 STP=1 LMT=17 ($1,656 MDD-361,
6-attempt convergence in R2, Rule-5 stable); NP-max high-LMT LEN=48 STP=1.25 LMT=26 ($1,752).
...
PREREQUISITES (MC64, elevated):
  - 20260101_SFJ_BASIC_OSC_AI.wsp open, ETHUSDT HOT HOURLY chart tab ACTIVE, _2021Basic_Osc_crypto ON
  - Full data to 2026/06/10 (the script sets the range itself). CLOSE the Study Editor window.

CLI:
  py search_eth_osc_hourly_oos_champion_select.py                  # merged: IS then FULL, verdict
  py search_eth_osc_hourly_oos_champion_select.py --candidate EO1  # smoke one candidate
  py search_eth_osc_hourly_oos_champion_select.py --from-csv       # recompute from existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_osc_hourly_oos_champion_select_search")
PREFIX      = "ETHOSCSEL_"
...
# Converged ETH Hourly _2021Basic_Osc_crypto candidates (eth_osc_hourly{,2}_search).
# id, regime, LEN, LE, SE, STP, LMT, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("EO1", "R2 Obj-max ***",     45.0, -3.0, 2.0, 1.0,  17.0, 1656.0, -361.0),  # 394tr, 6-attempt converged
    ("EO2", "R2 NP-max high-LMT", 48.0, -3.0, 2.0, 1.25, 26.0, 1752.0, -413.0),  # 343tr, max IS NP
    ("EO3", "R1 Obj-max tight",   46.0, -3.0, 2.0, 1.0,  16.5, 1387.0, -271.0),  # 383tr, lowest IS MDD
    ("EO4", "R1 alt momentum",    20.0,  0.0, 3.0, 2.0,  11.0, 1510.0, -645.0),  # 545tr, distinct regime
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_osc_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_osc_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_osc_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("EO1", "is", *CANDIDATES[0][2:7]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Hourly Osc -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly _2021Basic_Osc_crypto OOS champion selection")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="EOn", default=None)
