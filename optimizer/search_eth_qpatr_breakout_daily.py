"""
search_eth_qpatr_breakout_daily.py  QuantPassATR_Breakout on ETHUSDT HOT Daily, Round 1

Strategy: QuantPassATR_Breakout (long+short reversal, no STP/LMT)
  ...
  2-param: Len, Multiple

Timeframe: Daily (1440-min bars)
Target: NP > 10,000 USD

Reference ceilings:
  QPATR_Breakout BTC Daily: $2,544 (R3 A02, Len=13, Mult=1.845, 13tr); Obj $1,830 Len=10 Mult=1.05 (lowest MDD)
  QPATR_Breakout ETH Hourly: $4,444 (Len=32, Mult=3.37, 58tr, 8-conv)
  QPATR_Breakout BTC Hourly: $2,748 (Len=212, Mult=3.27, 90tr)
  QPATRex ETH Daily:        $3,161 (Len=14, Su=0.83, Ni=1.13, 41tr)
  QPATRex ETH Hourly:       $5,198 (Len=24, Su=1.48, Ni=2.47, 73tr)

Predictions:
  ETH Daily/Hourly QPATRex = 3,161/5,198 = 0.608 (worst Daily/Hourly across all)
  BTC QPATR_Breakout Daily/Hourly = 2,544/2,748 = 0.926
  Conservative: $4,444 x 0.608 = $2,702
  Optimistic:   $4,444 x 0.926 = $4,116
  Expected: ~$2,700-$4,100 (gap to $10K ~60-73%)

Attempt schedule (11 attempts, <=5000 combos each, 2D grids):
  Inspired by BTC Daily R1 + R2 lessons (where R2 revealed truth after R1 drift).

  A01 global_fine    Len(5-205 s5)   x Mult(0.2-5.0 s0.06)  [41x81=3321]
  A02 tight_multi    Len(5-105 s2)   x Mult(0.1-3.0 s0.03)  [51x97=4947]
  A03 wide_multi     Len(20-220 s5)  x Mult(2.0-8.0 s0.07)  [41x86=3526]
  A04 short_len      Len(2-52 s1)    x Mult(0.5-5.0 s0.05)  [51x91=4641]
  A05 long_len       Len(100-300 s5) x Mult(0.5-5.0 s0.05)  [41x91=3731]
  A06 near_default   Len(20-90 s2)   x Mult(1.5-4.0 s0.025) [36x101=3636]
  A07 Mult_below_1   Len(5-100 s2)   x Mult(0.1-1.0 s0.02)  [48x46=2208]  -- BTC R3 found Obj champ here
  A08 Mult_1_to_1.5  Len(5-100 s2)   x Mult(1.0-1.5 s0.01)  [48x51=2448]  -- BTC R3 Obj champ region
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatr_breakout_daily_search")
...
# ETH Hourly QPATR_Breakout champion as nominal seed
SEED_LEN  = 32.0
SEED_MULT = 3.37
SEED_NP   = 0.0
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatr_breakout_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "reference": "ETH QPATR_Breakout Hourly $4,444 (Len=32 Mult=3.37); BTC QPATR_Breakout Daily $2,544 (Len=13 Mult=1.845)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_eth_qpatr_breakout_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout on ETHUSDT HOT Daily -- Round 1")
    log.info("  Target: %.0f USD", TARGET_NP)
    log.info("  ETH Hourly: Len=32 Mult=3.37 NP=$4,444 (8-conv)")
    log.info("  BTC QPATR_Breakout Daily: $2,544 NP-max")
    log.info("  Expected: $2,700-$4,100 (gap to $10K ~60-73%)")
    log.info("==============================================================")
    ...
    attempts_config = [
        ...
        ("06_near_default",  (20, 90, 2),    (1.5, 4.0, 0.025)),
        ("07_Mult_below_1",  (5, 100, 2),    (0.1, 1.0, 0.02)),
        ("08_Mult_1_to_1.5", (5, 100, 2),    (1.0, 1.5, 0.01)),
    ]
    ...
    zoom_configs = [
        ...
        (10, "10_adaptive_zoom2", 0.12, 0.15, 25, 49),
        (11, "11_adaptive_zoom3", 0.06, 0.08, 17, 33),
    ]
    for A, _n, r_len_pct, r_mult_pct, n_len, n_mult in zoom_configs:
        ...
        if start_attempt <= A:
            r_len  = max(10.0 if A == 9 else (5.0 if A == 10 else 3.0),  best_LEN  * r_len_pct)
            r_mult = max(0.25 if A == 9 else (0.12 if A == 10 else 0.06), best_MULT * r_mult_pct)
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout ETHUSDT Daily Round-1 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATR_Breakout ETHUSDT HOT Daily R1 parameter search")
