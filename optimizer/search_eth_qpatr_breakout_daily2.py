"""
search_eth_qpatr_breakout_daily2.py  QuantPassATR_Breakout on ETHUSDT HOT Daily, Round 2

R1 Summary (11 attempts):
  NP-max ⭐⭐⭐ A09=A10=A11 (triple-conv): Len=9 Mult=1.706-1.71 NP=$3,986 MDD=-$767 14tr Obj=20,721 (highest)
  A02 sub-peak:  Len=7 Mult=1.78 NP=$3,853 14tr Obj=19,649
  A04 same Len:  Len=9 Mult=1.75 NP=$3,474 14tr (lower Mult)
  A07 high-freq: Len=31 Mult=0.84 NP=$2,263 97tr MDD=-$614 (low MDD)
  A03/A05 UI fails (rows truncated)
  ETH QPATR_Breakout Daily > ETH QPATRex Daily (+26.1%) -- rare reversal
  ETH Daily/Hourly only -10.3% (vs QPATRex -39%) -- QPATR_Breakout strong on ETH

R2 Plan (11 attempts, <=5000 combos each, 2D grids):
  Goal: confirm both regimes ultra-fine + redo R1 failures + explore micro-peaks.

  A01 A09_ultrafine        Len(7-12 s1)    xMult(1.65-1.78 s0.005)  [6x27=162]
  A02 A02_region_ultrafine Len(5-10 s1)    xMult(1.70-1.85 s0.005)  [6x31=186]
  A03 bridge_7_15          Len(7-15 s1)    xMult(1.65-1.85 s0.01)   [9x21=189]
  A04 redo_wide_multi      Len(20-220 s5)  xMult(2.0-6.0 s0.1)      [41x41=1681]
  A05 redo_long_len        Len(80-200 s5)  xMult(0.5-3.0 s0.05)     [25x51=1275]
  A06 A07_Mult_below_1     Len(20-50 s1)   xMult(0.5-1.0 s0.01)     [31x51=1581]
  A07 A08_Mult_1_to_1.5    Len(15-35 s1)   xMult(1.30-1.50 s0.005)  [21x41=861]
  A08 global_recheck       Len(5-300 s10)  xMult(0.5-5.0 s0.05)     [30x91=2730]
  A09-A11 adaptive zoom (ultra-tight from Len=9 Mult=1.71)
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatr_breakout_daily2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatr_breakout_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result":"R1 NP-max triple-conv A09=A10=A11: Len=9 Mult=1.706-1.71 NP=$3,986 MDD=-$767 14tr Obj=20,721; A02 sub Len=7 Mult=1.78 NP=$3,853",
            "r2_plan":  "Ultra-fine A09 + A02 region + bridge 7-15 + redo R1 failures + Mult<1 fine + Mult 1-1.5 fine + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_eth_qpatr_breakout_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout on ETHUSDT HOT Daily -- Round 2")
    log.info("  Target: %.0f USD", TARGET_NP)
    log.info("  R1 NP-max triple-conv: Len=9 Mult=1.71 NP=$3,986 Obj=20,721")
    log.info("==============================================================")
    ...
    attempts_config = [
        ("01_A09_ultrafine",        (7, 12, 1),       (1.65, 1.78, 0.005)),
        ("02_A02_region_ultrafine", (5, 10, 1),       (1.70, 1.85, 0.005)),
        ("03_bridge_7_15",          (7, 15, 1),       (1.65, 1.85, 0.01)),
        ("04_redo_wide_multi",      (20, 220, 5),     (2.0, 6.0, 0.1)),
        ("05_redo_long_len",        (80, 200, 5),     (0.5, 3.0, 0.05)),
        ("06_A07_Mult_below_1",     (20, 50, 1),      (0.5, 1.0, 0.01)),
        ("07_A08_Mult_1_to_1.5",    (15, 35, 1),      (1.30, 1.50, 0.005)),
    ...
    # Adaptive zooms (ultra-tight from R1 champion Len=9 Mult=1.71)
    zoom_configs = [
    ...
    log.info("==============================================================")
    log.info("  QuantPassATR_Breakout ETHUSDT Daily Round-2 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATR_Breakout ETHUSDT HOT Daily R2 parameter search")
