"""
search_eth_qpatrex_daily2.py  QuantPassATRex on ETHUSDT HOT Daily, Round 2

R1 Summary:
  A11 NP-max (zoom still climbing): Len=14 Su=0.82406 Ni=1.1296 NP=$3,161 MDD=-$703 41tr Obj=14,209
  A09/A10 prev: Len=17 Su=0.83 Ni=1.13 NP=$3,067 (35 trades)
  A04 alt regime: Len=32 Su=0.97 Ni=0.78 NP=$2,442 (59 trades) -- different sub-regime
  A02 vshort: Len=5 Su=1.62 Ni=2.05 NP=$2,316 (10 trades)
  ETH Daily REGRESSED -39% from ETH Hourly $5,198 (opposite of BTC Daily +6.6%).

R2 Plan (11 attempts, <=5000 combos each):
  A11 still climbing -- push further with ultra-fine zoom + adjacent exploration.

  A01 A11_ultrafine     Len(10-20 s1) xSu(0.75-0.92 s0.01)xNi(1.05-1.20 s0.01)  [11x18x16=3168]
  A02 vshort_zoom       Len(2-12 s1)  xSu(0.5-1.5 s0.05)  xNi(0.8-1.5 s0.05)    [11x21x15=3465]
  A03 A11_lateral_Su    Len(12-18 s1) xSu(0.5-1.2 s0.03)  xNi(1.0-1.3 s0.02)    [7x24x16=2688]
  A04 bridge_14_32      Len(14-34 s2) xSu(0.7-1.1 s0.04)  xNi(0.7-1.5 s0.05)    [11x11x17=2057]
  A05 highFreq          Len(20-60 s5) xSu(0.1-0.5 s0.025) xNi(0.1-0.5 s0.025)   [9x17x17=2601]
  A06 highNi_shortLen   Len(8-22 s1)  xSu(0.3-0.9 s0.05)  xNi(1.5-3.5 s0.1)     [15x13x21=4095]
  A07 A04_region_zoom   Len(28-38 s1) xSu(0.85-1.05 s0.02)xNi(0.65-0.95 s0.02)  [11x11x16=1936]
  A08 global_recheck    Len(5-200 s20)xSu(0.3-2.5 s0.2)   xNi(0.5-5.0 s0.4)     [11x12x12=1584]
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatrex_daily2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatrex_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result": "R1 A11 (still climbing): Len=14 Su=0.82406 Ni=1.1296 NP=$3,161 MDD=-$703 41tr Obj=14,209",
            "r2_plan":   "Ultra-fine A11 zoom + vshort Len + lateral Su + bridge + high-freq + high-Ni + A04 region + global recheck",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_eth_qpatrex_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on ETHUSDT HOT Daily -- Round 2")
    log.info("  Signal: %s  Symbol: %s  Timeframe: Daily (1440min)", SIGNAL, SYMBOL)
    log.info("  Target: %.0f USD", TARGET_NP)
    log.info("  R1 A11: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f (zoom still climbing)",
             SEED_LEN, SEED_SU, SEED_NI, SEED_NP)
    log.info("  Goal: ultra-fine A11 + vshort + lateral Su + bridges + high-freq")
    log.info("==============================================================")
    ...
    A = 1
    _n = "01_A11_ultrafine"
    _c = _cfg(_n, (10, 20, 1), (0.75, 0.92, 0.01), (1.05, 1.20, 0.01))
    log.info("A01  Len(10-20 s1)xSu(0.75-0.92 s0.01)xNi(1.05-1.20 s0.01)  %d combos", _c.total_runs())
    if start_attempt <= A:
    ...
    A = 2
    _n = "02_vshort_zoom"
    _c = _cfg(_n, (2, 12, 1), (0.5, 1.5, 0.05), (0.8, 1.5, 0.05))
    log.info("A02  Len(2-12 s1)xSu(0.5-1.5 s0.05)xNi(0.8-1.5 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # A03  A11_lateral_Su
    A = 3
    _n = "03_A11_lateral_Su"
    _c = _cfg(_n, (12, 18, 1), (0.5, 1.2, 0.03), (1.0, 1.3, 0.02))
    log.info("A03  Len(12-18 s1)xSu(0.5-1.2 s0.03)xNi(1.0-1.3 s0.02)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    A = 4
    _n = "04_bridge_14_32"
    _c = _cfg(_n, (14, 34, 2), (0.7, 1.1, 0.04), (0.7, 1.5, 0.05))
    log.info("A04  Len(14-34 s2)xSu(0.7-1.1 s0.04)xNi(0.7-1.5 s0.05)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    log.info("After A04 -- best NP=%.0f  (Len=%.4g Su=%.4g Ni=%.4g)",
             best_np, best_LEN, best_SU, best_NI)

    # A05  highFreq -- Su/Ni < 0.5 untested
    A = 5
    _n = "05_highFreq"
    _c = _cfg(_n, (20, 60, 5), (0.1, 0.5, 0.025), (0.1, 0.5, 0.025))
    log.info("A05  Len(20-60 s5)xSu(0.1-0.5 s0.025)xNi(0.1-0.5 s0.025)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # A06  highNi_shortLen
    A = 6
    _n = "06_highNi_shortLen"
    _c = _cfg(_n, (8, 22, 1), (0.3, 0.9, 0.05), (1.5, 3.5, 0.1))
    log.info("A06  Len(8-22 s1)xSu(0.3-0.9 s0.05)xNi(1.5-3.5 s0.1)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # A07  A04_region_zoom
    A = 7
    _n = "07_A04_region_zoom"
    _c = _cfg(_n, (28, 38, 1), (0.85, 1.05, 0.02), (0.65, 0.95, 0.02))
    log.info("A07  Len(28-38 s1)xSu(0.85-1.05 s0.02)xNi(0.65-0.95 s0.02)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    _c = _cfg(_n, (5, 200, 20), (0.3, 2.5, 0.2), (0.5, 5.0, 0.4))
    log.info("A08  Len(5-200 s20)xSu(0.3-2.5 s0.2)xNi(0.5-5.0 s0.4)  %d combos", _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    log.info("A11  adaptive_zoom3 -- center: Len=%.4g Su=%.4g Ni=%.4g  NP=%.0f",
             best_LEN, best_SU, best_NI, best_np)
    if start_attempt <= A:
        r_len = max(1.0,   best_LEN * 0.05)
        r_su  = max(0.03,  best_SU  * 0.05)
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex ETHUSDT Daily Round-2 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex ETHUSDT HOT Daily R2 parameter search")
