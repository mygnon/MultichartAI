"""
search_eth_qpatrex_hourly3.py  QuantPassATRex on ETHUSDT HOT Hourly, Round 3

R1 best NP=$5,030 (Len=25 Su=1.504 Ni=2.61)
R2 best NP=$5,198 (Len=24 Su=1.48 Ni=2.48) +3.35%
R2 A07 best Obj=46,902 (Len=24 Su=1.48 Ni=5.5 NP=$4,809 MDD=-$493) -- BEST risk-adjusted in R1+R2

Key R2 insights:
  - A01 ultrafine grid found NP champion; zooms (A09-A11) cannot beat it
  - A07 high-Ni regime is dramatically better risk-adjusted (R1 missed this with step=0.19)
  - Three regimes confirmed at fine resolution
  - Ni gap (2.5 to 5.5) untested -- may contain better optima

R3 Plan (11 attempts, <=5000 combos each):
  Headline: fine-zoom A07 high-Ni regime + bridge the Ni gap 2.5-5.5

  A01 highNi_finezoom   Len(22-28 s1) xSu(1.30-1.65 s0.02)  xNi(4.5-6.5 s0.10)   [7x18x21=2646]
  A02 superfine_A01     Len(22-26 s1) xSu(1.40-1.55 s0.01)  xNi(2.35-2.60 s0.015) [5x16x18=1440]
  A03 midNi_bridge      Len(22-28 s1) xSu(1.30-1.60 s0.025) xNi(2.5-5.5 s0.15)   [7x13x21=1911]
  A04 midLen_highNi     Len(30-55 s2) xSu(0.85-1.30 s0.04)  xNi(4.0-7.0 s0.3)    [13x12x11=1716]
  A05 longLen_highNi    Len(140-180 s4)xSu(0.55-0.80 s0.025)xNi(4.0-8.0 s0.3)    [11x11x14=1694]
  A06 vshort_detailed   Len(2-15 s1)  xSu(1.5-3.0 s0.10)    xNi(2.0-4.5 s0.15)   [14x16x17=3808]
  A07 lowSu_highNi      Len(23-27 s1) xSu(1.0-1.45 s0.025)  xNi(5.0-9.0 s0.4)    [5x19x11=1045]
  A08 global_recheck    Len(5-205 s20)xSu(0.5-2.5 s0.2)     xNi(1.0-15.0 s1.4)   [11x11x11=1331]
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatrex_hourly3_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatrex_hourly3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_result": "R1 best NP=$5,030 (Len=25 Su=1.504 Ni=2.61); zoom monotonic upward",
            "r2_result": "R2 best NP=$5,198 (Len=24 Su=1.48 Ni=2.48) +3.35%; A07 Obj=46,902 (Ni=5.5 MDD=-$493)",
            "r3_plan":   "Fine-zoom A07 high-Ni regime + bridge Ni 2.5-5.5 + mid/long-Len high-Ni combos",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_eth_qpatrex_hourly3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex on ETHUSDT HOT Hourly -- Round 3")
    ...
    log.info("  R2 A07 Obj-king: Len=24 Su=1.48 Ni=5.5 Obj=46,902 MDD=-$493")
    log.info("  Goal: fine-zoom high-Ni regime + bridge Ni 2.5-5.5 + mid/long-Len high-Ni")
    log.info("==============================================================")
    ...
    # -----------------------------------------------------------------------
    # A01  highNi_finezoom -- fine zoom of A07 high-Ni regime
    #      A07 found Len=24 Su=1.48 Ni=5.5 Obj=46,902 (best R1+R2)
    #      Step=0.10 on Ni catches finer optima
    #      Len(22-28 s1) x Su(1.30-1.65 s0.02) x Ni(4.5-6.5 s0.10) = 7x18x21=2646
    # -----------------------------------------------------------------------
    A = 1
    _n = "01_highNi_finezoom"
    _c = _cfg(_n, (22, 28, 1), (1.30, 1.65, 0.02), (4.5, 6.5, 0.10))
    log.info("A01  Len(22-28 s1)xSu(1.30-1.65 s0.02)xNi(4.5-6.5 s0.10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # -----------------------------------------------------------------------
    # A02  superfine_A01 -- ultra-fine zoom around R2 NP champion
    #      Step=0.01/0.015 catches micro-peaks Su=1.48 region missed
    #      Len(22-26 s1) x Su(1.40-1.55 s0.01) x Ni(2.35-2.60 s0.015) = 5x16x18=1440
    # -----------------------------------------------------------------------
    A = 2
    _n = "02_superfine_A01"
    _c = _cfg(_n, (22, 26, 1), (1.40, 1.55, 0.01), (2.35, 2.60, 0.015))
    log.info("A02  Len(22-26 s1)xSu(1.40-1.55 s0.01)xNi(2.35-2.60 s0.015)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A03  midNi_bridge -- bridge Ni gap 2.5-5.5 (untested zone)
    #      R2 jumped from Ni=2.48 (A01) to Ni=5.5 (A07) -- middle unexplored
    #      Len(22-28 s1) x Su(1.30-1.60 s0.025) x Ni(2.5-5.5 s0.15) = 7x13x21=1911
    # -----------------------------------------------------------------------
    A = 3
    _n = "03_midNi_bridge"
    _c = _cfg(_n, (22, 28, 1), (1.30, 1.60, 0.025), (2.5, 5.5, 0.15))
    log.info("A03  Len(22-28 s1)xSu(1.30-1.60 s0.025)xNi(2.5-5.5 s0.15)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A04  midLen_highNi -- mid-Len regime with high Ni (untested combo)
    #      R2 A03 found mid-Len at Ni=3.0; try higher Ni
    #      Len(30-55 s2) x Su(0.85-1.30 s0.04) x Ni(4.0-7.0 s0.3) = 13x12x11=1716
    # -----------------------------------------------------------------------
    A = 4
    _n = "04_midLen_highNi"
    _c = _cfg(_n, (30, 55, 2), (0.85, 1.30, 0.04), (4.0, 7.0, 0.3))
    log.info("A04  Len(30-55 s2)xSu(0.85-1.30 s0.04)xNi(4.0-7.0 s0.3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A05  longLen_highNi -- long-Len regime with higher Ni
    #      R2 A04 used Ni=3.0-5.5; extend to Ni=4-8
    #      Len(140-180 s4) x Su(0.55-0.80 s0.025) x Ni(4.0-8.0 s0.3) = 11x11x14=1694
    # -----------------------------------------------------------------------
    A = 5
    _n = "05_longLen_highNi"
    _c = _cfg(_n, (140, 180, 4), (0.55, 0.80, 0.025), (4.0, 8.0, 0.3))
    log.info("A05  Len(140-180 s4)xSu(0.55-0.80 s0.025)xNi(4.0-8.0 s0.3)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A06  vshort_detailed -- detailed v-short Len (Len 2-15)
    #      R2 A05 found Len=2 at NP=$4,398 -- check if Len=3-15 has better
    #      Len(2-15 s1) x Su(1.5-3.0 s0.10) x Ni(2.0-4.5 s0.15) = 14x16x17=3808
    # -----------------------------------------------------------------------
    A = 6
    _n = "06_vshort_detailed"
    _c = _cfg(_n, (2, 15, 1), (1.5, 3.0, 0.10), (2.0, 4.5, 0.15))
    log.info("A06  Len(2-15 s1)xSu(1.5-3.0 s0.10)xNi(2.0-4.5 s0.15)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A07  lowSu_highNi -- low Su + high Ni asymmetric variation
    #      A07 R2 had Su=1.48 Ni=5.5; try Su=1.0-1.45 Ni=5.0-9.0
    #      Len(23-27 s1) x Su(1.0-1.45 s0.025) x Ni(5.0-9.0 s0.4) = 5x19x11=1045
    # -----------------------------------------------------------------------
    A = 7
    _n = "07_lowSu_highNi"
    _c = _cfg(_n, (23, 27, 1), (1.0, 1.45, 0.025), (5.0, 9.0, 0.4))
    log.info("A07  Len(23-27 s1)xSu(1.0-1.45 s0.025)xNi(5.0-9.0 s0.4)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A08  global_recheck -- broad sanity sweep
    #      Different cell from R1/R2 -- confirm no missed territory
    #      Len(5-205 s20) x Su(0.5-2.5 s0.2) x Ni(1.0-15.0 s1.4) = 11x11x11=1331
    # -----------------------------------------------------------------------
    A = 8
    _n = "08_global_recheck"
    _c = _cfg(_n, (5, 205, 20), (0.5, 2.5, 0.2), (1.0, 15.0, 1.4))
    log.info("A08  Len(5-205 s20)xSu(0.5-2.5 s0.2)xNi(1.0-15.0 s1.4)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    if start_attempt <= A:
        r_len = max(8.0,   best_LEN * 0.20)
        r_su  = max(0.15,  best_SU  * 0.25)
    ...
    log.info("==============================================================")
    log.info("  QuantPassATRex ETHUSDT Hourly Round-3 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="QuantPassATRex ETHUSDT HOT Hourly R3 parameter search")
