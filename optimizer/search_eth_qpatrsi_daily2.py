"""
search_eth_qpatrsi_daily2.py — QuantPassRSI on ETHUSDT HOT Daily, Round 2 (IS 2022/01-2026/01)
...
R1 Obj-max = NP-max Len=50 RSI_Gap=47 NP=$1,218 MDD=-$755 Obj=1,965 10tr (weakest QPRSI; sparse, high-gap
near-degenerate). 2 attempts garbled. R2 = INTEGER confirm Len=50 Gap=47 + RULE-5 DRIFT retest (does
same-param NP hold or drift like BNB Daily?) + COARSE sparse re-tests + dense low-gap check.
ETH Daily<<Hourly ($1.2K vs $3.4K). $100K -98.8%.
...
SYMBOL     = "ETHUSDT HOT"
SIGNAL     = "QuantPassRSI"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatrsi_daily2_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatrsi_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_eth_qpatrsi_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI on ETHUSDT HOT Daily -- Round 2 (integer confirm + Rule-5 drift; IS 2022/01-2026/01)")
    ...
    if not from_csv and conn is not None:
        ...
        for _try in range(3):
            try:
                mc.ensure_chart_ready(conn, _cfg("seed", (48, 52, 2), (45, 49, 2)))
    ...
    # static attempts
    _do(1, "01_retest_center",  _cfg("01_retest_center",  (42,58,1),   (42,49,1)))
    _do(2, "02_fine_lengap",    _cfg("02_fine_lengap",    (30,80,1),   (40,49,1)))
    _do(3, "03_gap_fine",       _cfg("03_gap_fine",       (45,55,1),   (20,49,1)))
    _do(4, "04_mid_len_coarse", _cfg("04_mid_len_coarse", (15,115,5),  (40,49,1)))
    ...
    _do(7, "07_short_len_np",   _cfg("07_short_len_np",   (10,40,1),   (30,49,1)))
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI ETHUSDT Daily Round-2 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="QuantPassRSI ETHUSDT HOT Daily R2 (confirm) search")
