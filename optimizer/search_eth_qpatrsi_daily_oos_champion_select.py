"""
search_eth_qpatrsi_daily_oos_champion_select.py — OOS selection among the converged
ETH Daily QuantPassRSI champions (R1-R2, results/eth_qpatrsi_daily{,2}_search/).

QuantPassRSI (2 params Len, RSI_Gap; momentum; reversal exits, _Crypto1MUSD). ETH DAILY: IS data-STABLE
(Rule-5 retest PASS, Len=50 Gap=47=$1,218 R1=R2) but WEAKEST QPRSI ($1.2K, 10tr, Gap~47 near-degenerate).
...
PREREQUISITES (MC64, elevated):
  - 20260101_QuantPassRSI_AI.wsp open, ETHUSDT HOT DAILY chart tab ACTIVE, QuantPassRSI signal ON
  - Full data to 2026/06/10 (the script sets the range itself). CLOSE the Study Editor window.

CLI:
  py search_eth_qpatrsi_daily_oos_champion_select.py                 # merged: IS then FULL, verdict
  py search_eth_qpatrsi_daily_oos_champion_select.py --candidate ESD1 # smoke one candidate
  py search_eth_qpatrsi_daily_oos_champion_select.py --from-csv      # recompute from existing CSVs
"""
...
SYMBOL      = "ETHUSDT HOT"
SIGNAL      = "QuantPassRSI"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatrsi_daily_oos_champion_select_search")
PREFIX      = "ETHRSIDSEL_"
...
# 4 converged ETH Daily QuantPassRSI candidates (eth_qpatrsi_daily{,2}_search).
# id, regime, Len, RSI_Gap, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("ESD1", "R2 Obj-max mid-len ***", 50.0, 47.0, 1218.0, -755.0),  # 10tr, R2 ceiling Obj=1,965 (stable)
    ("ESD2", "short-len",              16.0, 36.0, 1008.0, -1047.0), # 11tr, alt NP, deep MDD
    ("ESD3", "dense low-gap",           2.0, 12.0,  707.0,  -940.0), # 74tr, most active
    ("ESD4", "mid-len contrast",       32.0, 44.0,  940.0,  -875.0), # 11tr, mid
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatrsi_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_qpatrsi_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_qpatrsi_daily_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("ESD1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Daily QuantPassRSI -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Daily QuantPassRSI OOS champion selection")
