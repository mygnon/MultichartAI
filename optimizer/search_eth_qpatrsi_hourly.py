"""
search_eth_qpatrsi_hourly.py — QuantPassRSI on ETHUSDT HOT Hourly, Round 1 (IS 2022/01-2026/01)
...
SYMBOL     = "ETHUSDT HOT"
SIGNAL     = "QuantPassRSI"
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatrsi_hourly_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatrsi_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(obj_best, np_best, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_eth_qpatrsi_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI on ETHUSDT HOT Hourly -- Round 1 (IS 2022/01-2026/01 chart-trimmed)")
    ...
    log.info("==============================================================")
    log.info("  QuantPassRSI ETHUSDT Hourly Round-1 COMPLETE (%.1f min)", (time.time()-_RUN_T0)/60)
    if obj_best:
...
def main():
    ap = argparse.ArgumentParser(description="QuantPassRSI ETHUSDT HOT Hourly R1 search")
