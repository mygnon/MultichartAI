"""
search_eth_qpatrsi_hourly_oos_champion_select.py — OOS selection among the converged
ETH Hourly QuantPassRSI champions (R1, results/eth_qpatrsi_hourly_search/).

QuantPassRSI (2 params Len, RSI_Gap; momentum: RSI>100-Gap -> BUY market, RSI<Gap -> SHORT market,
reversal exits, _Crypto1MUSD). IS converged to Obj-max Len=12 RSI_Gap=15 (short-len low-gap, 22tr, low MDD).
Candidates span: the Obj-max short-len, the NP-max, the mid-len DENSE regime (79tr), and a long-len — so the OOS test reveals which holds its Max Drawdown out-of-sample.
...
PREREQUISITES (MC64, elevated):
  - 20260101_QuantPassRSI_AI.wsp open, ETHUSDT HOT 60-Minute chart tab ACTIVE, QuantPassRSI signal ON
  - Full data to 2026/06/10 (the script sets the range itself). CLOSE the Study Editor window.

CLI:
  py search_eth_qpatrsi_hourly_oos_champion_select.py                 # merged: IS then FULL, verdict
  py search_eth_qpatrsi_hourly_oos_champion_select.py --candidate ES1 # smoke one candidate
  py search_eth_qpatrsi_hourly_oos_champion_select.py --from-csv      # recompute from existing CSVs
"""
...
SYMBOL      = "ETHUSDT HOT"
SIGNAL      = "QuantPassRSI"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_qpatrsi_hourly_oos_champion_select_search")
PREFIX      = "ETHRSISEL_"
...
# 4 converged ETH Hourly QuantPassRSI candidates (eth_qpatrsi_hourly_search, R1).
# id, regime, Len, RSI_Gap, prior IS NP, prior IS MDD (reference).
CANDIDATES = [
    ("ES1", "R1 Obj-max short-len ***", 12.0, 15.0, 3408.0, -704.0),  # 22tr, R1 ceiling Obj=16,489
    ("ES2", "NP-max",                   17.0, 19.0, 3408.0, -733.0),  # 22tr, same NP, deeper MDD
    ("ES3", "mid-len DENSE",            66.0, 41.0, 2108.0, -590.0),  # 79tr, lowest IS MDD, most active
    ("ES4", "long-len",                125.0, 45.0, 1839.0, -610.0),  # 70tr, long-RSI contrast
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_qpatrsi_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_qpatrsi_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_qpatrsi_hourly_oos_champion_select.json"
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Hourly QuantPassRSI -- OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly QuantPassRSI OOS champion selection")
