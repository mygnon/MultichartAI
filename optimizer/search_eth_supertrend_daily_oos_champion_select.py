"""
search_eth_supertrend_daily_oos_champion_select.py — OOS selection among the converged ETH Daily
SFJ_SuperTrend_crypto champions (round 1, results/eth_supertrend_daily_search/).

TASK: re-test each converged champion on full data and pick the parameter set most profitable
out-of-sample WITHOUT breaking its established Max Drawdown.

METHOD — MERGED single-run, AUTO data-range (proven route; set_instrument_data_range per pass):
  ...
  Each candidate = a 9-combo micro-grid (2 params ATRLength/Multiplier, all vary +/-1 step, Critical Rule 1);
  exact candidate row picked. Metric = NetProfit + Max INTRADAY Drawdown.

PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_SuperTrend_AI.wsp open, ETHUSDT HOT **DAILY** chart tab ACTIVE and VISIBLE
  - SFJ_SuperTrend_crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_eth_supertrend_daily_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_eth_supertrend_daily_oos_champion_select.py --period is
  py search_eth_supertrend_daily_oos_champion_select.py --candidate EDS1
  py search_eth_supertrend_daily_oos_champion_select.py --from-csv
  py search_eth_supertrend_daily_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_supertrend_daily_oos_champion_select_search")
PREFIX      = "ETHSTDSEL_"
...
STEP = {"ATRLength": 1.0, "Multiplier": 0.05}
LO   = {"ATRLength": 1.0, "Multiplier": 0.1}
HI   = {"ATRLength": 5000.0, "Multiplier": 50.0}

# 4 DISTINCT converged ETH Daily SuperTrend regimes (from eth_supertrend_daily_search).
# id, regime, ATRLength, Multiplier, prior IS NP, prior IS MDD  (reference only)
CANDIDATES = [
    ("EDS1", "short-ATR wide-band ***", 4.0,   4.5,  2413.0, -855.0),   # R1 6-conv peak, 12tr; NP/Obj-max
    ("EDS2", "low-mult high-freq",      4.0,   1.0,  1457.0, -797.0),   # 117tr, distinct high-freq
    ("EDS3", "long-ATR high-freq",    280.0,   1.5,  1415.0, -770.0),   # 80tr, distinct
    ("EDS4", "mid-ATR wide-band",      26.0,   4.375,1536.0, -1026.0),  # 12tr, distinct mid-ATR
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_supertrend_daily_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _cfg(cid, period, at, ml) -> StrategyConfig:
    return StrategyConfig(
        name=f"{PREFIX}{period}_{cid}",
        mc_signal_name=SIGNAL,
        timeframe="daily",
        bar_period=1440,
        params=[_axis("ATRLength", at), _axis("Multiplier", ml)],
...
def _pick(df, at, ml):
    m = df
    for nm, v in (("ATRLength", at), ("Multiplier", ml)):
        if nm in m.columns:
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_supertrend_daily_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_supertrend_daily_oos_champion_select.json"
    ...
    payload = {
        "symbol": SYMBOL, "signal": SIGNAL, "timeframe": "Daily",
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("EDS1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Daily SuperTrend — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        # Retry the chart-trim: the right-click -> Format Instruments can transiently miss.
        _trim_ok = False
        for _att in (1, 2, 3):
            try:
                _cleanup_stray_windows()
                ...
                log.warning("  set_instrument_data_range attempt %d FAILED for %s: %s",
                            _att, period, e)
                time.sleep(1.0)
        if not _trim_ok:
            log.error("  set_instrument_data_range FAILED for %s after 3 tries — aborting pass", period)
            continue
        pres = payload["results"].get(period, {})
        for (cid, regime, at, ml, pnp, pmdd) in CANDIDATES:
            if only_candidate and cid != only_candidate:
                continue
            t0 = time.time()
            cfg = _cfg(cid, period, at, ml)
            log.info("--- [%s] %s %s ATR=%g Mult=%g [%s] ---",
                     period, cid, regime, at, ml, datetime.now().strftime("%H:%M:%S"))
            df = run_or_load(cfg, conn, from_csv)
            ent = {"id": cid, "regime": regime,
                   "params": {"ATRLength": at, "Multiplier": ml},
                   "timestamp": datetime.now().isoformat(),
                   "elapsed_sec": round(time.time() - t0, 1),
                   "rows": len(df) if df is not None else 0}
            row = _pick(df, at, ml) if (df is not None and not df.empty) else None
            if row is not None:
    ...
    for (cid, regime, at, ml, pnp, pmdd) in CANDIDATES:
        ...
        c = {"id": cid, "regime": regime, "params": {"ATRLength": at, "Multiplier": ml},
    ...
    log.info("  %-4s %-24s %9s %9s %9s %9s %4s", "id", "regime", "NP_is", "NP_full", "OOS_NP", "MDD_full", "pass")
    for c in sorted(valid, key=lambda c: c["oos_np"], reverse=True):
        log.info("  %-4s %-24s %9.0f %9.0f %9.0f %9.0f %4s", c["id"], c["regime"],
...
def main():
    ap = argparse.ArgumentParser(description="ETH Daily SuperTrend OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="EDSn", default=None)
    ...
    if args.probe_instrument:
        ...
        print(f"\nProbe: setting Data Range -> {tgt_from} ~ {tgt_to}")
