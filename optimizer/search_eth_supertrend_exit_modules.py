"""
search_eth_supertrend_exit_modules.py — Exit-module independent optimization on top of the
SFJ_SuperTrend_crypto ETH Hourly champion.

Main signal FIXED at champion (ATRLength=45 Multiplier=10.45), Status ON.
...
the same-day baseline (A00) and the reference baseline NP=$3,004 / MDD=-$621 (R1=R2 ETH SuperTrend champion).
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_SuperTrend_AI.wsp open, ETHUSDT HOT 60-Minute tab active
  - Main signal SFJ_SuperTrend_crypto inputs set to ATRLength=45 Multiplier=10.45, Status ON
  - All 6 module signals inserted on the chart, Status OFF
  - Workspace saved

CLI:
  py search_eth_supertrend_exit_modules.py                  # A00 baseline + M1-M6 + teardown
  py search_eth_supertrend_exit_modules.py --module 3       # only module 3 (status set for it)
  py search_eth_supertrend_exit_modules.py --module 0       # baseline A00 only
  py search_eth_supertrend_exit_modules.py --smoke --module 1   # tiny end-to-end smoke test
  py search_eth_supertrend_exit_modules.py --manual-status  # user toggles Status by hand
  py search_eth_supertrend_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_supertrend_exit_modules_search")
...
# Reference baseline (R1=R2 ETH SuperTrend champion, IS 2022/01-2026/01)
BASELINE_REF = {"net_profit": 3004.0, "max_drawdown": -621.0, "total_trades": 103}
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_supertrend_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs ATRLength=45 Multiplier=10.45)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_eth_supertrend_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_eth_supertrend_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly SuperTrend exit-module optimization%s", " [SMOKE]" if smoke else "")
    log.info("  Main fixed: ATRLength=45 Multiplier=10.45 (Status ON); IS 2022/01-2026/01 (chart-trimmed)")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH SuperTrend exit-module independent optimization")
