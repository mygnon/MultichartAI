"""
search_eth_supertrend_hourly2.py — SFJ_SuperTrend_crypto on ETHUSDT HOT Hourly, Round 2
...
Priors (no SuperTrend crypto prior; 2-param ATR-breakout analog = QPATR_Breakout):
  ETH CT/QPATRex/Breakout Hourly $4.4-5.2K (ETH/BTC ~1.2x); SuperTrend BNB ATR=79 Mult=6.625, BTC ATR=151 Mult=9.15.
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_supertrend_hourly2_search")
...
SEED_AT, SEED_ML = 45.0, 10.45   # R1 champion
SEED_NP = 0.0

PREFIX = "ETHSTH2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_supertrend_hourly2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "priors":   "ETH CT Hourly $5,005 / QPATRex $5,198 / Breakout $4,444 (ETH/BTC ~1.2x); BNB SuperTrend ATR=79 Mult=6.625, BTC SuperTrend ATR=151 Mult=9.15. SuperTrend default ATRLength=10 Mult=3",
            "r1_plan":  "global + short/mid/long/vlong ATR + low/high mult + fine-default + adaptive zoom",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_eth_supertrend_hourly2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly SFJ_SuperTrend_crypto NP>100K -- Round 2")
    log.info("  IS window %s ~ %s  (chart-trimmed)", IS_RANGE[0], IS_RANGE[1])
    log.info("  R1 champ ATR=45 Mult=10.45 NP=3004 (mid-ATR wide-band 103tr; confirming peak)", )
    log.info("==============================================================")

    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (40, 50, 2), (9.75, 11.0, 0.25)))
    ...
    attempts_config = [
        ("01_Mult_sweep",      (38, 52, 2),    (7.0, 15.0, 0.125)),
        ("02_ATR_sweep",       (20, 90, 2),    (9.5, 11.5, 0.125)),
        ("03_high_mult_push",  (35, 60, 5),    (10.0, 18.0, 0.25)),
        ("04_fine_zoom",       (40, 50, 1),    (9.75, 11.0, 0.05)),
        ("05_ATR_bridge_low",  (10, 45, 2),    (8.0, 12.0, 0.25)),
        ("06_ATR_bridge_high", (45, 150, 5),   (7.0, 12.0, 0.25)),
        ("07_vhigh_mult",      (30, 70, 5),    (12.0, 20.0, 0.5)),
        ("08_short_ATR_recheck",(5, 40, 2),    (6.0, 12.0, 0.25)),
    ]
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly SuperTrend_crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH Hourly SFJ_SuperTrend_crypto NP>100K Round-2 search")
