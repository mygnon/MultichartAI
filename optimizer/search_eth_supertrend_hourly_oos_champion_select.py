"""
search_eth_supertrend_hourly_oos_champion_select.py — OOS selection among the converged ETH Hourly
SFJ_SuperTrend_crypto champions (rounds 1-2, results/eth_supertrend_hourly{,2}_search/).
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_SuperTrend_AI.wsp open, ETHUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
  - SFJ_SuperTrend_crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_eth_supertrend_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_eth_supertrend_hourly_oos_champion_select.py --period is
  py search_eth_supertrend_hourly_oos_champion_select.py --candidate ES1
  py search_eth_supertrend_hourly_oos_champion_select.py --from-csv
  py search_eth_supertrend_hourly_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_supertrend_hourly_oos_champion_select_search")
PREFIX      = "ETHSTHSEL_"
...
# 4 DISTINCT converged ETH Hourly SuperTrend regimes (from eth_supertrend_hourly{,2}_search).
# id, regime, ATRLength, Multiplier, prior IS NP, prior IS MDD  (reference only)
CANDIDATES = [
    ("ES1", "mid-ATR wide-band ***", 45.0, 10.45, 3004.0, -621.0),  # R1=R2 peak, 103tr; NP/Obj-max
    ("ES2", "short-ATR high-freq",   46.0, 7.0,   2339.0, -570.0),  # 205tr, distinct high-freq
    ("ES3", "vhigh-mult low-freq",   65.0, 14.0,  2355.0, -566.0),  # 66tr, lowest MDD, low-freq
    ("ES4", "mid-ATR very-high-freq",48.0, 6.0,   1867.0, -1004.0), # 263tr, distinct high-freq
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_supertrend_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_supertrend_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_supertrend_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("ES1", "is", *CANDIDATES[0][2:4]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Hourly SuperTrend — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly SuperTrend OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="ESn", default=None)
