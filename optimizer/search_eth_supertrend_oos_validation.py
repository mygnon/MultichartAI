"""
search_eth_supertrend_oos_validation.py — FULL-PERIOD (incl. OOS) validation of the IS-optimized
SuperTrend exit modules on ETHUSDT HOT Hourly.

Main signal SFJ_SuperTrend_crypto FIXED at champion (ATRLength=45 Multiplier=10.45), Status ON.
Each of the 6 exit modules is FIXED at its IS-best param (from
results/eth_supertrend_exit_modules_search) and re-tested over the FULL range 2021/03/01-2026/06/10
...
IS-best module params (from the exit-module optimization, IS 2022/01-2026/01):
  M1 ATRstop            STP=13.1
  M2 TrailingStop       ATRSTP=46.4
  M3 EntryBarsAfterExit EXITBAR=30
  M4 high_volatility    DAYRANGE=5.06
  M5 QuantPass_PT_Exit  PT_Base=0.647
  M6 RescueTeamExit     Length=60, std=3.0
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - Workspace 20260101_SFJ_SuperTrend_AI.wsp open, full data to 2026/06/10
  - ETHUSDT HOT 60-Minute chart tab ACTIVE and VISIBLE
  - Main signal SFJ_SuperTrend_crypto inputs = ATRLength 45 Multiplier 10.45, Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved

CLI:
  py search_eth_supertrend_oos_validation.py                 # A00 full baseline + M1-M6 + teardown
  py search_eth_supertrend_oos_validation.py --module 6      # only module 6
  py search_eth_supertrend_oos_validation.py --manual-status # toggle Status by hand
  py search_eth_supertrend_oos_validation.py --from-csv      # re-analyze existing CSVs
"""
...
SYMBOL      = "ETHUSDT HOT"
MAIN_SIGNAL = "SFJ_SuperTrend_crypto"
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_supertrend_oos_validation_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # WIDE no-op; chart-trim is the real control
FULL_RANGE  = ("2021/03/01", "2026/06/10")

CHAMPION = {"ATRLength": 45.0, "Multiplier": 10.45}
...
MODULES: List[Tuple[int, str, List[Tuple[str, float, float, float, float]]]] = [
    (1, "SFJ_15Dworkshop_lesson4_ATRstop",                 [("STP",      13.1,  0.1,   0.1,   100.0)]),
    (2, "SFJ_15Dworkshop_lesson9_1_TrailingStop",          [("ATRSTP",   46.4,  0.1,   0.1,   100.0)]),
    (3, "SFJ_15Dworkshop_lesson10_3_EntryBarsAfterExit",   [("EXITBAR",  30.0,  1.0,   1.0,   1000.0)]),
    (4, "SFJ_15Dworkshop_lesson11_3_high_volatility_exit", [("DAYRANGE", 5.06,  0.01,  0.01,  10.0)]),
    (5, "QuantPass_PT_Exit",                               [("PT_Base",  0.647, 0.001, 0.001, 1.0)]),
    (6, "RescueTeamExit",                                  [("Length",   60.0,  20.0,  20.0,  600.0),
                                                            ("std",      3.0,   0.1,   3.0,   6.0)]),
]
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_supertrend_oos_validation_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("ATRLength",  44.0, 46.0, 1.0),
            ParamAxis("Multiplier", 10.40, 10.50, 0.05),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (ATRLength=45 Multiplier=10.45)")
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_supertrend_oos_validation.json"
...
def run_search(conn, from_csv: bool, only_module: Optional[int], manual_status: bool):
    ...
    out_json = OUTPUT_DIR / "final_params_eth_supertrend_oos_validation.json"
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH SuperTrend exit-module FULL-PERIOD validation")
    log.info("  Main fixed ATRLength=45 Multiplier=10.45 (ON); range %s ~ %s",
             FULL_RANGE[0], FULL_RANGE[1])
    log.info("==============================================================")

    # ── Trim chart to FULL range ─────────────────────────────────────────────
    ...
    if only_module is None or only_module == 0:
        ...
        # the exact champion (45/10.45) row below.  That check is for MODULE runs only.
...
def main():
    ap = argparse.ArgumentParser(description="ETH SuperTrend exit-module full-period OOS validation")
