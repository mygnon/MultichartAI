"""
search_eth_xtreme_daily2.py — SFJ_XtremeStop_Crypto on ETHUSDT HOT Daily, Round 1
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\eth_xtreme_daily2_search")
...
SEED_X, SEED_LY, SEED_SY = 57.0, 3.45, 7.55
SEED_NP = 3386.7

PREFIX = "ETHXTD2_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_xtreme_daily2_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_eth_xtreme_daily2.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("==============================================================")
    log.info("  ETH Daily SFJ_XtremeStop_Crypto NP>100K -- Round 2 (confirm R1 X=57 LY=3.45 SY=7.55)")
    ...
    if not from_csv and conn is not None:
        mc.ensure_chart_ready(conn, _cfg("seed", (53, 61, 2), (3.25, 3.65, 0.05), (7.35, 7.75, 0.05)))
    ...
    attempts_config = [
        ("01_retest_seed",  (53, 61, 2),    (3.25, 3.65, 0.05), (7.35, 7.75, 0.05)),  # 5*9*9=405 Rule-5
        ("02_X_sweep",      (40, 80, 2),    (3.3, 3.6, 0.1),    (7.4, 7.7, 0.1)),     # 21*4*4=336
        ("03_LY_fine",      (55, 59, 2),    (1.0, 6.0, 0.1),    (7.4, 7.7, 0.1)),     # 3*51*4=612
        ("04_SY_fine",      (55, 59, 2),    (3.3, 3.6, 0.1),    (5.0, 12.0, 0.1)),    # 3*4*71=852
        ("05_X_wide_midX",  (20, 120, 5),   (3.0, 4.0, 0.25),   (7.0, 8.0, 0.25)),    # 21*5*5=525 re-cover mid-X
        ("06_dense_recheck",(30, 60, 2),    (0.4, 1.2, 0.2),    (2.0, 3.5, 0.25)),    # 16*5*7=560 low-pct 80tr
        ("07_high_pct_chk", (45, 65, 5),    (10.0, 16.0, 0.5),  (9.0, 13.0, 0.5)),    # 5*13*9=585
        ("08_combo_2d",     (48, 68, 2),    (2.5, 4.5, 0.2),    (6.5, 8.5, 0.25)),    # 11*11*9=1089
    ]
    ...
    log.info("==============================================================")
    log.info("  ETH Daily XtremeStop_Crypto Round-2 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="ETH Daily SFJ_XtremeStop_Crypto NP>100K Round-1 search")
