"""
search_eth_xtreme_exit_modules.py -- Exit-module independent optimization on top of the
SFJ_XtremeStop_Crypto ETH Hourly main (OOS-selected EX2).

Main signal FIXED at the user-selected EX2 (X=720 LY=3.0 SY=1.0; the only OOS-profitable
candidate), Status ON. Six exit modules tested INDEPENDENTLY (one enabled at a time),
...
PREREQUISITES (one-time manual setup in MC64):
  - Workspace 20260101_SFJ_XtremeStop_AI.wsp open, ETHUSDT HOT HOURLY tab active
  - Main signal SFJ_XtremeStop_Crypto inputs set to X=720 LY=3.0 SY=1.0 (EX2), Status ON
  - All 6 module signals inserted on the chart, Status OFF; workspace saved
  - CLOSE the Study Editor window

CLI:
  py search_eth_xtreme_exit_modules.py                  # IS-trim + A00 + M1-M6 + teardown
  py search_eth_xtreme_exit_modules.py --module 3       # only module 3
  py search_eth_xtreme_exit_modules.py --module 0       # baseline A00 only
  py search_eth_xtreme_exit_modules.py --smoke --module 1
  py search_eth_xtreme_exit_modules.py --manual-status  # user toggles Status by hand
  py search_eth_xtreme_exit_modules.py --no-trim        # skip chart-trim (chart already IS)
  py search_eth_xtreme_exit_modules.py --from-csv       # re-analyze existing CSVs
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_xtreme_exit_modules_search")
INSAMPLE    = DateRange("2019/01/01", "2027/01/01")   # signal date no-op; chart-trim is the control
IS_RANGE    = ("2022/01/01", "2026/01/01")

# XtremeStop ETH main = OOS-selected EX2 (only OOS-profitable candidate) — set in MC64 (Format Objects, Status ON)
CHAMPION = {
    "X":  720.0,
    "LY": 3.0,
    "SY": 1.0,
}
# Reference baseline = XtremeStop main-only IS (EX2 row, eth_xtreme_hourly_oos_champion_select)
BASELINE_REF = {"net_profit": 3343.8, "max_drawdown": -743.03, "total_trades": 134}
...
_LOG_FILE = OUTPUT_DIR / f"search_eth_xtreme_exit_modules_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def _baseline_cfg() -> StrategyConfig:
    # 27-combo micro-grid around EX2: all 3 params vary (Critical Rule 1);
    # grid points include the EXACT main X=720 LY=3.0 SY=1.0 so A00 reads main-only at EX2.
    return StrategyConfig(
        ...
        bar_period=60,
        params=[
            ParamAxis("X",  718.0, 722.0, 2.0),
            ParamAxis("LY", 2.75,  3.25,  0.25),
            ParamAxis("SY", 0.75,  1.25,  0.25),
        ],
        chart_workspace=WORKSPACE,
        chart_symbol=SYMBOL,
        insample=INSAMPLE,
...
def _apply_status(conn, active_module: Optional[str], manual: bool):
    ...
    if manual:
        ...
        print(f"  {MAIN_SIGNAL}: ON (inputs X=720 LY=3.0 SY=1.0)")
...
def save_json(payload):
    name = ("SMOKE_" if payload.get("smoke") else "") + "final_params_eth_xtreme_exit_modules.json"
    out = OUTPUT_DIR / name
...
def run_search(conn, from_csv: bool, only_module: Optional[int],
    ...
    out_json = OUTPUT_DIR / (("SMOKE_" if smoke else "")
                             + "final_params_eth_xtreme_exit_modules.json")
    if out_json.exists():
        try:
    ...
    log.info("==============================================================")
    log.info("  ETH Hourly XtremeStop exit-module optimization%s", " [SMOKE]" if smoke else "")
    log.info("  Main fixed: X=720 LY=3.0 SY=1.0 (EX2, Status ON)")
    log.info("  Baseline ref (XtremeStop IS EX2): NP=%.2f MDD=%.2f", BASELINE_REF["net_profit"],
...
def main():
    ap = argparse.ArgumentParser(description="ETH XtremeStop exit-module independent optimization")
