"""
search_eth_xtreme_hourly_oos_champion_select.py — OOS selection among the converged ETH Hourly
SFJ_XtremeStop_Crypto champions (rounds 1-3, results/eth_xtreme_hourly{,2,3}_search/).
...
PREREQUISITES (MC64, run AS ADMINISTRATOR):
  - 20260101_SFJ_XtremeStop_AI.wsp open, ETHUSDT HOT **60-Minute** chart tab ACTIVE and VISIBLE
  - SFJ_XtremeStop_Crypto signal applied (Status ON), data to 2026/06/10

CLI:
  py search_eth_xtreme_hourly_oos_champion_select.py                # merged: IS then FULL, verdict
  py search_eth_xtreme_hourly_oos_champion_select.py --period is
  py search_eth_xtreme_hourly_oos_champion_select.py --candidate EX1
  py search_eth_xtreme_hourly_oos_champion_select.py --from-csv
  py search_eth_xtreme_hourly_oos_champion_select.py --probe-instrument
"""
...
OUTPUT_DIR  = Path(r"C:\Users\Tim\MultichartAI\results\eth_xtreme_hourly_oos_champion_select_search")
PREFIX      = "ETHXTHSEL_"
...
# 4 DISTINCT converged ETH Hourly XtremeStop regimes (from eth_xtreme_hourly{,2,3}_search).
# id, regime, X, LY, SY, prior IS NP, prior IS MDD  (NP data-vintage-dependent, Rule 5 — reference only)
CANDIDATES = [
    ("EX1", "long-X asym champion ***", 796.0, 5.85, 3.35, 3682.0, -450.0),  # R2=R3 peak, 46tr; NP/Obj-max
    ("EX2", "long-X low-SY high-freq",  720.0, 3.0,  1.0,  3344.0, -743.0),  # 134tr, R2 asym_retry
    ("EX3", "short-X high-freq",          1.0, 3.0,  4.0,  2632.0, -894.0),  # 272tr, R1 short_X
    ("EX4", "mid-X high-freq",           70.0, 3.0,  6.0,  2569.0, -839.0),  # 209tr, R1 asym_SY
]

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_eth_xtreme_hourly_oos_champion_select_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(payload):
    out = OUTPUT_DIR / "final_params_eth_xtreme_hourly_oos_champion_select.json"
...
def run(conn, from_csv, only_candidate, only_period):
    run_t0 = time.time()
    out_path = OUTPUT_DIR / "final_params_eth_xtreme_hourly_oos_champion_select.json"
    ...
    if not from_csv:
        mc.ensure_chart_ready(conn, _cfg("EX1", "is", *CANDIDATES[0][2:5]))
        if not only_period:
    ...
    for period in periods:
        rng = RANGES[period]
        log.info("==============================================================")
        log.info("  ETH Hourly XtremeStop — OOS CHAMPION SELECTION  [PASS: %s]  range %s ~ %s",
                 period, rng[0], rng[1])
        log.info("==============================================================")
        try:
...
def main():
    ap = argparse.ArgumentParser(description="ETH Hourly XtremeStop OOS champion selection (auto-range merged)")
    ap.add_argument("--from-csv", action="store_true")
    ap.add_argument("--candidate", metavar="EXn", default=None)
