"""
search_gc_daily3.py — Breakout Daily NP > 700,000 on CME.GC HOT, Round 3
...
R2 precision findings (A10 dense zoom):
  STP profile at LE=4, SE=49:  0.6→286K / 0.7→309K / 0.8→307K / 0.9→312K / 1.0→309K / 1.1→306K
  LMT profile at STP≤1.0:      LMT=7→312K  LMT=8→249K  LMT=6→245K  (very sharp spike at LMT=7)
  SE plateau: 49-52 all give 312K; SE=46-48 give 309K; SE=41-45 unknown

Round-3 strategy — probe remaining uncertainty zones:
  1. Fine LMT around 7 (step 0.5) — is 6.5 or 7.5 better?
  2. SE below 46 fine — does the rise continue below 46?
  3. Low STP 0.3-0.7 precision — is the dropoff at 0.6 gradual or sharp?
  4. LE fine around 3-6 with optimal STP/LMT
  5. High LMT with tight STP — does LMT=10-50 at STP=0.9 do better?

Attempt schedule (12 attempts, ≤5,000 combos each):
  A01  LMT fine 6-10 s0.5:  LE(3-5 s1) × SE(46-52 s2) × STP(0.7-1.1 s0.1) × LMT(6-10 s0.5)  3×4×5×9=540
  A02  SE<46 fine:          LE(3-6 s1) × SE(30-46 s2)  × STP(0.7-1.1 s0.1) × LMT(5-11 s2)   4×9×5×4=720
  A03  Low STP 0.3-0.7:     LE(2-6 s1) × SE(44-54 s2)  × STP(0.3-0.7 s0.1) × LMT(5-11 s2)   5×6×5×4=600
  A04  LE fine 1-7:         LE(1-7 s1) × SE(46-52 s2)  × STP(0.7-1.1 s0.1) × LMT(6-10 s2)   7×4×5×3=420
  A05  Combined precision:  LE(3-5 s1) × SE(46-52 s1)  × STP(0.7-1.1 s0.1) × LMT(6-10 s1)   3×7×5×5=525
  A06  High LMT tight STP:  LE(3-5 s1) × SE(46-52 s2)  × STP(0.7-1.5 s0.2) × LMT(10-50 s5)  3×4×5×9=540
  A07  Adaptive zoom
  A08  SE 20-46 fine:       LE(3-5 s1) × SE(20-46 s2)  × STP(0.7-1.5 s0.4) × LMT(5-11 s2)   3×14×3×4=504
  A09  LE=4 SE=40-55 deep:  LE(2-6 s1) × SE(40-55 s1)  × STP(0.7-1.1 s0.1) × LMT(6-10 s2)   5×16×5×3=1200
  A10  Dense zoom (adaptive)
  A11  Sub-zone precision:  LE(3-6 s1) × SE(44-54 s1)  × STP(0.6-1.2 s0.1) × LMT(6-10 s2)   4×11×7×3=924
  A12  Wide boundary R3:    LE(1-9 s2) × SE(15-60 s5)  × STP(0.5-3.0 s0.5) × LMT(3-25 s2)   5×10×6×12=3600
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\gc_daily3_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_gc_daily3_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def csv_for(name: str) -> Path:
    return OUTPUT_DIR / f"GCD3_{name}_raw.csv"
...
def run_or_load(name, cfg, conn, from_csv):
    ...
    if from_csv or csv_path.exists():
        ...
        return None
    log.info("=== Starting GCD3_%s (%d combos) ===", name, cfg.total_runs())
    t0 = time.time()
    try:
...
def save_json(best, log_, met):
    ...
    payload = {
        "strategy":           "Breakout_GC_Daily  (target NP>700K round-3)",
        "symbol":             SYMBOL,
    ...
    out = OUTPUT_DIR / "final_params_gc_daily3.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  GC Daily Breakout NP>700K Round-3 — precision fine-tuning")
    log.info("  R2 best: LE=%.4g SE=%.4g STP=%.4g LMT=%.4g  NP=%.0f  Obj=%.0f",
             SEED_LE, SEED_SE, SEED_STP, SEED_LMT, SEED_NP, SEED_OBJ)
    log.info("  STP sweet spot: 0.9 (below 0.7 drops, above 1.0 drops)")
    log.info("  LMT=7 sharp spike (LMT=8 loses 60K)")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A01  Fine LMT 6-10 step 0.5  (3×4×5×9=540)
    # ──────────────────────────────────────────────────────────────────────
    A = 1
    _n = "01_lmt_fine"
    _c = _cfg(_n, (3, 5, 1), (46, 52, 2), (0.7, 1.1, 0.1), (6, 10, 0.5))
    log.info("A01  LE(3-5 s1) × SE(46-52 s2) × STP(0.7-1.1 s0.1) × LMT(6-10 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  SE below 46 fine step 2  (4×9×5×4=720)
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_se_below46"
    _c = _cfg(_n, (3, 6, 1), (30, 46, 2), (0.7, 1.1, 0.1), (5, 11, 2))
    log.info("A02  LE(3-6 s1) × SE(30-46 s2) × STP(0.7-1.1 s0.1) × LMT(5-11 s2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  Low STP 0.3-0.7  (5×6×5×4=600)
    # ──────────────────────────────────────────────────────────────────────
    A = 3
    _n = "03_low_stp"
    _c = _cfg(_n, (2, 6, 1), (44, 54, 2), (0.3, 0.7, 0.1), (5, 11, 2))
    log.info("A03  LE(2-6 s1) × SE(44-54 s2) × STP(0.3-0.7 s0.1) × LMT(5-11 s2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  LE fine 1-7  (7×4×5×3=420)
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_le_fine"
    _c = _cfg(_n, (1, 7, 1), (46, 52, 2), (0.7, 1.1, 0.1), (6, 10, 2))
    log.info("A04  LE(1-7 s1) × SE(46-52 s2) × STP(0.7-1.1 s0.1) × LMT(6-10 s2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  Combined precision LE+SE+STP+LMT step 1  (3×7×5×5=525)
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_combined_precision"
    _c = _cfg(_n, (3, 5, 1), (46, 52, 1), (0.7, 1.1, 0.1), (6, 10, 1))
    log.info("A05  LE(3-5 s1) × SE(46-52 s1) × STP(0.7-1.1 s0.1) × LMT(6-10 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  High LMT 10-50 with tight STP  (3×4×5×9=540)
    # ──────────────────────────────────────────────────────────────────────
    A = 6
    _n = "06_high_lmt"
    _c = _cfg(_n, (3, 5, 1), (46, 52, 2), (0.7, 1.5, 0.2), (10, 50, 5))
    log.info("A06  LE(3-5 s1) × SE(46-52 s2) × STP(0.7-1.5 s0.2) × LMT(10-50 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  Adaptive zoom (progressive shrink)
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_zoom_best"
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  SE=20-46 fine step 2  (3×14×3×4=504)
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_se_20_46"
    _c = _cfg(_n, (3, 5, 1), (20, 46, 2), (0.7, 1.5, 0.4), (5, 11, 2))
    log.info("A08  LE(3-5 s1) × SE(20-46 s2) × STP(0.7-1.5 s0.4) × LMT(5-11 s2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A09  LE=4 SE=40-55 deep step 1  (5×16×5×3=1200)
    # ──────────────────────────────────────────────────────────────────────
    A = 9
    _n = "09_le4_se40_55"
    _c = _cfg(_n, (2, 6, 1), (40, 55, 1), (0.7, 1.1, 0.1), (6, 10, 2))
    log.info("A09  LE(2-6 s1) × SE(40-55 s1) × STP(0.7-1.1 s0.1) × LMT(6-10 s2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A11  Sub-zone precision LE=3-6, SE=44-54  (4×11×7×3=924)
    # ──────────────────────────────────────────────────────────────────────
    A = 11
    _n = "11_subzone_precision"
    _c = _cfg(_n, (3, 6, 1), (44, 54, 1), (0.6, 1.2, 0.1), (6, 10, 2))
    log.info("A11  LE(3-6 s1) × SE(44-54 s1) × STP(0.6-1.2 s0.1) × LMT(6-10 s2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A12  Wide boundary R3  (5×10×6×12=3600)
    # ──────────────────────────────────────────────────────────────────────
    A = 12
    _n = "12_wide_boundary"
    _c = _cfg(_n, (1, 9, 2), (15, 60, 5), (0.5, 3.0, 0.5), (3, 25, 2))
    log.info("A12  LE(1-9 s2) × SE(15-60 s5) × STP(0.5-3.0 s0.5) × LMT(3-25 s2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    log.info("")
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  FINAL — GC Daily Breakout NP>700K Round-3")
    log.info("══════════════════════════════════════════════════════════════")
...
def main():
    ap = argparse.ArgumentParser(description="GC Daily Breakout NP>700K Round-3 search")
