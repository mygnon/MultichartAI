"""
search_gc_hunter2_hourly.py — SFJ_HUNTER2_NQ on CME.GC HOT Hourly, Round 1
...
Workspace: 20260101_SFJ_HUNTER_AI.wsp (CME.GC HOT Hourly chart with SFJ_HUNTER2_NQ applied)
Target: NP > 700,000 USD, maximise NP²/|MDD|

Reference:
  NQ Hourly: LEN_L=8, LEN_S=89, ATR_L=0.25, ATR_S=2.5 → $634,865 (883 trades/7yr)
  TXF Hourly: LEN_L=15, LEN_S=290, ATR_L=0.97, ATR_S=1.5 → 9.12M TWD (574 trades/7yr)
  NQ Daily: LEN_L=6, LEN_S=85, ATR_L=0.08, ATR_S=1.15 → $433,700 (72 trades/7yr)
  GC trades ~23h/day; 1-entry/day limit; potentially 500-2000 trades over 7yr.
...
Attempt schedule (11 attempts, ≤5,000 combos each):
  ...
  A02 nq_analog    : LEN_L(3-15 s2)×LEN_S(60-110 s5)×ATR_L(0.1-0.5 s0.1)×ATR_S(1.5-3.5 s0.5)   = 7×11×5×5=1925
  A03 txf_analog   : LEN_L(10-20 s2)×LEN_S(200-300 s20)×ATR_L(0.5-1.5 s0.2)×ATR_S(0.5-2.5 s0.5) = 6×6×6×5=1080
  A04 short_lenl   : LEN_L(1-10 s1)×LEN_S(30-200 s30)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  = 10×6×5×5=1500
  A05 medium_ma    : LEN_L(10-50 s5)×LEN_S(100-400 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 9×7×5×5=1575
  A06 tight_atrl   : LEN_L(2-20 s2)×LEN_S(50-300 s50)×ATR_L(0.02-0.2 s0.02)×ATR_S(0.5-2.5 s0.5)  = 10×6×10×5=3000
  A07 high_atrl    : LEN_L(5-25 s5)×LEN_S(50-300 s50)×ATR_L(2.0-8.0 s1.0)×ATR_S(1.0-4.0 s1.0)    = 5×6×7×4=840
  A08 asym_long_ls : LEN_L(1-10 s1)×LEN_S(200-500 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  = 10×7×5×5=1750
  A09 adaptive_zoom1: fixed — LL±3 LS±5 step=1, ATR_L±0.1 step=0.05, ATR_S±0.3 step=0.1 → ≤7×11×5×7=2695
  A10 adaptive_zoom2: fixed — LL±2 LS±3 step=1, ATR_L±0.05 step=0.025, ATR_S±0.2 step=0.05 → ≤5×7×5×9=1575
  A11 adaptive_zoom3: fixed — LL±2 LS±3 step=1, ATR_L±0.05 step=0.01, ATR_S±0.15 step=0.05 → ≤5×7×11×7=2695
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\gc_hunter2_hourly_search")
...
# Seeded from R1 A01 global_wide champion (A01 CSV already exists)
SEED_LL   = 10.0
SEED_LS   = 80.0
SEED_ATRL = 1.1
SEED_ATRS = 4.5
...
_LOG_FILE = OUTPUT_DIR / f"search_gc_hunter2_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "references": "NQ Hourly ceiling $634,865 (LEN_L=8 LEN_S=89 ATR_L=0.25 ATR_S=2.5 883 trades); TXF Hourly met 9.12M (LEN_L=15 LEN_S=290 ATR_L=0.97 ATR_S=1.5 574 trades)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_gc_hunter2_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_NQ on CME.GC HOT Hourly — Round 1")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A01  global_wide — broad landscape scan
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  nq_analog — probe NQ-like regime: short LEN_L, LEN_S≈60-110, moderate ATR
    #      LEN_L(3-15 s2)×LEN_S(60-110 s5)×ATR_L(0.1-0.5 s0.1)×ATR_S(1.5-3.5 s0.5) = 7×11×5×5=1925
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_nq_analog"
    _c = _cfg(_n, (3, 15, 2), (60, 110, 5), (0.1, 0.5, 0.1), (1.5, 3.5, 0.5))
    log.info("A02  LEN_L(3-15 s2)×LEN_S(60-110 s5)×ATR_L(0.1-0.5 s0.1)×ATR_S(1.5-3.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  txf_analog — probe TXF-like regime: medium LEN_L, long LEN_S, moderate ATR_L
    #      LEN_L(10-20 s2)×LEN_S(200-300 s20)×ATR_L(0.5-1.5 s0.2)×ATR_S(0.5-2.5 s0.5) = 6×6×6×5=1080
    # ──────────────────────────────────────────────────────────────────────
    A = 3
    _n = "03_txf_analog"
    _c = _cfg(_n, (10, 20, 2), (200, 300, 20), (0.5, 1.5, 0.2), (0.5, 2.5, 0.5))
    log.info("A03  LEN_L(10-20 s2)×LEN_S(200-300 s20)×ATR_L(0.5-1.5 s0.2)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  short_lenl — short LEN_L (1-10 step=1) × broad LEN_S range
    #      LEN_L(1-10 s1)×LEN_S(30-200 s30)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 10×6×5×5=1500
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_short_lenl"
    _c = _cfg(_n, (1, 10, 1), (30, 200, 30), (0.1, 1.1, 0.25), (0.5, 2.5, 0.5))
    log.info("A04  LEN_L(1-10 s1)×LEN_S(30-200 s30)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  medium_ma — medium LEN_L (10-50) with long LEN_S range
    #      LEN_L(10-50 s5)×LEN_S(100-400 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 9×7×5×5=1575
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_medium_ma"
    _c = _cfg(_n, (10, 50, 5), (100, 400, 50), (0.1, 1.1, 0.25), (0.5, 2.5, 0.5))
    log.info("A05  LEN_L(10-50 s5)×LEN_S(100-400 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  tight_atrl — tight ATR_L regime (small entry stop multiplier)
    ...
    log.info("A06  LEN_L(2-20 s2)×LEN_S(50-300 s50)×ATR_L(0.02-0.2 s0.02)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  high_atrl — high ATR_L regime (wide entry stop, momentum breakout)
    #      LEN_L(5-25 s5)×LEN_S(50-300 s50)×ATR_L(2.0-8.0 s1.0)×ATR_S(1.0-4.0 s1.0) = 5×6×7×4=840
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_high_atrl"
    _c = _cfg(_n, (5, 25, 5), (50, 300, 50), (2.0, 8.0, 1.0), (1.0, 4.0, 1.0))
    log.info("A07  LEN_L(5-25 s5)×LEN_S(50-300 s50)×ATR_L(2.0-8.0 s1.0)×ATR_S(1.0-4.0 s1.0)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  asym_long_ls — asymmetric: short LEN_L, very long LEN_S (rare short entries)
    #      LEN_L(1-10 s1)×LEN_S(200-500 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 10×7×5×5=1750
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_asym_long_ls"
    _c = _cfg(_n, (1, 10, 1), (200, 500, 50), (0.1, 1.1, 0.25), (0.5, 2.5, 0.5))
    log.info("A08  LEN_L(1-10 s1)×LEN_S(200-500 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A09  adaptive_zoom1 — LL±3 LS±5 step=1, ATR_L±0.1 step=0.05, ATR_S±0.3 step=0.1
    #      ≤ 7×11×5×7 = 2695 combos
    # ──────────────────────────────────────────────────────────────────────
    A = 9
    ...
    if start_attempt <= A:
        ...
        _atrs = zoom(best_atrs, 0.3, 0.1,  AS_LO, AS_HI)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A10  adaptive_zoom2 — LL±2 LS±3 step=1, ATR_L±0.05 step=0.025, ATR_S±0.2 step=0.05
    #      ≤ 5×7×5×9 = 1575 combos
    # ──────────────────────────────────────────────────────────────────────
    A = 10
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A11  adaptive_zoom3 — finest: LL±2 LS±3 step=1, ATR_L±0.05 step=0.01, ATR_S±0.15 step=0.05
    #      ≤ 5×7×11×7 = 2695 combos
    # ──────────────────────────────────────────────────────────────────────
    A = 11
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_NQ CME.GC HOT Hourly Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_NQ CME.GC HOT Hourly NP>700K USD Round-1 search")
