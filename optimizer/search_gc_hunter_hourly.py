"""
search_gc_hunter_hourly.py — SFJ_HUNTER_NQ on CME.GC HOT Hourly, Round 1
...
GC context:
  - GC (Gold) 100 oz/contract; ~$2500/oz → $250K face value
  - ATR(20) hourly ≈ $1,000–$2,000/contract; 2×ATR entry = ~$2K–$4K above prior close
  - NQ champion: STP=$550, LMT=$13,500 (R/R≈24.5:1) → GC will likely be in similar USD range
  - TXF champion: STP≈$730, LMT≈$19,400 USD-equivalent (R/R≈27:1)
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\gc_hunter_hourly_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_gc_hunter_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_gc_hunter_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER_NQ on CME.GC HOT Hourly — Round 1")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A01  global_wide — coarse survey of moderate USD STP/LMT range
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  short_len — small MA period (likely inert for LEN≤~10)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  tight_exits — very tight USD stop/target (high-frequency scalping)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  wide_exits — large USD stop/target (low-frequency trend-following)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  high_reward — low STP + high LMT (TXF/NQ-style R/R≈25:1)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  medium_fine — moderate finer grid (LMT step=1K)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  large_lmt — large LMT territory ($20K–$220K), low-frequency
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER_NQ GC Hourly Round-1 COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(
        description="SFJ_HUNTER_NQ CME.GC HOT Hourly R1 parameter search")
