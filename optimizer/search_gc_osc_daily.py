"""
search_gc_osc_daily.py — _2021Basic_Osc_NQ on CME.GC HOT Daily, Round 1
...
Workspace: needs CME.GC HOT DAILY chart with _2021Basic_Osc_NQ signal applied.

NOTE: Update WORKSPACE below to match your MC64 workspace file.
      The workspace must be OPEN in MC64 before running with a GC HOT DAILY chart.
      Only one CME.GC HOT chart should be visible in the workspace (daily).

Target: NP > 400,000 USD
Period: 2019/01/01 – 2026/01/01

Reference results:
  NQ Daily champion:  LEN=12 LE=0.1  SE=2.5 STP=0.5  LMT=25.5 NP=$456,190
  GC Hourly champion: LEN=2  LE=-0.75 SE=1.0 STP=1.6  LMT=30   NP=$312,250

R1 design — 8 fixed attempts + 3 adaptive zooms (11 total), ≤5,000 combos each:
  A01 global_d1  : LEN(2-18 s4)  × LE(-1-1 s0.5)    × SE(0.5-3.5 s1)  × STP(0.5-2 s0.5)   × LMT(5-30 s5)   = 5×5×4×4×6  = 2,400
  A02 short_len  : LEN(1-6 s1)   × LE(-1-0.5 s0.5)  × SE(0.5-2.5 s0.5)× STP(0.5-2 s0.5)   × LMT(10-30 s5)  = 6×4×5×4×5  = 2,400
  A03 high_len   : LEN(8-20 s2)  × LE(-0.5-1 s0.5)  × SE(1.5-3 s0.5)  × STP(0.3-1.3 s0.25) × LMT(15-30 s5)  = 7×4×4×5×4  = 2,240
  A04 tight_stp  : LEN(2-14 s3)  × LE(-0.5-1 s0.5)  × SE(1-3 s0.5)    × STP(0.1-1.0 s0.1)  × LMT(15-30 s5)  = 5×4×5×10×4 = 4,000
  A05 high_lmt   : LEN(2-22 s5)  × LE(-0.5-1 s0.5)  × SE(1-3 s1)      × STP(0.5-1.5 s0.5)  × LMT(30-80 s10) = 5×4×3×3×6  = 1,080
  A06 wide_se    : LEN(3-15 s3)  × LE(-1-0.5 s0.5)  × SE(3-8 s1)      × STP(0.5-2 s0.5)    × LMT(10-25 s5)  = 5×4×6×4×4  = 1,920
  A07 pos_le     : LEN(5-20 s5)  × LE(0-1.5 s0.25)  × SE(1.5-3 s0.5)  × STP(0.3-1.3 s0.25) × LMT(15-30 s5)  = 4×7×4×5×4  = 2,240
  A08 global_d2  : LEN(3-33 s6)  × LE(-2-2 s1)      × SE(0.5-4.5 s1)  × STP(0.25-1.75 s0.5) × LMT(5-35 s10) = 6×5×5×4×4  = 2,400
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\gc_osc_daily_search")
...
# Seed — NQ daily champion as starting point; no prior GC daily result
SEED_LEN, SEED_LE  = 12.0,  0.1
...
_LOG_FILE = OUTPUT_DIR / f"search_gc_osc_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "exits":      "STP×ATR(10) stop + LMT×ATR(10) limit",
            "r1_focus":   "Global sweep on daily bars; explores short/long LEN, high/low LMT, wide SE, tight STP, positive LE",
            "nq_daily_ref": "NQ Daily ceiling: LEN=12 LE=0.1 SE=2.5 STP=0.5 LMT=25.5 NP=$456,190",
            "gc_hourly_ref": "GC Hourly ceiling: LEN=2 LE=-0.75 SE=1 STP=1.6 LMT=30 NP=$312,250",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_gc_osc_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  GC Daily _2021Basic_Osc_NQ  NP>400K — Round 1")
    log.info("  Symbol: %s  Signal: %s  Timeframe: Daily", SYMBOL, SIGNAL)
    log.info("  Seed: LEN=%.4g LE=%.4g SE=%.4g STP=%.4g LMT=%.4g (NQ daily ref)",
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A01  global_d1 — wide initial sweep
    #      LEN(2-18 s4) × LE(-1-1 s0.5) × SE(0.5-3.5 s1) × STP(0.5-2 s0.5) × LMT(5-30 s5)
    #      = 5×5×4×4×6 = 2,400
    # ──────────────────────────────────────────────────────────────────────
    A = 1
    _n = "01_global_d1"
    _c = _cfg(_n, (2, 18, 4), (-1.0, 1.0, 0.5), (0.5, 3.5, 1.0), (0.5, 2.0, 0.5), (5, 30, 5))
    log.info("A01  LEN(2-18 s4)×LE(-1-1 s0.5)×SE(0.5-3.5 s1)×STP(0.5-2 s0.5)×LMT(5-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  short_len — GC hourly champion was LEN=2; test on daily
    #      LEN(1-6 s1) × LE(-1-0.5 s0.5) × SE(0.5-2.5 s0.5) × STP(0.5-2 s0.5) × LMT(10-30 s5)
    #      = 6×4×5×4×5 = 2,400
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_short_len"
    _c = _cfg(_n, (1, 6, 1), (-1.0, 0.5, 0.5), (0.5, 2.5, 0.5), (0.5, 2.0, 0.5), (10, 30, 5))
    log.info("A02  LEN(1-6 s1)×LE(-1-0.5 s0.5)×SE(0.5-2.5 s0.5)×STP(0.5-2 s0.5)×LMT(10-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  high_len — NQ daily champion was LEN=12; explore 8-20
    #      LEN(8-20 s2) × LE(-0.5-1 s0.5) × SE(1.5-3 s0.5) × STP(0.3-1.3 s0.25) × LMT(15-30 s5)
    #      = 7×4×4×5×4 = 2,240
    # ──────────────────────────────────────────────────────────────────────
    A = 3
    _n = "03_high_len"
    _c = _cfg(_n, (8, 20, 2), (-0.5, 1.0, 0.5), (1.5, 3.0, 0.5), (0.3, 1.3, 0.25), (15, 30, 5))
    log.info("A03  LEN(8-20 s2)×LE(-0.5-1 s0.5)×SE(1.5-3 s0.5)×STP(0.3-1.3 s0.25)×LMT(15-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  tight_stp — NQ daily needed tight STP=0.5; test 0.1-1.0
    #      LEN(2-14 s3) × LE(-0.5-1 s0.5) × SE(1-3 s0.5) × STP(0.1-1.0 s0.1) × LMT(15-30 s5)
    #      = 5×4×5×10×4 = 4,000
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_tight_stp"
    _c = _cfg(_n, (2, 14, 3), (-0.5, 1.0, 0.5), (1.0, 3.0, 0.5), (0.1, 1.0, 0.1), (15, 30, 5))
    log.info("A04  LEN(2-14 s3)×LE(-0.5-1 s0.5)×SE(1-3 s0.5)×STP(0.1-1.0 s0.1)×LMT(15-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  high_lmt — test LMT=30-80 (large ATR profit targets)
    #      LEN(2-22 s5) × LE(-0.5-1 s0.5) × SE(1-3 s1) × STP(0.5-1.5 s0.5) × LMT(30-80 s10)
    #      = 5×4×3×3×6 = 1,080
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_high_lmt"
    _c = _cfg(_n, (2, 22, 5), (-0.5, 1.0, 0.5), (1.0, 3.0, 1.0), (0.5, 1.5, 0.5), (30, 80, 10))
    log.info("A05  LEN(2-22 s5)×LE(-0.5-1 s0.5)×SE(1-3 s1)×STP(0.5-1.5 s0.5)×LMT(30-80 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  wide_se — wider BB (SE=3-8), fewer but bigger trades
    #      LEN(3-15 s3) × LE(-1-0.5 s0.5) × SE(3-8 s1) × STP(0.5-2 s0.5) × LMT(10-25 s5)
    #      = 5×4×6×4×4 = 1,920
    # ──────────────────────────────────────────────────────────────────────
    A = 6
    _n = "06_wide_se"
    _c = _cfg(_n, (3, 15, 3), (-1.0, 0.5, 0.5), (3.0, 8.0, 1.0), (0.5, 2.0, 0.5), (10, 25, 5))
    log.info("A06  LEN(3-15 s3)×LE(-1-0.5 s0.5)×SE(3-8 s1)×STP(0.5-2 s0.5)×LMT(10-25 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  pos_le — positive LE (NQ daily LE=0.1 was optimal); fine step
    #      LEN(5-20 s5) × LE(0-1.5 s0.25) × SE(1.5-3 s0.5) × STP(0.3-1.3 s0.25) × LMT(15-30 s5)
    #      = 4×7×4×5×4 = 2,240
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_pos_le"
    _c = _cfg(_n, (5, 20, 5), (0.0, 1.5, 0.25), (1.5, 3.0, 0.5), (0.3, 1.3, 0.25), (15, 30, 5))
    log.info("A07  LEN(5-20 s5)×LE(0-1.5 s0.25)×SE(1.5-3 s0.5)×STP(0.3-1.3 s0.25)×LMT(15-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    #      LEN(3-33 s6) × LE(-2-2 s1) × SE(0.5-4.5 s1) × STP(0.25-1.75 s0.5) × LMT(5-35 s10)
    #      = 6×5×5×4×4 = 2,400
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_global_d2"
    _c = _cfg(_n, (3, 33, 6), (-2.0, 2.0, 1.0), (0.5, 4.5, 1.0), (0.25, 1.75, 0.5), (5, 35, 10))
    log.info("A08  LEN(3-33 s6)×LE(-2-2 s1)×SE(0.5-4.5 s1)×STP(0.25-1.75 s0.5)×LMT(5-35 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
...
def main():
    ap = argparse.ArgumentParser(description="GC Daily _2021Basic_Osc_NQ NP>400K Round-1 search")
