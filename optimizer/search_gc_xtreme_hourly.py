"""
search_gc_xtreme_hourly.py — SFJ_XtremeStop_NQ on CME.GC HOT Hourly, Round 1
...
Target: NP > 800,000 USD on CME.GC HOT Hourly
IS period: 2019/01/01 - 2026/01/01 (7 years, ~40,000 hourly bars)

Cross-instrument knowledge:
  NQ Hourly: X=10 LY=1.48 SY=2.18 NP=624,015 (ceiling; tight X, tight pct)
  TXF Hourly: X=63 LY=5.155 SY=5.79 NP=5,411,000 TWD (ceiling; mid X, high pct, 11 trades)
  NQ Daily: X=9 LY=0.025 SY=3.7 NP=491,590 (ceiling; near-zero LY regime)
  TXF Daily: X=1 LY=3.1 SY=4.445 NP=4,820,400 TWD (ceiling; X=1, moderate pct)

Attempt schedule (11 attempts, <=5,000 combos each):
  A01 global_wide    : X(5-1005 s100)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5)  = 11x16x16=2816
  A02 large_x        : X(1000-5000 s250)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5) = 17x16x16=4352
  A03 small_x        : X(1-100 s5)xLY(0.5-7 s0.5)xSY(0.5-7 s0.5)    = 20x14x14=3920
  A04 tight_pct      : X(5-1005 s100)xLY(0.1-3 s0.2)xSY(0.1-3 s0.2) = 11x15x15=2475
  A05 high_pct       : X(5-1005 s100)xLY(5-15 s0.5)xSY(5-15 s0.5)   = 11x21x21=4851
  A06 medium_fine    : X(50-500 s25)xLY(0.5-7 s0.5)xSY(0.5-7 s0.5)  = 19x14x14=3724
  A07 asym_ly        : X(5-505 s50)xLY(1-12 s0.5)xSY(0.5-4 s0.5)    = 11x23x8=2024
  A08 asym_sy        : X(5-505 s50)xLY(0.5-4 s0.5)xSY(1-12 s0.5)    = 11x8x23=2024
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\gc_xtreme_hourly_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_gc_xtreme_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "cross_instrument": (
                "NQ Hourly: X=10 LY=1.48 SY=2.18 (ceiling); "
                "TXF Hourly: X=63 LY=5.155 SY=5.79 (ceiling); "
                "NQ Daily: X=9 LY=0.025 SY=3.7 (ceiling)"
            ),
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_gc_xtreme_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("=" * 62)
    log.info("  SFJ_XtremeStop_NQ on CME.GC HOT Hourly -- Round 1")
    ...
    # -----------------------------------------------------------------------
    # A01  global_wide — X(5-1005 s100)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5)=2816
    # -----------------------------------------------------------------------
    A = 1
    _n = "01_global_wide"
    ...
    # -----------------------------------------------------------------------
    # A02  large_x — X(1000-5000 s250)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5)=4352
    # -----------------------------------------------------------------------
    A = 2
    _n = "02_large_x"
    _c = _cfg(_n, (1000, 5000, 250), (0.5, 8.0, 0.5), (0.5, 8.0, 0.5))
    log.info("A02  X(1000-5000 s250)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A03  small_x — X(1-100 s5)xLY(0.5-7 s0.5)xSY(0.5-7 s0.5)=3920
    # -----------------------------------------------------------------------
    A = 3
    _n = "03_small_x"
    _c = _cfg(_n, (1, 100, 5), (0.5, 7.0, 0.5), (0.5, 7.0, 0.5))
    log.info("A03  X(1-100 s5)xLY(0.5-7 s0.5)xSY(0.5-7 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A04  tight_pct — X(5-1005 s100)xLY(0.1-3 s0.2)xSY(0.1-3 s0.2)=2475
    # -----------------------------------------------------------------------
    A = 4
    _n = "04_tight_pct"
    _c = _cfg(_n, (5, 1005, 100), (0.1, 3.0, 0.2), (0.1, 3.0, 0.2))
    log.info("A04  X(5-1005 s100)xLY(0.1-3 s0.2)xSY(0.1-3 s0.2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A05  high_pct — X(5-1005 s100)xLY(5-15 s0.5)xSY(5-15 s0.5)=4851
    # -----------------------------------------------------------------------
    A = 5
    _n = "05_high_pct"
    _c = _cfg(_n, (5, 1005, 100), (5.0, 15.0, 0.5), (5.0, 15.0, 0.5))
    log.info("A05  X(5-1005 s100)xLY(5-15 s0.5)xSY(5-15 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A06  medium_fine — X(50-500 s25)xLY(0.5-7 s0.5)xSY(0.5-7 s0.5)=3724
    # -----------------------------------------------------------------------
    A = 6
    _n = "06_medium_fine"
    _c = _cfg(_n, (50, 500, 25), (0.5, 7.0, 0.5), (0.5, 7.0, 0.5))
    log.info("A06  X(50-500 s25)xLY(0.5-7 s0.5)xSY(0.5-7 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A07  asym_ly — X(5-505 s50)xLY(1-12 s0.5)xSY(0.5-4 s0.5)=2024
    # -----------------------------------------------------------------------
    A = 7
    _n = "07_asym_ly"
    _c = _cfg(_n, (5, 505, 50), (1.0, 12.0, 0.5), (0.5, 4.0, 0.5))
    log.info("A07  X(5-505 s50)xLY(1-12 s0.5)xSY(0.5-4 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A08  asym_sy — X(5-505 s50)xLY(0.5-4 s0.5)xSY(1-12 s0.5)=2024
    # -----------------------------------------------------------------------
    A = 8
    _n = "08_asym_sy"
    _c = _cfg(_n, (5, 505, 50), (0.5, 4.0, 0.5), (1.0, 12.0, 0.5))
    log.info("A08  X(5-505 s50)xLY(0.5-4 s0.5)xSY(1-12 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    log.info("=" * 62)
    log.info("  SFJ_XtremeStop_NQ CME.GC HOT Hourly Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_XtremeStop_NQ CME.GC HOT Hourly NP>800K USD Round-1 search")
