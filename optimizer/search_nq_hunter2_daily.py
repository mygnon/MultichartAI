"""
search_nq_hunter2_daily.py — SFJ_HUNTER2_NQ on CME.NQ HOT Daily, Round 1
...
Workspace: 20260101_SFJ_HUNTER_AI.wsp (CME.NQ HOT Daily chart with SFJ_HUNTER2_NQ applied)
Target: NP > 900,000 USD, maximise NP²/|MDD|

NQ Hourly reference: ceiling $634,865 at LEN_L=8, LEN_S=89, ATR_L=0.25, ATR_S=2.5 (883 trades/7yr)
TXF Daily reference: ceiling 5,393,400 TWD at LEN_L=15, LEN_S=65, ATR_L=0.17, ATR_S=1.1 (79 trades/7yr)
→ Daily bars yield far fewer trades; per-trade NP must compensate. Exploring both tight-ATR and wide-ATR regimes.
...
Attempt schedule (11 attempts, ≤5,000 combos each):
  ...
  A02 short_lenl      : LEN_L(1-15 s1)×LEN_S(30-250 s30)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 15×8×5×5=3000
  A03 medium_ma       : LEN_L(10-60 s5)×LEN_S(50-300 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 11×6×5×5=1650
  A04 txf_analog      : LEN_L(5-25 s4)×LEN_S(30-100 s10)×ATR_L(0.05-0.5 s0.05)×ATR_S(0.5-2.5 s0.5)= 6×8×10×5=2400
  A05 high_atrs       : LEN_L(2-20 s2)×LEN_S(50-300 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(2.0-6.0 s0.5) = 10×6×5×9=2700
  A06 high_atr        : LEN_L(5-30 s5)×LEN_S(50-300 s50)×ATR_L(2.0-8.0 s1.0)×ATR_S(2.0-6.0 s1.0)  = 6×6×7×5=1260
  A07 short_lens      : LEN_L(2-20 s2)×LEN_S(5-80 s10)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)   = 10×9×5×5=2250
  A08 asym_ma         : LEN_L(1-10 s1)×LEN_S(100-400 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 10×7×5×5=1750
  A09 adaptive_zoom1  : fixed — LL±3 LS±5 s1, ATR_L±0.1 s0.05, ATR_S±0.2 s0.1 → ≤7×11×5×5=1925
  A10 adaptive_zoom2  : fixed — LL±2 LS±3 s1, ATR_L±0.05 s0.025, ATR_S±0.15 s0.05 → ≤5×7×5×7=1225
  A11 adaptive_zoom3  : fixed — LL±2 LS±3 s1, ATR_L±0.05 s0.01, ATR_S±0.1 s0.05 → ≤5×7×11×5=1925
"""
...
# Starting from strategy defaults (no prior NQ daily HUNTER2 search)
SEED_LL   = 250.0
SEED_LS   = 250.0
SEED_ATRL = 2.0
SEED_ATRS = 4.0
...
_LOG_FILE = OUTPUT_DIR / f"search_nq_hunter2_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "references": "NQ Hourly ceiling $634,865 (LEN_L=8 LEN_S=89 ATR_L=0.25 ATR_S=2.5 883 trades); TXF Daily ceiling 5.39M TWD (LEN_L=15 LEN_S=65 ATR_L=0.17 ATR_S=1.1 79 trades)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_nq_hunter2_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_NQ on CME.NQ HOT Daily — Round 1")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  short_lenl — short LEN_L (1-15) with moderate LEN_S range
    #      LEN_L(1-15 s1)×LEN_S(30-250 s30)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 15×8×5×5=3000
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_short_lenl"
    _c = _cfg(_n, (1, 15, 1), (30, 250, 30), (0.1, 1.1, 0.25), (0.5, 2.5, 0.5))
    log.info("A02  LEN_L(1-15 s1)×LEN_S(30-250 s30)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  medium_ma — medium LEN_L (10-60) with long LEN_S range
    #      LEN_L(10-60 s5)×LEN_S(50-300 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 11×6×5×5=1650
    # ──────────────────────────────────────────────────────────────────────
    A = 3
    _n = "03_medium_ma"
    _c = _cfg(_n, (10, 60, 5), (50, 300, 50), (0.1, 1.1, 0.25), (0.5, 2.5, 0.5))
    log.info("A03  LEN_L(10-60 s5)×LEN_S(50-300 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  txf_analog — TXF Daily analog: short LEN_L, medium LEN_S, tight ATR_L
    #      LEN_L(5-25 s4)×LEN_S(30-100 s10)×ATR_L(0.05-0.5 s0.05)×ATR_S(0.5-2.5 s0.5) = 6×8×10×5=2400
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_txf_analog"
    _c = _cfg(_n, (5, 25, 4), (30, 100, 10), (0.05, 0.5, 0.05), (0.5, 2.5, 0.5))
    log.info("A04  LEN_L(5-25 s4)×LEN_S(30-100 s10)×ATR_L(0.05-0.5 s0.05)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  high_atrs — high ATR_S regime (wider short entry stop)
    #      LEN_L(2-20 s2)×LEN_S(50-300 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(2.0-6.0 s0.5) = 10×6×5×9=2700
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_high_atrs"
    _c = _cfg(_n, (2, 20, 2), (50, 300, 50), (0.1, 1.1, 0.25), (2.0, 6.0, 0.5))
    log.info("A05  LEN_L(2-20 s2)×LEN_S(50-300 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(2.0-6.0 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  high_atr — both ATRs high (large momentum breakout regime)
    #      LEN_L(5-30 s5)×LEN_S(50-300 s50)×ATR_L(2.0-8.0 s1.0)×ATR_S(2.0-6.0 s1.0) = 6×6×7×5=1260
    # ──────────────────────────────────────────────────────────────────────
    A = 6
    _n = "06_high_atr"
    _c = _cfg(_n, (5, 30, 5), (50, 300, 50), (2.0, 8.0, 1.0), (2.0, 6.0, 1.0))
    log.info("A06  LEN_L(5-30 s5)×LEN_S(50-300 s50)×ATR_L(2.0-8.0 s1.0)×ATR_S(2.0-6.0 s1.0)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  short_lens — short LEN_S regime (faster short-side filter)
    #      LEN_L(2-20 s2)×LEN_S(5-80 s10)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 10×9×5×5=2250
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_short_lens"
    _c = _cfg(_n, (2, 20, 2), (5, 80, 10), (0.1, 1.1, 0.25), (0.5, 2.5, 0.5))
    log.info("A07  LEN_L(2-20 s2)×LEN_S(5-80 s10)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  asym_ma — asymmetric: short LEN_L, very long LEN_S (rare short entries)
    #      LEN_L(1-10 s1)×LEN_S(100-400 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5) = 10×7×5×5=1750
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_asym_ma"
    _c = _cfg(_n, (1, 10, 1), (100, 400, 50), (0.1, 1.1, 0.25), (0.5, 2.5, 0.5))
    log.info("A08  LEN_L(1-10 s1)×LEN_S(100-400 s50)×ATR_L(0.1-1.1 s0.25)×ATR_S(0.5-2.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_NQ CME.NQ HOT Daily Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_NQ CME.NQ HOT Daily NP>900K USD Round-1 search")
