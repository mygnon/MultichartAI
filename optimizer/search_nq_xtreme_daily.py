"""
search_nq_xtreme_daily.py — SFJ_XtremeStop_NQ on CME.NQ HOT Daily, Round 1

Strategy logic:
  BUY NEXT BAR C[X]*(1+LY*0.01) STOP
  SELLSHORT NEXT BAR C[X]*(1-SY*0.01) STOP
  Reference price = close X bars ago. Reversal exits only — no STP or LMT.
  3 params: X (lookback days), LY (long entry %), SY (short entry %)
  Defaults: X=150, LY=3.25, SY=3.25

Cross-instrument knowledge:
  NQ Hourly ceiling: X=10 LY=1.48 SY=2.18 NP=624,015 USD (R2 confirmed; tight pct; 350 trades/7yr)
  TXF Daily ceiling: X=1  LY=3.1  SY=4.445 NP=4,820,400 TWD (R3 confirmed; very short X)
  TXF Hourly ceiling: X=63 LY=5.155 SY=5.79 (low-trade regime, 11/7yr)
  --> NQ Daily: expect X=1-50 range, LY/SY=1-8% to be most productive
      High pct (5-15%) viable on daily (NQ can move 5%+ in a day; unlike hourly where it fails)
      SY>LY asymmetry confirmed across all XtremeStop instruments

INSAMPLE: 2019/01/01-2026/01/01 (~7 years, ~1,764 NQ HOT daily bars)

Attempt schedule (11 attempts, <=5,000 combos each):
  A01 global_wide   : X(5-1005 s100)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5)  = 11x16x16=2816
  A02 small_x_fine  : X(1-50 s2)xLY(1-6 s0.5)xSY(1-6 s0.5)           = 25x11x11=3025
  A03 x_unit        : X(1-20 s1)xLY(1-8 s0.5)xSY(1-8 s0.5)            = 20x15x15=4500
  A04 tight_pct     : X(1-50 s5)xLY(0.5-5 s0.25)xSY(0.5-5 s0.25)      = 10x19x19=3610
  A05 high_pct      : X(1-50 s5)xLY(5-15 s0.5)xSY(5-15 s0.5)          = 10x21x21=4410
  A06 medium_x      : X(50-500 s25)xLY(1-8 s0.5)xSY(1-8 s0.5)         = 19x15x15=4275
  A07 asym_ly       : X(1-100 s5)xLY(0.5-8 s0.5)xSY(0.5-4 s0.5)       = 20x16x8=2560
  A08 asym_sy       : X(1-100 s5)xLY(0.5-4 s0.5)xSY(0.5-8 s0.5)       = 20x8x16=2560
...
# Strategy defaults as seed (no prior NQ daily XtremeStop search)
SEED_X  = 150.0
SEED_LY = 3.25
SEED_SY = 3.25
SEED_NP = 0.0
...
_LOG_FILE = OUTPUT_DIR / f"search_nq_xtreme_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "defaults": "X=150, LY=3.25, SY=3.25",
            "r1_focus": "Global landscape: X=1-1005, LY/SY=0.5-15%; tight/wide/asym/high-pct sweeps",
            "cross_instrument": "TXF Daily: X=1 LY=3.1 SY=4.445 (ceiling); NQ Hourly: X=10 LY=1.48 SY=2.18 (ceiling)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_nq_xtreme_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("=" * 62)
    log.info("  SFJ_XtremeStop_NQ on CME.NQ HOT Daily -- Round 1")
    ...
    log.info("  INSAMPLE: 2019/01/01-2026/01/01 (~1,764 NQ daily bars)")
    log.info("  Cross-ref: TXF Daily X=1 LY=3.1 SY=4.445; NQ Hourly X=10 LY=1.48 SY=2.18")
    log.info("=" * 62)
    ...
    #      X(5-1005 s100)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5) = 11x16x16=2816
    # -----------------------------------------------------------------------
    A = 1
    _n = "01_global_wide"
    _c = _cfg(_n, (5, 1005, 100), (0.5, 8.0, 0.5), (0.5, 8.0, 0.5))
    log.info("A01  X(5-1005 s100)xLY(0.5-8 s0.5)xSY(0.5-8 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # -----------------------------------------------------------------------
    # A02  small_x_fine -- X=1-50 at step=2, fine pct
    #      X(1-50 s2)xLY(1-6 s0.5)xSY(1-6 s0.5) = 25x11x11=3025
    # -----------------------------------------------------------------------
    A = 2
    _n = "02_small_x_fine"
    _c = _cfg(_n, (1, 50, 2), (1.0, 6.0, 0.5), (1.0, 6.0, 0.5))
    log.info("A02  X(1-50 s2)xLY(1-6 s0.5)xSY(1-6 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A03  x_unit -- X=1-20 unit step, wider pct
    #      X(1-20 s1)xLY(1-8 s0.5)xSY(1-8 s0.5) = 20x15x15=4500
    # -----------------------------------------------------------------------
    A = 3
    _n = "03_x_unit"
    _c = _cfg(_n, (1, 20, 1), (1.0, 8.0, 0.5), (1.0, 8.0, 0.5))
    log.info("A03  X(1-20 s1)xLY(1-8 s0.5)xSY(1-8 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A04  tight_pct -- X=1-50 with very tight pct (0.5-5%)
    #      X(1-50 s5)xLY(0.5-5 s0.25)xSY(0.5-5 s0.25) = 10x19x19=3610
    # -----------------------------------------------------------------------
    A = 4
    _n = "04_tight_pct"
    _c = _cfg(_n, (1, 50, 5), (0.5, 5.0, 0.25), (0.5, 5.0, 0.25))
    log.info("A04  X(1-50 s5)xLY(0.5-5 s0.25)xSY(0.5-5 s0.25)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A05  high_pct -- daily moves can reach 5%+; test wide entry threshold
    #      X(1-50 s5)xLY(5-15 s0.5)xSY(5-15 s0.5) = 10x21x21=4410
    # -----------------------------------------------------------------------
    A = 5
    _n = "05_high_pct"
    _c = _cfg(_n, (1, 50, 5), (5.0, 15.0, 0.5), (5.0, 15.0, 0.5))
    log.info("A05  X(1-50 s5)xLY(5-15 s0.5)xSY(5-15 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A06  medium_x -- X=50-500, moderate pct (analog of TXF A01 X=205 best)
    #      X(50-500 s25)xLY(1-8 s0.5)xSY(1-8 s0.5) = 19x15x15=4275
    # -----------------------------------------------------------------------
    A = 6
    _n = "06_medium_x"
    _c = _cfg(_n, (50, 500, 25), (1.0, 8.0, 0.5), (1.0, 8.0, 0.5))
    log.info("A06  X(50-500 s25)xLY(1-8 s0.5)xSY(1-8 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A07  asym_ly -- asymmetric: wide LY, tight SY
    #      X(1-100 s5)xLY(0.5-8 s0.5)xSY(0.5-4 s0.5) = 20x16x8=2560
    # -----------------------------------------------------------------------
    A = 7
    _n = "07_asym_ly"
    _c = _cfg(_n, (1, 100, 5), (0.5, 8.0, 0.5), (0.5, 4.0, 0.5))
    log.info("A07  X(1-100 s5)xLY(0.5-8 s0.5)xSY(0.5-4 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    # A08  asym_sy -- asymmetric: tight LY, wide SY (SY>LY confirmed optimal)
    #      X(1-100 s5)xLY(0.5-4 s0.5)xSY(0.5-8 s0.5) = 20x8x16=2560
    # -----------------------------------------------------------------------
    A = 8
    _n = "08_asym_sy"
    _c = _cfg(_n, (1, 100, 5), (0.5, 4.0, 0.5), (0.5, 8.0, 0.5))
    log.info("A08  X(1-100 s5)xLY(0.5-4 s0.5)xSY(0.5-8 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # -----------------------------------------------------------------------
    log.info("=" * 62)
    log.info("  SFJ_XtremeStop_NQ CME.NQ HOT Daily Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_XtremeStop_NQ CME.NQ HOT Daily NP>800K USD Round-1 search")
