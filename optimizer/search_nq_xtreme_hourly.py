"""
search_nq_xtreme_hourly.py — SFJ_XtremeStop_NQ on CME.NQ HOT Hourly, Round 1
...
Target: NP > 800,000 USD on CME.NQ HOT Hourly
...
_LOG_FILE = OUTPUT_DIR / f"search_nq_xtreme_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_nq_xtreme_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("=" * 62)
    log.info("  SFJ_XtremeStop_NQ on CME.NQ HOT Hourly -- Round 1")
    ...
    # -----------------------------------------------------------------------
    log.info("=" * 62)
    log.info("  SFJ_XtremeStop_NQ CME.NQ HOT Hourly Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_XtremeStop_NQ CME.NQ HOT Hourly NP>800K USD Round-1 search")
