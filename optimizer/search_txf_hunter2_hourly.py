"""
search_txf_hunter2_hourly.py — SFJ_HUNTER2_NQ on TWF.TXF HOT Hourly, Round 1
...
4 active params, target NP > 9,000,000 TWD, ≤5,000 combos/attempt.
...
Attempt schedule (11 attempts):
  ...
  A05 long_ma_fine : LEN_L(100-500 s40)×LEN_S(100-500 s40)×ATR_L(1-5 s1)×ATR_S(1-5 s1) = 11×11×5×5=3025
  A06 fine_atr     : LEN_L(10-250 s60)×LEN_S(10-250 s60)×ATR_L(0.25-3.0 s0.25)×ATR_S(0.25-3.0 s0.25) = 5×5×12×12=3600
  A07 high_atr     : LEN_L(20-220 s50)×LEN_S(20-220 s50)×ATR_L(3-12 s1)×ATR_S(3-12 s1) = 5×5×10×10=2500
  A08 asym_ma      : LEN_L(10-150 s20)×LEN_S(100-400 s50)×ATR_L(0.5-5.5 s1)×ATR_S(0.5-5.5 s1) = 8×7×6×6=2016
  A09 adaptive_zoom1: dynamic from best
  A10 adaptive_zoom2: dynamic from best (tighter)
  A11 adaptive_zoom3: dynamic from best (tightest)
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\txf_hunter2_hourly_search")
...
SEED_LL   = 250.0   # LEN_L default
SEED_LS   = 250.0   # LEN_S default
SEED_ATRL = 2.0     # ATR_multiplier_L default
SEED_ATRS = 4.0     # ATR_multiplier_S default
SEED_NP   = 0.0     # no prior result

PREFIX = "TXFH2H_"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_txf_hunter2_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    ...
    out = OUTPUT_DIR / "final_params_txf_hunter2_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_NQ on TWF.TXF HOT Hourly — Round 1")
    ...
    A = 1
    _n = "01_global_wide"
    _c = _cfg(_n, (10, 250, 60), (10, 250, 60), (0.5, 5.0, 0.5), (0.5, 5.0, 0.5))
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  long_ma — larger MA periods (100-500)
    #      LEN_L(100-500 s50)×LEN_S(100-500 s50)×ATR_L(1-5 s1)×ATR_S(1-5 s1) = 9×9×5×5=2025
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_long_ma"
    _c = _cfg(_n, (100, 500, 50), (100, 500, 50), (1, 5, 1), (1, 5, 1))
    ...
    A = 3
    _n = "03_short_ma"
    _c = _cfg(_n, (5, 95, 10), (5, 95, 10), (0.5, 5.5, 1), (0.5, 5.5, 1))
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  vshort_ma — very short MA periods (3-30), wide ATR range
    #      LEN_L(3-30 s3)×LEN_S(3-30 s3)×ATR_L(0.5-6.5 s1)×ATR_S(0.5-6.5 s1) = 10×10×7×7=4900
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_vshort_ma"
    _c = _cfg(_n, (3, 30, 3), (3, 30, 3), (0.5, 6.5, 1), (0.5, 6.5, 1))
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  long_ma_fine — finer step for long MAs (100-500 s40)
    #      LEN_L(100-500 s40)×LEN_S(100-500 s40)×ATR_L(1-5 s1)×ATR_S(1-5 s1) = 11×11×5×5=3025
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_long_ma_fine"
    _c = _cfg(_n, (100, 500, 40), (100, 500, 40), (1, 5, 1), (1, 5, 1))
    log.info("A05  LEN_L(100-500 s40)×LEN_S(100-500 s40)×ATR_L(1-5 s1)×ATR_S(1-5 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  fine_atr — fine ATR resolution (0.25-3.0 s0.25) at moderate MA
    #      LEN_L(10-250 s60)×LEN_S(10-250 s60)×ATR_L(0.25-3.0 s0.25)×ATR_S(0.25-3.0 s0.25) = 5×5×12×12=3600
    # ──────────────────────────────────────────────────────────────────────
    A = 6
    _n = "06_fine_atr"
    _c = _cfg(_n, (10, 250, 60), (10, 250, 60), (0.25, 3.0, 0.25), (0.25, 3.0, 0.25))
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  high_atr — high ATR multipliers (3-12), moderate MA
    #      LEN_L(20-220 s50)×LEN_S(20-220 s50)×ATR_L(3-12 s1)×ATR_S(3-12 s1) = 5×5×10×10=2500
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_high_atr"
    _c = _cfg(_n, (20, 220, 50), (20, 220, 50), (3, 12, 1), (3, 12, 1))
    log.info("A07  LEN_L(20-220 s50)×LEN_S(20-220 s50)×ATR_L(3-12 s1)×ATR_S(3-12 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  asym_ma — asymmetric MA (short LEN_L 10-150, longer LEN_S 100-400)
    #      LEN_L(10-150 s20)×LEN_S(100-400 s50)×ATR_L(0.5-5.5 s1)×ATR_S(0.5-5.5 s1) = 8×7×6×6=2016
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_asym_ma"
    _c = _cfg(_n, (10, 150, 20), (100, 400, 50), (0.5, 5.5, 1), (0.5, 5.5, 1))
    log.info("A08  LEN_L(10-150 s20)×LEN_S(100-400 s50)×ATR_L(0.5-5.5 s1)×ATR_S(0.5-5.5 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A11  adaptive_zoom3 — finest zoom with step=1 for LEN, step=0.1 for ATR
    # ──────────────────────────────────────────────────────────────────────
    A = 11
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_HUNTER2_NQ TXF Hourly Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_HUNTER2_NQ TXF HOT Hourly NP>9M TWD Round-1 search")
