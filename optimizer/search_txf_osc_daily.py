"""
search_txf_osc_daily.py — _2021Basic_Osc_NQ on TWF.TXF HOT Daily, Round 1
...
Workspace: needs TWF.TXF HOT DAILY chart with _2021Basic_Osc_NQ signal applied.

NOTE: Update WORKSPACE below to match your MC64 workspace file.
      The workspace must be OPEN in MC64 before running.
      Only one TWF.TXF HOT chart should be visible in the workspace (daily).

Reference: TXF Hourly ceiling: LEN=11 LE=-1.30 SE=3.0 STP=0.975 LMT=33.0 NP=5,970,000 TWD
...
R1 design — 8 fixed attempts + 3 adaptive zooms (11 total), ≤5,000 combos each:
  A01 global_d1  : LEN(2-18 s4) × LE(-2-1 s1)      × SE(0.5-3.5 s1) × STP(0.5-2.5 s0.5) × LMT(5-30 s5)    = 5×4×4×5×6  = 2,400
  A02 short_len  : LEN(1-7 s1)  × LE(-1.5-0.5 s0.5) × SE(1.0-3.0 s0.5)× STP(0.5-2.0 s0.5) × LMT(15-45 s6) = 7×5×5×4×6  = 4,200
  A03 high_lmt   : LEN(5-25 s5) × LE(-1.5-0.5 s0.5) × SE(1.5-3.5 s0.5)× STP(0.5-1.5 s0.5) × LMT(30-80 s10)= 5×5×5×3×6  = 2,250
  A04 low_lmt    : LEN(2-10 s2) × LE(-1.5-0.5 s0.5) × SE(1.0-3.0 s0.5)× STP(0.5-2.0 s0.5) × LMT(1-10 s1)  = 5×5×5×4×10 = 5,000
  A05 wide_se    : LEN(3-15 s3) × LE(-2-0.5 s0.5)   × SE(4-9 s1)     × STP(0.5-2.0 s0.5) × LMT(10-30 s5)  = 5×6×6×4×5  = 3,600
  A06 high_len   : LEN(10-50 s8)× LE(-2-1 s0.5)     × SE(1.0-3.0 s0.5)× STP(0.5-1.5 s0.5) × LMT(10-30 s5) = 6×7×5×3×5  = 3,150
  A07 tight_stp  : LEN(5-20 s5) × LE(-1.5-0.5 s0.5) × SE(1.0-3.0 s0.5)× STP(0.05-0.5 s0.05)× LMT(5-25 s5) = 4×5×5×10×5 = 5,000
  A08 global_d2  : LEN(2-32 s5) × LE(-3-2 s1)       × SE(0.5-4.5 s1) × STP(0.25-1.75 s0.5)× LMT(5-35 s10) = 7×6×5×4×4  = 3,360
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\txf_osc_daily_search")
...
# Initial seed — use TXF hourly champion as starting point; no prior daily result
SEED_LEN, SEED_LE  = 11.0, -1.30
SEED_SE,  SEED_STP = 3.0,   0.975
SEED_LMT           = 33.0
...
_LOG_FILE = OUTPUT_DIR / f"search_txf_osc_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "exits":       "STP×ATR(10) stop + LMT×ATR(10) limit",
            "r1_focus":    "Global sweep on TXF daily bars; 8 fixed attempts (global, short/high LEN, high/low LMT, wide SE, tight STP) + 3 adaptive zooms",
            "hourly_ref":  "TXF Hourly ceiling: LEN=11 LE=-1.30 SE=3.0 STP=0.975 LMT=33.0 NP=5,970,000 TWD (R4 confirmed)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_txf_osc_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  TXF Daily _2021Basic_Osc_NQ  NP>7,000,000 TWD — Round 1")
    log.info("  Symbol: %s  Signal: %s  Timeframe: Daily", SYMBOL, SIGNAL)
    log.info("  Hourly ref: LEN=11 LE=-1.30 SE=3.0 STP=0.975 LMT=33.0  NP=5,970,000 TWD (ceiling)")
    ...
    #      LEN(2-18 s4) × LE(-2-1 s1) × SE(0.5-3.5 s1) × STP(0.5-2.5 s0.5) × LMT(5-30 s5)
    #      = 5×4×4×5×6 = 2,400
    # ──────────────────────────────────────────────────────────────────────
    A = 1
    _n = "01_global_d1"
    _c = _cfg(_n, (2, 18, 4), (-2.0, 1.0, 1.0), (0.5, 3.5, 1.0), (0.5, 2.5, 0.5), (5, 30, 5))
    log.info("A01  LEN(2-18 s4)×LE(-2-1 s1)×SE(0.5-3.5 s1)×STP(0.5-2.5 s0.5)×LMT(5-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  short_len — short LEN=1-7 with ranges around likely daily regime
    #      LEN(1-7 s1) × LE(-1.5-0.5 s0.5) × SE(1.0-3.0 s0.5) × STP(0.5-2.0 s0.5) × LMT(15-45 s6)
    #      = 7×5×5×4×6 = 4,200
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_short_len"
    _c = _cfg(_n, (1, 7, 1), (-1.5, 0.5, 0.5), (1.0, 3.0, 0.5), (0.5, 2.0, 0.5), (15, 45, 6))
    log.info("A02  LEN(1-7 s1)×LE(-1.5-0.5 s0.5)×SE(1.0-3.0 s0.5)×STP(0.5-2.0 s0.5)×LMT(15-45 s6)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  high_lmt — high LMT regime (30-80×ATR)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  low_lmt — tight LMT (1-10×ATR) scalp regime
    #      LEN(2-10 s2) × LE(-1.5-0.5 s0.5) × SE(1.0-3.0 s0.5) × STP(0.5-2.0 s0.5) × LMT(1-10 s1)
    #      = 5×5×5×4×10 = 5,000
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_low_lmt"
    _c = _cfg(_n, (2, 10, 2), (-1.5, 0.5, 0.5), (1.0, 3.0, 0.5), (0.5, 2.0, 0.5), (1, 10, 1))
    log.info("A04  LEN(2-10 s2)×LE(-1.5-0.5 s0.5)×SE(1.0-3.0 s0.5)×STP(0.5-2.0 s0.5)×LMT(1-10 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  wide_se — high SE regime (4-9)
    #      LEN(3-15 s3) × LE(-2-0.5 s0.5) × SE(4-9 s1) × STP(0.5-2.0 s0.5) × LMT(10-30 s5)
    #      = 5×6×6×4×5 = 3,600
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_wide_se"
    _c = _cfg(_n, (3, 15, 3), (-2.0, 0.5, 0.5), (4.0, 9.0, 1.0), (0.5, 2.0, 0.5), (10, 30, 5))
    log.info("A05  LEN(3-15 s3)×LE(-2-0.5 s0.5)×SE(4-9 s1)×STP(0.5-2.0 s0.5)×LMT(10-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  high_len — higher LEN range (10-50)
    ...
    log.info("A06  LEN(10-50 s8)×LE(-2-1 s0.5)×SE(1.0-3.0 s0.5)×STP(0.5-1.5 s0.5)×LMT(10-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  tight_stp — very tight STP (0.05-0.5) — known effective on some instruments
    #      LEN(5-20 s5) × LE(-1.5-0.5 s0.5) × SE(1.0-3.0 s0.5) × STP(0.05-0.5 s0.05) × LMT(5-25 s5)
    #      = 4×5×5×10×5 = 5,000
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_tight_stp"
    _c = _cfg(_n, (5, 20, 5), (-1.5, 0.5, 0.5), (1.0, 3.0, 0.5), (0.05, 0.5, 0.05), (5, 25, 5))
    log.info("A07  LEN(5-20 s5)×LE(-1.5-0.5 s0.5)×SE(1.0-3.0 s0.5)×STP(0.05-0.5 s0.05)×LMT(5-25 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  global_d2 — second global sweep with wider param ranges
    ...
    log.info("A08  LEN(2-32 s5)×LE(-3-2 s1)×SE(0.5-4.5 s1)×STP(0.25-1.75 s0.5)×LMT(5-35 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
...
def main():
    ap = argparse.ArgumentParser(description="TXF Daily _2021Basic_Osc_NQ NP>7M TWD Round-1 search")
