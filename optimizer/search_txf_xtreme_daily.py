"""
search_txf_xtreme_daily.py — SFJ_XtremeStop_NQ on TWF.TXF HOT Daily, Round 1
...
Params:
  X  (lookback bars, default 150): reference = close X daily bars ago
  LY (long entry %, default 3.25): buy when price >= C[X]*(1+LY%)
  SY (short entry %, default 3.25): short when price <= C[X]*(1-SY%)

Target: NP > 8,000,000 TWD on TWF.TXF HOT Daily
IS period: 2019/01/01 – 2026/01/01 (7 years, ~1,750 daily bars)

Daily vs Hourly differences:
  - X on daily: 1 bar = 1 trading day (150 bars ≈ 7.5 months)
  - Fewer trades per year expected (daily bars can only trigger once/day)
  - Daily TXF moves: 5-15% common over multi-day/week windows

Attempt schedule (11 attempts, ≤5,000 combos each):
  A01 global_wide    : X(5-1005 s100)×LY(0.5-10 s0.5)×SY(0.5-10 s0.5) = 11×20×20=4400
  A02 large_x        : X(1000-2500 s150)×LY(0.5-10 s0.5)×SY(0.5-10 s0.5)=11×20×20=4400
  A03 small_x        : X(1-100 s5)×LY(0.5-10 s0.5)×SY(0.5-10 s1.0)     = 20×20×10=4000
  A04 tight_pct      : X(5-1005 s100)×LY(0.1-3 s0.2)×SY(0.1-3 s0.2)    = 11×15×15=2475
  A05 high_pct       : X(5-505 s50)×LY(5-20 s1)×SY(5-20 s1)             = 11×16×16=2816
  A06 medium_fine    : X(50-500 s25)×LY(0.5-7.5 s0.5)×SY(0.5-7.5 s0.5) = 19×15×15=4275
  A07 asym_ly        : X(5-505 s50)×LY(3-15 s0.5)×SY(0.5-5 s0.5)       = 11×25×10=2750
  A08 asym_sy        : X(5-505 s50)×LY(0.5-5 s0.5)×SY(3-15 s0.5)       = 11×10×25=2750
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\txf_xtreme_daily_search")
...
_LOG_FILE = OUTPUT_DIR / f"search_txf_xtreme_daily_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_json(best_entry, attempt_log, target_met):
    payload = {
        ...
        "notes": {
            ...
            "r1_focus": "Global landscape: X=1-2500, LY/SY=0.1-20%; tight/wide/asym/fine sweeps (daily bars)",
        },
        "best_params":  best_entry,
        "all_attempts": attempt_log,
    }
    out = OUTPUT_DIR / "final_params_txf_xtreme_daily.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_XtremeStop_NQ on TWF.TXF HOT Daily — Round 1")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A01  global_wide — broad daily landscape: X=5-1005, LY/SY=0.5-10%
    #      X(5-1005 s100)×LY(0.5-10 s0.5)×SY(0.5-10 s0.5) = 11×20×20=4400
    # ──────────────────────────────────────────────────────────────────────
    A = 1
    _n = "01_global_wide"
    _c = _cfg(_n, (5, 1005, 100), (0.5, 10.0, 0.5), (0.5, 10.0, 0.5))
    log.info("A01  X(5-1005 s100)×LY(0.5-10 s0.5)×SY(0.5-10 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  large_x — long lookback (1000-2500 daily bars = 4-10 years ago)
    #      X(1000-2500 s150)×LY(0.5-10 s0.5)×SY(0.5-10 s0.5) = 11×20×20=4400
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_large_x"
    _c = _cfg(_n, (1000, 2500, 150), (0.5, 10.0, 0.5), (0.5, 10.0, 0.5))
    log.info("A02  X(1000-2500 s150)×LY(0.5-10 s0.5)×SY(0.5-10 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  small_x — short lookback (1-100 daily bars = 1-20 weeks)
    #      X(1-100 s5)×LY(0.5-10 s0.5)×SY(0.5-10 s1.0) = 20×20×10=4000
    # ──────────────────────────────────────────────────────────────────────
    A = 3
    _n = "03_small_x"
    _c = _cfg(_n, (1, 100, 5), (0.5, 10.0, 0.5), (0.5, 10.0, 1.0))
    log.info("A03  X(1-100 s5)×LY(0.5-10 s0.5)×SY(0.5-10 s1.0)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  tight_pct — small percentage thresholds (0.1-3%)
    #      X(5-1005 s100)×LY(0.1-3 s0.2)×SY(0.1-3 s0.2) = 11×15×15=2475
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_tight_pct"
    _c = _cfg(_n, (5, 1005, 100), (0.1, 3.0, 0.2), (0.1, 3.0, 0.2))
    log.info("A04  X(5-1005 s100)×LY(0.1-3 s0.2)×SY(0.1-3 s0.2)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  high_pct — large percentage thresholds (5-20%)
    #      X(5-505 s50)×LY(5-20 s1)×SY(5-20 s1) = 11×16×16=2816
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_high_pct"
    _c = _cfg(_n, (5, 505, 50), (5.0, 20.0, 1.0), (5.0, 20.0, 1.0))
    log.info("A05  X(5-505 s50)×LY(5-20 s1)×SY(5-20 s1)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  medium_fine — finer sweep at medium X range (50-500 daily bars)
    #      X(50-500 s25)×LY(0.5-7.5 s0.5)×SY(0.5-7.5 s0.5) = 19×15×15=4275
    # ──────────────────────────────────────────────────────────────────────
    A = 6
    _n = "06_medium_fine"
    _c = _cfg(_n, (50, 500, 25), (0.5, 7.5, 0.5), (0.5, 7.5, 0.5))
    log.info("A06  X(50-500 s25)×LY(0.5-7.5 s0.5)×SY(0.5-7.5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  asym_ly — LY wide range, SY tight (asymmetric long-biased)
    #      X(5-505 s50)×LY(3-15 s0.5)×SY(0.5-5 s0.5) = 11×25×10=2750
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_asym_ly"
    _c = _cfg(_n, (5, 505, 50), (3.0, 15.0, 0.5), (0.5, 5.0, 0.5))
    log.info("A07  X(5-505 s50)×LY(3-15 s0.5)×SY(0.5-5 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  asym_sy — SY wide range, LY tight (asymmetric short-biased)
    #      X(5-505 s50)×LY(0.5-5 s0.5)×SY(3-15 s0.5) = 11×10×25=2750
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_asym_sy"
    _c = _cfg(_n, (5, 505, 50), (0.5, 5.0, 0.5), (3.0, 15.0, 0.5))
    log.info("A08  X(5-505 s50)×LY(0.5-5 s0.5)×SY(3-15 s0.5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  SFJ_XtremeStop_NQ TWF.TXF HOT Daily Round-1 COMPLETE")
...
def main():
    ap = argparse.ArgumentParser(
        description="SFJ_XtremeStop_NQ TWF.TXF HOT Daily NP>8M TWD Round-1 search")
