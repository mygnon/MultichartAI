"""
search_zw_hourly.py — Breakout Hourly NP > 700,000 on CBOT.ZW HOT, Round 1

No prior knowledge — first exploration of ZW hourly bars.
Target: NP > 700,000 USD on CBOT.ZW HOT hourly bars, _2021Basic_Break_NQ
Insample: 2019/01/01 – 2026/01/01
Objective: NP² / |MaxDrawdown|

CBOT.ZW HOT context:
  5,000 bushels/contract
  Quoted in cents/bushel (e.g. 600 = $6.00/bu)
  1 point = 1 cent/bu × 5,000 = $50/contract
  Min tick = 0.25 cents = $12.50/contract
  Hourly ATR typically 5–20 cents
  To reach NP>700K: need ~14,000 pts profit over 7yr (~2,000 pts/yr)

Parameter ranges (all in cents/bushel):
  LE: 1–8   (entry bar lookback)
  SE: 5–150 (breakout signal length in bars)
  STP: 5–50 (stop loss; 5¢=$250/contract, 50¢=$2,500/contract)
  LMT: 10–100 (profit target; 10¢=$500, 100¢=$5,000/contract)

Attempt schedule (12 attempts, ≤5000 combos each):
  A01 coarse_survey:  LE(1-5 s2) × SE(10-80 s10) × STP(5-30 s5) × LMT(10-50 s10) 3×8×6×5=720
  A02 low_se:         LE(1-5 s2) × SE(5-30 s5)  × STP(5-25 s5)  × LMT(10-50 s10) 3×6×5×5=450
  A03 high_se:        LE(1-4 s1) × SE(60-150 s10)× STP(5-25 s5)  × LMT(10-50 s10) 4×10×5×5=1000
  A04 tight_stp:      LE(1-5 s2) × SE(10-60 s5) × STP(2-20 s2)  × LMT(10-40 s10) 3×11×10×4=1320
  A05 high_lmt:       LE(1-4 s1) × SE(20-60 s5) × STP(5-25 s5)  × LMT(50-150 s10)4×9×5×11=1980
  A06 tight_lmt:      LE(1-4 s1) × SE(10-60 s5) × STP(5-25 s5)  × LMT(5-30 s5)  4×11×5×6=1320
  A07 zoom_best:      adaptive zoom from best NP
  A08 le_fine:        LE(1-8 s1) × SE(15-50 s5) × STP(5-20 s5)  × LMT(10-40 s10) 8×8×4×4=1024
  A09 stp_fine:       LE(1-3 s1) × SE(15-50 s5) × STP(2-20 s2)  × LMT(10-40 s10) 3×8×10×4=960
  A10 dense_zoom:     adaptive dense zoom
  A11 lmt_fine:       LE(1-4 s1) × SE(15-50 s5) × STP(5-20 s5)  × LMT(5-60 s5)  4×8×4×12=1536
  A12 wide_boundary:  LE(1-5 s1) × SE(5-100 s10)× STP(2-30 s4)  × LMT(5-60 s5)  5×10×8×12=4800
"""
...
OUTPUT_DIR = Path(r"C:\Users\Tim\MultichartAI\results\zw_hourly_search")
...
# No prior knowledge — nominal seed
SEED_LE,  SEED_SE  = 3.0,  40.0
SEED_STP, SEED_LMT = 15.0, 30.0
SEED_NP   = 0.0
SEED_OBJ  = 0.0

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
_LOG_FILE = OUTPUT_DIR / f"search_zw_hourly_{int(time.time())}.log"
logging.basicConfig(
    level=logging.INFO,
...
def csv_for(name: str) -> Path:
    return OUTPUT_DIR / f"ZWH1_{name}_raw.csv"
...
def run_or_load(name, cfg, conn, from_csv):
    ...
    if from_csv or csv_path.exists():
        ...
        return None
    log.info("=== Starting ZWH1_%s (%d combos) ===", name, cfg.total_runs())
    t0 = time.time()
    try:
...
def save_json(best, log_, met):
    ...
    payload = {
        "strategy":           "Breakout_ZW_Hourly  (target NP>700K round-1)",
        "symbol":             SYMBOL,
    ...
    out = OUTPUT_DIR / "final_params_zw_hourly.json"
...
def run_search(conn, from_csv, start_attempt):
    ...
    log.info("══════════════════════════════════════════════════════════════")
    log.info("  ZW Hourly Breakout NP>700K Round-1 — initial exploration")
    log.info("  Symbol: %s  Timeframe: hourly (60 min)", SYMBOL)
    log.info("  No prior knowledge — wide parameter exploration")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A01  Coarse survey  (3×8×6×5=720)
    # ──────────────────────────────────────────────────────────────────────
    A = 1
    _n = "01_coarse_survey"
    _c = _cfg(_n, (1, 5, 2), (10, 80, 10), (5.0, 30.0, 5.0), (10, 50, 10))
    log.info("A01  LE(1-5 s2) × SE(10-80 s10) × STP(5-30 s5) × LMT(10-50 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A02  Low SE (fast signals)  (3×6×5×5=450)
    # ──────────────────────────────────────────────────────────────────────
    A = 2
    _n = "02_low_se"
    _c = _cfg(_n, (1, 5, 2), (5, 30, 5), (5.0, 25.0, 5.0), (10, 50, 10))
    log.info("A02  LE(1-5 s2) × SE(5-30 s5) × STP(5-25 s5) × LMT(10-50 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A03  High SE (slow signals)  (4×10×5×5=1000)
    # ──────────────────────────────────────────────────────────────────────
    A = 3
    _n = "03_high_se"
    _c = _cfg(_n, (1, 4, 1), (60, 150, 10), (5.0, 25.0, 5.0), (10, 50, 10))
    log.info("A03  LE(1-4 s1) × SE(60-150 s10) × STP(5-25 s5) × LMT(10-50 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A04  Tight STP 2-20  (3×11×10×4=1320)
    # ──────────────────────────────────────────────────────────────────────
    A = 4
    _n = "04_tight_stp"
    _c = _cfg(_n, (1, 5, 2), (10, 60, 5), (2.0, 20.0, 2.0), (10, 40, 10))
    log.info("A04  LE(1-5 s2) × SE(10-60 s5) × STP(2-20 s2) × LMT(10-40 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A05  High LMT 50-150  (4×9×5×11=1980)
    # ──────────────────────────────────────────────────────────────────────
    A = 5
    _n = "05_high_lmt"
    _c = _cfg(_n, (1, 4, 1), (20, 60, 5), (5.0, 25.0, 5.0), (50, 150, 10))
    log.info("A05  LE(1-4 s1) × SE(20-60 s5) × STP(5-25 s5) × LMT(50-150 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A06  Tight LMT 5-30  (4×11×5×6=1320)
    # ──────────────────────────────────────────────────────────────────────
    A = 6
    _n = "06_tight_lmt"
    _c = _cfg(_n, (1, 4, 1), (10, 60, 5), (5.0, 25.0, 5.0), (5, 30, 5))
    log.info("A06  LE(1-4 s1) × SE(10-60 s5) × STP(5-25 s5) × LMT(5-30 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A07  Zoom toward best NP
    # ──────────────────────────────────────────────────────────────────────
    A = 7
    _n = "07_zoom_best"
    for _r_le, _r_se, _r_stp, _r_lmt, _le_step, _se_step, _stp_step, _lmt_step in [
        (3, 20, 8.0, 20, 1, 4, 2.0, 5),
        (2, 15, 6.0, 15, 1, 3, 1.0, 5),
        (2, 10, 4.0, 10, 1, 2, 1.0, 5),
        (1,  6, 2.0,  6, 1, 1, 0.5, 5),
    ]:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A08  LE fine  (8×8×4×4=1024)
    # ──────────────────────────────────────────────────────────────────────
    A = 8
    _n = "08_le_fine"
    _c = _cfg(_n, (1, 8, 1), (15, 50, 5), (5.0, 20.0, 5.0), (10, 40, 10))
    log.info("A08  LE(1-8 s1) × SE(15-50 s5) × STP(5-20 s5) × LMT(10-40 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A09  STP fine  (3×8×10×4=960)
    # ──────────────────────────────────────────────────────────────────────
    A = 9
    _n = "09_stp_fine"
    _c = _cfg(_n, (1, 3, 1), (15, 50, 5), (2.0, 20.0, 2.0), (10, 40, 10))
    log.info("A09  LE(1-3 s1) × SE(15-50 s5) × STP(2-20 s2) × LMT(10-40 s10)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A11  LMT fine  (4×8×4×12=1536)
    # ──────────────────────────────────────────────────────────────────────
    A = 11
    _n = "11_lmt_fine"
    _c = _cfg(_n, (1, 4, 1), (15, 50, 5), (5.0, 20.0, 5.0), (5, 60, 5))
    log.info("A11  LE(1-4 s1) × SE(15-50 s5) × STP(5-20 s5) × LMT(5-60 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
    ...
    # ──────────────────────────────────────────────────────────────────────
    # A12  Wide boundary  (5×10×8×12=4800)
    # ──────────────────────────────────────────────────────────────────────
    A = 12
    _n = "12_wide_boundary"
    _c = _cfg(_n, (1, 5, 1), (5, 100, 10), (2.0, 30.0, 4.0), (5, 60, 5))
    log.info("A12  LE(1-5 s1) × SE(5-100 s10) × STP(2-30 s4) × LMT(5-60 s5)  %d combos",
             _c.total_runs())
    if start_attempt <= A:
        _update(run_or_load(_n, _c, conn, from_csv), _c, _n, A, _c.total_runs())
...
def main():
    ap = argparse.ArgumentParser(description="ZW Hourly Breakout NP>700K Round-1 search")
